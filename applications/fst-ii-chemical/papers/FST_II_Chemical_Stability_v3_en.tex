...
% ===================================================================
\section{Optional Structural Analogy: FST-II as a Pattern A Instance}
...
Table~\ref{tab:prediction-ledger} states the six predictions as measurement
gates. This separates the proposed game-theoretic status of a network from
independent thermodynamic observables and keeps the toy calculations below from
being read as validation evidence.

\begin{table}[htbp]
\centering
\caption{Prediction-to-measurement ledger. Each prediction requires an
observable that is independent of the payoff model used to define the
game-theoretic state.}
\label{tab:prediction-ledger}
\footnotesize
\setlength{\tabcolsep}{3pt}
\begin{tabularx}{\textwidth}{@{}>{\raggedright\arraybackslash}p{0.11\textwidth}>{\raggedright\arraybackslash}p{0.21\textwidth}>{\raggedright\arraybackslash}p{0.34\textwidth}>{\raggedright\arraybackslash}X@{}}
\toprule
\textbf{ID} & \textbf{Target} & \textbf{Independent observables} & \textbf{Current gate} \\
\midrule
P1 & Cycle persistence & Calorimetric $\dot{S}_{\mathrm{prod}}$ plus perturbation recovery time for matched cycles & Proposed; needs matched cycles above and below the England-bound viability threshold \\
P2 & Chirality threshold & $ee(t)$ as a function of initial $ee$, driving strength, autocatalytic rates, and cross-inhibition & Proposed; needs a racemic baseline and a Soai-type positive-control regime \\
P3 & Network selection & Game status from population sequencing or payoff replay; $\dot{S}_{\mathrm{prod}}$ from calorimetry & Not yet tested; the existing replicator table is only a toy throughput proxy \\
P4 & Compartment threshold & Parasite fraction, compartment size, diffusion length, and persistence under controlled flux & Proposed; compare compartmentalized and well-mixed controls with the same feed chemistry \\
P5 & Spatial protection & Invasion resistance index $\eta$ under identical parasite input with and without spatial structure & Proposed; requires gel or surface-structured replicator experiments \\
P6 & CGT versus kinetics & Initial growth rate, long-run dominance, crossover time, and independent $\dot{S}_{\mathrm{prod}}$ & Direct falsification gate against the standard fastest-growth expectation \\
\bottomrule
\end{tabularx}
\end{table}
...
Homochirality as pure-strategy Nash transition & \textsc{Conjectured} & Sec.~\ref{sec:chirality}; reinterpretation of Frank (1953) \\
