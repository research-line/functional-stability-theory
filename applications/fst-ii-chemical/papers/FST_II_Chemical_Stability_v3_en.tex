...
\renewcommand*{\HyperDestNameFilter}[1]{fst2chem-en-#1}
\makeatother
...
\begin{enumerate}[label=\alph*)]
    ...
    \item \textbf{Finite Negativity as Gauge-Signal:} Parasitic sequences and local instabilities---rather than being mere defects---signal the absence of a stabilizing organizational constraint. In chemical terms, these destabilizing modes may be suppressed in specific regimes by compartmentalization and spatial symmetry breaking (spiral waves), analogous to how gauge constraints resolve finite-negativity modes in the arithmetical setting.
    \item \textbf{Constrained Functional Positivity (Chemical ESS):} The ESS of a chemical network is the ``gauge-stable'' state where the total dissipation functional is resolvent-stabilized (not necessarily globally maximized; see the stability-filter interpretation in Section~\ref{sec:mepp}). By analogy with the rank-finite stabilization language used in the RH program, chemical homochirality and hypercycles are treated here as candidate outcomes of finite constraints that can stabilize replication dynamics in specific models.
\end{enumerate}

This alignment is a structural analogy of ``Functional Positivity under Constraint,'' not an independent chemical theorem.
...
\begin{table}[htbp]
\centering
\caption{Systematic comparison of prebiotic evolutionary and stability paradigms: Thermodynamic Equilibrium Stability, Dynamic Kinetic Stability (DKS; Pross/Pascal), Maximum Entropy Production Principle (MEPP; Dewar/England), and Functional Stability Theory II / Chemical Game Theory (FST-II / CGT).}
...
\toprule
\textbf{Dimension} & \textbf{Thermo\-dynamic Equilibrium} & \textbf{Dynamic Kinetic Stability (DKS)} & \textbf{Maximum Entropy Production (MEPP)} & \textbf{FST-II / Chemical Game Theory (CGT)} \\
\midrule
System State & Static equilibrium & Non-equilibrium steady state (NESS) & Far-from-equilibrium steady state & Heterogeneous game-theoretic NESS networks \\
Energy Level & Absolute minimum & High-energy (sustained by flux) & High-energy (dissipation channel) & High-energy (network payoffs) \\
Entropy Balance & Maximum internal entropy & Local minimum via external dissipation & Maximization of $\dot{S}_{\text{prod}}$ subject to bounds & Stability filtering under England bound \\
Core Variables & Free energy $\Delta G$, entropy $S$ & Kinetic rates $k_{\text{rep}}, k_{\text{deg}}$, DKS index & Heat dissipation $\Delta Q$, entropy flux $J_S$ & Payoff matrix $\mathbf{A}$, spectral radius $\rho(J)$ \\
Stability Criterion & Energetic isolation / minimum & $k_{\text{rep}} > k_{\text{deg}}$ (net kinetic growth) & Maximization of entropy production rate & Nash equilibrium / ESS under dissipation bound \\
Empirical Role & Endpoint (heat death; e.g., salt) & Starting point (e.g., simple replicator) & Phenomenological macro-filter & Testable predictions P1--P6 ($Azoarcus$, spiral waves) \\
\bottomrule
\end{tabularx}
\end{table}
...
Consider an isothermal system coupled to a heat bath at temperature $T$ and a driven protocol with forward and time-reversed dynamics. Let $A \to B$ denote a coarse-grained macrotransition (e.g., a successful replication event), with forward probability $p_{\mathrm{grow}} := p_F(A \to B)$ and reverse probability $p_{\mathrm{decay}} := p_R(B \to A)$. Crooks' fluctuation theorem relates forward and reverse microscopic trajectory probabilities to exponentiated entropy production. After coarse-graining from microtrajectories to the macrotransition and applying Jensen's inequality, the relevant consequence is the lower bound below. We state this Jensen consequence directly because the sign convention of the exponential average depends on whether one parametrizes heat released to the bath or work done on the system; writing $p_{\mathrm{grow}}/p_{\mathrm{decay}}$ as a simple $\langle e^{+\beta W_{\mathrm{diss}}}\rangle$ average would reverse the Jensen inequality.
...
The resolvent analysis from the gauge-theoretic Riemann Hypothesis program~\citep{GeigerRH2026b} provides the structural analogy used here for interpreting England's inequality. The dissipation bound is not a causal driver of replication but a \emph{viability filter}: replicating systems that violate the macrotransition bound fall outside the thermodynamic cost condition under England's assumptions. This is not, by itself, a Jacobian or resolvent-instability certificate; dynamic elimination requires an explicit kinetic model. Among the many molecular configurations that satisfy the bound at leading order (the degenerate manifold), the framework proposes second-order selection through coupling between the replicative and dissipative degrees of freedom. This perspective organizes the causal question in two stages: England's inequality demarcates the \emph{boundary of the thermodynamically admissible manifold}, while MEPP-as-stability-filter identifies candidate dynamically stable fixed points within it. Systems are not driven toward high entropy production; rather, configurations must first satisfy the dissipation-cost condition and then remain stable under the relevant kinetic perturbations.
\end{remark}
...
{\emergencystretch=4em \textbf{Note on the direction of the CGT metaphor.} Velegol et al.~\citep{CGT2018} introduced CGT as a framework in which \emph{human strategic decisions} are modeled using the formalism of chemical reactions---players' choices are treated as metaphorical ``molecules'' (called ``knowlecules''), and equilibrium outcomes are calculated using chemical thermodynamics. In other words, Velegol's CGT applies chemistry \emph{to} social games, not game theory \emph{to} chemistry. The present paper inverts this metaphor direction: we apply game-theoretic concepts \emph{to} actual chemical systems, treating real molecular species as carriers of strategy-like degrees of freedom. This inversion is independently motivated by the experimental work of Yeates et al.~\citep{AzoarcusPNAS}, who demonstrated that prebiotic RNA replicator dynamics can be directly mapped onto standard $2\times 2$ payoff matrices. We retain the ``CGT'' label for continuity with the literature, but the reader should be aware that our application to prebiotic chemistry constitutes an extension of---not a direct application of---Velegol's original framework.\par}
...
\begin{table}[htbp]
\centering
\caption{Classification of the four chemical games observed in the Azoarcus ribozyme system~\citep{AzoarcusPMC}, mapping kinetic conditions to molecular evolutionary dynamics.}
...
There is a well-defined formal relation between the asymptotics of the replicator equation and Nash equilibria~\citep{HofbauerSigmund1998,HofbauerSigmund2003}:
...
The resolvent framework from the Riemann Hypothesis analysis~\citep{GeigerRH2026b} offers a structural lens for why specific autocatalytic networks (hypercycles, spiral waves) may persist while others decay in a given kinetic model. These structures are not \emph{global} MEPP-maxima---they do not globally maximize entropy production over all conceivable configurations. Rather, they are treated here as \emph{second-order resolvent-stabilized network candidates}: local Nash equilibria that are stable under perturbation. At leading order, many network topologies can produce comparable replication rates and satisfy the England dissipation bound. Parasitic invasion, spatial instability, and concentration collapse are second-order destabilization mechanisms that can favor a locally stable configuration without proving that all alternatives are eliminated. The replicator equation can thus select a resolvent-stable fixed point, not necessarily the most productive one. This organizes the role of MEPP in this framework: MEPP serves as a necessary viability heuristic (stability filter) rather than as a sufficient selection principle (maximization). Robustness in real prebiotic networks would have to be shown by kinetic stability and invasion tests, not inferred from high entropy production alone.
\end{remark}
...
\end{equation}
where the consistency condition $m^* = \mathcal{L}(\mathbf{x}^*)$ distinguishes this from a global (planner) optimum. In the special case where the chemical interaction admits a potential $\Phi$ defined as a payoff or negative free energy, Nash equilibria coincide with maximizers of $\Phi$, and the $\arg\max$ formulation is exact. When $\Phi$ denotes thermodynamic free energy itself, the corresponding statement is an $\arg\min$ statement. The optimization direction is therefore a sign convention, not an additional thermodynamic theorem.
...
The emergence of spiral waves is treated here not as a random pattern but as a model-dependent robust topological feature; we interpret it as maintaining a DESS consistent with the dissipative constraints of MEPP in the reaction-diffusion regimes studied by the cited hypercycle simulations~\citep{HypercycleInfinite}.
...
The instability claim used here is narrower than ``open driven system'' alone. In driven systems that also contain Frank-/Viedma-type autocatalytic amplification and mutual inhibition or recycling, the mixed Nash equilibrium of the racemate can lose its Lyapunov stability and transform into an energetic saddle point~\citep{Frank1953,PlassonReview2007}.
...
\paragraph{Minimal open-chemostat guardrail.}
To separate openness from antagonistic kinetics, consider the dimensionless symmetric model
\begin{align}
    \dot A &= q(A_{\mathrm{in}}-A)-kA(L+D), \\
    \dot L &= (kA-q)L-hLD, \\
    \dot D &= (kA-q)D-hLD,
\end{align}
where $q$ is the common feed/dilution rate, $k$ is the autocatalytic rate, and $h$ is the mutual-antagonism rate. For $h=0$ and $L,D>0$, the two enantiomers have identical per-capita growth rates, so
\begin{equation}
    \frac{d}{dt}\log\!\left(\frac{L}{D}\right)=0
    \qquad\Longrightarrow\qquad
    \dot{ee}=0,
\end{equation}
with $ee=(L-D)/(L+D)$. Open feed can therefore change total chiral mass without amplifying an initial enantiomeric excess. On a positive racemic fixed-point branch $L^*=D^*=x^*>0$ with $h>0$, stationarity gives $kA^*-q=h x^*$, while the antisymmetric mode $\delta=L-D$ obeys $\dot\delta=(kA-q)\delta$. Its chiral eigenvalue is thus $\lambda_{\chi}=h x^*>0$. This matched comparison makes the claim boundary explicit: antagonistic coupling opens the unstable chiral mode; openness and symmetric autocatalysis alone do not. An exactly racemic deterministic trajectory remains symmetric, so branch selection still requires a perturbation or noise source. The result is an exact guardrail within this minimal model, not evidence that real prebiotic systems universally realize its premises.
...
The resolvent analysis from the Riemann Hypothesis program~\citep{GeigerRH2026b} provides a structural analogy for chirality symmetry breaking. The racemate is a leading-order degenerate equilibrium: L- and D-enantiomers have identical thermodynamic payoffs at first order, analogous to the even and odd sectors of the Weil quadratic form sharing identical Laplace limits. No energetic advantage distinguishes the two at leading order. Chirality emerges as a \emph{second-order effect} through coupling between the reaction subspaces: autocatalytic amplification and cross-inhibition constitute the resolvent coupling that splits the degenerate manifold. The selected homochiral state (pure L or pure D) is one stable attractor in the two-enantiomer Frank idealization---not because it produces more entropy than the racemate at first order, but because the second-order cross-couplings between chiral sectors stabilize one pure branch.
\end{remark}
...
The architecture of prebiotic evolution can be described within FST-II as a three-stage, statistically biased cascade~\citep{ThermodynamicsLife}, summarized in Table~\ref{tab:hierarchical-cascade}:

\begin{table}[htbp]
\centering
\caption{The three-level hierarchical cascade of Functional Stability Theory II (FST-II), unifying local reaction thermodynamics, network game dynamics, and non-equilibrium macroscopic stability constraints.}
\label{tab:hierarchical-cascade}
\scriptsize
\setlength{\tabcolsep}{3pt}
\begin{tabularx}{\textwidth}{@{}>{\raggedright\arraybackslash}p{0.12\textwidth}>{\raggedright\arraybackslash}p{0.17\textwidth}>{\raggedright\arraybackslash}p{0.17\textwidth}>{\raggedright\arraybackslash}p{0.21\textwidth}>{\raggedright\arraybackslash}X@{}}
\toprule
\textbf{Cascade Level} & \textbf{Physical \& Game Domain} & \textbf{Key Quantitative Variables} & \textbf{Governing Equations \& Bounds} & \textbf{Prebiotic Selection \& Diagnostic Role} \\
\midrule
\textbf{Micro-Level} & Thermodynamics of Nodes \& Elementary Reactions & Affinity $A_r$, Gibbs free energy $\Delta G_r$, fluxes $J_r$, rate constants $k_{ij}$ & De Donder relation $A_r = -\sum \nu_{ir}\mu_i$, Mass action kinetics $\dot{c}_i = f(c,k)$ & Local equilibrium convergence; sets baseline reaction rates and pairwise kinetic matrix entries $k_{ij}$. \\
\midrule
\textbf{Meso-Level} & Network Topology \& Chemical Game Theory (CGT) & Payoff matrix $\mathbf{A}$, frequency vector $\mathbf{x}$, spectral radius $\rho(J)$, Lyapunov $V(\mathbf{x})$ & Replicator dynamics $\dot{x}_i = x_i [(A\mathbf{x})_i - \mathbf{x}^T A\mathbf{x}]$, Spatial diffusion $\partial_t x_i = D_i \nabla^2 x_i + f_i(\mathbf{x})$ & Selection of Nash equilibria, ESS, and spatial spiral patterns; prevents parasitic invasion via spatial/network structure. \\
\midrule
\textbf{Macro-Level} & Non-Equilibrium Thermodynamics \& MEPP & Entropy production rate $\dot{S}_{\text{prod}}$, dissipation $\Delta Q$, DKS index & England bound $\beta \Delta Q \ge \ln(k_f/k_b)$, MEPP condition $\max \dot{S}_{\text{prod}}$, Resolvent bound & Macro-stability filter; eliminates locally stable Nash equilibria that fail global dissipation and persistence bounds. \\
\bottomrule
\end{tabularx}
\end{table}
...
Table~\ref{tab:prediction-ledger} states the six predictions as measurement gates. This separates the proposed game-theoretic status of a network from independent thermodynamic observables and keeps the toy calculations below from being read as validation evidence.

\begin{table}[htbp]
\centering
\caption{Prediction-to-measurement ledger. Each prediction requires an observable that is independent of the payoff model used to define the game-theoretic state.}
...
\toprule
\textbf{No.} & \textbf{Target} & \textbf{Independent Observables} & \textbf{Current Gate} & \textbf{Falsification Criterion} \\
\midrule
P1 & Cycle Persistence & Calorimetric $\dot{S}_{\mathrm{prod}}$ plus perturbation recovery time for matched cycles & Proposed; requires matched cycles above/below England bound & No recovery transition at bound or purely linear $k_{\mathrm{rep}}$ correlation \\
P2 & Chirality Threshold & $ee(t)$ vs. initial $ee$, drive strength, autocatalytic rates, and cross-inhibition & Minimal ODE guardrail passed: Open drive w/o antagonism preserves $ee$; experimental controls open & $ee(t)$ decay despite strong autocatalysis or drive-coupled racemization \\
P3 & Network Selection & Game status from sequencing/payoff replay; $\dot{S}_{\mathrm{prod}}$ from calorimetry & Untested; existing replicator table is a toy throughput proxy & Lower entropy production dominates given identical payoff profiles \\
P4 & Compartment Bound & Parasite fraction, compartment size, diffusion length, persistence under flux & Proposed; compare compartmentalized vs. well-mixed controls with matched feed & Identical parasite accumulation in compartments vs. homogeneous solutions \\
P5 & Spatial Protection & Invasion resistance index $\eta$ given identical parasite input with/without spatial structure & Proposed; requires gel or surface-structure experiments with replicators & No increased invasion resistance ($\eta \le 0$) under spatial spiral-wave structure \\
P6 & CGT vs. Kinetics & Initial growth rate, long-term dominance, crossover time, independent $\dot{S}_{\mathrm{prod}}$ & Direct falsification gate against standard fastest-growth expectation & Persistent victory of highest initial growth rate $k_0$ over Nash equilibrium \\
\bottomrule
\end{tabularx}
\end{table}
...
\begin{proposition}
In a specified symmetric autocatalytic model, antagonistic coupling can create a model-dependent chirality bifurcation; open driving alone need not amplify enantiomeric excess.
\end{proposition}

\textbf{Test:} In asymmetric autocatalytic systems (e.g., the Soai reaction~\citep{Soai1995}), measure the amplification dynamics of enantiomeric excess (ee) as a function of initial ee, driving strength (energy flux), autocatalytic rates, and antagonistic coupling. A matched no-antagonism control must not amplify $ee$ merely because the system is open. In a specified Frank-/Soai-type positive-control regime, the racemic branch should instead acquire a positive antisymmetric eigenvalue and amplify small nonzero $ee$. The non-trivial empirical prediction is therefore a model-dependent bifurcation surface in these kinetic parameters, not a universal driving threshold. The minimal chemostat calculation in Section~\ref{sec:chirality} establishes only the negative- and positive-control logic; its realization and location in real chemistry remain experimental questions.
...
\textbf{Test:} Set up competition between distinct autocatalytic systems. Measure entropy production rates independently, then run competition. The predicted tendency is that the higher-$\dot{S}$ network outcompetes the lower-$\dot{S}$ network. \textbf{Qualification:} As the illustrative example below demonstrates, this prediction holds only in the long run and conditional on sufficient initial cooperator density. In bistable systems, the higher-$\dot{S}$ network may lose from unfavorable initial conditions (coordination barrier). The precise prediction is therefore: \emph{conditional on exceeding the coordination threshold}, the higher-$\dot{S}$ network dominates.
...
\begin{table}[htbp]
\centering
\caption{Fixed points of the 3-species replicator dynamics with Azoarcus-type fitness parameters. Mean fitness $\langle f \rangle$ is used here as a catalytic-throughput proxy; it is not an independently measured thermodynamic entropy-production rate. In this toy parameterization, the cooperative network achieves twice the throughput proxy of the selfish equilibrium.}
...
The cooperative ESS has twice the catalytic-throughput proxy of the selfish ESS, which is qualitatively compatible with P3 but does not confirm it. The proxy is derived from the payoff model itself, whereas P3 requires an independent thermodynamic observable such as calorimetric $\dot{S}_{\mathrm{prod}}$. The example therefore functions only as a dynamical illustration of the coordination problem. The system exhibits \emph{bistability}: from a uniform initial composition, the dynamics converge to the selfish equilibrium because the selfish replicator has a higher initial growth rate ($f_S = 3.0$ vs.\ $f_C = 1.0 + 5.0 x_C^2$ for cooperators). The cooperative ESS is reached only when the initial cooperator fraction exceeds a critical threshold ($x_C \gtrsim 0.4$). This coordination barrier explains why Vaidya et al.\ \citep{AzoarcusPNAS} found that spatial structure (compartmentalization, surface adsorption) was essential for cooperative network establishment---it locally concentrates cooperators above the critical threshold.

\subsection{P4: Compartmentalization Threshold}

\begin{proposition}
The critical parasite density at which compartmentalization becomes advantageous can be estimated in a minimal two-type payoff model.
\end{proposition}

\textbf{Test:} In RNA replicator experiments with varying parasite fractions, determine the critical parasite frequency $x_P^*$ above which compartmentalized systems outperform well-mixed systems. In the minimal mean-field two-type approximation with payoff matrix $\mathbf{A}$, this baseline threshold is predicted to satisfy $x_P^* = (a_{CC} - a_{PC})/(a_{CC} - a_{PC} + a_{PP} - a_{CP})$, where $a_{ij}$ are the pairwise catalytic rates between cooperator ($C$) and parasite ($P$) species. This expression is not a universal threshold: fitted payoffs, diffusion length, compartment size, occupancy statistics, and stochastic extinction can shift or remove it. The non-trivial prediction is that, within a calibrated compartment model, compartmentalization shifts the fitted $x_P^*$ upward (increasing parasite tolerance) by a factor controlled by compartment size relative to diffusion length.

\subsection{P5: Spiral Wave Protection}

\begin{proposition}
Spiral wave spatial organization can increase hypercycle resistance against parasitic invasion in specified reaction-diffusion regimes.
\end{proposition}

\textbf{Test:} Compare invasion success of parasitic RNA sequences in well-mixed vs.\ spatially structured (gel-based) replicator experiments. Quantitatively, define the \emph{invasion resistance index} $\eta = 1 - x_P(t_{\mathrm{eq}})/x_P(0)$, where $x_P(0)$ is the initial parasite fraction and $x_P(t_{\mathrm{eq}})$ is the parasite fraction at demographic equilibrium. Note that $\eta > 0$ indicates parasite reduction (successful resistance), $\eta = 1$ complete parasite elimination, and $\eta < 0$ parasite growth (failed resistance). FST-II predicts that, within Boerlijst--Hogeweg-like reaction-diffusion regimes and for initial conditions that do not already nucleate global collapse, $\eta_{\mathrm{spatial}} > \eta_{\mathrm{mixed}}$ below the model-specific threshold $x_P^*$ from P4. The gap $\Delta\eta = \eta_{\mathrm{spatial}} - \eta_{\mathrm{mixed}}$ should increase with the ratio of domain size to diffusion length only within the calibrated spatial regime. This provides a quantitative target for gel-based experiments with tunable mesh size.

\subsection{P6: Distinguishing CGT Predictions from Standard Kinetics}

A direct experimental test distinguishing the game-theoretic prediction from standard chemical kinetics would involve two competing autocatalytic networks under controlled resource flux. Standard kinetics predicts that the network with the faster initial growth rate dominates. The game-theoretic framework instead raises the testable possibility that the network achieving a resolvent-stable Nash equilibrium---which may have a slower initial rate but is robust against parasitic invasion---will dominate at long times. This could be tested using the Azoarcus ribozyme system with competing cooperative and selfish RNA populations under continuous-flow conditions, measuring both population fractions and calorimetric entropy production independently. The key observable is whether the slower-growing cooperative network overtakes the faster-growing selfish network after a characteristic crossover time, as suggested by the bistability analysis of P3.
...
\begin{enumerate}
    ...
    \item \textbf{Cross-scale architecture:} Integration into the FST framework connecting particle physics (FST-I), chemistry (FST-II), and biology (FST-III) through a recurring Pattern A stability logic---this cross-scale structural correspondence is a proposed original contribution
\end{enumerate}
...
The mathematical connection between Eigen--Schuster stability and Nash stability is exact in the limited sense described below: \emph{at fixed-point solutions of the replicator equation}, every Nash equilibrium of the payoff matrix $\mathbf{A}$ is a stationary point, and every asymptotically stable stationary point is a Nash equilibrium (Theorem~\ref{thm:replicator-nash}). This correspondence is well-established~\citep{HofbauerSigmund2003}.

\subsection{Stability vs.\ Maximization: The Resolvent Perspective}

The FST framework describes \emph{stability principles}, not maximization principles. Leading-order dynamics are degenerate: many configurations of prebiotic networks are equally viable at first order. Selection occurs at second order via resolvent-type coupling between degenerate and complementary subspaces. MEPP acts as a stability filter that identifies candidate resolvent-stable fixed points within the degenerate manifold. This structural logic---developed as structural analogy in the gauge-theoretic reformulation of the Riemann Hypothesis~\citep{GeigerRH2026b}---helps address the MEPP controversy (the principle is used as a stability heuristic, not as maximal-dissipation law), the causal direction problem of England's inequality (dissipation bounds demarcate a thermodynamic viability condition; kinetic stability selects within it), and the chirality puzzle (homochirality is modeled as a resolvent-stable fixed point of a leading-order degenerate equilibrium under Frank-type premises). Across the FST papers, the program proposes the same second-order selection logic: Nash equilibria are not optima but candidate resolvent-stable fixed points.
...
encodes the local coupling between all species.  For open systems without exact
conservation laws, resolvent stability requires that every eigenvalue of~$A$
has strictly negative real part.  For conservative reaction networks, the same
criterion must first restrict~$A$ to the stoichiometric compatibility class (or
equivalently the tangent space after quotienting conserved quantities); neutral
mass-conservation modes are not instability modes.  In either case, the
stability boundary is controlled by the spectral abscissa
$\alpha(A)=\max \operatorname{Re}\lambda(A)$ on the relevant subspace, not by
the spectral radius alone.  The stationary state is asymptotically stable when
this restricted spectral abscissa is negative and trajectories return to
$\mathbf{c}^{*}$ after any sufficiently small admissible perturbation.
...
(ii)~\emph{systematic perturbation scans} that map how the restricted spectral
abscissa~$\alpha(A)$ approaches zero across environmental parameters, thereby
charting the boundary of the resolvent-stable region in parameter space.
The spectral radius~$\rho(A)$ remains useful as a stiffness or oscillation
scale, but it is not by itself a stability-margin certificate.

Within this picture, England's dissipation bound~\citep{England2013} demarcates
a \emph{thermodynamic viability condition}: networks that fail the
macrotransition cost bound are not viable under the assumptions of
Lemma~\ref{lem:england}.  Persistence still depends on the kinetic model.
Second-order selection then operates \emph{within} this admissible region.
Parasite resistance, spatial structure (spiral waves in hypercycles), and
resource-flow topology help determine \emph{which} of the viable networks is
locally stable and can therefore dominate the long-term dynamics.
This two-tier architecture---first-order thermodynamic cost filter plus
second-order kinetic stability selection---connects the structural analogy to
the FST-RH framework~\citep{GeigerRH2026b} with quantities that are directly
...
The circularity risk identified in Section~\ref{sec:cgt} (MEPP simultaneously defines the payoff function and the predicted outcome) can be empirically tested by an experiment in which entropy production and game-theoretic status are measured through \emph{independent} observables. We propose the following protocol:
...
Because the dynamics conserve total concentration on the simplex, the full Jacobian contains a neutral mass-conservation mode. Restricted to the tangent space of the concentration simplex, the non-neutral eigenvalues have negative real part (approximately $-0.3125 \pm 0.2997\,i$), confirming asymptotic stability within the physically relevant simplex. The reported spectral radius $\rho(J) = 0.433$ is therefore best read as a local stiffness/oscillation scale, not as a literal distance to the stability boundary.
...
\begin{table}[htbp]
\centering
\caption{Status of claims. \textbf{Established}: proven or widely accepted results cited here; \textbf{Literature-supported heuristic}: motivated by prior literature but not used as a proven general law; \textbf{Conjectured}: new but testable claims; \textbf{Speculative}: framework-level proposals without direct empirical test.}
...
\midrule
Replicator--Nash correspondence & \textsc{Established} & Hofbauer \& Sigmund (1998, 2003); Theorem~\ref{thm:replicator-nash} \\
England dissipation bound & \textsc{Established} & England (2013); Crooks (1999); Lemma~\ref{lem:england} \\
MEPP as heuristic organizing principle & \textsc{Literature-supported heuristic} / contested far from eq. & Martyushev \& Seleznev (2006); Grinstein \& Linsker (2007); not used here as a proven general law \\
Azoarcus ribozymes as game-theoretic systems & \textsc{Established} & Vaidya et al.\ (2012);~\citet{SieversKiedrowski1994} \\
Frank model for chirality amplification & \textsc{Established} & Frank (1953); Viedma ripening experiments \\
Gibbs minimization--Nash equilibrium analogy & \textsc{Conjectured} & Sec.~\ref{sec:cgt}; formal analogy, not isomorphism \\
Resolvent stability as selection mechanism & \textsc{Conjectured} & Remark~\ref{rem:autocatalysis-resolvent}; structural analogy, no domain-specific formalization \\
Homochirality as pure-strategy Nash transition & \textsc{Conjectured} & Sec.~\ref{sec:chirality}; reinterpretation of Frank (1953), with a minimal matched chemostat guardrail but no experimental FST-II test \\
MEPP network competition (P3) & \textsc{Conjectured} & Sec.~\ref{sec:predictions}; illustrative simulation, no experiment \\
Spiral waves as distributed ESS & \textsc{Conjectured} & Sec.~\ref{sec:hypercycles}; qualitative, no quantitative DESS proof \\
Compartmentalization threshold (P4) & \textsc{Conjectured} & Testable; no experimental data yet \\
Hierarchical micro/meso/macro cascade & \textsc{Speculative} & Sec.~\ref{sec:synthesis}; conceptual architecture \\
\bottomrule
\end{tabularx}
\end{table}
...
The FST-II framework offers a structured interpretive model of prebiotic chemical evolution. Through the mathematical connection between the Lyapunov metric of differential replicator equations and thermodynamic entropy production~\citep{ReplicatorEntropy}, it suggests that the emergence of life can be investigated as a problem of stability selection under dissipative constraints in open material systems.

Prebiological reactions are treated here not merely as isolated stochastic collisions, but as networked processes that can be represented as thermodynamic multi-player games. The central ``payoff'' in these molecular games is a candidate dissipation measure under stability selection. Phenomena such as homochirality and autocatalytic hypercycles can then be read as Nash-equilibrium-like structures within these energetic competitions.
...
\end{enumerate}
As a framework (program) paper, FST-II does not present new experimental data or novel mathematical proofs; its contribution is the synthesis of established results into a coherent architecture that generates testable predictions. The six testable predictions (P1--P6) provide a concrete experimental program to distinguish this framework from mere reformulation of known results. The value of the framework will ultimately depend on whether these predictions are confirmed, refined, or refuted by future experiments.

Despite these limitations, FST-II still points to suggestive---though as yet untested---ways of constraining the expected architectures of exobiological systems on other planets (for example, by treating homochirality as a potentially generic feature of autocatalytic networks) and of guiding the rational design of artificial dissipative networks in modern synthetic systems chemistry.
...
\end{thebibliography}
\medskip
\paragraph*{AI Disclosure.} This work was assisted by Claude (Anthropic), Gemini (Google), and GPT (OpenAI) for literature synthesis, mathematical and structural checks, bilingual editing, and text drafting. All scientific claims, original arguments, and theoretical contributions are the sole responsibility of the human author. No AI system is listed as co-author.
\endgroup
