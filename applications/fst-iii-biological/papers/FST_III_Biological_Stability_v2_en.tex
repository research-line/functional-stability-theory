...
We validated the Nash frustration metric against experimental NMR chemical shift perturbation (CSP) data from BMRB entry~4428 \cite{BMRB4428} for HP35 (villin headpiece subdomain, 1YRF). The CSP per residue is computed as $\text{CSP}_i = \sqrt{\Delta\delta_{\text{H}}^2 + (\Delta\delta_{\text{N}}/5)^2}$, where $\Delta\delta$ denotes the secondary chemical shift relative to random-coil reference values \cite{Wishart1995}; the factor $1/5$ is the standard weighting for ${}^{15}$N chemical shifts \cite{Williamson2013}.
