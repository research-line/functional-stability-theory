...
\paragraph{Aktueller Evidenzstatus.} Die Titelformulierung ``thermodynamische Stabilität'' ist als Name des Forschungsprogramms zu lesen, nicht als bereits abgeschlossenes Stabilitätszertifikat. Die Evidenzhierarchie dieses Beitrags ist streng: Der eindimensionale Stellarscan ist ein Konsistenzcheck; der zweidimensionale $\alpha$/$m_e$-Scan ist eine Negativkontrolle gegen uneingeschränkte einzelskalige MEPP; der lokale Hessian ist wegen des nichtverschwindenden uneingeschränkten Gradienten nur eine Landschaftsdiagnostik. Die hierarchische Constraint-Hypothese ist daher noch keine eigenständige Evidenz. Sie wird erst dann zu Evidenz, wenn die aktiven atomaren, chemischen, nuklearen und stellaren Constraints unabhängig vor der Optimierung spezifiziert werden und anschließend projizierten Gradienten-, Tangential-Hessian-, Leakage- und Matched-Control-Tests standhalten.
...
\textbf{FST-I (diese Arbeit)} & Kandidatentest f\"ur stabilit\"atsbeschr\"ankte Entropie & $\alpha$ im breiten Entropieplateau (98,8\,\%); $m_e/m_p$ erfordert mehrskalige Constraints (47\,\% in 2D) & Falsifizierbar \"uber vorregistrierte Constraints und Mehrparameter-St\"orung; nur Pilotstudie \\
\bottomrule
\end{tabularx}
\end{table}
...
Constraint-Leakage-Test & Hierarchie noch nicht gegen gekoppelte entropiesteigernde Ausweichrinnen stressgetestet & offene Falsifikationsleitplanke & Zeigen, dass der Ausschluss der entropiesteigernden 2D-Rinne aus vorab spezifizierter Physik folgt, nicht aus nachträglichem Ausfiltern; Roh-Payoff-Quelle, aktive Constraint-IDs, projizierte Gradientennorm, Tangential-$\lambda_{\min}/\mathrm{trace}$, effektiven Rang, Residuum-zu-Lücke, Same-Fit-Referenzrisiko und Matched-Control-Familie berichten. \\
Abgeglichene Kontrollen & No-Gamow-Variante, verschobene Constraint-Box und zuf\"allige Plateaupunkte ausstehend & noch nicht gerechnet & Notwendige Leitplanke gegen nachtr\"agliche Anpassung und modellspezifische Hessian-Artefakte. \\
\bottomrule
\end{tabularx}
\endgroup
\end{table}
...
Dieses Kriterium ist ein Mindest-Gate, keine hinreichende Validierung. Ein vollst\"andiger FST-I-Stabilit\"atsclaim muss auch gekoppelte entropiesteigernde Ausweichrinnen wie die aktuelle 2D-Negativkontrolle zur\"uckweisen oder anhand eines unabh\"angig fixierten Constraint-Ledgers zeigen, warum diese Rinnen physikalisch unzul\"assig sind. Wenn die zum Ausschluss einer solchen Rinne n\"otigen Constraints erst nach Beobachtung der Rinne eingef\"uhrt werden, bleibt der Entropiedominanz-Claim f\"ur diesen Slice unvalidiert.
...
Hierarchische Constraints als Reparatur des 2D-Fehlschlags & \textsc{Konjektur} & Erforderliche Arbeitshypothese nach der 2D-Negativkontrolle; erst Evidenz nach unabh\"angig spezifizierten Constraints, projiziertem Gradienten, Tangential-Hessian und Leakage-Tests \\
...
\begin{enumerate}
    \item \textbf{Constraint- und KKT-Vorregistrierung (kritisch):} Aktive atomare, chemische, nukleare und stellare Constraints vor der Optimierung fixieren; Rohgradient, Projektionsmatrix, projizierten Gradienten, Tangential-Hessian und Leakage-Ledger am beobachteten Parameterpunkt berechnen. Ein negativ-definiter Tangential-Hessian nach unabh\"angig spezifizierter KKT-Pr\"ufung w\"are der st\"arkste Nachweis f\"ur den Rahmen.
    \item \textbf{Mehrparameter-Stellarscan (kritisch):} Nach Fixierung des Constraint-Ledgers das semi-analytische Modell auf gleichzeitige Variation von $\alpha_s$, $G_F$ und $m_u/m_d$ erweitern und die Kernstabilit\"atsgrenzen aus \citep{Adams2019} einbeziehen. Dies ist der direkteste Weg, die Entropiedominanz-Vermutung \"uber die hier pr\"asentierten Einzelparameter-Ergebnisse hinaus zu testen.
    \item \textbf{Matched Controls (kritisch):} No-Gamow-, verschobene-Constraint-Box-, Random-Plateau-Point- und Same-Fit-Reference-Kontrollen rechnen, bevor ein Hessian- oder Plateau-Befund als Evidenz interpretiert wird.
