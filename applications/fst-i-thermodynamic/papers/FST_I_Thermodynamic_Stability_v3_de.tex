...
% Version: 3.0 (lokaler Post-v1.5-Kandidat zum Alpha-Toleranzgate)
% ===================================================================
...
\paragraph{Aktueller Evidenzstatus.} Die Titelformulierung ``thermodynamische Stabilität'' ist als Name des Forschungsprogramms zu lesen, nicht als bereits abgeschlossenes Stabilitätszertifikat. Die Evidenzhierarchie dieses Beitrags ist streng: Der eindimensionale Stellarscan ist ein Konsistenzcheck; der zweidimensionale $\alpha$/$m_e$-Scan ist eine Negativkontrolle gegen uneingeschränkte einzelskalige MEPP; der lokale Hessian ist wegen des nichtverschwindenden uneingeschränkten Gradienten nur eine Landschaftsdiagnostik. Die Hypothese einer hierarchischen Einschränkungsstruktur ist daher noch keine eigenständige Evidenz. Sie wird erst dann zu Evidenz, wenn die aktiven atomaren, chemischen, nuklearen und stellaren Einschränkungen unabhängig vor der Optimierung spezifiziert werden und anschließend projizierten Gradienten-, Tangential-Hessian-, Leakage- und Vergleichskontrolltests standhalten. Ein erster Proxy-Tangenten-Ledger verfehlt dieses Gate bereits: Keine vorab benannte Tangentialfamilie besteht am beobachteten Punkt das projizierte Stationaritätskriterium, sodass der aktuelle Ledger die Leitplanke stärkt, nicht den Stabilitätsclaim.
...
\begin{itemize}
    ...
    \item \textbf{Sawada, Daigaku \& Toma (2025)} best\"atigten in einem aktuellen Review, dass MEPP makroskopische evolution\"are Trends gut beschreibt, eine rigorose Herleitung aus ersten Prinzipien fern vom Gleichgewicht jedoch unvollst\"andig bleibt~\citep{Sawada2025}.
\end{itemize}
...
Der gr\"o\ss{}te Teil des kosmischen Kohlenstoffs entsteht durch die Triple-Alpha-Reaktion \citep{Adams2019,Oberhummer2000}. Ihre Rate h\"angt empfindlich von der $0^+$-Resonanz in Kohlenstoff-12 ab. Oberhummer, Cs\'ot\'o und Schlattl variierten in mikroskopischen Kern- und Stellarrechnungen die Coulombwechselwirkung, die sie ausdr\"ucklich als proportional zur Feinstrukturkonstante ansetzen, um ihren beobachteten Wert. \"Anderungen von mehr als ungef\"ahr $4\%$ unterdr\"uckten in den modellierten Sternen entweder die Kohlenstoff- oder die Sauerstoffproduktion stark \citep{Oberhummer2000}. Diese quellengebundene Sensitivit\"at ist ein physikalisches Stressfenster, aber f\"ur sich genommen weder eine universelle Gangbarkeitsgrenze noch Evidenz daf\"ur, dass der beobachtete Wert von $\alpha_{EM}$ das FST-I-Funktional maximiert.
...
Das semi-analytische Ergebnis ist als Landschaftsdiagnostik n\"utzlich---das beobachtete $\alpha$ liegt auf einem breiten stellaren Hochentropie-Plateau---ist aber keine unabh\"angige Evidenz f\"ur Parameterselektion. Insbesondere zeigt die monotone $m_e/m_p$-Abh\"angigkeit, dass mehrskalige Einschr\"ankungen (Atomstabilit\"at, Chemie-Gangbarkeit) vor jedem Stabilit\"atsclaim ber\"ucksichtigt werden m\"ussen. Die Erweiterung dieser Berechnung ist die prim\"are Aufgabe f\"ur zuk\"unftige Arbeit.
...
Nachdem wir die Kernstruktur von FST-I dargelegt haben --- die Quantenmechanik als Analogie f\"ur gemischte Strategien (Abschnitt~\ref{sec:quantum}), MEPP als Kandidat f\"ur die globale Payoff-Funktion (Abschnitt~\ref{sec:mepp}), Fine-Tuning als Hypothese einer eingeschr\"ankten Optimierung (Abschnitt~\ref{sec:finetuning}), Symmetriebrechung als Phasen\"ubergangsanalogie (Abschnitt~\ref{sec:symmetry}) und Dekoh\"arenz als Analogie zur strategischen Durchsetzung (Abschnitt~\ref{sec:decoherence}) --- verorten wir diesen Rahmen nun in der breiteren Landschaft verwandter Ans\"atze.
...
\textbf{FST-I (diese Arbeit)} & Kandidatentest f\"ur stabilit\"atsbeschr\"ankte Entropie & $\alpha$ im breiten Entropieplateau (98,8\,\%); $m_e/m_p$ erfordert mehrskalige Einschr\"ankungen (47\,\% in 2D) & Falsifizierbar \"uber vorregistrierte Einschr\"ankungen und Mehrparameter-St\"orung; nur Pilotstudie \\
\bottomrule
\end{tabularx}
\end{table}

FST-I kombiniert drei Elemente: (i)~eine vorgeschlagene Nash-Lesart eines thermodynamischen Spiels, (ii)~einen ersten Pilotstudien-Konsistenzcheck (das $\alpha$-Scan-Plateau bei 98,8\,\% Entropieoptimalit\"at f\"ur einen Einzelparameter in einem vereinfachten Sternmodell) und (iii)~ein explizites Falsifikationskriterium (Abschnitt~\ref{sec:predictions}). Wir betonen, dass (ii) keine Vorhersage ist, sondern ein post-hoc-Konsistenzcheck mit bekannter Physik; der Gamow-Tunnelfaktor, der das 98,8\,\%-Resultat tr\"agt, ist selbst Standard-Kernphysik. Das anthropische Prinzip und die kosmologische nat\"urliche Selektion liefern Selektionsdruck, aber keine dynamischen Gleichungen; das Free Energy Principle operiert auf der biologischen Skala und wurde nicht auf fundamentale Konstanten erweitert; 't~Hoofts Zellul\"arer-Automat-Programm adressiert den Ursprung der Quantenmechanik, aber nicht die Werte der Standardmodell-Parameter.
...
\textbf{Das Plateau-Problem.} Ein kritisches Merkmal von Tabelle~\ref{tab:alpha_scan} ist die Flachheit der Entropielandschaft: $\mathcal{S}/\mathcal{S}_{\max} > 0{,}92$ \"uber den gesamten Bereich $0{,}5 \leq \alpha/\alpha_{\text{obs}} \leq 5{,}0$ und $> 0{,}96$ f\"ur $\alpha/\alpha_{\text{obs}} \in [0{,}75;\; 3{,}0]$. Dies bedeutet, dass \emph{fast jeder} Wert von $\alpha$ innerhalb des physikalisch gangbaren Bereichs eine nahezu maximale stellare Entropie erzeugt. Der Wert von 98,8\% demonstriert daher keine scharfe Selektion von $\alpha_{\text{obs}}$, sondern vielmehr die Konsistenz mit einem breiten Plateau. Dar\"uber hinaus liegt das uneingeschr\"ankte Maximum bei $\alpha/\alpha_{\text{obs}} \approx 1{,}6$, nicht bei 1,0 --- der beobachtete Wert liegt etwa 37\% unter dem Maximum. Das Framework interpretiert dies als ein m\"ogliches Zeichen einer Stabilit\"atsselektion aus einer entarteten Mannigfaltigkeit anstelle einer scharfen Optimierung: Viele Konfigurationen sind in erster Ordnung nahezu optimal, und Kreuzkopplungen zweiter Ordnung zwischen Parametersektoren k\"onnten die Entartung aufheben. Dieser Stabilit\"atsselektionsmechanismus wurde jedoch nicht zertifiziert. Die neue Zweiparameter-Hessian-Diagnostik ist informativ, aber nicht station\"ar, sodass sie noch kein KKT- oder Resolventenstabilit\"ats-Zertifikat liefert. Bis ein eingeschr\"ankter Mehrparameter-Hessian und ein projizierter Gradiententest evaluiert sind, sollte der Wert von 98,8\% als eine \emph{notwendige, aber nicht hinreichende} Bedingung f\"ur die Konsistenz des Frameworks verstanden werden.

\textbf{Strukturelle Stabilit\"atseinschr\"ankungen.} Das uneingeschr\"ankte Maximum bei $\alpha / \alpha_{\text{obs}} \approx 1{,}6$ muss gegen physikalische Gangbarkeit bewertet werden: f\"ur $\alpha \gtrsim 1/85$ h\"oren stabile Atome schwerer als Eisen auf zu existieren (die Feinstrukturentwicklung bricht zusammen), und f\"ur $\alpha \gtrsim 1/60$ verschiebt sich die Triple-Alpha-Resonanz aus ihrem gangbaren Fenster \citep{Adams2019}. Wenn diese Einschr\"ankungen unabh\"angig vor dem Stabilit\"atstest spezifiziert werden, verengt sich das gangbare Parameterfenster auf ungef\"ahr $0{,}5 \lesssim \alpha/\alpha_{\text{obs}} \lesssim 2{,}0$, innerhalb dessen der beobachtete Wert mit dem eingeschr\"ankten Hochentropiebereich kompatibel ist. Solange kein unabh\"angig spezifiziertes Register aktiver Einschr\"ankungen existiert, klassifizieren diese Einschr\"ankungen Kandidatenausschl\"usse, liefern aber keine Evidenz f\"ur KKT- oder Resolventenstabilit\"at.

\textbf{Quellengebundenes Alpha-Toleranzgate.} Das engere Coulombexperiment von Oberhummer et al.\ liefert ein direktes Diagnoseintervall von $0{,}96\leq\alpha/\alpha_{\mathrm{obs}}\leq1{,}04$, das aber auf den beobachteten Wert zentriert und dort nicht bindend ist \citep{Oberhummer2000}. Eine dependency-freie Neuauswertung des vorliegenden Spielzeugfunktionals auf 801 Punkten in diesem Intervall steigt strikt an: Ihr Maximum liegt an der oberen Kante, wo $\mathcal{S}(1{,}04\,\alpha_{\mathrm{obs}})/\mathcal{S}(\alpha_{\mathrm{obs}})=1{,}001998387$ gilt. Gleichwertig erreicht der Zielpunkt $0{,}998005599$ des Source-Box-Maximums, w\"ahrend die schrittweitenstabile lokale Log-Steigung positiv ist, $d\log\mathcal{S}/d\log\alpha=0{,}0531918\ldots0{,}0531939$. Das Quellenfenster erh\"alt damit nur die Kompatibilit\"at mit einem nahezu optimalen Plateau; es macht $\alpha_{\mathrm{obs}}$ weder station\"ar noch exakt ausgew\"ahlt. Weil das Fenster auf den Zielwert zentriert ist und keine unabh\"angige Einschr\"ankung f\"ur $m_e$ liefert, kann es weder als unabh\"angige Vorhersage noch als mehrparametriges KKT-Zertifikat dienen.

\textbf{Elektronenmasse-Variation: mehrskalige Einschr\"ankungen.} \emph{Physikalische G\"ultigkeitsgrenzen m\"ussen vor dem Modellergebnis angegeben werden.} Die Abh\"angigkeit des Modells von $m_e$ operiert \"uber die Thomson-Opazit\"at $\kappa \propto m_e^{-2}$. Bei hinreichend kleinem $m_e$ brechen die Annahmen des Modells jedoch zusammen: Ionisationsenergien skalieren wie $E_I \sim m_e \alpha^2 c^2$, und unterhalb von $m_e/m_{e,\text{obs}} \lesssim 0{,}01$ sind Atome bei stellaren Oberfl\"achentemperaturen vollst\"andig ionisiert, wodurch die Konzepte von ``Hauptreihenstern'' und ``stellarer Opazit\"at'' hinf\"allig werden. Der Bohr-Radius $a_0 = \hbar/(m_e \alpha c)$ legt die atomare L\"angenskala fest; wenn $a_0$ die intermolekularen Abst\"ande \"uberschreitet, h\"ort kondensierte Materie auf zu existieren. Chemische Bindungsenergien $E_{\text{chem}} \sim m_e \alpha^2 c^2$ m\"ussen $E_{\text{chem}} \gg k_B T$ f\"ur eine stabile Chemie erf\"ullen, was eine untere Grenze von $m_e \approx 10^{-2}\,m_{e,\text{obs}}$ ergibt. Diese Einschr\"ankungen erzwingen eine harte untere Grenze bei ungef\"ahr $m_e/m_p \gtrsim 10^{-5}$--$10^{-4}$.

\emph{Innerhalb} dieser physikalischen G\"ultigkeitsgrenzen zeigt das Sternmodell eine monotone Zunahme von $\mathcal{S}_{\text{total}}$ mit abnehmendem $m_e$, was das Fehlen einer intrinsischen unteren Grenze f\"ur $m_e$ allein auf der stellaren Skala widerspiegelt. Dies ist ein strukturell wichtiges Ergebnis: \emph{Stellare Entropieproduktion allein kann $m_e/m_p$ nicht fixieren}. Der beobachtete Wert wird stattdessen durch die oben aufgef\"uhrten mikroskopischen Konsistenzbedingungen eingeschr\"ankt, nicht durch einen stellaren Entropie-Peak.
...
\textbf{Lokaler Hessian-Status.} Eine anschlie\ss{}ende 2D-Log-Hessian-Diagnostik am beobachteten Punkt, mit Koordinaten $u=\log(\alpha/\alpha_{\mathrm{obs}})$ und $v=\log(m_e/m_{e,\mathrm{obs}})$, ergibt eine steife konkave Richtung ($\lambda \approx -0{,}12344$ f\"ur $\log \mathcal{S}$) und eine nullnahe schlappe Richtung ($|\lambda|<10^{-7}$ \"uber die getesteten Schrittweiten). Dieselbe Diagnostik hat im uneingeschr\"ankten Sternmodell jedoch einen nichtverschwindenden lokalen Gradienten von ungef\"ahr $(0{,}053,\,-0{,}628)$ in $(u,v)$ am beobachteten Punkt. Der Hessian beschreibt daher die Kr\"ummung der lokalen Landschaft, zertifiziert aber \emph{keine} Stationarit\"at, KKT-Optimalit\"at oder Resolventenstabilit\"at. Daf\"ur braucht es eine unabh\"angig fixierte Menge von Einschr\"ankungen, einen projizierten Gradienten-Test und eine Tangential-Hessian-Pr\"ufung.

\textbf{Proxy-Tangenten-Ledger.} Ein explizites Tangential-KKT-/Leakage-Ledger vom 2026-06-20 versch\"arft diese Leitplanke. Sein Befund ist auf Evidenzebene negativ: Keine vorab benannte Tangentialfamilie besteht am beobachteten Punkt das Gate $\|P_T\nabla \log\mathcal{S}\|\leq 0{,}01$. Die beste einfache Solar-Projektion \texttt{fixed\_me} hat die projizierte Gradientennorm $0{,}053194$, der physikalisch benannte Proxy \texttt{fixed\_thomson\_opacity} liegt bei $0{,}406209$. Das st\"arkt nicht den Stabilit\"atsclaim, sondern die Falsifizierbarkeit: Als n\"achstes muss ein unabh\"angig begr\"undetes Register aktiver Einschr\"ankungen spezifiziert werden, nicht eine weitere post-hoc-Hessian-Richtung.

\begin{table}[htbp]
\centering
\caption{Status\"ubersicht f\"ur stellare Entropie- und lokale Hessian-Diagnostik. Die Tabelle trennt numerische Evidenz vom Zertifizierungsstatus, der f\"ur eine eingeschr\"ankte FST-I-Stabilit\"atsaussage n\"otig ist.}
\label{tab:kkt-ledger}
\begingroup\footnotesize
\begin{tabularx}{\textwidth}{p{0.18\textwidth}p{0.26\textwidth}p{0.18\textwidth}Y}
\toprule
\textbf{Diagnostik} & \textbf{Numerische Evidenz} & \textbf{Aktueller Status} & \textbf{N\"achster Pr\"ufschritt} \\
\midrule
1D-$\alpha$-Plateau & $\mathcal{S}(\alpha_{\mathrm{obs}})/\mathcal{S}_{\max}=0{,}988$; Plateau $>0{,}92$ im gangbaren Bereich & Konsistenzcheck & Selektiert $\alpha$ nicht scharf; muss mit unabh\"angigen sektor\"ubergreifenden Einschr\"ankungen gekoppelt werden. \\
...
Proxy-Tangenten-Ledger (2026-06-20) & Beste Projektion \texttt{fixed\_me}: projizierte Gradientennorm $0{,}053194>0{,}01$; \texttt{fixed\_thomson\_opacity}: $0{,}406209$ & negative Diagnostik & Keine getestete Proxy-Tangentialfamilie besteht Stationarit\"at; nicht als KKT- oder Tangential-Hessian-Zertifikat darstellen. \\
Unabh\"angiger eingeschr\"ankter Tangentialtest & aktive Einschr\"ankungen und Projektion noch nicht fixiert & offener Zertifizierungsschritt & Ein aktives Constraint-Register vorregistrieren, Gradienten projizieren und Tangential-Hessian/Resolvente testen. \\
Leakage-Test f\"ur Einschr\"ankungen & Hierarchie noch nicht gegen gekoppelte entropiesteigernde Ausweichrinnen systematisch gepr\"uft & offene Falsifikationsleitplanke & Zeigen, dass der Ausschluss der entropiesteigernden 2D-Rinne aus vorab spezifizierter Physik folgt, nicht aus nachträglichem Ausfiltern; Roh-Payoff-Quelle, Kennungen aktiver Einschr\"ankungen, projizierte Gradientennorm, Tangential-$\lambda_{\min}/\mathrm{trace}$, effektiven Rang, Residuum-zu-Lücke, Risiko einer gleich gut passenden Referenz und Familie der Vergleichskontrollen berichten. \\
Vergleichs\-kontrollen & No-Gamow-Variante, verschobene Einschr\"ankungsbox und zuf\"allige Plateaupunkte ausstehend & noch nicht gerechnet & Notwendige Leitplanke gegen nachtr\"agliche Anpassung und modellspezifische Hessian-Artefakte. \\
\bottomrule
\end{tabularx}
\endgroup
\end{table}

Innerhalb dieses chemisch erlaubten Fensters operiert die Sternphysik nahe maximaler Entropieproduktion. Somit wird $m_e/m_p$ als durch \emph{mehrskalige Kompatibilit\"at} eingeschr\"ankt behandelt statt durch ein Einzelskalen-MEPP-Extremum: die engste Einschr\"ankung kommt von der atomaren/chemischen Skala, und gr\"o\ss{}ere Skalen (stellar, galaktisch) optimieren innerhalb dieses erlaubten Bereichs. Diese Lesart einer hierarchischen Einschr\"ankungsstruktur ist eine zentrale Arbeitshypothese des FST-Rahmens (vgl.\ FST-II \citep{FSTII}).

\textbf{Falsifizierbarkeits-Bewertung.} Unter 13 getesteten Parametervariationen ($\pm 10\%$, $\pm 20\%$, $\pm 50\%$, $+100\%$ in sowohl $\alpha$ als auch $m_e/m_p$) ergeben 7 ein $\mathcal{S} > \mathcal{S}_{\text{obs}}$ im uneingeschr\"ankten Einzelsternmodell. Die Verletzungen bei $\alpha$ sind jedoch klein (maximal 1{,}2\% \"uber dem beobachteten Wert), und alle $m_e$-Verletzungen treten bei unphysikalisch kleinen Elektronenmassen auf. Unabh\"angig spezifizierte strukturelle Stabilit\"atseinschr\"ankungen aus Atomphysik, Kernstabilit\"at und chemischer Gangbarkeit k\"onnen die Anzahl der echten Verletzungen verringern, aber der aktuelle Proxy-Tangentialledger hat eine solche Einschr\"ankungsmenge noch nicht geliefert. Das 2D-Scan-Ergebnis ($\mathcal{S}(\text{obs})/\mathcal{S}_{\text{max}}^{2D} = 0{,}47$) demonstriert, dass das uneingeschr\"ankte MEPP-Argument unzureichend ist: die vollst\"andige FST-I-Behauptung erfordert die oben entwickelte hierarchische Einschr\"ankungsstruktur. Ein umfassender Mehrparameterscan unter Einbeziehung aller relevanten physikalischen Einschr\"ankungen ist in Vorbereitung.
...
\textbf{Konkretes quantitatives Kriterium.} Um zu verhindern, dass die strukturellen Stabilit\"atseinschr\"ankungen als unfalsifizierbare Ausweichklausel dienen, legen wir uns auf den folgenden vorregistrierten Test fest: Wenn der Mehrparameter-Scan (Schlussfolgerung, Punkt~1) \"uber die f\"unf Kernparameter $\{\alpha_{EM}, \alpha_s, G_F, m_e/m_p, m_u/m_d\}$ abgeschlossen ist und mehr als zwei dieser f\"unf Parameter au\ss{}erhalb des 80\%-Plateaus von $\mathcal{S}/\mathcal{S}_{\max}$ liegen (d.\,h.\ $\mathcal{S}(\text{obs})/\mathcal{S}_{\max} < 0{,}80$ entlang des jeweiligen Einparameter-Querschnitts, w\"ahrend alle anderen Parameter auf ihren beobachteten Werten gehalten werden und alle strukturellen Stabilit\"atseinschr\"ankungen erf\"ullt sind), dann ist die Entropiedominanz-Konjektur in ihrer aktuellen Form falsifiziert. Die Liste der strukturellen Stabilit\"atseinschr\"ankungen ist auf die aus etablierter Kern- und Atomphysik aufz\"ahlbaren Einschr\"ankungen festgelegt: Vakuumstabilit\"at, BBN-Ertrag innerhalb von $2\sigma$ der beobachteten H\"aufigkeiten, stellare Z\"undung mit $T_c > 4 \times 10^6\,\text{K}$ und chemische Bindungsenergien $E_{\text{chem}} \gg k_B T_{\text{surface}}$. Eine nachtr\"agliche Erg\"anzung neuer Einschr\"ankungen ist nicht zul\"assig.

Dieses Kriterium ist ein Mindest-Gate, keine hinreichende Validierung. Ein vollst\"andiger FST-I-Stabilit\"atsclaim muss auch gekoppelte entropiesteigernde Ausweichrinnen wie die aktuelle 2D-Negativkontrolle zur\"uckweisen oder anhand einer unabh\"angig fixierten Einschr\"ankungs\"ubersicht zeigen, warum diese Rinnen physikalisch unzul\"assig sind. Der Ausschluss des entropiesteigernden 2D-Korridors z\"ahlt nur, wenn die ausschlie\ss{}ende Einschr\"ankung vor dem Scan identifiziert wird, eine vom Payoff unabh\"angige physikalische Quelle hat und die verworfene Gradientenkomponente als normale Constraint-Kraft erkl\"art. Wenn die zum Ausschluss einer solchen Rinne n\"otigen Einschr\"ankungen erst nach Beobachtung der Rinne eingef\"uhrt werden, bleibt der Entropiedominanz-Claim f\"ur diesen Slice unvalidiert.
...
\begin{remark}[Leitplanke f\"ur eingeschr\"ankte Optima]\label{rem:kkt-guardrail}
Bei eingeschr\"ankten Parameterproblemen wie FST-I ist Hessian-Definitheit
allein kein Stabilit\"atszertifikat. Die aktiven Einschr\"ankungen m\"ussen vor dem
...
Die flache Entropielandschaft (Tabelle~\ref{tab:alpha_scan}: $\mathcal{S}/\mathcal{S}_{\max} > 0{,}92$ \"uber den gangbaren Bereich) legt nahe, dass das Fine-Tuning-Problem besser als Stabilit\"atsproblem denn als Maximierungsr\"atsel formuliert werden sollte. Viele Konfigurationen liefern vergleichbare Entropieproduktion erster Ordnung; die beobachtete Konfiguration k\"onnte durch Kreuzkopplungen zweiter Ordnung zwischen Parametersektoren selektiert werden. Diese Hypothese der ``Stabilit\"atsselektion'' bietet eine Alternative sowohl zum Landscape-Ansatz als auch zum anthropischen Argument, bleibt jedoch eine \emph{strukturelle Konjektur}. Die neue 2D-Log-Hessian-Diagnostik zeigt eine steife konkave Richtung und eine nullnahe Richtung, w\"ahrend der uneingeschr\"ankte Gradient nicht verschwindet. Der Proxy-Tangenten-Ledger vom 2026-06-20 macht das n\"achste Gate strenger: Bevor ein voller Mehrparameterscan als Stabilit\"atsevidenz interpretierbar ist, m\"ussen die aktiven Einschr\"ankungen unabh\"angig fixiert werden und das Gate f\"ur den projizierten Gradienten bestehen. Ob der eingeschr\"ankte Tangential-Hessian in allen unabh\"angig zul\"assigen Nicht-Eich-Richtungen negativ definit ist---zusammen mit den erforderlichen KKT-Bedingungen---bleibt daher offen.
...
\begin{table}[htbp]
\centering
\caption{Status der Behauptungen. \textsc{Etabliert}: bewiesene oder weithin akzeptierte Ergebnisse, die hier zitiert werden; \textsc{Konjektur}: neue, aber testbare Behauptungen; \textsc{Diagnostik}: numerische Leitplanken ohne Zertifikatsstatus; \textsc{Spekulativ}: Rahmenvorschl\"age ohne direkten empirischen Test.}
...
SM-Parameter als Nash-Gleichgewicht des Entropiespiels & \textsc{Konjektur} & Konjektur~\ref{conj:entropy-dominance}; nur Pilotstudie \\
...
Proxy-Tangential-/KKT-Ledger & \textsc{Diagnostik} & Ledger vom 2026-06-20: Keine getestete Tangentialfamilie besteht projizierte Stationarit\"at; beste Zeile \texttt{fixed\_me} mit $0{,}053194>0{,}01$; kein KKT-Zertifikat \\
Quellengebundenes $\alpha$-Toleranzgate & \textsc{Diagnostik} & Oberhummer et al.\ (2000), $\pm4\%$-Coulombfenster: Ziel/Boxmaximum $0{,}998005599$, aber das Funktional steigt strikt an und maximiert bei $1{,}04\,\alpha_{\mathrm{obs}}$; keine exakte Auswahl und kein KKT-Zertifikat \\
Hierarchische Einschr\"ankungen als Reparatur des 2D-Fehlschlags & \textsc{Konjektur} & Erforderliche Arbeitshypothese nach der 2D-Negativkontrolle; erst Evidenz nach unabh\"angig spezifizierten Einschr\"ankungen, projiziertem Gradienten, Tangential-Hessian und Leakage-Tests \\
...
Der Ansatz umfasst explizite Falsifizierbarkeitskriterien (Abschnitt~\ref{sec:predictions}), einschlie\ss{}lich eines vorregistrierten quantitativen Kriteriums f\"ur den geplanten Mehrparameter-Scan. Eine erste semi-analytische Pilotstudie zeigt, dass die beobachtete Feinstrukturkonstante innerhalb eines breiten Plateaus nahezu maximaler stellarer Entropieproduktion liegt ($\mathcal{S}/\mathcal{S}_{\max} = 0{,}988$, aber mit $\mathcal{S}/\mathcal{S}_{\max} > 0{,}92$ \"uber den gesamten lebensf\"ahigen Bereich)---ein Konsistenzcheck, kein entscheidender Test. Die Flachheit dieses Plateaus bedeutet, dass das 1D-Ergebnis zwar f\"ur die Tragf\"ahigkeit des Rahmens notwendig ist, aber keine scharfe Parameterselektion demonstriert. Ein zweidimensionaler Scan ergibt $\mathcal{S}/\mathcal{S}_{\max}^{2D} = 0{,}47$ und zeigt damit, dass eine einzelskalige stellare MEPP-Argumentation als eigenst\"andiges Selektionsprinzip scheitert: Ohne hierarchische mehrskalige Einschr\"ankungen aus Atom- und chemischer Physik scheitert die Entropiedominanz-Vermutung bereits in zwei Dimensionen. Die volle Hierarchie von Einschr\"ankungen ist daher eine erforderliche Arbeitshypothese von FST-I, ihre unabh\"angige KKT-/Resolventenvalidierung bleibt aber offen; das erste Proxy-Tangenten-Ledger verfehlte das projizierte Stationarit\"atsgate und dient daher als negative Diagnostik. Das quellengebundene $\pm4\%$-Coulombfenster liefert einen sch\"arferen Alpha-Stresstest, repariert dieses Scheitern aber nicht: Das Spielzeugfunktional steigt im gesamten Fenster strikt an und erreicht sein Maximum bei $1{,}04\,\alpha_{\mathrm{obs}}$, w\"ahrend der beobachtete Wert mit $0{,}998005599$ des Boxmaximums nur plateauartig nahezu optimal bleibt. Diese Pilotstudie auf den gesamten Standardmodell-Parameterraum zu erweitern, bleibt die zentrale rechnerische Herausforderung.
...
\begin{enumerate}
    \item \textbf{Vorregistrierung von Einschr\"ankungen und KKT-Bedingungen (kritisch):} Aktive atomare, chemische, nukleare und stellare Einschr\"ankungen vor der Optimierung in einem eigenen aktiven Constraint-Register fixieren; Rohgradient, Projektionsmatrix, projizierten Gradienten, Tangential-Hessian und Leakage-\"Ubersicht am beobachteten Parameterpunkt berechnen. Das aktuelle Proxy-Tangenten-Ledger verfehlt diese Stationarit\"atsschwelle (beste Zeile \texttt{fixed\_me}: $0{,}053194>0{,}01$), daher ist dieser Schritt Voraussetzung statt Best\"atigung. Ein negativ-definiter Tangential-Hessian nach unabh\"angig spezifizierter KKT-Pr\"ufung w\"are der st\"arkste Nachweis f\"ur den Rahmen.
    \item \textbf{Mehrparameter-Stellarscan (kritisch):} Nach Fixierung der Einschr\"ankungs\"ubersicht das semi-analytische Modell auf gleichzeitige Variation von $\alpha_s$, $G_F$ und $m_u/m_d$ erweitern und die Kernstabilit\"atsgrenzen aus \citep{Adams2019} einbeziehen. Dies ist der direkteste Weg, die Entropiedominanz-Vermutung \"uber die hier pr\"asentierten Einzelparameter-Ergebnisse hinaus zu testen.
    \item \textbf{Vergleichskontrollen (kritisch):} No-Gamow-, verschobene-Einschr\"ankungsbox-, Zufalls-Plateaupunkt- und Referenzkontrollen mit gleich guter Passung rechnen, bevor ein Hessian- oder Plateau-Befund als Evidenz interpretiert wird.
