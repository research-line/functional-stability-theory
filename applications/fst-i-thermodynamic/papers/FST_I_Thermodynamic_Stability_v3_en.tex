...
\begin{enumerate}
    \item \textbf{Constraint and KKT pre-registration (critical):} Fix the active atomic, chemical, nuclear, and stellar constraints before optimization; compute the raw gradient, projection matrix, projected gradient, tangent-space Hessian and leakage ledger at the observed parameter point. A negative-definite tangent Hessian after an independently specified KKT check would provide the strongest evidence for the framework.
