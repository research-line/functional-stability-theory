...
constrains the effective cosmological constant~$\Leff$ without introducing
a fitted late-time $\Lambda$ parameter; the quantitative scaling is
conditional on the RG-matching assumptions stated below.
...
we argue that, under this matching, the unique minimum of~$\Phi$ yields a residual vacuum
...
target for the gravitational slip parameter $\eta = 5/4$ in the
quasi-static, linear, unscreened sub-Compton regime of metric $f(R)$
theories, including the Hu--Sawicki class used for chameleon screening
(see Remark~\ref{rem:hu-sawicki-fst}).
The framework naturally addresses the coincidence
problem by linking the vacuum energy to the current expansion rate.

\medskip\noindent
{\footnotesize
\textit{Limitations (honest scope statement).} The scaling
$\Leff \sim H_0^2/\ln(M_\mathrm{Pl}/H_0)$ follows from the variational
principle \emph{conditional} on a renormalisation-group matching that
contributes a second logarithm $\ln(M_\mathrm{Pl}/H_0)$ from integrating
$\beta_\Lambda^\mathrm{QQG}$ between $H_0$ and $M_\mathrm{Pl}$. This
RG-matching is not derived from first principles in the present paper.
A structured reduction onto two explicit open sub-conditions
((B1)~existence of a perturbatively accessible QQG renormalisation-group
trajectory ending in a Hu--Sawicki-type effective $f(R)$; (B2)~the
$\beta_\Lambda^\mathrm{QQG}$-integral evaluating to $\ln(M_\mathrm{Pl}/H_0)$
with controlled $O(1)$ prefactor) is documented in the companion proof
note on the bridge to Hu--Sawicki screening. We classify this as a
Cosmology-Pattern-A reduction (sub-reduction with unsolved root): the
present paper structures the cosmological constant problem rather than
closing it.
}
...
invoking selection effects, we model the \emph{observed} cosmological
constant as a candidate thermodynamic equilibrium value of a free-energy
functional defined over vacuum fluctuations. The functional is bounded
below by the infrared geometry of spacetime, and, once the RG-matching
assumption is granted, its unique minimum selects a residual energy
density of order~$H_0^{2}/G$.

This variational architecture is intended as a \textbf{``Pattern~B''
(Flow Selection)} instance within the FST framework. The analogy with the
gauge-theoretic reformulation of the Riemann Hypothesis
(see~\cite{GeigerRH2026c}) is structural rather than evidential: both
settings use a stability functional to single out a distinguished branch.
In the present paper, $\Phi(\rho)$ plays the role of a branch-selection
functional for the vacuum sector. The analogy does not transfer the RH
result to cosmology; it only motivates the form of the stability ledger.
...
\textbf{Warning:} Until this reformulation is carried out, the
implicit dimensional factors are not merely ``of order unity'' but
necessarily involve powers of $M_{\mathrm{Pl}}$ and/or $H_0$---precisely
the scales whose ratio determines the result.  The claim that the
stationarity condition $\dd\Phi/\dd\rho = 0$ involves only the
ratio~$B/C$ is correct, but the physical content of $B/C$ depends
on these implicit factors, which must be made explicit to verify the
scaling law.
\end{remark}
...
models~\cite{Li2004} and in the UV-IR cutoff relation
of Cohen, Kaplan, and Nelson~\cite{CohenKaplanNelson1999},
who argue that in an effective field theory coupled to gravity
the IR and UV cutoffs are related by $\Lambda_{\mathrm{UV}}^2\,L \lesssim M_{\mathrm{Pl}}$,
with $L \sim H_0^{-1}$ the cosmological horizon size.
The identification is physically motivated by the causal structure
of de~Sitter space.
...
\begin{remark}
Lemma~\ref{lem:minimality} addresses the principal criticism that $V_{\mathrm{grav}}$ is an \emph{ansatz} rather than a first-principles derivation. While the lemma does not derive $V_{\mathrm{grav}}$ from the Einstein equations, it shows that the functional form~\eqref{eq:Vgrav} is the leading term selected by the stated dimensional and weak-field axioms, up to a numerical coefficient. Alternative back-reaction terms would either violate those assumptions, introduce higher-order gravitational coupling ($G^2$ terms, suppressed by $M_{\mathrm{Pl}}^{-2}$), or be sub-leading in $\rho$ within the regime considered here.
\end{remark}

% ============================================================
\section{Main Result}\label{sec:main}

\begin{theorem}[Thermodynamic bound on $\Leff$ -- conditional]\label{thm:main}
Under the assumptions of Definition~\ref{def:functional}, the function
$\Phi(\rho)$ attains a unique global minimum at
$\rho = \rho_{\ast} > 0$\textup{;} this part (existence, uniqueness,
strict convexity) is proven.  The corresponding effective cosmological
constant
\begin{equation}\label{eq:Leff}
  \Leff \;=\; \frac{8\pi G}{c^{4}}\;\rho_{\ast}
\end{equation}
satisfies the \emph{conjectured} bound
\begin{equation}\label{eq:bound}
  \Leff \;\sim\;
  \frac{H_0^{2}}{c^{2}}\;
  \bigl[\ln\!\left(\frac{M_{\mathrm{Pl}}}{H_0}\right)\bigr]^{-1}.
\end{equation}
The scaling~\eqref{eq:bound} does \textbf{not} follow from the
raw mode sum alone\textup{;} the raw stationarity condition yields
$\rho_\ast \sim H_0 M_{\mathrm{Pl}}^4 \ln(M_{\mathrm{Pl}}/H_0)$,
which is $\sim 10^{60}$ times too large.
The physically relevant scaling requires the
renormalisation-group matching of Section~\ref{sec:renorm},
where the UV-divergent contributions are absorbed into
counterterms and only the infrared running survives.
This matching is \emph{externally imposed}, not derived from the
variational principle itself\textup{;} its rigorous derivation
from the functional~$\Phi$ remains the principal open problem
(see ``Proof status summary'' below).
\end{theorem}
...
divergences. In particular, Step~3 above establishes only the
structural competition between entropic and gravitational terms;
the physical scaling $\rho_\ast^{\mathrm{ren}} \sim
H_0^2/(8\pi G\,\ln(M_{\mathrm{Pl}}/H_0))$ is obtained only after
the renormalisation-group matching of Section~\ref{sec:renorm} and
does not follow from the raw mode-sum functional~\eqref{eq:Phi}
alone. The full derivation will be addressed in the complete
version.}
\end{proof}
...
\label{sec:saturation}% second label for cross-references to saturation dynamics
...
quantum-mechanical potentials, the P\"oschl--Teller form is the simplest
that produces a hyperbolic-tangent saturation profile for the scalar-field
energy density (see Proposition~\ref{prop:saturation} below).
Its key properties---exact solvability, asymptotic
saturation ($V \to 0$ as $|\phi| \to \infty$), and a single
characteristic scale~$\phi_0$---make it a natural minimal choice for
modelling late-time dark-energy dynamics.  We emphasise that the
P\"oschl--Teller potential is \emph{not} the Starobinsky
potential: the Einstein-frame potential of Starobinsky
$R + \gamma R^2$ gravity~\cite{Starobinsky1980}
is the exponential plateau
$V(\phi) = V_0(1 - e^{-\sqrt{2/3}\,\phi/M_{\mathrm{Pl}}})^2$,
which is asymmetric and qualitatively different.  The connection
between the two sectors is indirect: both arise in scalar-tensor
reformulations of higher-curvature gravity, but they describe
different physical regimes (inflation vs.\ late-time
dark energy).
...
solution~\eqref{eq:tanh-solution} and the \textbf{illustrative}
(not observationally favoured) parameters
$\kappa \approx 1.5$, $a_{\mathrm{trans}} \approx 0.50$ yields
$a_{\mathrm{acc}} \approx 0.59$, corresponding to
$z_{\mathrm{acc}} \approx 0.70$.  \emph{These illustrative
values are superseded by the DESI-compatible parameters
$\kappa = 2.63$, $a_{\mathrm{trans}} = 0.326$
(Remark~\ref{rem:desi-comparison}); the numerical result
should be recomputed with the updated parameters.}

\smallskip\noindent
\textbf{Note.}\;
The illustrative values $\kappa = 1.5$,
$a_{\mathrm{trans}} = 0.50$ are superseded by the DESI~DR2
parameter scan of Remark~\ref{rem:desi-comparison}, which
requires $a_{\mathrm{trans}} \lesssim 0.33$ for observational
viability.  With the revised parameters ($\kappa = 2.63$,
$a_{\mathrm{trans}} = 0.326$), $z_{\mathrm{acc}}$ should be
recomputed from the full tanh profile; the qualitative
coincidence-problem resolution (tracking of $\rho_{\mathrm{vac}}$
with $H^2$) remains unchanged.
...
\medskip\noindent\textbf{Warning (illustrative parameters outdated).}\;
The values $\kappa \approx 1.5$--$2.0$ and $a_{\mathrm{trans}} \approx 0.50$
are inconsistent with the DESI~DR2 parameter scan of
Remark~\ref{rem:desi-comparison}, which requires
$a_{\mathrm{trans}} \lesssim 0.33$ for observational viability.
With the revised best-fit parameters ($\kappa = 2.63$,
$a_{\mathrm{trans}} = 0.326$), $w_{\mathrm{eff}}(z=0)$ shifts
significantly and must be recomputed.  The illustrative values
above should not be taken as model predictions.
A dedicated numerical fit to supernova and BAO data is needed to
fix $\kappa$ and $a_{\mathrm{trans}}$ and to derive a reliable
$w_{\mathrm{eff}}(z=0)$.
...
The DESI~DR2 analysis~\cite{DESI2025} (arXiv:2503.14738;
Phys.\ Rev.\ D 112, 083515) reports
evidence for dynamical dark energy at the $2.8$--$4.2\sigma$ level
(depending on the data combination and supernova sample).  The
best-fit values in the $w_{0}w_{a}$CDM parameterisation depend
sensitively on the data combination:
\begin{itemize}
  \item \emph{DESI BAO $+$ CMB (without SNe):}
        $w_0 = -0.42 \pm 0.21$, $w_a = -1.75 \pm 0.58$
        (Table~IX of \cite{DESI2025}; $3.1\sigma$ preference);
  \item \emph{DESI BAO $+$ CMB $+$ SNe:}
        $w_0 \approx -0.7$ to $-0.8$, $w_a \approx -0.6$ to $-0.9$
        (depending on the supernova sample; $2.8$--$4.2\sigma$).
\end{itemize}
A direct comparison with our $w_{\mathrm{eff}}(z=0) \approx -1.4$
to $-1.5$ is not straightforward, since the DESI~$w_{0}$ corresponds
to a different parameterisation (CPL vs.\ tanh saturation).
...
\smallskip\noindent\textbf{Best fit to DESI DR2 (2025):} $\kappa = 2.63$, $a_{\mathrm{trans}} = 0.326$ (corresponding to $z_{\mathrm{trans}} \approx 2.07$), yielding $w_0^{\mathrm{DE}} = -0.470$ and $w_a^{\mathrm{DE}} = -2.00$, with $\chi^2 = 0.15$ (the comparison target should be updated to the
actual DESI~DR2 best-fit values, which depend on the data combination;
see~\cite{DESI2025} for details). The compatible region ($<2\sigma$) spans $\kappa \in [1.2, 5.0]$ and $a_{\mathrm{trans}} \in [0.30, 0.33]$.
...
\begin{theorem}[Convexity--screening incompatibility]\label{thm:screening-nogo}
Let $\Phi: (0,\infty) \to \mathbb{R}$ be strictly convex with
$\Phi''(\rho) > 0$ for all $\rho > 0$, $\Phi'(\rho) \to -\infty$
as $\rho \to 0^+$ and $\Phi'(\rho) \to +\infty$ as $\rho \to +\infty$.
Then $\Phi$ admits a \emph{unique} global minimiser $\rho_*$,
and $\Phi$ has no secondary local minima.
In particular, no density-dependent mechanism can produce
a second, locally stable vacuum state $\rho_{\mathrm{local}}(\varrho) \neq \rho_*$
as a function of the ambient matter density~$\varrho$.
\end{theorem}

\begin{proof}
Strict convexity implies $\Phi'$ is strictly increasing.
The derivative boundary conditions ensure exactly one zero of $\Phi'$
(intermediate value theorem + monotonicity).
Any local minimum $\rho_{\mathrm{local}}$ with $\Phi'(\rho_{\mathrm{local}}) = 0$
must equal $\rho_*$ (uniqueness of the zero).
A chameleon mechanism requires $\rho_{\mathrm{local}}(\varrho)$ to vary with $\varrho$,
which is impossible when $\rho_*$ is $\varrho$-independent.
\end{proof}

\begin{corollary}[Linear $R^2$ screening failure]\label{cor:R2-nogo}
The thermodynamic functional $\Phi(\rho)$ of Section~\ref{sec:functional}
is strictly convex and satisfies the derivative boundary conditions
used above (Theorem~\ref{thm:main}, Steps~1--2).
Therefore, the linear $R^2$ model cannot produce density-dependent
screening. The resulting PPN parameter $\gamma_{\mathrm{PPN}} = 1/2$
is excluded by Cassini tracking at $> 10^4\sigma$.
Resolution requires a \emph{non-convex} extension of $\Phi$
(e.g.\ the Hu--Sawicki parametrisation, which introduces a
density-dependent scalaron mass $m_s(\varrho) \sim \sqrt{\varrho}$).
\end{corollary}

\subsection{Screening as \texorpdfstring{$\Gamma$}{Gamma}-convergent phase separation (heuristic)}\label{sec:screening-gamma}

We present a heuristic argument that screening can be understood as a
phase-separation phenomenon in the $\Gamma$-limit of a non-convex
thermodynamic functional.  The following is \emph{not} a rigorous
$\Gamma$-convergence proof (which would require establishing the
liminf inequality and the existence of a recovery sequence) but a
physical motivation for the screening mechanism.
...
\subsubsection*{Step 2: \texorpdfstring{$\Gamma$}{Gamma}-convergence and phase separation}
...
\end{itemize}
This has the formal shape of a chameleon screening channel, but in the
present paper it is a variational motivation rather than a completed
derivation of solar-system-safe screening.
...
\end{itemize}
\emph{In this heuristic description, screening is represented as a
thermodynamic phase separation rather than as an independently postulated
dynamical add-on.}

\subsubsection*{Step 4: Relation to Hu--Sawicki models}

Hu--Sawicki $f(R)$ theories provide a concrete parametric model for the
kind of density-dependent branch structure that the $\Gamma$-limit picture
requires. The key difference is interpretive: in standard $f(R)$,
screening is built into the model class; in the FST formulation, it is
treated as a consistency gate for any viable IR completion of the
$\Lambda$-selection principle of Sections~\ref{sec:functional}--\ref{sec:selection}.
...
\item[(iii)] If the dense-regime parameters satisfy the usual thin-shell
and scalaron-mass inequalities, the Yukawa suppression length
$\lambda_Y = 1/m_s$ can satisfy $\lambda_Y(\rho_\odot) \ll 1\,\mathrm{AU}$
for solar-system density $\rho_\odot$, making
$|\gamma_{\mathrm{PPN}} - 1| \lesssim (m_s \cdot r)^{-2} e^{-m_s r}$
compatible with the Cassini bound
...
Proposition~\ref{prop:screening-viability} records the conditional
screening pattern suggested by the phase-separation picture: a dense
phase can have $m_s \gg H_0$ and suppress fifth forces, while a dilute
cosmological phase can keep $m_s \sim H_0$ and support late-time
acceleration. This is a viability criterion, not a closed Cassini proof:
the thin-shell calculation and conservative $|f_{R0}|$ guardrail remain
separate checks. Numerical support for the toy phase transition is given
by \texttt{compute\_screening\_phase.py} (Remark~\ref{rem:screening-numerics}).
\end{remark}
...
\begin{remark}[Convexity and stability of the minimum]
\label{rem:talagrand}
The functional $\Phi(\rho)$ is strictly convex on $(0,\infty)$
(cf.\ Step~2 of the proof of Theorem~\ref{thm:main}:
$\dd^{2}\Phi/\dd\rho^{2} = B/\rho + 2C > 0$).  This guarantees
that the de~Sitter vacuum $\rho^{*}$ is the unique global minimum
and that perturbations $\delta\rho$ decay at a rate controlled by
the curvature $\Phi''(\rho^{*}) = B/\rho^{*} + 2C$.
The stability is intrinsic to the functional structure and does not
depend on the UV cutoff.

\smallskip\noindent
\textbf{Erratum (v1).}\;
An earlier version of this remark invoked Wasserstein-$2$ convexity
and the Otto--Villani/Talagrand inequality.  That language was
inappropriate: $\Phi$ is a function of a single scalar
parameter~$\rho > 0$, not a functional on a space of probability
measures, so the Wasserstein framework does not apply in this
context.  The convexity statement above is the correct
...
\begin{tabularx}{0.97\textwidth}{|>{\raggedright\arraybackslash}p{0.34\textwidth}|Y|}
\hline
...
\hline
$\Lambda_{\mathrm{eff}}$ selection from free energy & Conditional theorem (existence/uniqueness proven; scaling requires RG matching) \\
$\Lambda \sim H^2 / \log(M_{\mathrm{Pl}}/H)$ scaling & Conjectured (follows only after renormalisation, not from raw mode sum) \\
Saturation dynamics (tanh profile) & Proposition \\
$\Gamma$-convergence of screening (Steps 1--4) & Argued (conceptual) \\
Screening viability ($m_s^2 \sim \rho$) & Conditional criterion; thin-shell gate open \\
\hline
\multicolumn{2}{|l|}{\textit{Numerically confirmed:}} \\
\hline
DESI DR2 compatibility & Preliminary (target values need update; see Remark~\ref{rem:desi-comparison}) \\
...
The target scaling $\Lambda_{\mathrm{eff}} \sim H_0^2/\ln(M_{\mathrm{Pl}}/H_0)$ contains no fitted late-time dark-energy parameter once the RG-matching assumptions are fixed; the logarithmic factor is then set by the separation between the Hubble and Planck scales. The remaining factor-of-few discrepancy should be read as an $O(1)$ uncertainty of the semiclassical matching, not as a precision prediction.
...
\begin{remark}[Circularity of the de~Sitter temperature]\label{rem:circularity}
The de Sitter temperature $T_{\mathrm{dS}} = H/(2\pi)$ depends on $H$, which in turn depends on $\Lambda_{\mathrm{eff}}$, which is determined by the minimum of $\Phi$ that contains $T_{\mathrm{dS}}$. This circularity is resolved by noting that $\Phi(\rho)$ is to be understood as a fixed-point equation: the self-consistent solution $\rho_*$ satisfying $\partial_\rho \Phi|_{\rho_*} = 0$ with $T_{\mathrm{dS}}(\rho_*) = H(\rho_*)/(2\pi)$ exists and is unique by the strict convexity of $\Phi$ (Theorem~\ref{thm:main}, Step~2).
\end{remark}
...
\begin{enumerate}
  ...
  \item \textbf{Full stability analysis:} The de~Sitter stability
        ...
        future work.
  \item \textbf{Dimensional homogeneity of the functional:}
        The three terms under the integral
        in~\eqref{eq:Phi} carry different engineering dimensions
        (see Remark~2.2). Although the stationarity condition
        $\dd\Phi/\dd\rho = 0$ involves only the ratio~$B/C$
        where the dimensional factors cancel, the \emph{absolute}
        value of~$\Phi(\rho)$ is not well-defined without
        specifying the implicit coupling constants explicitly.
        A fully dimensionless formulation---e.g., in terms
        of~$\rho/\rho_{\mathrm{Pl}}$ and~$k/M_{\mathrm{Pl}}$---is
        necessary to make the variational principle self-contained
        and to verify that no hidden scale dependence enters the
...
This perspective is closely related to the Running Vacuum Model
(RVM) of Sol\`{a} and
collaborators~\cite{ShapiroSola2009,SolaPeracaula2022}, who derive
$\delta\rho_{\mathrm{vac}} \sim \nu_{\mathrm{eff}}\,M_{\mathrm{Pl}}^2\,H^2$
from the RG running of the cosmological constant.  Our approach
differs in that we motivate the running from a thermodynamic
variational principle rather than from loop calculations, but
the scaling structure is analogous.
...
\begin{remark}
Proposition~\ref{prop:beta-Lambda} is a \emph{consistency check},
not an independent derivation.  It presupposes the conjectured
renormalised scaling
$\rho_\ast^{\mathrm{ren}} \sim H^2 M_{\mathrm{Pl}}^2/\ln(M_{\mathrm{Pl}}/H)$
(cf.\ the conditional status of Theorem~\ref{thm:main}) and
computes its logarithmic derivative---the resulting beta function
is therefore a \emph{tautological consequence} of the postulated
scaling, not evidence for it.  It does show that the sign and
magnitude are internally consistent, but provides no independent
support for the scaling itself.
...
without adding a separate scalar-mass fine-tune beyond the
gravitational-sector assumptions.  If, however, non-gravitational couplings were
...
\subsection{Renormalisation-group interpretation of \texorpdfstring{$\Phi$}{Phi}}
...
This is a monotonicity property reminiscent of Zamolodchikov's
$c$-theorem, though it follows from simpler statistical-mechanical
arguments (the data-processing inequality for the KL divergence)
rather than the 2D conformal field theory machinery of the
original theorem.
\end{proposition}
...
\begin{theorem}[Cosmological RFEP fixed-point universality -- conditional]
\label{thm:rfep-fixed-point-universality}
Let $\Phi[\rho;\,\mu]$ be a running free-energy functional satisfying:
\begin{enumerate}
\item[\textup{(C1)}] \emph{IR cutoff by causal structure:} the infrared scale $k_{\mathrm{IR}}(\mu)$ is bounded below by the Hubble parameter $H(\mu)$;
\item[\textup{(C2)}] \emph{Positive gravitational feedback:} the self-interaction term satisfies $V[\rho] \geq c\,G\rho^2/k_{\mathrm{IR}}^2$ for some $c > 0$;
\item[\textup{(C3)}] \emph{Entropic regularisation:} $\Phi$ contains a KL-divergence term $D_{\mathrm{KL}}(\rho\|\rho_0)$ ensuring strict convexity.
\end{enumerate}
Then the RG flow $\mu \mapsto \Phi[\rho^*(\mu);\,\mu]$ possesses a non-zero IR fixed point with
\[
\Leff \;\sim\; \frac{H_0^2}{\ln(M_{\mathrm{Pl}}/H_0)} \;\neq\; 0.
\]
In particular, conditions \textup{(C1)--(C3)} make $\Leff = 0$ non-generic within this IR-selection model: a vanishing cosmological constant would require an additional cancellation or boundary condition not supplied by the stated flow assumptions.
\end{theorem}

\begin{proof}[Proof sketch]
By (C3), $\Phi$ is strictly convex, so $\rho^*(\mu)$ exists and is unique at each scale. By (C2), the gravitational term is a relevant perturbation (dimension~2 in 4D) that grows under coarse-graining $\mu \to 0$. By (C1), the flow cannot proceed below $\mu = H_0$, fixing the IR endpoint. The monotonicity $\mu\,\frac{d}{d\mu}\Phi \leq 0$ (data-processing inequality for KL divergence, cf.\ Proposition~\ref{prop:rg-fixed-points}) ensures convergence to a fixed point. At the fixed point, the balance between entropic penalty and gravitational feedback gives $\rho^* \sim H_0^2/(8\pi G \ln(M_{\mathrm{Pl}}/H_0))$, hence $\Leff = 8\pi G\rho^* \neq 0$.

\smallskip\noindent
\emph{Note:} The scaling $\rho^* \sim H_0^2/(8\pi G\,\ln(M_{\mathrm{Pl}}/H_0))$
relies on the same renormalisation-group matching argument as
Theorem~\ref{thm:main} and carries the same ``conditional'' status:
it is conjectured, not derived from the raw mode sum alone.
\end{proof}

\begin{remark}[UV-completion independence]
\label{rem:uv-independence}
Conditions (C1)--(C3) are imposed at the \emph{IR} end of the RG flow. A UV completion---string landscape, asymptotic safety \`{a} la Wetterich~\cite{Wetterich2024}, loop quantum gravity---would have to reproduce an IR flow satisfying these conditions for the present mechanism to apply.  This suggests a possible universality class behind $\Leff \sim H_0^2/\ln(M_{\mathrm{Pl}}/H_0)$, but it is not derived here as a theorem about all UV-complete quantum-gravity theories.  In this paper, the cosmological constant problem is reduced to an IR-selection problem once the RG-matching assumption is granted; the UV matching itself remains an explicit open gate.
\end{remark}

\begin{remark}[RG time as cosmic time]
\label{rem:rg-cosmic-time}
The identification $\mu \leftrightarrow H(t)$ maps the RG flow to
cosmological evolution: coarse-graining ($\mu$ decreasing) corresponds to
expansion ($H$ decreasing). Under this identification, the Friedmann
equation $H^2 = 8\pi G\rho/3$ can be read as both a background evolution
equation and an IR-flow constraint, with
$\Leff(t) \sim H(t)^2/\ln(M_{\mathrm{Pl}}/H(t))$ tracking the Hubble rate
adiabatically. This reframes the coincidence problem: $\Leff$ and $H^2$
are comparable because the assumed RG matching ties the selected vacuum
branch to the same IR scale. The beta function
$\beta_\Lambda = \mu\,\partial\Lambda/\partial\mu$ is therefore a
formal analogue of the Friedmann flow, not an independently derived
identity.
\end{remark}

\subsection{Predictions: Slow evolution of \texorpdfstring{$\Leff$}{Lambda eff}}
...
The variational result of Theorem~\ref{thm:main} admits a complementary thermodynamic interpretation through the Maximum Entropy Production Principle (MEPP)~\cite{MartyushevSeleznev2006}. Among all kinematically admissible vacuum energy densities, the realised value~$\rho_\ast$ may be interpreted as the one that maximises the rate of entropy production~$\dot{S}$ over cosmological time scales. This selection criterion is consistent with our variational bound: the minimum of the free-energy functional~$\Phi(\rho)$ may be identified with the maximum of~$\dot{S}$, because the entropy term in~$\Phi$ acts as a thermodynamic penalty that suppresses both excessively large and vanishingly small vacuum energies. At the saddle-point, the de~Sitter horizon radiates at temperature $T_{\mathrm{dS}} = H/(2\pi)$, and the associated Gibbons--Hawking entropy $S_{\mathrm{dS}} = \pi c^5/(G\hbar\,H^2)$ reaches a quasi-stationary maximum---the universe expands just fast enough to maximise the accessible phase-space volume.

From this perspective, accelerated expansion is interpreted as a
candidate thermodynamic selection effect rather than as a pure
coincidence. Once stellar nucleosynthesis exhausts the available baryonic
free energy, cosmological expansion becomes a dominant remaining channel
for entropy production. The vacuum energy density
$\rho_\ast \sim H_0^2/G$ is then the value at which the horizon entropy
production rate is stationary within the model, subject to the constraint
that~$\rho$ be sustained self-consistently by the gravitational
back-reaction term~\eqref{eq:Vgrav}. This offers a possible physical
interpretation of the selected scale; it does not by itself replace the
open RG-matching proof.

In this reading, the coincidence problem is softened rather than solved
outright: the vacuum energy tracks the expansion scale because the
infrared cutoff $\LIR = H$ couples the vacuum sector to the expansion
history. Whether this adiabatic tracking survives a full perturbative and
observational treatment remains one of the follow-up tests.
...
\begin{proposition}[Thermodynamic selection of $\Lambda_{\mathrm{eff}}$ -- conditional on (C2)]
...
\item[\textup{(C2)}] \emph{Minkowski subtraction:} The bare cosmological constant $\Lambda_0 \sim M_{\mathrm{Pl}}^4$ is absorbed by renormalisation: $\Phi[\rho] = \Phi_{\mathrm{ren}}[\rho - \rho_0]$ where $\rho_0$ is the Minkowski vacuum. \textbf{Warning:} This condition assumes that a consistent renormalisation scheme exists in which the Minkowski vacuum has $\Lambda = 0$. In the Standard Model, this is a \emph{physical input} (renormalisation condition), not a derived result. Condition~(C2) therefore \emph{presupposes} a solution to the classical cosmological constant problem at the level of UV subtraction.
...
The proposition is conditional: it shows that \emph{if} the UV problem (C2) is resolved, then the IR dynamics uniquely select the observed order of magnitude. It does not solve the UV problem itself. Note that the logical content is modest: given (C2), the scaling $\Lambda_{\mathrm{eff}} \sim H_0^2$ follows essentially from dimensional analysis in the renormalised theory plus (C3). The non-trivial open question is establishing (C2) itself.
\end{proposition}
...
\smallskip\noindent
\textbf{Caveat regarding (C2).}\;
Condition~(C2) assumes that the bare cosmological constant
$\Lambda_0 \sim M_{\mathrm{Pl}}^4$ ``is absorbed by
renormalisation''---but this absorption is precisely the step that
constitutes the cosmological constant problem in its classical form.
In the Standard Model, the Minkowski vacuum does \emph{not} have
$\Lambda = 0$; that assignment is a \emph{physical
renormalisation condition}, not a derived result.
Condition~(C2) therefore encodes the assumption that a
consistent renormalisation scheme exists in which the
Minkowski vacuum has vanishing cosmological constant.
This is a necessary input, not a consequence of the
thermodynamic selection mechanism.
The theorem should be read as: \emph{given} that (C2) can be
imposed consistently, the IR dynamics select
$\Lambda_{\mathrm{eff}} \sim H_0^2$.
A proof that (C2) is self-consistent (i.e., that no
obstruction prevents setting $\Lambda_{\mathrm{Mink}} = 0$)
...
\begin{enumerate}
  ...
  \item \textbf{Connection to the MOND acceleration scale.}\;
        ...
        10^{-10}\;\mathrm{m\,s^{-2}}$ to within 20\%.
        This numerical coincidence is \emph{suggestive but not
        derived from the model}: the relation $a_0 \sim cH_0$
        is a generic dimensional consequence of any cosmological
        acceleration scale and does not, by itself, establish a
        causal connection between dark energy and MOND dynamics.
        The factor $2\pi$ from the de~Sitter temperature
        $T_{\mathrm{dS}} = \hbar\,a_{0}/(2\pi c\,k_{\mathrm{B}})$
        narrows the agreement but a physical mechanism linking the
        P\"oschl--Teller scalar field to galactic-scale modified
        dynamics remains to be established.
...
\mu^2 &= H_0^2 \Omega_m,\qquad
\frac{c_1}{c_2} = \frac{6\Omega_\Lambda}{\Omega_m}.
\end{aligned}
\]
...
\small
\setlength{\tabcolsep}{5pt}
\begin{tabularx}{0.97\textwidth}{>{\raggedright\arraybackslash}p{0.46\textwidth}|Y}
\textbf{Hu--Sawicki} & \textbf{FST functional $\Phi[\rho]$} \\
\hline
\(
\begin{aligned}[t]
f_{R0} &= -n\, c_1 / c_2^2\\
&\quad \times (H_0^2 \Omega_m / R_0)^{n+1}
\end{aligned}
\) & Coupling strength $\gamma_{\mathrm{eff}}$ \\
...
\textbf{Unified $\eta = 5/4$ prediction.}\;
The quasi-static modified Poisson equations~\eqref{eq:Poisson-Phi}--\eqref{eq:Poisson-Psi} with coefficients $1/3$ (for~$\Phi_N$) and $2/3$ (for~$\Psi$) are \emph{universal} for all metric $f(R)$ theories---they follow from the linearised trace equation and are independent of the specific functional form of~$f(R)$ (cf.~Hu \& Sawicki 2007, eq.~12--13; Sotiriou \& Faraoni 2010, Sec.~V.B).  The Hu--Sawicki model differs from the quadratic $R + \gamma R^2$ theory only in the \emph{density dependence} of the scalaron mass $m_s^2(\rho)$, not in the Poisson-equation coefficients.  In the sub-Compton limit ($k \gg a\,m_s$), both yield $\eta = (5/3)/(4/3) = 5/4$.  The crucial advantage of Hu--Sawicki is that $m_s(\rho)$ grows with ambient density, providing chameleon screening in the solar system while preserving $\eta = 5/4$ on cosmological sub-Compton scales.

\smallskip\noindent
\textbf{Regime guardrail.}\;
The value $\eta = 5/4$ is a quasi-static, linear-perturbation, unscreened sub-Compton statement: $k/a \gg m_s(z,\rho)$, outside the screened high-density regime and outside the super-Compton GR-recovery regime.  It is not a solar-system prediction and not a background-level dark-energy fit parameter.

\smallskip\noindent
\textbf{Erratum (v1).}\;
An earlier version of this remark stated that the Hu--Sawicki model
yields $\eta \to 4/3$ due to different Poisson-equation coefficients.
This was incorrect: the coefficients $1/3$ and $2/3$ are universal
for all $f(R)$ theories.  The coefficient ratio giving $\eta = 5/4$
applies uniformly across the linearised unscreened sub-Compton $f(R)$
regime, including Hu--Sawicki.
\end{remark}

\subsection{Exploratory MCMC Scan for Hu--Sawicki Parameters}\label{sec:mcmc-results}

We report an exploratory Markov chain Monte Carlo (MCMC) likelihood scan of the Hu--Sawicki $f(R)$ parametrisation, not a production cosmological parameter inference.  The scan combines simplified diagonal Planck~2018 (TT/TE/EE) and DESI~DR2 (BAO) terms with a Cassini hard prior on $|\gamma_{\mathrm{PPN}}-1|$.  The sampler employs 32 walkers over 3\,000 steps with a 500-step burn-in, yielding 80\,000 post-burn-in samples.  These samples should be read as posterior-like stress-test samples, not as independent constraints from the official Planck/DESI likelihoods.  Table~\ref{tab:mcmc-results} summarises the sampled parameters and two derived quantities.

The best-fit $\chi^2 = 6811.4$ for a combined data set with
$N_{\mathrm{data}} \approx 7200$ effective data points
(Planck TT/TE/EE $\sim 7000$ $+$ DESI BAO $\sim 12$ $+$ Cassini
$1$) gives $\chi^2/N_{\mathrm{data}} \approx 0.95$, indicating an
acceptable fit.  The modified-gravity amplitude
$\alpha_{M,0} = 1.5 \times 10^{-5}\;({}^{+2.0 \times 10^{-5}}_{-1.0
\times 10^{-5}})$ is consistent with zero at $< 2\sigma$.  This
means that the data \emph{do not require} a departure from GR; the
GR limit ($\alpha_{M,0} = 0$) lies within the $2\sigma$ posterior.
The honest interpretation is that current data are compatible with
both GR and the mild FST modifications, but do not favour FST over
$\Lambda$CDM.  The steepness parameter $n_{\exp} \approx 0.71$ favours moderate nonlinearity, broadly consistent with the $n = 1$ Hu--Sawicki benchmark.  The standard cosmological parameters---cold dark matter density $\omega_{\mathrm{cdm}}$, primordial amplitude $\ln(10^{10}A_s)$, and spectral index $n_s$---are all consistent with the Planck~2018 best-fit values, confirming that the Hu--Sawicki extension does not distort the well-constrained $\Lambda$CDM sector.

\smallskip\noindent
\textbf{Methodological caveats.}\;
(i)~The likelihood is deliberately simplified: it uses diagonal CMB and BAO summary terms rather than the official Planck and DESI likelihood pipelines, and it treats the Cassini condition as a hard prior rather than a full screened-scalar calculation.
(ii)~Chain convergence has not been verified by
Gelman--Rubin ($\hat{R}$) or Geweke diagnostics; the 500-step
burn-in and 3\,000-step chain length should be treated as
preliminary.  A production-quality analysis would require
$\hat{R} < 1.01$ for all parameters and an effective sample size
$n_{\mathrm{eff}} > 1000$.
(iii)~The posterior $\alpha_{M,0} \approx 0$ is equally
consistent with GR ($\alpha_{M,0} = 0$) and with the FST
prediction ($\alpha_{M,0} > 0$); the data do not currently
discriminate between the two.  A Bayesian model comparison
(Bayes factor $\Lambda$CDM vs.\ Hu--Sawicki) would quantify
this more precisely but is beyond the scope of this skeleton
draft.
(iv)~The best-fit $|f_{R0}| = 1.2 \times 10^{-5}$ is above
the commonly cited cosmological upper bound
$|f_{R0}| < 6 \times 10^{-5}$ (Cataneo et al.\ 2015, 95\% CL
from cluster abundances) but may be in tension with the Cassini-derived
bound $|f_{R0}| \lesssim 10^{-6}$ in the linearised regime.
The compatibility of our best-fit value with solar-system constraints
relies on the nonlinear chameleon mechanism at this specific $f_{R0}$,
which has not been explicitly verified here.
(v)~Prior sensitivity: the flat prior $\alpha_{M,0} \in [0, 0.01]$
includes GR ($\alpha_{M,0} = 0$) at the boundary; a prior excluding
zero would produce a qualitatively different posterior.

\begin{remark}[$\Lambda$CDM comparison]\label{rem:lcdm-comparison}
The best-fit $\chi^2 = 6811.4$ for the Hu--Sawicki model should be compared with the $\Lambda$CDM baseline. For the same data combination (Planck 2018 TT/TE/EE + DESI DR2 BAO), $\Lambda$CDM achieves $\chi^2 \approx 6810$--$6815$ (depending on the exact likelihood implementation). The Hu--Sawicki model with its additional parameters ($\alpha_{M,0}$, $n_{\exp}$) therefore does \emph{not} improve the fit---the extra parameters are unconstrained, consistent with GR ($\alpha_{M,0} = 0$ within $2\sigma$). This is explicitly acknowledged: the data do not currently require modified gravity.
\end{remark}
...
\caption{Exploratory Hu--Sawicki MCMC scan from simplified Planck~2018 + DESI~DR2 + Cassini terms (80\,000 post-burn-in samples, 32 walkers, 3\,000 steps).}
\label{tab:mcmc-results}
\resizebox{\textwidth}{!}{%
\begin{tabular}{lcccc}
\hline
Parameter & Median & $+1\sigma$ & $-1\sigma$ & Prior \\
\hline
$\alpha_{M,0}$ & $1.5\times10^{-5}$ & $+2.0\times10^{-5}$ & $-1.0\times10^{-5}$ & $[0,\; 0.01]$ \\
$n_{\exp}$ & $0.711$ & $+0.576$ & $-0.319$ & $[0.1,\; 4]$ \\
$\omega_{\mathrm{cdm}}$ & $0.12005$ & $+0.00031$ & $-0.00030$ & $[0.10,\; 0.14]$ \\
$\ln(10^{10}A_s)$ & $3.0447$ & $+0.0020$ & $-0.0019$ & $[2.9,\; 3.2]$ \\
$n_s$ & $0.9654$ & $+0.0024$ & $-0.0025$ & $[0.9,\; 1.0]$ \\
\hline
$|f_{R0}|$ & \multicolumn{3}{c}{$1.2\times10^{-5}\;({}^{+7.9\times10^{-6}}_{-7.9\times10^{-6}})$} & derived \\
$|\gamma_{\mathrm{PPN}}-1|$ proxy & \multicolumn{3}{c}{$1.2\times10^{-5} < 2.3\times10^{-5}$} & exploratory prior \\
\hline
\end{tabular}
}
\end{table}

The derived scalar-field amplitude $|f_{R0}| = 1.2 \times 10^{-5}$ passes only the linear proxy $|\gamma_{\mathrm{PPN}}-1| < 2.3 \times 10^{-5}$ used in this exploratory likelihood.  It does \emph{not} close the solar-system viability question, because Lemma~\ref{lem:rg-viability} uses the conservative guardrail $|f_{R0}| \leq 10^{-6}$ and the chameleon thin-shell condition has not been explicitly verified for this point.  The MCMC scan is therefore evidence for background-level compatibility, not a proof of Cassini-safe screening.

The small but non-zero $|f_{R0}|$ leaves room for the regime-limited FST target $\eta = 5/4$ on unscreened quasi-static sub-Compton scales (cf.\ Remark~\ref{rem:hu-sawicki-fst}), while the solar-system screening gate remains open until the thin-shell calculation is supplied.  These results show only that the simplified Planck+DESI+Cassini stress test is compatible with the FST/$f(R)$ framework and does not exclude mild modifications, but it does not prefer them over $\Lambda$CDM.
...
\emph{Holographic entropy and MOND.}\; Sevin\c{c}, \"Okc\"u, and
Aydiner~\cite{MONDEntropy2025} derive modified Friedmann equations from
...
\emph{Finsler gravity.}\; Pfeifer et al.~\cite{FinslerJCAP2025} demonstrate that
...
\emph{Informational persistence.}\; As a non-peer-reviewed Zenodo preprint,
Hunt~\cite{HuntMOND2026} interprets dark-matter phenomenology as the survival
of curvature information under coarse-graining.  This is useful only as a
low-weight conceptual comparator for information-theoretic modified-gravity
language, not as empirical support for the CRM.

\subsection{Post-Zookeeper transfer: DESI DR2, JWST and TDCOSMO as a data overlay}
\label{subsec:zookeeper-crm-transfer}

The post-Zookeeper role of the present framework is to make the CRM
observable rather than merely structural.  The next version should add a
data-overlay layer that maps CRM parameters directly to the standard
late-time descriptors used by current surveys.

Relative to the current CORE status, the Zookeeper closure of
Riemann-$C2/MS2$ is not a cosmology theorem.  CRM inherits only the
RFEP~v1.8 adapter normal form: operator/form, defect, approximation, gap,
and limiting passage must be made explicit in the Dark-Energy domain
itself.  The open work therefore remains the CRM-to-CPL map, the joint
DESI/JWST/TDCOSMO overlay, and the Cassini/BBN screening guardrails.

The transfer slots are:
\begin{itemize}
  \item \textbf{Operator/form.} The Flow-Zeta or RG operator for $w(z)$,
        $H(z)$, and $\Lambda_{\mathrm{eff}}$.
  \item \textbf{Defect.} The joint residual against DESI BAO, CMB, local
        Cepheid/SN measurements, strong-lensing time delays, UV matching,
        and screening constraints.
  \item \textbf{Approximation.} CPL $(w_0,w_a)$, Hu--Sawicki parameter
        grids, MCMC samples, and redshift-bin likelihoods.
  \item \textbf{Gap.} RFEP fixed-point curvature, de Sitter stability, and
        the screening viability margin.
  \item \textbf{Limit.} Background-only fits $\to$ perturbations/CMB/BBN
        consistency $\to$ local-gravity tests such as Cassini.
\end{itemize}

In Zookeeper/RFEP language this is a residual-over-gap programme.  The
paper proves the convex existence/uniqueness part of the free-energy
selection mechanism and leaves the RG-matching and screening gates
explicitly open; a future data version should additionally display the estimate
\[
  \|(I-P_{\mathrm{CRM}})\Theta_{\mathrm{data}}\|
  \lesssim
  \frac{r_{\mathrm{DESI}}+r_{\mathrm{local}}+r_{\mathrm{screen}}}
       {g_{\mathrm{dS}}} ,
\]
where the numerator collects survey, local-distance and screening
residuals, while $g_{\mathrm{dS}}$ denotes the curvature of the selected
de~Sitter/free-energy branch.  This is a bookkeeping target, not a new
claim already established in the present version.

Concretely, the next technical target is a CRM-to-CPL map
\[
  \Theta_{\mathrm{CRM}}
  \longmapsto
  (w_0,w_a,H_0^{\mathrm{eff}},\Omega_m,\Omega_{\mathrm{DE}}),
\]
followed by a comparison table for the main DESI DR2 combinations:
\begin{itemize}
  \item DESI+CMB
  \item DESI+CMB+Pantheon+
  \item DESI+CMB+Union3
  \item DESI+CMB+DESY5
\end{itemize}
The same table should also record whether the parameter point remains
compatible with JWST/Cepheid local-distance results, TDCOSMO
strong-lensing constraints, BBN protection of the sound horizon, and the
Cassini bound on post-Newtonian deviations.

This reframes the claim.  The model should no longer merely state that
evolving dark energy is qualitatively compatible with CRM.  It should
identify a CRM parameter window that simultaneously fits the DESI DR2
preference for dynamical dark energy, stays compatible with the
local-$H_0$ triangle, and does not violate BBN or Solar-System screening.
...
\item \emph{Different:} DVFT lacks a thermodynamic/entropy argument; the FST functional $\Phi[\rho]$ is derived from the RFEP (a variational principle), not postulated. DVFT does not predict a specific value of $\eta$; FST targets $\eta = 5/4$ on quasi-static, linear, unscreened sub-Compton metric-$f(R)$ scales.
\end{itemize}
...
\item \emph{Sub-Compton and screening regime:} For $k < a m_s$, or in screened high-density regions where the scalar degree of freedom decouples, the target $\eta = 5/4$ breaks down and $\eta \to 1$ (GR recovery). The transition scale $k_{\mathrm{cross}} = a m_s$ depends on $\gamma$ and on the ambient-density dependence of $m_s$.
\item \emph{Renormalisation scheme:} The precise value of $\gamma_0$ is scheme-dependent at the $O(10\%)$ level. The coefficient ratio producing $\eta = 5/4$ is stable only within the linearised metric-$f(R)$ sub-Compton approximation, not across arbitrary nonlinear screening regimes.
\end{enumerate}
\end{remark}
...
For a comprehensive review of the asymptotic safety programme and its compatibility with Standard Model matter content, see~\cite{Eichhorn2020}.
\end{remark}
...
\textbf{Speculative (requires complex scalar field).}\;
For a real scalar field $\phi: M \to \mathbb{R}$, the fundamental
group $\pi_1(\mathbb{R}) = 0$ implies that the winding number
$\oint \nabla\phi \cdot d\ell / (2\pi)$ vanishes identically by
Stokes' theorem for smooth configurations.  Non-trivial winding
numbers require either a complex scalar field ($\pi_1(U(1)) = \mathbb{Z}$)
or a field with singularities.  If the dark-energy sector were
extended to include a complex scalar field, the resulting winding
numbers could provide an Aharonov--Bohm-type screening mechanism
independent of the scalaron mass.  This possibility remains
speculative and requires a significant extension of the model.
\end{remark}
