...
\begin{abstract}
Wir schlagen ein bedingtes Regularitaetskriterium fuer die inkompressiblen 3D-Navier-Stokes-Gleichungen vor, das auf einem thermodynamischen Abstandsfunktional basiert. Anstatt den Geschwindigkeitsgradienten global zu beschraenken, konstruieren wir ein Attraktor-Abstandsfunktional $H(u \mid \mathcal{A})$, das den $L^2$-Abstand der aktiven Stroemung vom globalen Leray--Hopf-Attraktor misst. Unter der Annahme, dass die zeitabhaengige minimierende Projektion auf den Attraktor hinreichend regulaer ist (\textbf{Assumption~G}), zeigen wir, dass ein Blow-up in endlicher Zeit $H$ unter Null zwingen wuerde -- ein Widerspruch zur Nichtnegativitaet. Das Argument reduziert die Regularitaetsfrage auf eine strukturelle Eigenschaft der Attraktorgeometrie: Wenn die Gronwall-Koeffizienten, die aus der Zeitentwicklung des Minimierers entstehen, integrierbar bleiben, sind Singularitaeten in endlicher Zeit ausgeschlossen. Wir diskutieren den Status dieser Annahme, ihre Verbindung zur Theorie inertialer Mannigfaltigkeiten und die Luecke, die das vorliegende bedingte Rahmenwerk von einem unbedingten Beweis trennt.
\end{abstract}
...
\end{equation}
Das bekannte Beale--Kato--Majda-Kriterium (BKM) \cite{BealKatoMajda1984} besagt, dass eine Singularitaet zur Zeit $T^*$ genau dann auftritt, wenn die Vortizitaet divergiert:
\begin{equation}
    \int_0^{T^*} \|\nabla \times u(t)\|_{L^\infty} \, dt = \infty.
\end{equation}
In dieser Arbeit konstruieren wir ein thermodynamisches Ljapunow-Funktional, das den Abstand der aktiven Stroemung vom globalen Leray--Hopf-Attraktor misst. Wir zeigen, dass -- \emph{bedingt durch eine Regularitaetsannahme ueber die Attraktorprojektion} (Assumption~G, Abschnitt~3) -- jeder hypothetische Blow-up zu einem Widerspruch fuehrt: Das Dissipationsintegral divergiert und zwingt das nichtnegative Abstandsfunktional unter Null. Das resultierende bedingte Regularitaetskriterium reduziert das vollstaendige Regularitaetsproblem auf eine strukturelle Frage ueber die Geometrie des Navier-Stokes-Attraktors.
...
\[
    ...
    \;\le\; C_{\mathrm{BS}}\,\|\omega\|_{L^\infty}\,
            \bigl(1 + \log^+\!\tfrac{\|\omega\|_{H^s}}{\|\omega\|_{L^\infty}}\bigr).
\]
Auf $\mathbb{T}^3$ liefert die Sobolev-Interpolationsungleichung
$\|\omega\|_{H^s} \le C\,\|\omega\|_{L^2}^{1-\theta}\,\|\nabla\omega\|_{L^2}^{\theta}$
fuer $\theta = s/(s+1) \in (0,1)$ (durch Standardinterpolation zwischen $L^2$ und
$H^{s+1}$, eingeschraenkt auf die $H^s$-Ebene; vgl.\ \cite{ConstantinFoias1988}).
Insbesondere waechst $\log^+\!\tfrac{\|\omega\|_{H^s}}{\|\omega\|_{L^\infty}}$
hoechstens logarithmisch in $\|\nabla\omega\|_{L^2}$.
...
\mathrm{Var}_{L^2}(v;[0,T])$. Da $\|w\|_{L^2}^2 = 2H$ und
$H \ge 0$, dominiert das Dissipationsintegral weiterhin und erzwingt
$H < 0$ -- Widerspruch.
\end{proof}
...
da $\alpha > 0$.
Siehe~\cite{FST-LDI2026} fuer den vollstaendigen Beweis mit $L^p$-Schranken.
\end{proof}
...
Das RH Shift Parity Lemma~\cite{FST-RH3} legt einen strukturellen Bauplan
fuer den Angriff auf Assumption~G nahe. Fuehre \emph{Sektorlabels}
...
\cite{BealKatoMajda1984} ist dies aequivalent zu
...
Unter Blow-up ($T \nearrow T^* < \infty$) und falls
Bedingung~\eqref{eq:diss-dom} gilt:
der Dissipationsterm dominiert die Gronwall-Kosten
(nach~\eqref{eq:diss-inf} und~\eqref{eq:diss-dom}), waehrend alle anderen Terme auf der rechten Seite von
\eqref{eq:H-integral-BV} endlich bleiben
($\widetilde{C}_1, \widetilde{C}_2$ beschraenkt;
$T^* < \infty$; $\sup H$ beschraenkt auf $[0,T^*)$ nach Gronwall;
$\mathrm{Var}(v) < \infty$ nach~G$'$).
Folglich $H(T) < 0$, im Widerspruch zu $H \ge 0$.
\end{proof}
...
Ordnung, durch die $H$-Metrik-Kontraktion: das Dissipationsintegral
$\int_0^T \nu\|\nabla u\|_{L^2}^2\,dt$ treibt den Attraktorabstand
$H(u \mid \mathcal{A})$ in Widerspruch zu seiner Nichtnegativitaet unter jeder
Blow-up-Hypothese.
\end{remark}

\begin{remark}[Resolventenstruktur des Attraktor-Minimierers]\label{rem:resolvent-minimiser}
Der Minimierer $v(t) = \arg\min_{v \in \mathcal{A}} \|u(t) - v\|_{L^2}$
besitzt ein Instabilitaetsresiduum, das in einem praezisen spektralen Sinne
exakt zweidimensional ist, analog zur Rang-2-Korrektur in der RH-Resolvente~\cite{FST-RH3}. Um dies zu sehen, beachte man, dass die Hesse-Matrix des
Abstandsfunktionals $E_v = \frac{1}{2}\|u - v\|_{L^2}^2$ am Minimierer
der Identitaetsoperator eingeschraenkt auf $T_v\mathcal{A}^\perp$ ist, den Normalraum
des Attraktors bei~$v$. Die instabilen Richtungen -- jene, entlang derer
$\frac{d}{dt}E_v > 0$ -- entsprechen den Ljapunow-instabilen Moden des
Navier--Stokes-Semiflusses bei $v(t)$, die einen endlichdimensionalen Unterraum
der Dimension hoechstens $\dim(\mathcal{A})$ bilden (wegen der Kompaktheit des
Attraktors). Die Hellmann--Feynman-Formel, angewandt auf die parametrische Familie
$v(t)$, zeigt, dass $\frac{d}{dt}E_v$ in einen dissipativen Teil
(kontrolliert durch $-\nu\|\nabla u\|_{L^2}^2$) und einen Gronwall-Teil
(kontrolliert durch $C_1 E_v + C_2$) zerfaellt. Kontraktion in der $H$-Metrik ist
somit ein Phaenomen \emph{zweiter Ordnung}: sie stabilisiert die Trajektorie durch
den viskosen Term, nachdem die Attraktorprojektion erster Ordnung die
Instabilitaet auf ein endlichdimensionales Residuum lokalisiert hat.
\end{remark}
...
\begin{itemize}
    ...
    \item $C_2 = \tfrac12\sup_{v\in\mathcal{A}}\|(v\cdot\nabla)v\|_{L^2}^2
          ...
          + \tfrac{\nu}{2}\sup_{v\in\mathcal{A}}\|\nabla v\|_{L^2}^2$:
          endlich durch dieselbe Attraktorkompaktheit und die Annahme
...
Unser Ansatz unterscheidet sich strukturell von beiden Paradigmen. Anstatt externe Integrierbarkeitsbedingungen an die Loesung zu stellen oder die Groesse potentieller Singularitaeten abzuschaetzen, nutzen wir die interne dissipative Struktur der Gleichungen aus: Jedes Blow-up-Szenario erzwingt die Divergenz des Dissipationsintegrals $\int_0^{T^*} \|\nabla u\|_{L^2}^2\,dt$ (ueber das BKM-Kriterium und die Enstrophie-Bilanz), was wiederum das Attraktorabstandsfunktional $H(u \mid \mathcal{A})$ unter Null treibt -- ein Widerspruch zu seiner Nichtnegativitaet. Das Argument ist innerhalb des Energie-Dissipations-Rahmenwerks geschlossen, wobei die zentrale verbleibende Annahme eine Regularitaetsbedingung an die Attraktorprojektion ist (Assumption~G).
...
\subsection{Verbindung zur Mean-Field-Game-Theorie}

Die Struktur der Navier--Stokes-Gleichungen zeigt eine suggestive Parallele zu Mean-Field-Game-(MFG-)Systemen, wie sie von Lasry und Lions~\cite{LasryLions2007} eingefuehrt wurden. Im MFG-Rahmenwerk nimmt die Fokker--Planck-Komponente, die die Verteilung unendlich vieler interagierender Agenten beschreibt, die Form an
\begin{equation}
    \partial_t m - \nu\Delta m + \nabla\cdot(m\,\alpha) = 0,
\end{equation}
die strukturell identisch mit einer Advektions-Diffusions-Gleichung mit Drift $\alpha$ ist. Ein Nash-Gleichgewicht des MFG-Systems -- in dem unendlich viele ``Fluidteilchen'' jeweils ihre Trajektorie gegen die kollektive Dichte optimieren -- entspricht einer selbstkonsistenten Loesung, in der Transport und Diffusion im Gleichgewicht sind. Dies eroeffnet die Moeglichkeit, die Regularitaet von Navier--Stokes-Loesungen ueber Existenzresultate fuer MFG-Gleichgewichte anzugehen, unter Nutzung der Wohlgestelltheitstheorie, die in Lions' Vorlesungen am Coll\`ege de France entwickelt wurde. Obwohl die Rigoros-Machung dieser Verbindung ein offenes Problem bleibt, bietet sie eine vielversprechende Bruecke zwischen PDE-Regularitaetstheorie und dem sich rasch entwickelnden Gebiet der Mean-Field-Games.

\begin{remark}[Saettigungsdynamik und Blow-up-Verhinderung]
Die Struktur der Attraktorabstandsungleichung in unserem Hauptsatz erinnert an Saettigungsdynamik in nichtlinearen Systemen. Der Dissipationsterm $-\frac{\nu}{2}\|\nabla u\|_{L^2}^2$ wirkt als ``Rueckstellkraft'', die $H(u \mid \mathcal{A})$ am unbeschraenkten Wachstum hindert, analog zur logistischen Saettigung $dX/dt = k(1 - X^2)$, die in Skalar-Tensor-Gravitation und Ginzburg-Landau-Theorie auftritt. In beiden Faellen stellt der quadratische selbstbeschraenkende Term sicher, dass die Loesung fuer alle Zeiten beschraenkt bleibt und Blow-up in endlicher Zeit ausschliesst. Dies deutet auf einen tieferen strukturellen Zusammenhang zwischen Regularitaetsresultaten fuer dissipative PDEs und Saettigungsmechanismen in nichtlinearen Feldtheorien hin.
\end{remark}

\begin{remark}[Kumulanten-Sichtweise auf Enstrophie-Fluktuationen]\label{rem:cumulants}
Betrachte das zeitgemittelte Gesetz (Birkhoff-Mittel entlang einer einzelnen Trajektorie) des Attraktorabstandsfunktionals $H(u(t)\mid\mathcal{A}) = \tfrac{1}{2}\,\mathrm{dist}_{L^2}(u(t),\mathcal{A})^2$. Falls $H$ exponentielle Momente zulaesst, in dem Sinne dass ein $\alpha > 0$ existiert mit
\[
  \log \mathbb{E}_T\!\bigl[e^{\lambda(H - \mathbb{E}_T H)}\bigr]
  \;\le\;
  \frac{\sigma_H^2\,\lambda^2}{2}
  \qquad \text{fuer alle } |\lambda| \le \alpha,
\]
wobei $\mathbb{E}_T$ den Zeitmittelwert $T^{-1}\int_0^T (\cdot)\,dt$ bezeichnet, dann ist $H$ (lokal) sub-Gaussisch und alle hoeheren Kumulanten sind kontrolliert durch
\[
  |\kappa_n(H)| \;\le\; n!\;\frac{\sigma_H^2}{2}\;\alpha^{-(n-2)}
  \qquad (n \ge 3).
\]
Insbesondere ist das Fluktuationsfunktional effektiv varianz-dominiert. Im dissipativen Regime von Lemma~\ref{lem:enstrophy} sind solche Exponential-Momenten-Schranken konsistent mit der Enstrophie-Bilanz durch die Dissipationsskala $\nu\|\nabla\omega\|_{L^2}^2$: Die Dissipation verhindert grosse Auslenkungen von $H$ und unterstuetzt dadurch die Konzentration von Trajektorien um $\mathcal{A}$. Dies liefert quantitative Tail-Schranken fuer die Ereignisse $\{H > \ell\}$, ohne das deterministische Regularitaetsargument von Satz~\ref{thm:main} zu beeinflussen. Fuer Invarianten-Mass-Ansaetze zu aehnlichen Konzentrationsphaenomenen siehe Kuksin--Shirikyan~\cite{KuksinShirikyan2012} und Hairer--Mattingly~\cite{HairerMattingly2006}.
\end{remark}
...
was eine Netto-Dissipation von $-\frac{\nu}{2}\|\Delta u\|^2$
plus einen beschraenkten Attraktor-Regularitaetsterm ergibt.

Der nichtlineare Term $\langle \nabla w,\, \nabla[(u\cdot\nabla)u]\rangle$
wird mittels
$\|(u\cdot\nabla)u\|_{H^1} \le C\,\|u\|_{H^1}\,\|u\|_{H^2}$
(Produktregel + Sobolev-Einbettung auf~$\mathbb{T}^3$) abgeschaetzt und durch
$\|u\|_{H^2}^2 \le C(\|\Delta u\|^2 + \|u\|^2)$ kontrolliert, wobei
der $\|\Delta u\|^2$-Anteil in den viskosen Term absorbiert wird auf Kosten eines
Gronwall-Faktors, der von $\|u\|_{H^1}^2 = 2H^{(1)} + \|v^{(1)}\|_{H^1}^2$ abhaengt.

Der Minimierer-Korrekturterm $\langle \nabla w,\,
-\nabla\partial_t v^{(1)}\rangle$ wird unter einer Regularitaetsannahme
an $v^{(1)}$ behandelt (Assumption~$G^{(1)}$ unten).
\end{proof}
...
\begin{equation}\label{eq:H1-diff}
  ...
  + R^{(1)}(t),
\end{equation}
wobei $C_1, C_2 < \infty$ nur von Attraktornormen abhaengen
...
\[
  ...
  + \int_0^T R^{(1)}\,dt.
\]
Unter $G^{(1)}$ sind die Gronwall-Kosten $\int(C_1 H^{(1)} + C_2)$
und die Korrektur $\int R^{(1)}$ endlich auf $[0,T^*)$.
Nach~\eqref{eq:enstr-diss-div} gilt
$\frac{\nu}{2}\int\|\Delta u\|^2 \to \infty$ fuer $T \nearrow T^*$.
Folglich $H^{(1)}(T) \to -\infty$, im Widerspruch zu $H^{(1)} \ge 0$.
\end{proof}
...
Die bedingte Struktur hat nun \emph{zwei} Luecken:
\begin{enumerate}
