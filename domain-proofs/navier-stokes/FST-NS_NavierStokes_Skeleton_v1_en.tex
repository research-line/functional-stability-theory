...
This paper establishes a \emph{conditional reduction}: global regularity is equivalent to the existence of a BV-regular selection on the attractor (Assumption~G), which is addressed in the companion paper on log-distance integrability \cite{Geiger2026-LDI}.
\end{abstract}
...
\begin{proposition}[Squeezing implies TLL]\label{prop:squeezing-tll}
...
where $L_N$ is the Lipschitz constant of the graph. For points $x$ at distance $d$ from $\mathcal{A}$, the log factor arises from the cone condition: the high-mode error $\|Q_N(x - P_\mathcal{A}(x))\|$ is controlled by $d \cdot (1 + \log(R/d))$ when $\mathcal{A}$ has finite fractal dimension (Mañé projection theorem). Combining yields TLL with $C_{\mathrm{TLL}} = (1 + L_N)(1 + \dim_F(\mathcal{A})/N)$.
\end{proof}

\begin{remark}[Relevance for 3D Navier--Stokes]\label{rem:squeezing-ns}
The squeezing property is established for 2D Navier--Stokes (Foias--Temam 1984), reaction-diffusion systems, and damped wave equations. For 3D Navier--Stokes, squeezing on the attractor would follow from the \emph{determining modes} property, which is known to hold for generic forcing (Foias--Prodi 1967, Jones--Titi 1992). The gap is that determining modes control asymptotic behavior, while squeezing requires uniform-in-time contraction. Proposition~\ref{prop:squeezing-tll} shows that closing this gap would immediately yield Assumption~G via the TLL/LDI route.
\end{remark}
...
\begin{proposition}[Equivalences of G-NS]\label{prop:g-equivalences}
...
\item[(iii)] \textbf{Strong squeezing:} The squeezing property (Proposition~\ref{prop:squeezing-tll}) holds uniformly for all solutions on the attractor.
...
\end{enumerate}
The implication (i)$\Rightarrow$(iv) is classical; (iv)$\Rightarrow$(i) follows from the smoothness of the attractor and the finite-dimensionality forced by the BV regularity.
\end{proposition}
...
This paper is a \textbf{conditional reduction framework}: it reduces global regularity of 3D Navier--Stokes to a geometric property of the attractor (Selection Regularity).
...
\hline
$H(u|\mathcal{A})$ Gronwall inequality & Theorem \\
Assumption G $\Rightarrow$ regularity & Theorem \\
G' (BV) suffices for G & Proposition \\
Squeezing $\Rightarrow$ TLL $\Rightarrow$ G & Proposition \\
\hline
\multicolumn{2}{|l|}{\textit{Numerically confirmed:}} \\
\hline
BV selection on Lorenz/R\"ossler/Chen & 3/3 passed \\
Balanced viscosity convergence & Cauchy sequence \\
Condition D (dissipation bounded) & 3/3 passed \\
\hline
\multicolumn{2}{|l|}{\textit{Open (the core problem):}} \\
\hline
