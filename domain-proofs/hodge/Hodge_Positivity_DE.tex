...
\usepackage{hyperref}
\hypersetup{
    pdftitle={Die Hodge-Vermutung als Positivitaets-No-Go-Theorem: Arithmetische Stabilitaet und die Unmoeglichkeit nichtalgebraischer Hodge-Klassen},
...
\title{Die Hodge-Vermutung als Positivitaets-No-Go-Theorem:\\Arithmetische Stabilitaet und die Unmoeglichkeit\\nichtalgebraischer Hodge-Klassen}
...
\begin{abstract}
Wir schlagen einen neuen Zugang zur Hodge-Vermutung vor, der auf \textit{Positivitaetsobstruktion} statt auf Zykuskonstruktion basiert. Wir fuehren das Konzept der \textit{arithmetischen Positivitaet} ein: eine rationale $(p,p)$-Klasse $\alpha \in H^{2p}(X, \mathbb{Q})$ auf einer glatten projektiven Varietaet $X/\mathbb{C}$ heisst arithmetisch positiv, wenn sie unter allen Polarisierungen, allen Automorphismen von $\mathbb{C}$ und allen Lefschetz-Operationen gleichzeitig nichtnegativ bleibt. Wir formulieren die \textit{Arithmetische Positivitaetsvermutung}: eine rationale Hodge-Klasse ist genau dann algebraisch, wenn sie arithmetisch positiv ist. Wir beweisen, dass algebraische Klassen arithmetisch positiv sind (die ,,leichte Richtung''), etablieren Funktorialitaet des arithmetisch positiven Kegels unter Rueckzug, Vorwaertsschub und K\"unneth-Produkt und zeigen, dass die Vermutung fuer abelsche Varietaeten aequivalent zur Hodge-Vermutung ist. Die zentrale strukturelle Einsicht besteht darin, dass die Hodge-Vermutung, umformuliert als No-Go-Theorem, die \textit{Unmoeglichkeit} nichtalgebraischer Hodge-Klassen behauptet: jede rationale $(p,p)$-Klasse, die nicht algebraisch ist, muss arithmetische Positivitaet unter mindestens einer zulaessigen Operation verletzen. Wir identifizieren die praezise Obstruktion -- das Versagen absoluter Galois-Stabilitaet der Hodge-Riemann-Form -- und zeigen, dass diese Obstruktion fuer algebraische Klassen trivialerweise abwesend ist, wodurch die Hodge-Vermutung auf eine Positivitaets-Stabilitaetsaussage reduziert wird. Der Ansatz vermeidet explizite Zykuskonstruktion, vollstaendige motivische Maschinerie und mod-$p$-Liftung und nutzt stattdessen das Zusammenspiel von Hodge-Riemann-Bilinearformen, Hard-Lefschetz-Dualitaet und absoluter Hodge-Theorie. Wir diskutieren die Verbindung zu Delignes Framework absoluter Hodge-Klassen und identifizieren die verbleibende Luecke: den Beweis, dass arithmetische Positivitaet die Klasse zwingt, im Bild der Zykusklassenabbildung zu liegen.
\end{abstract}
...
Riemannsche Vermutung & Positivitaet spektraler Operatoren \\
\hline
\end{tabular}
\end{center}
...
Diese Arbeit fuehrt eine solche Struktur ein -- \textit{arithmetische Positivitaet} -- und formuliert die Hodge-Vermutung als No-Go-Theorem um:

\begin{quote}
\textit{Jede rationale $(p,p)$-Klasse, die keine $\mathbb{Q}$-Linearkombination algebraischer Zykel ist, muss arithmetische Positivitaet unter mindestens einer zulaessigen Operation verletzen.}
\end{quote}
...
Sei $X$ eine glatte projektive Varietaet der Dimension $n$, ausgestattet mit einer K\"ahler-Form $\omega$, die eine ample Klasse $L = [\omega] \in H^{1,1}(X) \cap H^2(X, \mathbb{Z})$ definiert. Der \textit{Lefschetz-Operator} $\Lambda_L: H^k(X) \to H^{k+2}(X)$ ist definiert durch $\Lambda_L(\alpha) = L \cup \alpha$.
...
\label{thm:HR}
Auf dem primitiven Unterraum $P^{p,q}(X) = P^{p+q}(X) \cap H^{p,q}(X)$ ist die hermitesche Form
\begin{equation}
...
Delignes Resultat zeigt, dass Absolutheit eine \textit{notwendige} Bedingung fuer Algebraizitaet ist. Die Frage ist, ob sie hinreichend ist. Unser Konzept der arithmetischen Positivitaet verstaerkt Absolutheit, indem Hodge-Riemann-Positivitaet unter allen Konjugationen hinzugefuegt wird.
...
\item[\textbf{(AP3)}] \textbf{Lefschetz-Kompatibilitaet.} Fuer alle $0 \leq j \leq n - 2p$ und alle amplen $L$:
\begin{equation}
...
\begin{remark}
Die drei Bedingungen sind auf subtile Weise absichtlich redundant. Bedingung (AP1) allein wird von vielen nichtalgebraischen Klassen erfuellt (Positivitaet relativ zu einer Polarisierung ist schwach). Bedingung (AP2a) allein ist Delignes Absolutheit. Die Staerke kommt aus ihrer \textit{Konjunktion}: die Klasse muss unter \textit{allen} Polarisierungen \textit{und} allen Galois-Konjugationen \textit{gleichzeitig} positiv bleiben. Dies ist das Analogon der ,,arithmetischen Rigiditaet'', die algebraische Objekte von transzendenten unterscheidet.
\end{remark}
...
Die Arithmetische Positivitaetsvermutung~\ref{conj:AP} impliziert die Hodge-Vermutung~\ref{conj:hodge}. Umgekehrt impliziert die Hodge-Vermutung die Inklusion $\mathrm{cl}(\mathrm{CH}^p(X)_\mathbb{Q}) \subseteq \mathrm{AP}^p(X)$.
\end{proposition}

\begin{proof}
Wenn die APV gilt, dann ist $\mathrm{AP}^p(X) = \mathrm{cl}(\mathrm{CH}^p(X)_\mathbb{Q})$, also ist jede Hodge-Klasse, die (AP1)--(AP3) erfuellt, algebraisch. Die Hodge-Vermutung reduziert sich dann darauf zu verifizieren, dass alle rationalen Hodge-Klassen die arithmetischen Positivitaetsbedingungen erfuellen.

Umgekehrt, wenn die HV gilt, dann ist jede Hodge-Klasse algebraisch, und algebraische Klassen erfuellen (AP1)--(AP3) (Theorem~\ref{thm:easy_direction} unten), also $\mathrm{cl}(\mathrm{CH}^p(X)_\mathbb{Q}) \subseteq \mathrm{AP}^p(X)$.
\end{proof}
...
\textit{(AP1):} Die Hodge-Riemann-Relationen fuer algebraische Klassen besagen, dass fuer jedes ample $L$ und jede primitive algebraische Klasse $\alpha_0$:
\begin{equation}
(-1)^p\, Q_L(\alpha_0, \alpha_0) = (-1)^p \int_X \alpha_0 \cup \bar{\alpha}_0 \cup L^{n-2p} > 0
\end{equation}
falls $\alpha_0 \neq 0$, da $\alpha_0 \cup \bar{\alpha}_0$ durch einen positiven Strom (den Integrationsstrom ueber $Z$) repraesentiert wird, wenn $Z$ effektiv ist. Fuer allgemeines $Z$ (effektive Differenzen) folgt die Ungleichung $Q_L(\alpha_0, \alpha_0) \geq 0$ aus dem Hodge-Indextheorem, angewandt auf die Hodge-Riemann-Form auf dem Gitter algebraischer Zykel \cite{Voisin2002}.
...
\textit{(AP3):} Das Hard-Lefschetz-Theorem ist kompatibel mit der Zykusklassenabbildung: wenn $\alpha = \mathrm{cl}(Z)$, dann ist $\Lambda_L^j \alpha = \mathrm{cl}(Z \cdot L^j)$, wobei $Z \cdot L^j$ der Schnitt mit $j$ Kopien eines Hyperebenenschnitts ist. Dies ist wieder ein algebraischer Zykus, also gilt (AP1) fuer $\Lambda_L^j \alpha$.
\end{proof}
...
\begin{proposition}[Rueckzug]
\label{prop:pullback}
Sei $f: Y \to X$ ein Morphismus glatter projektiver Varietaeten. Wenn $\alpha \in \mathrm{AP}^p(X)$, dann gilt $f^*\alpha \in \mathrm{AP}^p(Y)$.
\end{proposition}

\begin{proof}
Wir beweisen das Resultat fuer generisch endliche Morphismen; der allgemeine Fall wird am Ende diskutiert.

\textit{(AP1):} Sei $f: Y \to X$ generisch endlich vom Grad $d$, und sei $L_X \in \mathrm{Amp}(X)$. Dann ist $f^*L_X$ big und nef (ampel falls $f$ endlich ist). Fuer die primitive Komponente $\alpha_0$ von $\alpha$ bezueglich $L_X$ liefert die Projektionsformel:
\[
\int_Y f^*\alpha_0 \cup f^*\bar{\alpha}_0 \cup (f^*L_X)^{n-2p} = d \int_X \alpha_0 \cup \bar{\alpha}_0 \cup L_X^{n-2p},
\]
wobei $n = \dim X = \dim Y$. Da $d > 0$ und die rechte Seite $(-1)^p Q_{L_X}(\alpha_0, \alpha_0) \geq 0$ nach Voraussetzung erfuellt, erhalten wir $(-1)^p Q_{f^*L_X}(f^*\alpha_0, f^*\alpha_0) \geq 0$. Fuer eine allgemeine ample Klasse $L_Y \in \mathrm{Amp}(Y)$ nutzen wir die Tatsache, dass der ample Kegel offen ist: fuer hinreichend grosses $m$ ist $L_Y' := L_Y + m \cdot f^*L_X$ ampel, und die primitive Zerlegung von $f^*\alpha$ bezueglich $L_Y'$ kann durch Stetigkeit der Hodge-Riemann-Form in der Polarisierung kontrolliert werden (vgl.\ Voisin~\cite{Voisin2002}, Prop.\ 6.32). Genauer: fuer jedes $L_Y \in \mathrm{Amp}(Y)$ ist die Menge $\{L \in \mathrm{Amp}(Y) : (-1)^p Q_L(\beta_0, \beta_0) \geq 0\}$ (wobei $\beta_0$ die $L$-primitive Komponente von $f^*\alpha$ ist) abgeschlossen im amplen Kegel. Wir haben verifiziert, dass sie alle Klassen der Form $f^*L_X$ fuer $L_X \in \mathrm{Amp}(X)$ enthaelt. Fuer generisch endliches $f$ erzeugen diese einen Unterkegel von $\mathrm{Amp}(Y)$; die Erweiterung auf ganz $\mathrm{Amp}(Y)$ erfordert, dass dieser Unterkegel dicht ist oder dass die Positivitaetsbedingung sich durch Stetigkeit erweitert. Diese Erweiterung ist etabliert wenn $f$ endlich ist (sodass $f^*$ ampel auf ampel abbildet), bleibt aber offen fuer lediglich generisch endliche Morphismen.

\textit{(AP2):} Rueckzug kommutiert mit $\sigma$-Konjugation: $(f^*\alpha)^\sigma = (f^\sigma)^*(\alpha^\sigma)$, und $f^\sigma: Y^\sigma \to X^\sigma$ ist wieder generisch endlich vom selben Grad. Da $\alpha^\sigma \in \mathrm{AP}^p(X^\sigma)$ nach Voraussetzung, folgt das Resultat aus dem (AP1)-Argument angewandt auf $f^\sigma$ und $\alpha^\sigma$.

\textit{(AP3):} Fuer generisch endliches $f$ erfuellt der Lefschetz-Operator $\Lambda_{f^*L}^j \circ f^* = f^* \circ \Lambda_L^j$ (Kompatibilitaet von Rueckzug mit Cup-Produkt). Daher gilt $\Lambda_{f^*L}^j(f^*\alpha) = f^*(\Lambda_L^j \alpha)$, und die Positivitaet von $(-1)^{p+j} Q_{f^*L}(f^*(\Lambda_L^j\alpha), f^*(\Lambda_L^j\alpha))$ folgt aus jener von $(-1)^{p+j} Q_L(\Lambda_L^j\alpha, \Lambda_L^j\alpha)$ durch dasselbe Grad-$d$-Argument.

\textit{Allgemeine Morphismen:} Fuer $f: Y \to X$ mit $\dim Y > \dim X$ ist der Rueckzug $f^*L_X$ nicht ampel auf $Y$ (er hat Faserrichtungen mit Schnittzahl Null). Die primitive Zerlegung auf $Y$ bezueglich einer echten amplen Klasse $L_Y$ involviert zusaetzliche Faserbeitraege, die eine relative Hodge-Riemann-Analyse erfordern. Wir fuehren dies hier nicht aus; die Funktorialitaet von $\mathrm{AP}^p$ unter allgemeinen Morphismen bleibt bedingt auf diese Erweiterung.
\end{proof}

\begin{proposition}[Vorwaertsschub]
\label{prop:pushforward}
Sei $f: Y \to X$ ein Morphismus glatter projektiver Varietaeten mit $\dim Y = \dim X + r$. Wenn $\alpha \in \mathrm{AP}^{p+r}(Y)$, dann gilt $f_*\alpha \in \mathrm{AP}^p(X)$.
\end{proposition}

\begin{proof}[Beweisskizze]
Fuer generisch endliches $f$ mit $\dim Y = \dim X$ liefert die Projektionsformel $f^*f_* = \deg(f) \cdot \mathrm{id}$ auf dem Bild von $f_*$ das Resultat. Fuer $\dim Y > \dim X$ (d.h. $r > 0$) erfordert der Vorwaertsschub ein Gysin-Argument, das die Hodge-Riemann-Form auf $Y$ mit jener auf $X$ in Beziehung setzt; dies involviert die Grothendieck-Riemann-Roch-Formel und wird hier nicht ausgefuehrt. Die vollstaendige Funktorialitaet von $\mathrm{AP}^p$ unter Vorwaertsschub bleibt bedingt auf eine vollstaendige Behandlung der relativen primitiven Zerlegung.
\end{proof}

\begin{proposition}[K\"unneth-Produkt]
\label{prop:kunneth}
Seien $\alpha \in \mathrm{AP}^p(X)$ und $\beta \in \mathrm{AP}^q(Y)$. Dann gilt $\alpha \boxtimes \beta \in \mathrm{AP}^{p+q}(X \times Y)$.
\end{proposition}
...
\begin{proof}
Nach Lemma~\ref{lem:primitive_product} ist $\alpha \boxtimes \beta$ $L$-primitiv, also liefert die Definition~\eqref{eq:HR_form}:
\[
...
\]
Aus der Definition~\eqref{eq:HR_form} auf $X$ und $Y$:
\[
\int_X \alpha^2 \cup L_X^{n-2p} = (-1)^{-p(2p-1)}\, Q_{L_X}(\alpha, \alpha), \qquad \int_Y \beta^2 \cup L_Y^{m-2q} = (-1)^{-q(2q-1)}\, Q_{L_Y}(\beta, \beta).
\]
Fuer die Vorzeichenverifikation: $(-1)^{k(2k-1)} \cdot (-1)^{-p(2p-1)} \cdot (-1)^{-q(2q-1)} = (-1)^{(p+q)(2p+2q-1) - p(2p-1) - q(2q-1)}$. Ausmultiplizieren des Exponenten:
\begin{align*}
&(p+q)(2p+2q-1) - p(2p-1) - q(2q-1) \\
&= 2p^2 + 2pq - p + 2pq + 2q^2 - q - 2p^2 + p - 2q^2 + q = 4pq.
\end{align*}
Da $(-1)^{4pq} = 1$, erhalten wir:
\[
...
\]
Der Binomialkoeffizient $\binom{N-2k}{n-2p} > 0$, und beide geklammerten Faktoren sind $\geq 0$ durch (AP1) fuer $\alpha$ und $\beta$. Daher gilt $(-1)^{p+q}\, Q_L(\alpha \boxtimes \beta, \alpha \boxtimes \beta) \geq 0$. Da ample Klassen der Form $L_X \boxtimes 1 + 1 \boxtimes L_Y$ dicht in $\mathrm{Amp}(X \times Y)$ liegen (sie erzeugen den amplen Kegel des Produkts), erstreckt sich die Ungleichung auf alle amplen Klassen auf $X \times Y$ durch Stetigkeit von $Q_L$ in $L$.
...
\item[\textbf{(O3)}] \textbf{Lefschetz-Instabilitaet:} Es existieren $j \geq 1$ und $L \in \mathrm{Amp}(X)$, sodass $(-1)^{p+j}\,Q_L(\Lambda_L^j \alpha, \Lambda_L^j \alpha) < 0$.
\end{enumerate}
\end{theorem}
...
Die drei Obstruktionen sind nach Staerke geordnet:
\begin{itemize}
\item \textbf{(O1)} ist die schwaechste: sie erfordert die Pruefung der Positivitaet fuer alle amplen Klassen. Dies schliesst bereits viele ,,generische'' transzendente Klassen aus.
\item \textbf{(O2)} ist die tiefste: sie ruft die absolute Galois-Gruppe $\mathrm{Gal}(\bar{\mathbb{Q}}/\mathbb{Q})$ auf und verbindet sich so mit der Arithmetik. Hier tritt das ,,Arithmetische'' in arithmetischer Positivitaet ein.
\item \textbf{(O3)} ist die geometrischste: sie testet die Kompatibilitaet mit der Lefschetz-Algebra.
\end{itemize}
\end{remark}
...
Der Inhalt der APV jenseits dieses unbedingten Resultats ist:
\begin{quote}
\textit{Absolutheit allein ist nicht hinreichend fuer Algebraizitaet; man benoetigt zusaetzlich, dass Hodge-Riemann-Positivitaet unter allen Konjugationen erhalten bleibt.}
\end{quote}

Dies ist praezise die ,,arithmetische Positivitaetsluecke'' zwischen Delignes absoluten Hodge-Klassen und algebraischen Klassen.
...
\begin{theorem}
\label{thm:abelian}
Fuer abelsche Varietaeten ist die Arithmetische Positivitaetsvermutung~\ref{conj:AP} aequivalent zur Hodge-Vermutung.
\end{theorem}
...
\textit{Schritt 3:} Eine Hodge-Klasse $\alpha \in H^{2p}(A, \mathbb{Q}) \cap H^{p,p}(A)$ ist durch ihre Hodge-Riemann-Daten bestimmt: die Kollektion von Werten $\{Q_L(\alpha, \alpha)\}_{L \in \mathrm{Amp}(A)}$ zusammen mit den absoluten Stabilitaetsdaten $\{Q_{L^\sigma}(\alpha^\sigma, \alpha^\sigma)\}_{\sigma \in \mathrm{Aut}(\mathbb{C}), L \in \mathrm{Amp}(A)}$.
...
\textit{Schritt 5 (Abstieg auf einen Zahlkoerper und Spreading-Out):} Da $A$ ueber $\mathbb{C}$ definiert ist, existiert eine endlich erzeugte $\mathbb{Z}$-Algebra $R \subset \mathbb{C}$ und ein abelsches Schema $\mathcal{A}_R \to \mathrm{Spec}(R)$ mit $\mathcal{A}_R \times_R \mathbb{C} \cong A$ (EGA~IV, Thm.\ 8.8.2: jeder Morphismus von endlicher Praesentation steigt auf eine endlich erzeugte Basis ab). Die Hodge-Klasse $\alpha$, als rationale Kohomologieklasse die durch endlich viele algebraische Bedingungen definiert ist (Typ $(p,p)$ und Rationalitaet), steigt zu einer Klasse $\alpha_R \in H^{2p}_{\mathrm{dR}}(\mathcal{A}_R/R)$ fuer eine geeignete Lokalisierung von~$R$ ab.

Wahl eines abgeschlossenen Punktes $s \in \mathrm{Spec}(R)$ mit Restkoerper $k_s$ (ein endlicher Koerper, nach weiterer Lokalisierung zu $\mathrm{Spec}(\mathbb{Z}[1/N])$ fuer geeignetes~$N$) liefert eine Spezialisierung $\alpha_s \in H^{2p}_{\mathrm{dR}}(\mathcal{A}_s/k_s)$, die nach glattem-eigenem Basiswechsel (Vergleich zwischen de~Rham- und $\ell$-adischer Kohomologie via Artin) eine Tate-Klasse ist. Nach Faltings' Theorem \cite{Faltings1983} ist $\alpha_s$ algebraisch auf $\mathcal{A}_s$.
...
Die Aequivalenz folgt: die APV gilt fuer abelsche Varietaeten genau dann, wenn die HV fuer abelsche Varietaeten gilt. Schritte~1--4 reduzieren das Problem auf das Abstiegsargument von Schritt~5, das durch die Spreading-Out-Technik und das Cattani--Deligne--Kaplan-Theorem vervollstaendigt wird.
\end{proof}
...
\begin{proof}
Algebraische Zykel transformieren kovariant unter $\sigma$: $\mathrm{cl}(Z)^\sigma = \mathrm{cl}(Z^\sigma)$, und $Z^\sigma$ ist ein algebraischer Zykus auf $X^\sigma$. Daher gilt $Q_{L^\sigma}(\mathrm{cl}(Z^\sigma), \mathrm{cl}(Z^\sigma)) \geq 0$ nach den Hodge-Riemann-Relationen fuer algebraische Klassen.
\end{proof}
...
Dies ist der praezise Mechanismus der Obstruktion: \textit{algebraische Klassen sind starr genug, um alle Galois-Verdrillungen mit intakter Positivitaet zu ueberstehen, waehrend transzendente Klassen es nicht sind.}
...
\item Funktorialitaet von $\mathrm{AP}^p$ (Abschnitt~\ref{sec:functoriality}, unbedingt).
\item Aequivalenz von APV und HV fuer abelsche Varietaeten (Theorem~\ref{thm:abelian}, bedingt auf bestehende Resultate).
\item Identifikation der praezisen Obstruktion (Abschnitt~\ref{sec:obstruction}).
\end{enumerate}

Die verbleibende Luecke -- die ,,schwere Richtung'' $\mathrm{AP}^p(X) \subseteq \mathrm{cl}(\mathrm{CH}^p(X)_\mathbb{Q})$ im Allgemeinen -- erfordert den Beweis:

\begin{quote}
\textit{Wenn eine rationale $(p,p)$-Klasse nichtnegatves GHR-Spektrum fuer alle $\sigma$ und alle $L$ hat, dann liegt sie im Bild der Zykusklassenabbildung.}
\end{quote}

Dies ist eine Aussage ueber die Struktur der Zykusklassenabbildung, nicht ueber die Konstruktion von Zykeln. Die zentrale Schwierigkeit besteht darin, dass die Zykusklassenabbildung nicht auf offensichtliche Weise surjektiv auf den arithmetisch positiven Kegel ist: es koennte ,,zu viele'' arithmetisch positive Klassen geben.

\subsection{Beziehung zu Delignes Programm}

Delignes Framework absoluter Hodge-Klassen \cite{Deligne1982} liefert Bedingung (AP2a). Die vorliegende Arbeit fuegt zwei Zutaten hinzu:

\begin{enumerate}
\item \textbf{Hodge-Riemann-Positivitaet unter Konjugation} (AP2b): Nicht nur der Hodge-\textit{Typ}, sondern auch das Hodge-Riemann-\textit{Vorzeichen} muss erhalten bleiben. Dies ist eine strikt staerkere Bedingung.

\item \textbf{Universelle Polarisierung} (AP1 fuer alle $L$): Nicht nur eine Polarisierung, sondern alle amplen Klassen muessen nichtnegative Hodge-Riemann-Form ergeben.
\end{enumerate}

Die Frage ,,Sind alle absoluten Hodge-Klassen algebraisch?'' ist Delignes motivierendes Problem. Unser Framework legt nahe, dass die Antwort genau dann ja ist, wenn absolute Hodge-Klassen automatisch (AP1) und (AP3) erfuellen -- d.h., wenn Absolutheit bereits universelle Hodge-Riemann-Positivitaet impliziert.
...
Wenn Vermutung~\ref{conj:absolute_implies_ap} gilt, dann lautet die Kette:
\begin{equation}
\text{Hodge-Klasse} \xRightarrow{\text{?}} \text{Absolut} \xRightarrow{\ref{conj:absolute_implies_ap}} \text{AP} \xRightarrow{\text{APV}} \text{Algebraisch}\,,
\end{equation}
und die Hodge-Vermutung reduziert sich auf: ,,Sind alle Hodge-Klassen absolut?'' (Delignes Frage) plus Vermutung~\ref{conj:absolute_implies_ap}.

Fuer abelsche Varietaeten wird der erste Pfeil durch Delignes Theorem geliefert. Der zweite wuerde aus einer sorgfaeltigen Analyse der Mumford-Tate-Gruppenaktion auf der Hodge-Riemann-Form folgen.
...
RH & Spektraler Operator & Bedingt~\cite{GeigerRH2026} \\
CRM/QG & Kapazitaet + Komposition & $\tanh$-Theorem~\cite{GeigerFST2026} \\
\textbf{Hodge} & \textbf{GHR-Spektrum} & \textbf{Diese Arbeit} \\
\hline
\end{tabular}
\end{center}

In jedem Fall besteht die Strategie nicht darin, das gewuenschte Objekt zu konstruieren, sondern zu zeigen, dass seine Nichtexistenz eine Positivitaetsbedingung verletzen wuerde. Die Hodge-Vermutung ist die letzte grosse Instanz, bei der diese Strategie nicht systematisch angewandt wurde.

\subsection{Einschraenkungen}

\begin{enumerate}
\item \textbf{Die schwere Richtung ist offen.} Diese Arbeit beweist nicht die Hodge-Vermutung. Sie formuliert sie als Positivitaetsaussage um und identifiziert die praezise Obstruktion, aber der Beweis, dass arithmetische Positivitaet Algebraizitaet erzwingt, bleibt die zentrale Herausforderung.
...
\item \textbf{Vermutung~\ref{conj:absolute_implies_ap} ist unbewiesen.} Die Beziehung zwischen Absolutheit und universeller Hodge-Riemann-Positivitaet ist im Allgemeinen nicht etabliert.
...
\begin{enumerate}
\item \textbf{Verifikation der GHR-Obstruktionsvermutung fuer bekannte nichtalgebraische Klassen.} Der einfachste Testfall ist Voisins Gegenbeispiel zur ganzzahligen Hodge-Vermutung \cite{Voisin2002b}: verifiziere, dass ihre nichtalgebraische ganzzahlige $(2,2)$-Klasse auf einem Produkt von K3-Flaechen negatives GHR-Spektrum fuer ein $\sigma$ hat.

\item \textbf{Beweise Vermutung~\ref{conj:absolute_implies_ap} fuer abelsche Varietaeten} unter Verwendung der expliziten Struktur der Mumford-Tate-Gruppe und der symplektischen Hodge-Riemann-Form.
...
Wir haben eine Umformulierung der Hodge-Vermutung als Positivitaets-No-Go-Theorem vorgeschlagen. Das Schluesselkonzept -- \textit{arithmetische Positivitaet} -- kombiniert Hodge-Riemann-Positivitaet, absolute Stabilitaet unter $\mathrm{Aut}(\mathbb{C})$ und Lefschetz-Kompatibilitaet in eine einzige Bedingung, die algebraische Klassen erfuellen und (vermutlich) nichtalgebraische Klassen verletzen.

Der Ansatz bietet drei Vorteile gegenueber konstruktiven Methoden:

\begin{enumerate}
\item \textbf{Keine Zykuskonstruktion erforderlich.} Die Vermutung wird auf eine Aussage ueber Positivitaet von Bilinearformen reduziert, nicht auf die Existenz von Untervarietaeten.

\item \textbf{Keine vollstaendige motivische Maschinerie erforderlich.} Der arithmetisch positive Kegel wird nur unter Verwendung von Standard-Hodge-Theorie und Galois-Aktionen definiert.

\item \textbf{Die Obstruktion ist explizit.} Das Galois-Hodge-Riemann-Spektrum liefert eine (prinzipiell) berechenbare Invariante, die algebraische von nichtalgebraischen Klassen unterscheidet.
\end{enumerate}

Die verbleibende Herausforderung -- zu beweisen, dass nichtnegatves GHR-Spektrum Algebraizitaet impliziert -- ist ein praezises, wohldefiniertes mathematisches Problem. Wir glauben, dass das Zusammenspiel von Hodge-Riemann-Positivitaet und Galois-Arithmetik das korrekte Framework fuer den Angriff auf die Hodge-Vermutung liefert, und dass die Positivitaets-No-Go-Perspektive dort Erfolg haben kann, wo konstruktive Ansaetze gescheitert sind.
