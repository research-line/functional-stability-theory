...
\begin{remark}[SAT slice entropy at the phase transition]
\label{rem:sat-entropy}
The slice entropy $H_{\mathrm{slice}}(\mathrm{SAT}_n)$ was computed for random 3-SAT instances at the phase transition $\alpha = m/n \approx 4.267$ for $n = 8, 10, 12, 14, 16$. The entropy grows from $H \approx 1.5$ ($n=8$) to $H \approx 2.3$ ($n=14$), consistent with subexponential growth $H \sim n^{0.71}$ (polynomial fit $R^2 = 0.82$). The entropy is maximal on the satisfiable side ($\alpha \approx 2$--$3$) and drops to zero for $\alpha \geq 8$ (UNSAT regime), confirming the phase transition structure. Full validation of the ESC hypothesis $H_{\mathrm{slice}} \geq c \cdot 2^{n/2}$ requires $n \geq 30$, which is computationally infeasible on standard hardware. Script: \texttt{compute\_sat\_entropy.py}.
\end{remark}
...
If compression is impossible (under PH non-collapse), then for infinitely many instances $x$, every polynomial-time witness production must fail---i.e., $K^{n^c}(w | x) \geq |w| - O(\log n)$ for all witnesses $w$. This is the Uniformity Bridge in operational form.
...
Let $\mathcal{W}_n$ denote the set of high-entropy witnesses for SAT instances of size $n$: pairs $(\varphi_n, w)$ with $K^t(w \mid \varphi_n) \geq n^\varepsilon$. The \emph{non-uniform} statement (Theorem~\ref{thm:slice_nogo}) asserts:
\[
  \forall\, \text{algorithm } A,\quad \forall\, n,\quad \exists\, \varphi_n \in \mathrm{SAT}_n : \text{all witnesses of } \varphi_n \text{ have } K^t(w \mid \varphi_n) \geq n^\varepsilon.
\]
The \textbf{Uniformity Bridge} demands a \emph{uniform} construction: a single deterministic procedure $\mathcal{U}$ such that, given $1^n$, $\mathcal{U}(1^n)$ produces an instance $\varphi_n$ with the above property, using resources polynomial in $n$.
\end{axiom}
...
\begin{proposition}[Equivalences of UB-PvNP]\label{prop:ub-equivalences}
The following are equivalent formulations of the Uniformity Bridge:
\begin{enumerate}
...
\end{enumerate}
Each of (i)--(iii) implies P$\,\neq\,$NP. The equivalence (i)$\Leftrightarrow$(ii) follows from the MCSP connection (Remark~\ref{rem:mcsp-internalisation}); (ii)$\Leftrightarrow$(iii) is the Impagliazzo--Wigderson hardness-randomness tradeoff.
\end{proposition}
...
\item[(b)] One-way functions exist (via Impagliazzo--Levin: average-case hardness from worst-case).
...
Ilango \cite{Ilango2025} proved that MCSP is NP-hard under deterministic quasi-polynomial non-adaptive reductions, answering the oldest open question from the inception of NP-completeness theory. A key corollary is that $\mathrm{MCSP} \notin \mathrm{coNP}$: there is no short proof for the hardness of a Boolean function. This is precisely the entropy obstruction of our framework---the impossibility of compressing the incompressibility proof. The cryptographic primitives (NIWI proof systems) used in the reduction are inherently immune to the algebrization barrier, since algebraic identity tests cannot invert cryptographic hash functions.
\end{remark}
...
This paper is a \textbf{reformulation paper}: it establishes an equivalence between the P\,$\neq$\,NP conjecture and an entropic condition (ESC), without proving the separation itself.
...
\hline
Barrier immunity (S1--S3) & Analysis & Sec.~\ref{sec:barriers} \\
...
\hline
Phase transition at $\alpha_c \approx 4.267$ & Monte Carlo & Rem.~\ref{rem:sat-entropy} \\
$H_{\mathrm{slice}}$ growth with $n$ & Monte Carlo & Rem.~\ref{rem:sat-entropy} \\
\hline
...
\bibitem{Ilango2025} R.~Ilango, \textit{NP-hardness of the Minimum Circuit Size Problem from well-studied assumptions}, Proceedings, 2025.
