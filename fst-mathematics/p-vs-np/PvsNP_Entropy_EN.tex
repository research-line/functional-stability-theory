...
positive witness mass.  A useful residual diagnostic is
\[
\begin{aligned}
R_{\mathrm{front}}(x,t)
...
leaves $R_{\mathrm{front}}\geq n^\varepsilon$, then $\mathcal{C}$ cannot
solve the family with constant success probability.  This is still a
restricted-model statement.  To use it against all polynomial-time
algorithms one would have to prove a front-shadow lemma showing that every
uniform witness producer induces such a low-complexity visible front without
