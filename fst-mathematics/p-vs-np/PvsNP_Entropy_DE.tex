...
\label{conj:entropic}
Keine NP-vollständige Sprache ist algorithmisch positiv. Das heisst: für jede NP-vollständige Sprache $L$ und jede Polynomialzeit-Zeugenproduzenten-Funktion $W$ existieren unendlich viele Ja-Instanzen $x \in L$, sodass $W$ keinen gültigen Zeugen für $x$ produziert (d.h. $V(x, W(x)) = 0$), oder $K^{q(|x|)}(W(x) \mid x) \geq \omega(\log|x|)$ für alle Polynome $q$.

Eine äquivalente Formulierung in Wahrheitstabellen-Komplexität (siehe Theorem~\ref{thm:slice_nogo}): für eine NP-vollständige Sprache $L$ und jedes Polynom $q$ erfüllen unendlich viele $n$ die Bedingung $K^{q(2^n)}(L_n) \geq n$. (Beide Formulierungen sind äquivalent zu $\PP \neq \NP$; ihre gegenseitige Äquivalenz folgt transitiv.)
\end{conjecture}
...
Die Umkehrung von Proposition~\ref{prop:equiv} gilt ebenfalls: $\PP \neq \NP$ impliziert ESC. Tatsächlich: wenn $\PP = \NP$, dann hat jede NP-vollständige Sprache $L$ einen Polynomialzeit-Algorithmus, was $K^{\mathrm{poly}(2^n)}(L_n) = O(\log n)$ ergibt nach Lemma~\ref{lem:p_low_entropy}, was die ESC-Bedingung $K^{q(2^n)}(L_n) \geq n$ verletzt. Somit ESC $\Leftrightarrow$ $\PP \neq \NP$: die Entropische Separationsvermutung ist eine \textit{Umformulierung} des P-vs-NP-Problems in informationstheoretischer Sprache, keine unabhängige Verstärkung.
\end{remark}
...
\item[(iii)] Keine NP-vollständige Sprache ist in Polynomialzeit entscheidbar. Das heisst: für jede NP-vollständige Sprache $L$ und jeden Polynomialzeit-Algorithmus $A$ existieren Instanzen $x$ mit $A(x) \neq L(x)$.
\end{enumerate}
...
Dies ist die \textit{sauberste} No-Go-Formulierung: die ,,leichte Richtung'' ($L \in \PP \Rightarrow$ niedrige Entropie) ist ein Theorem; die ,,schwere Richtung'' (EH für SAT) ist der gesamte Inhalt von $\PP \neq \NP$.
...
Die Entropiehärte-Hypothese ist eng mit Schaltkreisschranken verwandt. Eine Boolesche Funktion $f_n$, die von einem Schaltkreis der Größe $s$ berechnet wird, hat eine Wahrheitstabelle, die in $O(s \log s)$ Bits beschreibbar ist, also $K^{O(s \cdot 2^n)}(f_n) \leq O(s \log s) + O(\log n)$. Daher:
\begin{itemize}
\item EH mit Schwellwert $n$ impliziert, dass $\SAT_n$ Schaltkreise der Größe $s \geq 2^{\Omega(n / \log n)}$ benötigt, d.h.\ $\NP \not\subseteq \mathrm{SIZE}(2^{o(n)})$.
\item Insbesondere impliziert EH $\NP \not\subseteq \PP/\poly$ (da Schaltkreise polynomialer Größe $K^{\poly(2^n)}(f_n) \leq O(n^c \log n)$ ergeben, was $o(n)$ für großes $n$ ist).
\end{itemize}
Dies zeigt, dass das Entropie-Framework und der Schaltkreiskomplexitäts-Ansatz zwei Perspektiven auf dieselbe zugrundeliegende Trennung sind.
\end{remark}

\begin{remark}[SAT-Schnittentropie am Phasenübergang]
\label{rem:sat-entropy}
Die Schnittentropie $H_{\mathrm{slice}}(\mathrm{SAT}_n)$ wurde für zufällige 3-SAT-Instanzen am Phasenübergang $\alpha = m/n \approx 4{,}267$ für $n = 8, 10, 12, 14, 16$ berechnet. Die Entropie wächst von $H \approx 1{,}5$ ($n=8$) auf $H \approx 2{,}3$ ($n=14$), konsistent mit subexponentiellem Wachstum $H \sim n^{0{,}71}$ (polynomialer Fit $R^2 = 0{,}82$). Die Entropie ist maximal auf der erfüllbaren Seite ($\alpha \approx 2$--$3$) und fällt auf Null für $\alpha \geq 8$ (UNSAT-Regime), was die Phasenübergangsstruktur bestätigt. Eine vollständige Validierung der ESC-Hypothese $H_{\mathrm{slice}} \geq c \cdot 2^{n/2}$ erfordert $n \geq 30$, was auf Standard-Hardware rechnerisch nicht durchführbar ist. Skript: \texttt{compute\_sat\_entropy.py}.
\end{remark}
...
\item \textbf{Kryptographische Evidenz.} Wenn (EH) versagt, dann würden effiziente PRGs aus Einwegfunktionen nicht existieren \cite{ImpagliazzoWigderson1997}, was viel der modernen Kryptographie zum Einsturz brächte.
\end{enumerate}
...
Die Resolutionsbreite $w(\varphi_n \vdash \bot)$ ist robust unter Zeitbeschr\"ankung: Anders als die Kolmogorov-Komplexit\"at~$K$ ist die Breite einer Resolutionswiderlegung eine \emph{syntaktische} Invariante, die nicht von Rechenressourcen abh\"angt. Dies macht sie zu einer nat\"urlichen "`thermodynamischen Observablen"' in der Beweiskomplexit\"ats-Analogie: Große Breite erzwingt, dass jede zeitbeschr\"ankte Beschreibung komplex ist, und liefert damit einen konkreten Weg von $K$-unteren Schranken zu $K^t$-unteren Schranken f\"ur spezifische Formelfamilien. Die conditionalen MCSP-Härteresultate von Hirahara und Ilango~\cite{Ilango2025} motivieren MCSP als Brückenobjekt, ohne allgemeine Entscheidbarkeit von~$K$ vorauszusetzen.
\end{remark}
...
\emph{Warum dies genügt:} Instanzkompressionshärte ist ein gut untersuchter Begriff in der parametrisierten Komplexitätstheorie~\cite{FortnowSanthanam2011,DellMelkebeek2014}. Die Verbindung zu $K^t$-unteren-Schranken würde aus einem Simulationsargument folgen: wenn ein kurzes Programm $w$ aus $\varphi_n$ in Zeit $n^c$ berechnen könnte, würde dies eine Kompression von $\varphi_n$ über den Zeugen darstellen.
...
Jedes \emph{einzelne} der drei Ziele genügt, um die Entropische Separationsvermutung von einer Äquivalenz (Umformulierung von P $\neq$ NP) zu einem bedingten Beweis aufzuwerten (P $\neq$ NP unter einer konkreten, falsifizierbaren Hypothese). Ziel~3 ist wohl das zugänglichste, da es auf kryptographische Standardannahmen reduziert. Ziel~1 schließt an das aktive Forschungsprogramm zur Instanzkompression an~\cite{Drucker2015}. Ziel~2 ist das ambitionierteste, würde aber gleichzeitig Fragen in der Beweiskomplexität lösen.
\end{remark}
...
\textbf{Schritt A: Zeugenfinder $\Rightarrow$ Kompressor.}
Wenn ein Polynomialzeit-Algorithmus $A$ einen gültigen Zeugen $w = A(x)$ für $x \in \mathrm{SAT}$ erzeugt, definiert $A$ implizit eine \emph{Kompression}: aus $x$ kann man eine kürzere Darstellung $y = C(x)$ (über den Zeugen) erzeugen, sodass ein Dekompressor $D$ genügend Struktur rekonstruiert, um SAT-Zugehörigkeit zu entscheiden. Konkret gilt $|y| \leq |A| + O(\log|x|)$, da $A$ ein festes Programm ist. Der Schlüssel ist, dass diese Kompression \emph{nichttrivial} ist: sie bildet eine Familie von $2^{|\varphi_n|}$ möglichen Instanzen auf $O(1)$ Bits Rat (das Programm $A$) ab, was einer exponentiellen Kompression entspricht.

\textbf{Schritt B: Kompressions-No-Go $\Rightarrow$ Uniformität.}
Die bekannten Instanzkompressionsunmöglichkeitsresultate~\cite{FortnowSanthanam2011,DellMelkebeek2014,Drucker2015} zeigen: wenn SAT-Instanzen der Länge $n$ auf $n^{1-\varepsilon}$ Bits komprimiert werden können, kollabiert die Polynomhierarchie (NP $\subseteq$ coNP/poly). Angewendet auf Schritt~A: wenn $A$ als Polynomialzeit-Zeugenfinder für \emph{alle} Instanzen existiert, widerspricht die resultierende Kompression dem PH-Nichtkollaps. Dies ist präzise das ,,Positivitäts-No-Go''-Muster der FST: nicht ,,wir konstruieren schwere Instanzen'', sondern ,,jede globale Vereinfachung hat katastrophale strukturelle Konsequenzen.''

\textbf{Schritt C: Übersetzung nach $K^t$.}
Wenn Kompression unmöglich ist (unter PH-Nichtkollaps), dann muss für unendlich viele Instanzen $x$ jede Polynomialzeit-Zeugenproduktion im logarithmischen Beschreibungsregime scheitern; ein zusätzliches quantitatives Argument wäre nötig, um dies zu $K^{n^c}(w | x) \geq |w| - O(\log n)$ für alle Zeugen $w$ zu verstärken. Diese quantitative Verstärkung ist die Nahe-Maximal-Brücke, keine Folgerung aus der Basisumformulierung.

Die verbleibende Lücke betrifft die Quantorenreihenfolge: Schritt~B liefert ,,für jeden $A$ existieren schwere Instanzen'' (nicht-uniform), während die Uniformitätsbrücke ,,es existieren Instanzen, die für alle $A$ schwer sind'' (uniform) benötigt. Das Schließen dieser Lücke erfordert entweder (i) eine \emph{universelle} Kompressionsunmöglichkeit (nicht nur für jeden Kompressor einzeln), oder (ii) ein Diagonalisierungsargument, das über alle Polynomialzeit-Algorithmen gleichzeitig uniformisiert.
\end{remark}
...
\]
wobei $H_x$ eine beliebige Energiefunktion auf dem Zeugenraum ist (z.B.\ $H_x \equiv 0$ für das Gleichverteilungsmass über gültige Zeugen). Ein Polynomialzeit-Suchalgorithmus entspricht einer \emph{lokalen Markov-Dynamik} (Glauber-/Heat-Bath-Typ) auf dem Zeugenraum, die schnell mischt -- d.h., eine Region niedriger Entropie von $\pi_x$ in $\mathrm{poly}(|x|)$ Schritten erreicht.

Die Entropische Separationsvermutung (ESC) lässt sich dann umformulieren:
\begin{quote}
...
Die OGP besagt, dass für zufällige Constraint-Satisfaction-Probleme nahe der Erfüllbarkeitsschwelle der Lösungsraum \emph{topologische Diskonnektivität} aufweist: Der Overlap (normierte Hamming-Distanz) zwischen zwei beliebigen nahezu optimalen Lösungen ist entweder sehr klein oder sehr groß, ohne Zwischenwerte. Dies ist präzise der geometrische Gehalt der Dobrushin-Obstruktion $\rho(C) \geq 1$ aus Bemerkung~\ref{rem:mixing-obstruction}: Das Gibbs-Maß über Zeugen fragmentiert in exponentiell viele wohlseparierte Cluster (der \emph{Shattering-Übergang} von Achlioptas und Coja-Oghlan~\cite{AchlioptasCO2008}), und keine lokale Markov-Dynamik kann in Polynomialzeit zwischen ihnen mischen.
...
\end{aligned}
\]
Die ersten beiden Pfeile sind etabliert: Gamarnik, Jagannath und Wein~\cite{GamarnikJW2020} beweisen, dass OGP das Versagen von niedriggradigen Polynomen, Booleschen Schaltkreisen und Langevin-Dynamik impliziert; Bresler und Huang~\cite{BreslerHuang2022} etablieren scharfe algorithmische Phasenübergänge für zufälliges $k$-SAT mithilfe der Overlap Gap Property.

Der dritte Pfeil -- \textbf{OGP $\Rightarrow$ hohe bedingte Kolmogorov-Komplexität $K^t(w|x)$} -- ist das \emph{zentrale offene Brückenziel} dieses Papers. Ein Beweis würde wie folgt vorgehen: Wenn der Lösungsraum OGP hat, dann muss jedes kurze Programm, das einen Zeugen $w$ aus $x$ extrahiert, eine Freie-Energie-Barriere zwischen Clustern ,,überqueren'', was entweder superpolynomielle Zeit oder superlogarithmische Beschreibungslänge erfordert. Dies würde die geometrische Obstruktion (OGP) in eine informationstheoretische Obstruktion (hohes $K^t$) umwandeln und die Uniformitätsbrücke schließen.
\end{remark}
...
\item[(ii)] \textbf{Uniforme Schaltkreis-untere-Schranken:} Es existiert eine explizite Boolesche Funktionenfamilie $(f_n)_{n \geq 1}$, konstruierbar in $\mathrm{P}$, mit $K^t(f_n) \geq n^\varepsilon$ (d.h., keine kleinen Schaltkreise).
...
\item \textbf{Diagonalisierung ohne Uniformisierung:} Ein Objekt, das durch ein reines Existenzargument (Cantor/K\"onig) definiert ist und keine uniforme Konstruktion liefert.
...
\begin{remark}[No-Go-Mechanismen als H\"artelokalisierung]\label{rem:ub-nogo}
...
Wenn UB-PvNP (Axiom~\ref{ax:ub-pvnp}) gilt, dann:
\begin{enumerate}
\item[(a)] P $\neq$ NP (unmittelbar aus der Existenz uniformer schwerer Instanzen).
\item[(b)] Einwegfunktionen existieren (über Impagliazzo--Levin: Average-Case-Härte aus Worst-Case).
\item[(c)] Die Entropielücke $H_{\mathrm{slice}}(\mathrm{SAT}_n) \geq c \cdot 2^{n^\varepsilon}$ gilt uniform.
\end{enumerate}
...
\item \emph{MCSP $\notin$ P/poly:} Das Minimum Circuit Size Problem (Allender--Hirahara~\cite{AllenderHirahara2019}) ist die natürliche Entscheidungsversion der Kolmogorov-Komplexität für Schaltkreise. Der Beweis MCSP $\notin$ P/poly würde uniforme Entropiehärte (EH) implizieren und über unser Framework P $\neq$ NP. Dies gilt als eines der zugänglichsten bedingungslosen Ziele in der Komplexitätstheorie.
\end{enumerate}
\end{remark}
...
Dieses Paper ist ein \textbf{Reformulierungspaper}: Es etabliert eine \"Aquivalenz zwischen der P\,$\neq$\,NP-Vermutung und einer entropischen Bedingung (ESC), ohne die Separation selbst zu beweisen.
...
\midrule
\multicolumn{3}{@{}l}{\textit{Bewiesene \"Aquivalenzen:}} \\
...
\midrule
\multicolumn{3}{@{}l}{\textit{Strukturelle Beitr\"age:}} \\
Barriere-Immunit\"at (S1--S5 f\"ur $K$; heuristisch f\"ur $K^t$) & Analyse & Abschn.~\ref{sec:barriers}, Bem.~\ref{rem:barrier-K-vs-Kt} \\
...
\midrule
\multicolumn{3}{@{}l}{\textit{Offen (\"aquivalent zu P\,$\neq$\,NP):}} \\
Uniformitätsbrücke & Offenes Problem & Abschn.~\ref{sec:uniformity-bridge} \\
Entropie-Härte (EH) & Vermutung & Abschn.~\ref{sec:slices} \\
\bottomrule
\end{tabular}
}
\end{table*}
...
Die verbleibende Herausforderung ist die Uniformitätsbrücke: die Konvertierung der existenziellen Entropieschranke in eine universelle untere Schranke gegen alle Polynomialzeit-Algorithmen. Die identifizierten Brückenstrategien (Instanzkompression, Beweiskomplexität, PRG-basierte Familien, MCSP) sind ihrerseits große offene Probleme, jeweils im Wesentlichen äquivalent zu oder schwerer als P vs NP selbst. Der Wert des Frameworks liegt darin, diese Struktur explizit zu machen.
