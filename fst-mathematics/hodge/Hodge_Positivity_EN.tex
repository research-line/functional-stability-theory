...
Our main result is a \emph{no-go theorem}: the Hodge--Riemann relations alone cannot distinguish algebraic from non-algebraic Hodge classes (Theorem: AP\,$=$\,AbsHodge). This precisely delineates the boundary of positivity-based approaches and identifies Deligne's question (1982) as the exact obstruction. As a constructive complement, we introduce the Galois--Hodge--Riemann spectrum as a potential tool for detecting non-algebraicity beyond the Hodge--Riemann barrier.
\end{abstract}
...
The full AP condition is strictly stronger than Deligne's absoluteness alone: the conjunction AP1+AP2+AP3 has no precedent in the literature and provides a novel obstruction criterion via the GHR spectrum.
...
The extension from $f^*\mathrm{Amp}(X)$ to all of $\mathrm{Amp}(Y)$ requires that $f^*\mathrm{Amp}(X)$ is dense in $\mathrm{Amp}(Y)$ in the Euclidean topology on $N^1(Y)_{\mathbb{R}}$. This holds when $f$ is surjective and $\rho(Y) = 1$ (Picard number one), but may fail for higher Picard number. In the general case, the proposition applies to the subcone $f^*\mathrm{Amp}(X) \subset \mathrm{Amp}(Y)$; extension to all ample classes remains conditional on the density hypothesis.
\end{remark}
...
\subsubsection*{Threshold axiom: beyond Deligne}

\begin{axiom}[Language Bridge -- JD-Hodge]\label{ax:jd-hodge}
There exists a category $\mathcal{C}$ of ``Hodge carriers'' (objects representing cohomological data), equipped with:
\begin{enumerate}
...
\end{enumerate}
In this language, the Hodge conjecture becomes a \emph{formal consequence}: positivity in $\mathcal{C}$ implies algebraicity. Deligne's absoluteness is then a shadow (necessary condition) of the richer structure in $\mathcal{C}$.
\end{axiom}
...
The na\"ive averaging $\bar{\alpha} = \frac{1}{|\mathrm{Aut}(\mathbb{C}/\mathbb{Q})|} \sum_\sigma \sigma^*\alpha$ is ill-defined since $\mathrm{Aut}(\mathbb{C}/\mathbb{Q})$ is a profinite group of cardinality $2^{\mathfrak{c}}$. The correct formulation uses the projective limit: for each finite Galois extension $K/\mathbb{Q}$, form $\bar{\alpha}_K = \frac{1}{|\mathrm{Gal}(K/\mathbb{Q})|} \sum_{\sigma \in \mathrm{Gal}(K/\mathbb{Q})} \sigma^*\alpha$. The Galois average is then $\bar{\alpha} = \lim_K \bar{\alpha}_K$, which exists in $H^{2p}(X, \mathbb{Q})$ by the absoluteness of $\alpha$.
\end{remark}
...
\begin{remark}[Voisin-type GHR obstruction test]
\label{rem:voisin-ghr}
The GHR obstruction was tested on six varieties: generic $T^4$, Weil torus (CM by $\mathbb{Z}[i]$), $K3 \times K3$, Fermat quintic $X_5$, Voisin-type non-projective torus, and an SDP relaxation of the AP cone. For all projective examples, AP1 is satisfied as guaranteed by the Hodge--Riemann relations. The $K3 \times K3$ test with non-tensor-product perturbations produces 3 negative eigenvalues of $(-1)^p Q_L$, confirming that the GHR obstruction has non-trivial detection power for non-algebraic classes. The AP cone analysis shows all random classes inside the cone with positive margin ($0.24$), indicating robustness. Script: \texttt{compute\_voisin\_test.py}.
\end{remark}
...
\hline
\multicolumn{2}{|l|}{\textit{Constructive tools:}} \\
\hline
GHR spectrum definition & Definition \\
Functoriality (finite morphisms) & Proposition \\
Voisin-type GHR detection & Numerical \\
\hline
\multicolumn{2}{|l|}{\textit{Open (equivalent to Deligne 1982):}} \\
\hline
