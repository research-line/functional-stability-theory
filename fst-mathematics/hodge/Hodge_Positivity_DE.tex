...
\hypersetup{
    ...
    pdfsubject={Grenzresultat zu Hodge-Riemann-Positivitaet und absoluten Hodge-Klassen},
    pdfkeywords={Hodge-Vermutung, absolute Hodge-Klassen, arithmetische Positivitaet, Hodge-Riemann-Relationen, Deligne-Absolutheit},
    colorlinks=true,
    linkcolor=black,
    urlcolor=blue,
    citecolor=black
...
\author{Lukas Geiger}
\affiliation{Unabhaengiger Forscher, Bernau, Deutschland}

\date{\today}

\begin{abstract}
Wir fuehren das Konzept der \textit{arithmetischen Positivitaet} fuer die Hodge-Vermutung ein: eine rationale $(p,p)$-Klasse $\alpha \in H^{2p}(X, \mathbb{Q})$ auf einer glatten projektiven Varietaet $X/\mathbb{C}$ heisst arithmetisch positiv, wenn sie gleichzeitig Hodge-Riemann-Positivitaet unter allen Polarisierungen, Delignes Absolutheit unter allen Automorphismen von $\mathbb{C}$ und Lefschetz-Kompatibilitaet erfuellt. Wir beweisen, dass algebraische Klassen arithmetisch positiv sind, und dokumentieren begrenzte Funktorialitaetstests: endlicher Rueckzug ist auf dem Rueckzugskegel ampler Klassen kontrolliert und wird nur unter einer expliziten Ample-Kegel-Vergleichshypothese zu einer vollen AP-Aussage; K\"unneth-Produkte werden fuer primitive Klassen und von Produktpolarisierungen erzeugte ample Kegel behandelt; endlicher Vorwaertsschub bleibt konjektural.

Unser \textbf{Hauptresultat ist ein No-Go-Theorem}: Der arithmetisch positive Kegel stimmt exakt mit Delignes absoluten Hodge-Klassen ueberein ($\mathrm{AP}^p(X) = \mathrm{AbsHodge}^p(X)$), weil die Hodge-Riemann-Bilinearrelationen die Positivitaetsbedingungen fuer jede Klasse vom Typ $(p,p)$ automatisch garantieren. Der natuerliche Versuch, Delignes Absolutheit durch Hodge-Riemann-Positivitaet zu verstaerken, liefert daher keine neuen Einschraenkungen. Die Arithmetische Positivitaetsvermutung reduziert sich auf Delignes Frage (1982): ,,Sind alle absoluten Hodge-Klassen algebraisch?'' Ferner sind die Obstruktionen (O1) und (O3) in der No-Go-Formulierung fuer rationale $(p,p)$-Klassen vakuos; nur (O2), Versagen der Absolutheit, ist effektiv. Wir fuehren das Galois-Hodge-Riemann-Spektrum als Visualisierung des Absolutheitstests ein und diskutieren kontrollierte Testfamilien fuer Bedingungen, die genuein staerker als die Hodge-Riemann-Relationen sein muessten.

Diese Arbeit ist ein expositorischer Beitrag, der eine strukturelle Limitation dokumentiert; sie enthaelt keine neuen Resultate ueber die Beobachtung $\mathrm{AP} = \mathrm{AbsHodge}$ hinaus.
\end{abstract}
...
KI-Werkzeuge (Claude von Anthropic) wurden f\"ur mathematische Formalisierung, Literaturverifikation und redaktionelle \"Uberarbeitung eingesetzt. Alle mathematischen Inhalte wurden vom Autor entwickelt, verifiziert und freigegeben.
\end{center}
...
\label{prop:pullback}
Sei $f: Y \to X$ ein endlicher Morphismus glatter projektiver Varietaeten. Wenn $\alpha \in \mathrm{AP}^p(X)$, dann erfuellt $f^*\alpha$ (AP1) fuer ample Klassen im Rueckzugsunterkegel $f^*\mathrm{Amp}(X) \subset \mathrm{Amp}(Y)$ und erfuellt (AP2)--(AP3) auf diesem Unterkegel. Ein volles Element von $\mathrm{AP}^p(Y)$ erhaelt man nur unter einer expliziten Ample-Kegel-Vergleichshypothese, etwa wenn die relevanten Positivitaetsungleichungen durch Dichtheit oder Gleichheit der numerischen ample Kegel von $f^*\mathrm{Amp}(X)$ auf ganz $\mathrm{Amp}(Y)$ fortgesetzt werden koennen.
\end{proposition}
...
\]
wobei $n = \dim X = \dim Y$. Da $d > 0$ und die rechte Seite $(-1)^p Q_{L_X}(\alpha_0, \alpha_0) \geq 0$ nach Voraussetzung erfuellt, erhalten wir $(-1)^p Q_{f^*L_X}(f^*\alpha_0, f^*\alpha_0) \geq 0$ auf dem Rueckzugsunterkegel. Fuer eine allgemeine ample Klasse $L_Y \in \mathrm{Amp}(Y)$ folgt aus Endlichkeit allein keine Fortsetzung: Die primitive Komponente von $f^*\alpha$ haengt von $L_Y$ ab, und der Rueckzugsunterkegel muss nicht dicht im amplen Kegel von $Y$ liegen. Wenn eine zusaetzliche Ample-Kegel-Vergleichshypothese Dichtheit oder Gleichheit des relevanten Kegels liefert, setzen Stetigkeit der Hodge-Riemann-Form und der Lefschetz-Zerlegung die Ungleichung auf alle amplen Klassen fort.
...
Die Erweiterung von $f^*\mathrm{Amp}(X)$ auf ganz $\mathrm{Amp}(Y)$ erfordert einen expliziten Vergleich der amplen Kegel. Eine nuetzliche hinreichende Bedingung ist, dass der Rueckzugsunterkegel ein dichtes Bild im relevanten numerischen amplen Kegel hat oder dass die numerischen ample Kegel nach Rueckzug uebereinstimmen. Das kann in Picard-Zahl-eins-Situationen gelten, scheitert aber im Allgemeinen. Ohne solche Zusatzannahme ist Proposition~\ref{prop:pullback} nur eine Funktorialitaetsaussage auf dem Rueckzugsunterkegel.
\end{remark}

\begin{conjecture}[Vorwaertsschub -- endliche Morphismen]
...
\label{prop:kunneth}
Seien $\alpha \in \mathrm{AP}^p(X)$ und $\beta \in \mathrm{AP}^q(Y)$ primitiv bezueglich amplen Klassen $L_X$ bzw.\ $L_Y$. Dann erfuellt $\alpha \boxtimes \beta$ (AP1) fuer alle Produktpolarisierungen $L = L_X \boxtimes 1 + 1 \boxtimes L_Y$ und erfuellt (AP2)--(AP3). Insbesondere gilt $\alpha \boxtimes \beta \in \mathrm{AP}^{p+q}(X \times Y)$, wenn Produktpolarisierungen $\mathrm{Amp}(X \times Y)$ erzeugen (z.B.\ wenn $X$ oder $Y$ Picard-Zahl~$1$ hat); fuer allgemeine Produktvarietaeten bleibt die Erweiterung auf alle amplen Klassen offen.
\end{proposition}
...
\begin{remark}[Barriere-Lemma: HR-Positivitaet ist automatisch gesaettigt]
\label{rem:barrier-lemma}
Bevor wir zum No-Go-Theorem uebergehen, heben wir den strukturellen Grund hervor, warum jede positivitaetsbasierte Verstaerkung der Hodge-Vermutung das Hodge-Riemann-Framework verlassen muss. Fuer eine glatte projektive Varietaet $X$ und jede ample Klasse $L$ garantieren die Hodge-Riemann-Bilinearrelationen:
\begin{enumerate}
\item $(-1)^p Q_L(\alpha, \bar{\alpha}) > 0$ fuer jede nichtverschwindende primitive $(p,p)$-Klasse $\alpha$;
\item $Q_L$ ist nichtentartet auf $H^{p,p}_{\mathrm{prim}}(X)$.
\end{enumerate}
Diese Bedingungen sind \emph{automatisch erfuellt} fuer alle rationalen $(p,p)$-Klassen -- algebraische wie nichtalgebraische. Folglich kann keine Positivitaetsbedingung, die durch die HR-Relationen impliziert wird, algebraische von nichtalgebraischen Klassen unterscheiden. Dies ist die Barriere: die ,,offensichtliche'' Verstaerkung (Positivitaet unter allen Polarisierungen, allen Automorphismen, allen Lefschetz-Zerlegungen fordern) kollabiert zur Bedingung, eine absolute Hodge-Klasse zu sein (Theorem~\ref{thm:nogo}). Die einzigen Auswege erfordern \emph{nichtbilineare} Positivitaetsbedingungen (motivische Faltung, Galois-Mittelung, prismatische Einschraenkungen) oder Informationen von \emph{ausserhalb} der Hodge-Riemann-Struktur (o-Minimalitaet, derivierte Kategorien).
\end{remark}
...
\begin{proposition}[Aequivalenz fuer abelsche Varietaeten]
\label{thm:abelian}
...
\begin{enumerate}
\item \emph{Positiver Test:} Falls eine Klasse $\alpha$ die Eigenschaft $(-1)^p Q_L^\sigma(\alpha^\sigma) < 0$ fuer ein $\sigma \in \mathrm{Aut}(\mathbb{C})$ und ein $L$ besitzt, dann ist $\alpha$ beweisbar nichtalgebraisch (nach Theorem~\ref{thm:easy_direction}).
\item \emph{Null-Test:} Falls $(-1)^p Q_L^\sigma(\alpha^\sigma) \geq 0$ fuer alle $\sigma$ und alle $L$, dann ist $\alpha$ arithmetisch positiv -- somit absolut Hodge -- aber nicht notwendigerweise algebraisch. Dies ist der Bereich, in dem die GHR-Diagnostik \emph{stumm} ist.
\item \emph{Struktureller Test:} Das GHR-Spektrum detektiert Nichtalgebraizitaet nur fuer Klassen, die \emph{nicht} absolut Hodge sind. Fuer absolute nichtalgebraische Klassen (falls sie existieren) wird eine strikt staerkere Diagnostik benoetigt.
\end{enumerate}
Das GHR-Spektrum fungiert somit als \emph{notwendiger-Bedingung-Filter}: es kann Nichtalgebraizitaet zertifizieren, aber nicht Algebraizitaet. Diese Asymmetrie spiegelt die Pattern-A-Architektur im gesamten FST-Programm wider, wo Positivitaetszertifikate Alternativen ausschliessen, aber keine Loesungen konstruieren.
\end{remark}
...
\begin{remark}[Meta-Muster: versuchte Verstaerkung kollabiert an der Saettigung]
\label{rem:meta-pattern-saturation}
Die strukturelle Lektion dieser Arbeit geht ueber die Hodge-Vermutung hinaus. In jeder Pattern-A-Instanziierung im FST-Programm ist die ,,offensichtliche'' Verstaerkung der Positivitaetsbedingung automatisch durch das Strukturtheorem gesaettigt:
\begin{itemize}
\item \emph{Hodge:} HR-Positivitaet $\Rightarrow$ AP $=$ AbsHodge (diese Arbeit).
\item \emph{NS-Skelett:} Lipschitz-Regularitaet der Attraktor-Projektion $\Rightarrow$ erfordert positive Reichweite $\Rightarrow$ aequivalent zur Existenz einer Inertialmannigfaltigkeit (No-Go-Zone, vgl.\ die Begleitarbeit zu NS).
\item \emph{BSD:} Hoehenpositivitaet fuer Rang $\leq 1$ $\Rightarrow$ gesaettigt durch Gross--Zagier + Kolyvagin; die ,,naechste'' Positivitaet (Rang $\geq 2$) erfordert Objekte, die noch nicht existieren.
\end{itemize}
In jedem Fall erfordert Fortschritt das \emph{Verlassen} des gesaettigten Positivitaets-Frameworks -- entweder durch Einfuehrung nichtlinearer/nichtbilinearer Bedingungen oder durch Import von Regularitaet aus einer anderen Quelle (z.B.\ die TLL/LDI-Maschinerie fuer NS oder prismatische Kohomologie fuer Hodge). Das Saettigungsphaenomen ist kein Versagen der Methode, sondern eine \emph{Diagnostik}: es identifiziert die praezise Grenze, an der die aktuellen Werkzeuge ihre Reichweite erschoepfen.
\end{remark}
...
Nie \cite{Nie2021} konstruierte absolute Hodge-Zykel in der prismatischen Kohomologie abelscher Varietaeten ueber $p$-adischen Koerpern unter Verwendung von Andr\'es motivierten Zykeln im Bhatt--Scholze-Framework. Prismatische Kohomologie $R\Gamma_\Delta(X/S)$ interpoliert \'etale, de Rham- und kristalline Kohomologie integral und liefert einen universellen kohomologischen Rahmen. Die Verbindung zur Positivitaet ueber Bridgeland-Stabilitaetsbedingungen auf derivierten Kategorien legt nahe, dass nichtlineare, derivierte Positivitaetsbedingungen -- die die rigide Galois-Struktur prismatischer Kristalle mit geometrischer Stabilitaet kombinieren -- die Luecke zwischen Absolutheit und Algebraizitaet ueberbruecken koennten.
\end{remark}

\subsubsection*{Schwellenaxiom: jenseits von Deligne}

\begin{axiom}[Sprachbruecke -- JD-Hodge]\label{ax:jd-hodge}
Es existiert eine Kategorie $\mathcal{C}$ von ,,Hodge-Traegern'' (Objekte, die kohomologische Daten repraesentieren), ausgestattet mit:
\begin{enumerate}
\item[(JD1)] \textbf{Funktorialitaet:} Rueckzug, Vorwaertsbild und Produkte wirken auf $\mathcal{C}$ ohne nicht-kanonische Wahlen.
\item[(JD2)] \textbf{Intrinsische Filtration:} Eine funktorielle Hodge-Filtration $F^\bullet$ ist kanonisch definiert (nicht ueber eine Wahl von Einbettung oder Modell).
\item[(JD3)] \textbf{Kanonische Polarisation:} Eine mit (JD1) und (JD2) kompatible Positivitaetsstruktur existiert, die die Hodge--Riemann-Form verallgemeinert.
\end{enumerate}
In dieser Sprache wird die Hodge-Vermutung zu einer \emph{formalen Konsequenz}: Positivitaet in $\mathcal{C}$ impliziert Algebraizitaet. Delignes Absolutheit ist dann ein Schatten (notwendige Bedingung) der reicheren Struktur in $\mathcal{C}$.
\end{axiom}

\begin{remark}[Der Nicht-Kanonizitaets-Scanner]\label{rem:noncanonicity-scanner}
Um zu lokalisieren, wo die aktuelle Theorie hinter JD-Hodge zurueckbleibt, suche nach drei Signaturen:
\begin{enumerate}
\item \textbf{Wahlabhaengigkeit:} Wo treten Spaltungen oder Vergleichsisomorphismen (de Rham/Betti/$\ell$-adisch) auf, die ,,existieren'', aber keine natuerlichen Transformationen sind?
\item \textbf{Inkompatible Operationen:} Welche geometrische Operation (Rueckzug unter nicht-endlichen Morphismen, Gysin-Vorwaertsbild) zerstoert die Polarisation oder destabilisiert die Filtration?
\item \textbf{Vergleichsbrueche:} Wo haengt die Aussage von einem Vergleich zwischen Realisierungen ab, der nicht strukturell kontrolliert ist?
\end{enumerate}
Im aktuellen AP-Framework tritt der Bruch bei AP2a (Absolutheit) auf: Der Vergleichsisomorphismus $H^*_{\mathrm{dR}}(X) \otimes \mathbb{C} \cong H^*_{\mathrm{Betti}}(X^\sigma) \otimes \mathbb{C}$ ist \emph{kein} Morphismus in einer natuerlichen Kategorie -- er erfordert die Wahl von $\sigma \in \mathrm{Aut}(\mathbb{C}/\mathbb{Q})$.
\end{remark}

\begin{remark}[Kandidaten fuer $\mathcal{C}$]\label{rem:candidates-C}
Drei Kandidaten fuer die Kategorie $\mathcal{C}$ werden derzeit untersucht:
\begin{enumerate}
\item \textbf{Prismatische Kohomologie} (Bhatt--Scholze, Nie \cite{Nie2021}): $R\Gamma_\Delta(X/S)$ interpoliert alle klassischen Realisierungen integral, mit einer kanonischen Frobenius-Aktion fuer (JD2). Die Positivitaet (JD3) ist noch nicht formuliert, aber strukturell moeglich via Bridgeland-Stabilitaet.
\item \textbf{Motivische Kohomologie} (Voevodsky): Die derivierte Kategorie der Motive $\mathrm{DM}(k)$ erfuellt (JD1) per Konstruktion. Die Standard-Vermutungen wuerden (JD2) und (JD3) liefern, sind aber unbewiesen.
\item \textbf{Gemischte Hodge-Moduln} (Saito): Erfuellen (JD1)--(JD2) fuer glatte projektive Varietaeten, aber (JD3) ist auf die klassische HR-Form beschraenkt -- genau die AP$=$AbsHodge-Barriere dieses Papers.
\end{enumerate}
\end{remark}

\begin{lemma}[JD-Hodge impliziert die Hodge-Vermutung fuer abelsche Varietaeten]\label{lem:jd-abelian}
Wenn $\mathcal{C}$ (JD1)--(JD3) erfuellt und mit der Tannakian-Struktur der Kategorie abelscher Motive kompatibel ist, dann ist jede Hodge-Klasse auf einer abelschen Varietaet algebraisch.
\end{lemma}

\begin{remark}[Die Deligne-Grenze als Sprachdefekt]\label{rem:deligne-language}
Axiom~\ref{ax:jd-hodge} identifiziert die Obstruktion als \emph{linguistisch}, nicht rechnerisch: Die Vergleichsisomorphismen $\mathrm{comp}_{\mathrm{dR} \leftrightarrow B}$ und $\mathrm{comp}_{\ell\text{-adic} \leftrightarrow B}$ existieren, sind aber nicht funktoriell in $X$. Absolute Hodge-Klassen sind genau diejenigen, die unter allen Realisierungen kompatibel sind -- und Algebraizitaet wird zu $\mathrm{AH}(X) = \mathrm{Im}(\mathrm{CH}^*(X) \to H^*(X))$. Waeren die Vergleiche natuerliche Transformationen in einer Kategorie $\mathcal{C}_{\mathrm{abs}}$, waere dies eine formale Konsequenz. Die Deligne-Grenze lautet daher: \emph{Nicht-kanonische Spaltungen machen jeden Algebraizitaetsbeweis instabil unter Funktoren}.
\end{remark}
...
Die Theorie o-minimaler Strukturen (Bakker--Brunebarbe--Tsimerman~\cite{BBT2020}) wurde angewendet, um Definierbarkeitsresultate fuer Periodenabbildungen und Hodge-Loki zu beweisen. Eine moegliche AP$'$-Bedingung: Fordere, dass die ,,Umlaufbahn'' von $\alpha$ unter der definierbaren Periodenabbildung algebraisch ist. Nach CDK gilt dies fuer algebraische Klassen. Fuer nicht-algebraische Hodge-Klassen koennte die Umlaufbahn ,,wild'' (transzendent) sein, was die Zahmheitsbedingung verletzt. Dies steht in Verbindung mit der Arbeit von Binyamini--Novikov~\cite{BinyaminiNovikov2017} zum Zaehlen rationaler Punkte auf definierbaren Mengen.
\end{remark}
...
\emph{Rand-Positivitaet.}
Fuer eine Familie ampler Klassen $L_t \to L_0$, wobei $L_0$ nef aber nicht ample ist, koennen die vorzeichenkorrigierten Werte $(-1)^p Q_{L_t}$ fuer $t \to 0$ nichtnegative Grenzwerte haben, selbst wenn $Q_{L_0}$ nicht definiert ist. Diese ,,Rand-Positivitaet'' erfasst Klassen, die im Grenzwert algebraisch sind, aber von keiner einzelnen Polarisierung detektiert werden.

\emph{Positivitaet unter motivischer Faltung.}
Fuer eine Produktvarietaet $X \times Y$ liefert die K\"unneth-Zerlegung $H^{p,p}(X \times Y) = \bigoplus_{a+b=p} H^{a,a}(X) \otimes H^{b,b}(Y)$. Die Positivitaetsbedingung auf dem Produkt ist \emph{multiplikativ}: $(-1)^{a+b} Q_{L_X \boxtimes L_Y} = (-1)^a Q_{L_X} \cdot (-1)^b Q_{L_Y}$. Dies ist staerker als K\"unneth-Positivitaet auf jedem Faktor und kann nichtalgebraische Klassen detektieren, die sich in den Kreuztermen ,,verstecken''.

\emph{Positivitaet nach Galois-Mittelung.}
Fuer eine absolute Hodge-Klasse $\alpha$ ist der Mittelwert $\bar\alpha = \frac{1}{|\mathrm{Aut}(\mathbb{C}/\mathbb{Q})|} \sum_{\sigma} \sigma^* \alpha$ rational und daher algebraisch (durch die Hodge-Vermutung fuer rationale Klassen, die bekannt ist). Die arithmetischen Positivitaetsbedingungen AP1--AP3 lassen sich umformulieren als: $\alpha$ ist algebraisch genau dann, wenn $\alpha$ ,,nahe'' an seinem Galois-Mittelwert in der $Q_L$-Metrik liegt. Dies ist das Hodge-theoretische Analogon thermodynamischer Selektion: Der Galois-Mittelwert wirkt als ,,IR-Regularisierung.''
\end{remark}

\begin{remark}[Galois-Mittelung: profiniter Limes]\label{rem:galois-profinite}
Die naive Mittelung $\bar{\alpha} = \frac{1}{|\mathrm{Aut}(\mathbb{C}/\mathbb{Q})|} \sum_\sigma \sigma^*\alpha$ ist nicht wohldefiniert, da $\mathrm{Aut}(\mathbb{C}/\mathbb{Q})$ eine profinite Gruppe der Kardinalitaet $2^{\mathfrak{c}}$ ist. Die korrekte Formulierung verwendet den projektiven Limes: Fuer jede endliche Galois-Erweiterung $K/\mathbb{Q}$ bilde man $\bar{\alpha}_K = \frac{1}{|\mathrm{Gal}(K/\mathbb{Q})|} \sum_{\sigma \in \mathrm{Gal}(K/\mathbb{Q})} \sigma^*\alpha$. Der Galois-Mittelwert ist dann $\bar{\alpha} = \lim_K \bar{\alpha}_K$, der in $H^{2p}(X, \mathbb{Q})$ durch die Absolutheit von $\alpha$ existiert.
\end{remark}
...
Die GHR-Obstruktion wurde an sechs Varietaeten getestet: generischer $T^4$, Weil-Torus (CM durch $\mathbb{Z}[i]$), $K3 \times K3$, Fermat-Quintik $X_5$, Voisin-Typ nicht-projektiver Torus und eine SDP-Relaxierung des AP-Kegels. Fuer alle projektiven Beispiele ist AP1 durch die Hodge--Riemann-Relationen garantiert. Der $K3 \times K3$-Test mit Nicht-Tensorprodukt-Stoerungen erzeugt 3 negative Eigenwerte von $(-1)^p Q_L$, was bestaetigt, dass die GHR-Obstruktion nicht-triviale Detektionskraft fuer nicht-algebraische Klassen besitzt. Die AP-Kegel-Analyse zeigt alle zufaelligen Klassen innerhalb des Kegels mit positiver Marge ($0{,}24$), was Robustheit anzeigt. Skript: \texttt{compute\_voisin\_test.py}.
\end{remark}
...
arithmetische Positivitaet auf absolute Hodge-Klassen kollabiert, ist der
naechste Schritt kein weiteres globales Positivitaetsaxiom, sondern eine
Testfamilien-Leiter fuer moegliche Verstaerkungen jenseits von Delignes
Absolutheit.
Die Symbole $\mathrm{AP}'$, $\mathrm{AP}''$ und
$\mathrm{AP}^{\prime\prime\prime}$ bezeichnen keine globalen
Ersatzvermutungen fuer die Hodge-Vermutung. Sie bezeichnen zunehmend
strengere Tests auf kontrollierten Spezialfamilien: $\mathrm{AP}'$ erfasst
Provenienzdefekte, $\mathrm{AP}''$ fordert ein beschraenktes
Orbit-Rigiditaetszertifikat, und $\mathrm{AP}^{\prime\prime\prime}$ filtert
...
Die aktuelle CORE-Schliessung von Riemann-$C2/MS2$ verstaerkt das
Hodge-Resultat dieses Papers nicht.  Hodge bleibt ein negativer
Kontrollfall des Programms: Hodge--Riemann-Positivitaet saettigt zu breit,
sodass jeder kuenftige Transfer wirklich neue arithmetische oder motivische
Zusatzbedingungen braucht und nicht die Riemann-Kernel-Schliessung
wiederverwenden darf.

Die fuenf Transferfelder lauten:
\begin{description}
  ...
  \item[Approximation.] Abelsche Vierfalten vom Weil-Typ, singulaere
  OG6-hyperkaehlersche Modelle, Potenzen bekannter Beispiele und
  Reduktionen in Charakteristik $p$.
  \item[Gap.] Zusatzbedingungen jenseits der Hodge--Riemann-Positivitaet:
  $p$-adische Slopes, Tate-Constraints, Mumford--Tate-Restriktionen und
  o-minimale Algebraizitaet.
  \item[Grenzuebergang.] Deformation, Potenzen und Spezialisierung von
  kontrollierten Familien in einen breiteren motivischen Rahmen.
\end{description}

Eine nuetzliche naechste Version sollte deshalb eine Tabelle von
Testfamilien enthalten.  Fuer jede Familie werden festgehalten: bekannter
Hodge-/Tate-Status, der in der Literatur verwendete Zykelmechanismus, die
vorgeschlagene staerkere Bedingung $\mathrm{AP}'$ und ob der Defekt
verschwindet.  Weil-Vierfalten mit Diskriminante eins, OG6-Konstruktionen,
abelsche Varietaeten ueber endlichen Koerpern mit Standardvermutung vom
Hodge-Typ und Voisin-artige Stresstests bilden die natuerliche erste
...
einem Stresstest trennt, wird sie zu einem echten Kandidaten fuer weitere
Formalisierung.

\subsubsection*{Hin zu \texorpdfstring{$\mathrm{AP}'$}{AP'}: Provenienzdefekte}
\label{subsec:provenance-defects}

Die erste Verschaerfung ist als Provenienzscanner zu lesen, nicht als ein
weiterer Positivitaetskegel. Relativ zu einer benannten Testfamilie definiert
...
\]
dessen Eintraege Steigungs-Kompatibilitaet an guten Reduktionen, Erklaerung
durch den Tate-/Galois-Fixteil, Mumford--Tate-Kompatibilitaet,
...
Fingerprint-Aussage wuerde besagen, dass in einer Testfamilie mit unabhaengig
verfuegbaren Tate-/Standardvermutungsdaten und Vergleichsfunktorialitaet aus
...
\subsubsection*{Hin zu \texorpdfstring{$\mathrm{AP}''$}{AP''}: Bedingte Orbit-Rigiditaet}
\label{subsec:orbit-rigidity}

Die fuenf Transferkanaele legen eine prototypische Verschaerfung nahe.  Sei
$f\colon \mathcal{X}_{S}\to S$ ein Spread-out von $X$ ueber einer glatten
Basis~$S$, sei $Z\subset S$ eine irreduzible algebraische Komponente des
Hodge-Lokus von~$\alpha$, sei $B$ die zugehoerige relative Chow- oder
...
\item[(i)] \emph{Properness.} Die relativen Chow-/Hilbert-Komponenten,
  die die relevanten Zykelklassen parametrisieren, sind proper ueber~$S$.
\item[(ii)] \emph{Dichtheit.} $T$ ist Zariski-dicht in~$Z$.
\item[(iii)] \emph{Vergleichsfunktorialitaet.}  Die Vergleichsisomorphismen
  ...
  Restklassen.
\item[(iv)] \emph{Spezialisierungsrigiditaet.}  Basiswechsel entlang
  $Z\to S$ erzeugt keine neuen residualen Zykelklassen.
\end{enumerate}

\begin{lemma}[Bedingte Orbit-Rigiditaet]
\label{lem:orbit-rigidity}
Sei $\alpha\in\mathrm{AbsHodge}^{p}(X)$ orbit-rigide auf $(f,Z,B)$
im obigen Sinne.  Dann liegt die Einschraenkung $\alpha_{\eta}$ auf die
generische Faser $\mathcal{X}_{\eta}$ ueber dem generischen Punkt $\eta$
...
implizieren ueber das Bewertungskriterium, dass der Abschluss des
algebraischen Lokus in $B$ den generischen Punkt~$\eta$ von~$Z$ enthaelt.
Vergleichsfunktorialitaet \emph{(iii)} stellt sicher, dass jede algebraische
Klasse ueber $t\in T$ konsistent in allen kohomologischen Realisierungen
liftet.  Spezialisierungsrigiditaet \emph{(iv)} verhindert die Akkumulation
transzendenter Restklassen am Limes.  Ein vollstaendiger Beweis erfordert
das Cattani--Deligne--Kaplan-Prinzip fuer Hodge-Loki
\cite{CattaniDeligneKaplan1995} zusammen mit einem Properness-Argument
fuer das relative Hilbert-Schema; beide Schritte sind bedingt durch die
Standardvermutungen in den relevanten Charakteristik-$p$-Reduktionen.
\end{proof}

\begin{remark}
\label{rem:ap-double-prime}
Lemma~\ref{lem:orbit-rigidity} beweist nicht die Hodge-Vermutung.
Es ist als bedingtes Schliessungsschema zu lesen, nicht als neues
unbedingtes Algebraizitaetstheorem. Seine Aussagekraft liegt vollstaendig in
der unabhaengigen Verifikation beschraenkter relativer Chow-/Hilbert-Daten,
der Vergleichsfunktorialitaet und der Spezialisierungsrigiditaet in einer
benannten Testfamilie. Unter diesen separat geprueften Voraussetzungen
propagiert Algebraizitaet von einer Zariski-dichten Menge zum generischen
Punkt.  Weil-abelsche Vierfalten mit Diskriminante eins sind ein
natuerlicher erster Kandidat fuer einen Positivtest; $K3\times K3$-Stoerungen
ausserhalb des Tensorprodukt-Lokus dienen als erste Negativkontrolle.
Ob $\mathrm{AP}''$ diese beiden Familien trennt, entscheidet darueber,
ob es eine echte Verschaerfung jenseits von $\mathrm{AbsHodge}$ darstellt.
\end{remark}

\subsubsection*{Hin zu \texorpdfstring{$\mathrm{AP}^{\prime\prime\prime}$}{AP'''}: Uniformitaet und Anti-Advice}
\label{subsec:ap-triple-prime}

Das Orbit-Rigiditaetszertifikat besitzt weiterhin einen Fehlermodus: Die
Familie $f$, die Hodge-Lokus-Komponente $Z$, die Chow-/Hilbert-Komponente
$B$ oder die dichte Menge $T$ koennten erst nach Kenntnis der Zielklasse
$\alpha$ gewaehlt werden. Dann kann das Zertifikat $\alpha$ kodieren, statt
es zu erklaeren. Wir verwenden daher
\[
\mathrm{AP}^{\prime\prime\prime}_{\mathrm{unif}}(\alpha;R,C)
\]
als Anti-Tautologie-Filter fuer ein $\mathrm{AP}''$-Zertifikat $C$. Gefordert
...
Testfamilienleiter abhaengt, mit beschraenktem Advice, Erosionsstabilitaet
nach Entfernen einer $\alpha$-spezifischen Inzidenzbedingung, Traeger auf
einer Nachbarfamilie oder Zariski-dichten Menge und Scheitern auf
Negativkontrollen, die nur durch Hodge--Riemann-Formstabilitaet getrieben
sind. Diese Bedingung fuegt keine neue globale Hodge-Vermutung hinzu; sie
...
\toprule
\textbf{Familie} & \textbf{Zykelmechanismus} & \textbf{AP$'$} & \textbf{AP$''$} & \textbf{AP$^{\prime\prime\prime}$} & \textbf{Status} \\
\midrule
CM-/Weil-Vierfalten & Endomorphismuszyklen & Slope/Tate/MT & Kandidat & Regel aus Endomorphismen & Positivkontrolle \\
Abelsche Vierfalten & bekannte Zykelspanne & Tate/MT & Kandidat & ambiente Familienregel & Positivkontrolle \\
OG6-Typ-Familien & hyperkaehlersche Zykel & Monodromie/Zykel & offen & Registry noetig & Testfall \\
Char-$p$-Tate-Faelle & Tate-Klassen & Slope/Tate & bedingt & Reduktionsregel & arithmetischer Test \\
...
Diese Arbeit ist ein \textbf{Grenzresultat}: Sie bestimmt praezise, was positivitaetsbasierte Methoden fuer die Hodge-Vermutung leisten koennen und was nicht.
...
\midrule
\multicolumn{2}{l}{\textit{No-Go-Theorem:}} \\
AP $=$ AbsHodge (HR liefert nichts Neues) & Theorem \\
HC reduziert sich auf Delignes Frage & Korollar \\
\midrule
\multicolumn{2}{l}{\textit{Konstruktive Werkzeuge:}} \\
GHR-Spektrum-Definition & Definition \\
Endlicher Rueckzug / primitive K\"unneth & Partiell, bedingt \\
Voisin-Typ-GHR-Detektion & Numerisch \\
Bedingte Orbit-Rigiditaet $(\mathrm{AP}'')$ & Lemma (bedingt) \\
Uniformer Anti-Advice-Filter $(\mathrm{AP}''')$ & Methodisches Kriterium \\
\midrule
\multicolumn{2}{l}{\textit{Offen (aequivalent zu Deligne 1982):}} \\
AbsHodge $\stackrel{?}{=}$ Algebraisch & Offen \\
Staerkere Positivitaet jenseits HR & Forschungsrichtung \\
\bottomrule
\end{tabular}}
\end{center}
...
\item \textbf{Partielle Funktorialitaetstests:} Die Arbeit dokumentiert partielle Funktorialitaetstests, keine vollstaendige Funktorialitaetstheorie: endlicher Rueckzug ist an den Vergleich der amplen Kegel gebunden, primitive K\"unneth-Produkte sind fuer Produktpolarisierungen kontrolliert, und endlicher Vorwaertsschub bleibt offen.
\end{enumerate}
