...
Unser Hauptresultat ist ein \emph{No-Go-Theorem}: Die Hodge--Riemann-Relationen allein koennen algebraische nicht von nicht-algebraischen Hodge-Klassen unterscheiden (Theorem: AP\,$=$\,AbsHodge). Dies grenzt praezise die Reichweite positivitaetsbasierter Ansaetze ab und identifiziert Delignes Frage (1982) als die exakte Obstruktion. Als konstruktive Ergaenzung fuehren wir das Galois--Hodge--Riemann-Spektrum als potentielles Werkzeug zur Erkennung von Nicht-Algebraizitaet jenseits der Hodge--Riemann-Schranke ein.
...
\label{prop:pullback}
Sei $f: Y \to X$ ein endlicher Morphismus glatter projektiver Varietaeten. Wenn $\alpha \in \mathrm{AP}^p(X)$, dann erfuellt $f^*\alpha$ (AP1) fuer alle amplen Klassen im Unterkegel $f^*\mathrm{Amp}(X) \subset \mathrm{Amp}(Y)$ und erfuellt (AP2)--(AP3). Insbesondere gilt $f^*\alpha \in \mathrm{AP}^p(Y)$, wann immer $f^*\mathrm{Amp}(X)$ dicht in $\mathrm{Amp}(Y)$ liegt (z.B.\ wenn $\rho(Y) = 1$).
\end{proposition}
...
Die Erweiterung von $f^*\mathrm{Amp}(X)$ auf ganz $\mathrm{Amp}(Y)$ erfordert, dass $f^*\mathrm{Amp}(X)$ dicht in $\mathrm{Amp}(Y)$ in der euklidischen Topologie auf $N^1(Y)_{\mathbb{R}}$ liegt. Dies gilt, wenn $f$ surjektiv ist und $\rho(Y) = 1$ (Picard-Zahl Eins), kann aber fuer hoehere Picard-Zahl versagen. Im allgemeinen Fall gilt die Proposition fuer den Unterkegel $f^*\mathrm{Amp}(X) \subset \mathrm{Amp}(Y)$; die Erweiterung auf alle amplen Klassen bleibt bedingt durch die Dichte-Hypothese.
\end{remark}
...
\subsection{Post-Zookeeper-Transfer: Testfamilien vor allgemeinem Hodge}
...
Funktorialitaet (endliche Morphismen) & Proposition \\
...
\item \textbf{Funktorialitaetsresultate:} Der arithmetisch positive Kegel ist funktoriell unter endlichem Rueckzug und K\"unneth-Produkt fuer primitive Klassen; endlicher Vorwaertsschub bleibt konjektural.
\end{enumerate}
