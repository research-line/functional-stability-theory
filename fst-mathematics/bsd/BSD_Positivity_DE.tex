...
\usepackage{hyperref}
\hypersetup{
    pdftitle={Die BSD-Vermutung als Positivitaets-Normalform: Eine strukturelle Uebersicht},
...
\title{Die BSD-Vermutung als Positivitaets-Normalform:\\Eine strukturelle Uebersicht}
...
\end{equation}
wobei $r = \rank E(\mathbb{Q})$, $\Omega_E$ die reelle Periode, $R_E$ der Regulator (Determinante der N\'eron-Tate-Hoehenpaarung), $c_p$ die Tamagawa-Zahlen und $\Sha(E/\mathbb{Q})$ die Tate-Shafarevich-Gruppe ist. Wir schreiben durchgaengig $E_{\mathrm{tors}} := E(\mathbb{Q})_{\mathrm{tors}}$ zur Kurzform.
\end{conjecture}
...
Diese Arbeit ist in sich geschlossen; alle Hauptresultate haengen nur von Definitionen und Beweisen innerhalb dieses Dokuments ab. Verbindungen zum breiteren FST-Programm werden als Kontext zitiert, sind aber logisch nicht erforderlich.
...
\label{ax:iwasawa}
In der zyklotomischen $\mathbb{Z}_p$-Erweiterung $\mathbb{Q}_\infty/\mathbb{Q}$ wachsen die $p$-primaeren Selmer-Gruppen kontrolliert und asymptotisch vorhersagbar. Schreibe $X_n = \Sel_{p^\infty}(E/\mathbb{Q}_n)^\vee$ fuer den Pontryagin-Dual (einen endlich erzeugten $\mathbb{Z}_p$-Modul, dessen $\mathbb{Z}_p$-Rang gleich $\rank E(\mathbb{Q}_n)$ ist):
\begin{equation}
\mathrm{ord}_p\!\bigl(|X_n^{\mathrm{tors}}|\bigr) = \mu p^n + \lambda n + \nu + O(1)\,,
\label{eq:iwasawa_growth}
\end{equation}
wobei $X_n^{\mathrm{tors}}$ den $\mathbb{Z}_p$-Torsionsuntermodul von $X_n$ bezeichnet und $\mu, \lambda \geq 0$ die Iwasawa-Invarianten sind. Die Iwasawa-Hauptvermutung liefert eine $p$-adische $L$-Funktion $L_p(E,s)$, deren charakteristisches Ideal mit dem Selmer-Modul uebereinstimmt.
\end{axiom}
...
\begin{itemize}
\item \textbf{Kato \cite{Kato2004}:} Beweist eine Teilbarkeitsrichtung ($\mathrm{char}_\Lambda(\Sel^\vee) \subseteq (\mathcal{L}_p)$, d.h.\ das charakteristische Ideal des Selmer-Duals ist im vom $p$-adischen $L$-Funktion erzeugten Ideal enthalten) fuer alle Primzahlen~$p$, bei denen die Modulform ordinaer ist und die residuale Darstellung milde Bedingungen erfuellt. Dies gilt ohne Einschraenkung an den Rang und liefert eine obere Schranke fuer den Selmer-Modul.
...
\textit{Rolle im No-Go-Argument.} Die kontrollierte Wachstumsformel~\eqref{eq:iwasawa_growth} (Anmerkung: ,,kontrolliert'' bezieht sich auf das asymptotisch vorhersagbare Wachstum, gesteuert durch $\mu$ und $\lambda$, nicht auf strikte Monotonie, da der $O(1)$-Fehlerterm beschraenkte Schwankungen erlaubt) verhindert, dass der analytische Rang unbegrenzte ,,Oszillationen'' zwischen Schichten des $\mathbb{Z}_p$-Turms aufweist. Zusammen mit der Hauptvermutung koppelt sie die $p$-adische $L$-Funktion an den Selmer-Modul und liefert eine $p$-adische Positivitaetszwangsbedingung.
...
\item \textbf{Axiom~III $\leftrightarrow$ Kontrolliertes Wachstum (Axiom~C).} Beim Aufsteigen im $\mathbb{Z}_p$-Turm waechst die arithmetische Komplexitaet kontrolliert und vorhersagbar -- keine Oszillationen oder Spruenge im Selmer-Rang treten auf.

\item \textbf{Axiom~IV $\leftrightarrow$ Kompositionsgesetz (Axiom~D).} Norm-kompatible Schritte im Euler-System komponieren kohaerrent. Die Norm-Kompatibilitaet~\eqref{eq:euler_norm} stellt sicher, dass das Euler-System eine global konsistente Familie ist, nicht eine Sammlung von Ad-hoc-Lokalkonstruktionen.
\end{itemize}
...
\begin{proposition}[BSD-Normalform-Charakterisierung -- Bedingtes strukturelles Resultat]
\label{thm:bsd_normal}
\textbf{(Bedingt auf Axiome~I--IV.)} Die folgenden Schlussfolgerungen gelten \textit{nur unter der Annahme}, dass die Axiome~I--IV erfuellt sind. Fuer Rang~$\leq 1$ ist diese Annahme durch etablierte Theoreme erfuellt; fuer Rang~$\geq 2$ haengen die Axiome~I und~IV selbst von unbewiesenen Vermutungen ab (siehe Bemerkung~\ref{rem:axiom-independence}). \textbf{Dies ist eine strukturelle Charakterisierung, kein neuer Beweis von BSD.}
...
\begin{remark}[Bedingter Status und logische Zirkularitaet bei Rang~$\geq 2$]
\label{rem:conditional_status}
Proposition~\ref{thm:bsd_normal} ist eine \textit{bedingte strukturelle Charakterisierung}: sie zeigt, dass die BSD-Formel die einzige Normalform ist, \textit{gegeben} die Axiome~I--IV. \textbf{Sie liefert keinen unbedingten Beweis von BSD, und fuer Rang~$\geq 2$ ist das Argument logisch zirkulaer.} Fuer Rang~$\leq 1$ sind alle vier Axiome unabhaengig etablierte Theoreme (Kolyvagin, N\'eron-Tate, Kato/Skinner-Urban, Heegner-Punkte), sodass die Schlussfolgerung unbedingt gilt und eine genueine strukturelle Reformulierung bekannter Resultate darstellt. Fuer Rang~$\geq 2$ ist jedoch Axiom~I (Endlichkeit von $\Sha$) selbst eine Konsequenz der vollen BSD-Vermutung, und Axiom~IV (Existenz eines vollstaendigen Kolyvagin-Systems hinreichender Tiefe) ist nicht unabhaengig von BSD etabliert. Die Proposition hat daher die logische Struktur ,,BSD + bekannte Fakten $\Rightarrow$ BSD ist die einzige Normalform'' bei Rang~$\geq 2$, was logisch gueltig, aber mathematisch trivial als Beweisstrategie ist. Ihr Wert liegt in der \textit{strukturellen Analyse}: die Identifikation der BSD-Formel als einziger Output der vier Axiome klaert, \textit{welche} Komponenten fuer einen Beweis benoetigt werden, auch wenn sie keinen liefert.

Die Analogie zu den Weil-Vermutungen erfordert Qualifikation: Grothendiecks Reformulierung ueber \'etale Kohomologie war \textit{nicht} zirkulaer -- die \'etale Kohomologie war ein unabhaengig konstruiertes mathematisches Objekt, und die Reformulierung bewies Rationalitaet und Funktionalgleichung direkt, reduzierte das Problem auf die Riemannsche Hypothese fuer Varietaeten. Im Gegensatz dazu sind unsere Axiome fuer Rang~$\geq 2$ nicht unabhaengig etabliert. Die Analogie gilt \textit{im Geiste} (Identifikation des korrekten strukturellen Rahmens), aber nicht \textit{in logischer Kraft}.
\end{remark}
...
Wir argumentieren \textit{heuristisch}, dass (U1)--(U5) gemeinsam $\Phi(E)$ bestimmen. \textbf{Vorbehalt:} Dieses Argument ist eine Plausibilitaetsskizze, kein rigoroser Beweis. Der rigorose Rahmen fuer die Eindeutigkeitsfrage ist die \textit{Bloch-Kato-Vermutung} (Tamagawa-Zahl-Vermutung) \cite{BlochKato1990,FontainePerrinRiou1994}, die Tamagawa-Zahlen fuer Motive definiert und die BSD-Formel als Spezialfall fuer das Motiv $h^1(E)(1)$ ableitet und die rationale Mehrdeutigkeit der Beilinson-Vermutungen beseitigt. Unser heuristisches Argument sollte als Motivation gelesen werden, \textit{warum} die BSD-Formel die erwartete Antwort ist; der Bloch-Kato-Rahmen liefert die korrekte mathematische Umgebung.

\textit{Schritt~1: Der Positivitaetsfaktor muss $R_E$ sein.} Durch (U2) gilt $\Phi(E) > 0$ fuer $r \geq 1$, sodass $\Phi(E)$ eine positiv-definite Invariante von $E(\mathbb{Q})/E(\mathbb{Q})_{\mathrm{tors}}$ enthalten muss. Die einzige kanonische solche Invariante ist die Gram-Determinante $R_E = \det(\langle P_i, P_j \rangle)$ der N\'eron-Tate-Hoehe, die die \textit{einzige} positiv-definite Bilinearform auf $E(\mathbb{Q})/E(\mathbb{Q})_{\mathrm{tors}}$ mit quadratischer Skalierung $\hat{h}(nP) = n^2 \hat{h}(P)$, Uebereinstimmung mit der Weil-Hoehe bis auf $O(1)$ und Positiv-Definitheit modulo Torsion ist (vgl.\ \cite{Silverman2009}, Theorem~VIII.9.3). \textit{Anmerkung:} Dieser Schritt zeigt nur, dass \textit{falls} $\Phi(E)$ einen positiv-definiten Faktor enthaelt, dieser $R_E$ sein muss; er zeigt nicht, dass $\Phi(E)$ einen solchen Faktor enthalten \textit{muss}. Letzteres erfordert die Hoehenkopplungshypothese~(H1).

\textit{Schritt~2: Die archimedischen und lokalen Faktoren sind bestimmt.} (U4) (Kompatibilitaet mit der $p$-adischen Interpolationsformel und Funktionalgleichung) bestimmt den archimedischen Faktor $\Omega_E$ und die lokalen Faktoren $c_p$ durch das Birch-Lemma und die Spezialisierung der Iwasawa-Hauptvermutung.

\textit{Schritt~3: Der verbleibende Faktor ist durch Ganzzahligkeit eingeschraenkt.} Nach Extraktion von $\Omega_E \cdot R_E \cdot \prod_p c_p$ erfordert (U3), dass das verbleibende Verhaeltnis $\Phi(E)/(\Omega_E \cdot R_E \cdot \prod_p c_p)$ gleich $|\Sha(E/\mathbb{Q})|/|E(\mathbb{Q})_{\mathrm{tors}}|^2$ mit $|\Sha| \in \mathbb{Z}_{>0}$ ist. \textit{Anmerkung:} Dieser Schritt setzt voraus, dass die Zerlegung $\Phi(E) = \Omega_E \cdot R_E \cdot (\text{Rest})$ gueltig ist, was praezise der Inhalt der BSD-Formel ist. Die ,,Ganzzahligkeits-Einschraenkung'' leitet diese Zerlegung nicht unabhaengig her; vielmehr ist sie \textit{konsistent mit} der BSD-Formel, sobald diese angenommen wird.

\textit{Schritt~4: Isogenie-Invarianz schliesst Ad-hoc-Konstruktionen aus.} (U5) stellt sicher, dass $\Phi(E)$ nur von der Isogenieklasse abhaengt, nicht von Hilfswahlentscheidungen (Basis von $E(\mathbb{Q})$, Weierstrass-Modell). Die einzelnen Groessen $\Omega_E$, $R_E$, $c_p$, $|\Sha|$, $|E(\mathbb{Q})_{\mathrm{tors}}|$ sind \textit{nicht} einzeln isogenie-invariant, aber ihre Kombination in der BSD-Formel ist es, da $L(E,s)$ nur von der Isogenieklasse abhaengt (Faltings). Dies erzwingt, dass $\Phi(E)$ das einzige isogenie-invariante Produkt kanonischer arithmetischer Daten ist.

Zusammenfassend machen die fuenf Bedingungen die BSD-Formel zum einzigen \textit{natuerlichen Kandidaten} der Form ,,Produkt kanonischer arithmetischer Invarianten''. Dies ist eine schwaechere Behauptung als ein rigoroser Eindeutigkeitsbeweis: sie zeigt, dass die BSD-Formel der einzige Ausdruck \textit{innerhalb der Klasse von Produkten bekannter kanonischer Invarianten} ist, nicht dass sie die einzige Formel ist, die (U1)--(U5) im Raum aller moeglichen Funktionen von~$E$ erfuellt. Die Bloch-Kato-Vermutung \cite{BlochKato1990} liefert den rigorosen motivischen Rahmen, in dem diese Eindeutigkeit praezisiert werden kann.
\end{proof}
...
\begin{proposition}[Konsistenzpruefung: durch die Axiome ausgeschlossene Szenarien]
\label{prop:exclusion}
Die folgenden Szenarien werden durch die Axiome ausgeschlossen. \textit{Anmerkung:} Fuer Rang~$\leq 1$ geben die Ausschluesse bekannte Resultate (Kato, Kolyvagin) in der Sprache unseres Axiomsystems wieder. Fuer Rang~$\geq 2$ werden nur (E3) und (E4) ausgeschlossen, und beide folgen trivial aus den Axiomannahmen. Die nichttrivialen Faelle (E1) und (E2) bleiben fuer Rang~$\geq 2$ bedingt.
...
\item[\textbf{(E2)}] \textbf{Geometrischer Rang ohne analytische Ableitung:} $\rank E(\mathbb{Q}) = r > \ord_{s=1} L(E,s) = s$. \textit{Unter der Annahme der Leitterm-Formel} und der Euler-System-Schranke (H2) ist dies ausgeschlossen: (H2) ergibt $r = \rank E(\mathbb{Q}) \leq \ord_{s=1} L(E,s) = s$, im direkten Widerspruch zu $r > s$. Fuer Rang~$\leq 1$ (wo Axiom~IV unbedingt ueber Katos Euler-System bewiesen ist) ist (E2) somit sofort ausgeschlossen. \textbf{Fuer Rang~$\geq 2$ ist der Ausschluss bedingt:} er haengt von der vollen Euler-System-Schranke (H2) ab, die ihrerseits die Existenz eines Kolyvagin-Systems hinreichender Tiefe erfordert -- ein grosses offenes Problem. Ohne (H2) ist die Ungleichung $\rank \leq \ord$ nicht etabliert und es ergibt sich kein Widerspruch; (E2) \textit{kann derzeit fuer Rang~$\geq 2$ nicht ausgeschlossen werden}. Ergaenzend liefert, unter der Annahme dass die Leitterm-Formel gilt, die Hoehenkopplung (H1) eine kompatible Perspektive: sie behauptet, dass $L^{(r)}(E,1)/r!$ gleich $C_r \cdot R_E$ als \textit{Leitterm-Identitaet} ist und die $r$-te Ableitung an den Regulator koppelt genau dann, wenn $\ord_{s=1} L(E,s) = r$; wenn $\ord = s < r$, ist die Voraussetzung der Kopplungsformel nicht erfuellt.
...
Das Gross-Zagier-Theorem \cite{GrossZagier1986} besagt: fuer einen geeigneten imaginaer-quadratischen Koerper $K$, der die Heegner-Hypothese erfuellt (jede Primzahl, die den Fuehrer $N$ teilt, zerfaellt in $K$ -- ein solches $K$ existiert stets nach dem Chinesischen Restsatz und quadratischer Reziprozitaet), gilt
\begin{equation}
L'(E,1) = \frac{8\pi^2 \|f\|^2}{\sqrt{|D_K|}} \cdot \hat{h}(P_K)\,,
\label{eq:gross_zagier}
\end{equation}
wobei $f$ die Neuform ist, die $E$ zugeordnet ist, $\|f\|^2 = \int_{\Gamma_0(N) \backslash \mathcal{H}} |f(z)|^2 \, \frac{dx\,dy}{y^2}$ die Petersson-Norm (die $L^2$-Norm von $f$ bezueglich des hyperbolischen Masses auf der Modulkurve) bezeichnet, $D_K$ die Diskriminante und $P_K \in E(K)$ der Heegner-Punkt. (Die Formel gilt bis auf einen expliziten rationalen Faktor ungleich Null, der von $K$ abhaengt; fuer $|D_K| > 4$ ist dieser Faktor gleich~$1$. Die Beziehung zwischen der Petersson-Norm $\|f\|^2$ und der N\'eron-Periode $\Omega_E$ in der BSD-Formel involviert die Manin-Konstante~$c_M$; fuer die optimale Kurve in jeder Isogenieklasse ist $c_M = 1$ rechnerisch verifiziert fuer alle Fuehrer bis $500\,000$ in der Cremona-Datenbank \cite{LMFDB}\footnote{Cremonas \texttt{ecdata} liefert vollstaendige Tabellen bis Fuehrer $N = 500\,000$; siehe \url{https://johncremona.github.io/ecdata/}.} (unbedingt fuer $N \leq 130\,000$; bedingt auf Optimalitaet der ersten Kurve fuer $N > 400\,000$). Allgemeiner bewies \v{C}esnavi\v{c}ius~\cite{Cesnavicius2018} $c_M = 1$ fuer alle semistabilen elliptischen Kurven ueber~$\mathbb{Q}$.)
...
Vermutung~\ref{conj:height_saturation} ist \textit{aequivalent} zur starken BSD-Vermutung (Vermutung~\ref{conj:bsd_strong}): die ,,Hoehensaettigungs''-Formulierung formuliert die BSD-Leitterm-Formel lediglich in der Sprache des Positivitaets-Frameworks dieser Arbeit um. Der Name ,,Hoehensaettigung'' betont die Rolle des positiv-definiten Regulators $R_E > 0$ als dominantem geometrischen Faktor, aber der mathematische Inhalt ist identisch zur klassischen Formulierung. Wir behalten diese Umformulierung bei, weil sie die Positivitaetsstruktur hervorhebt, nicht weil sie neuen mathematischen Inhalt liefert.
\end{remark}
...
\begin{remark}[Scheibenmittelungs-Formulierung]
\label{conj:height-saturation-identity}
Die BSD-Leitterm-Formel kann als Konturintegral-Grenzwert ausdrueckt werden, der die Rolle des Taylor-Koeffizienten explizit macht. Dies ist eine komplexanalytische Standardidentitaet, keine unabhaengige Vermutung. Fuer eine elliptische Kurve $E/\mathbb{Q}$ vom analytischen Rang $r = \mathrm{ord}_{s=1} L(E,s)$:
\[
\frac{L^{(r)}(E, 1)}{r!} = \lim_{\varepsilon \to 0} \varepsilon^{-r} \cdot \frac{1}{2\pi i} \oint_{|s-1|=\varepsilon} L(E, s) \, ds = \Omega_E \cdot \det(\langle P_i, P_j \rangle_{\mathrm{NT}}) \cdot \frac{\#\mathrm{Sha}(E) \cdot \prod c_p}{\#E(\mathbb{Q})_{\mathrm{tors}}^2},
\]
wobei $P_1, \ldots, P_r$ Erzeuger von $E(\mathbb{Q}) / E(\mathbb{Q})_{\mathrm{tors}}$ sind.
\end{remark}

\begin{remark}[Normierung des Scheibenmittelwerts]\label{rem:disc-normalisation}
Die Scheibenmittelungsformel in Vermutung~\ref{conj:height_saturation} ist mit der komplexanalytischen Standardnormierung zu verstehen: $\frac{1}{2\pi i} \oint_{|s-1|=\varepsilon} L(E,s)\, ds = \frac{L^{(r)}(E,1)}{r!} \varepsilon^r + O(\varepsilon^{r+1})$ fuer $\varepsilon \to 0$. Das Hoehensaettigungsverhaeltnis ist daher $\lim_{\varepsilon \to 0} \varepsilon^{-r} \cdot \frac{1}{2\pi i} \oint_{|s-1|=\varepsilon} L(E,s)\, ds = \frac{L^{(r)}(E,1)}{r!}$, was nach BSD gleich $\Omega_E \cdot R_E \cdot \prod c_p \cdot |\mathrm{Sha}| / |E_{\mathrm{tors}}|^2$ ist.
\end{remark}

\begin{remark}[Hoehensaettigung als Selektionsprinzip]
\label{rem:height-saturation}
Die Vermutung reformuliert die BSD-Leitterm-Formel als eine \emph{Grenzwertidentitaet} statt einer punktweisen Auswertung: der Konturmittelwert extrahiert den fuehrenden Taylor-Koeffizienten, den BSD mit einem Produkt arithmetischer Invarianten gleichsetzt, dominiert vom positiv-definiten Regulator $R_E$. Diese Perspektive legt nahe, die BSD-Formel als \emph{Selektionsprinzip} zu betrachten: unter allen Fuehrungstermstrukturen \textit{der Form ,,Produkt kanonischer arithmetischer Invarianten''} (siehe das heuristische Argument von Proposition~\ref{thm:uniqueness}) macht die Positivitaet der N\'eron-Tate-Hoehenpaarung die BSD-Formel zum einzigen natuerlichen Kandidaten, wobei $\Sha$ die verbleibende arithmetische Korrektur liefert. Die rigorose Formulierung dieser Eindeutigkeit wird durch die Bloch-Kato-Vermutung \cite{BlochKato1990} geliefert.
\end{remark}
...
Fuer 17 elliptische Kurven vom Rang~2 aus der Cremona-Datenbank (Fuehrer $389 \leq N \leq 5077$) erfuellt der Regulator $R > 0$ in allen Faellen ($R_{\min} = 0{,}095$, $R_{\max} = 1{,}700$). Log-Log-Regression ergibt $R \sim N^{0{,}099}$, konsistent mit sublinearem Wachstum. \textbf{Vorbehalt:} Die Stichprobengroesse ($n = 17$) ist zu klein fuer statistisch signifikante Schlussfolgerungen; diese Resultate sollten als vorlaeuige Konsistenzpruefungen betrachtet werden, nicht als statistische Bestaetigung eines Skalierungsgesetzes. Die Positivitaet $R > 0$ ist a priori durch die Positiv-Definitheit der N\'eron-Tate-Hoehe garantiert (Theorem~\ref{thm:NT_positive}). Skript: \texttt{compute\_rank2\_lmfdb.py}.
\end{remark}
...
\textit{Status:} Fuer $r = 1$ ist dies Gross-Zagier. Fuer $r = 2$ existieren Teilresultate ueber Diagonalrestriktion $p$-adischer Familien \cite{DarmonRotger2017}. Kim \cite{Kim2024} hat eine hoehere Gross-Zagier-Formel erhalten, die die Verbindung zwischen Heegner-Punkt-Kolyvagin-Systemen und Selmer-Gruppen-Struktur verfeinert. Fuer allgemeines $r$ ist dies ein grosses offenes Problem. Juengere Arbeiten zu Euler-Systemen fuer hoeherrangige Motive \cite{LSZ2020} liefern Fortschritt, aber keine vollstaendige Formel.
...
\textit{Status:} Fuer $r = 1$ ist dies Kolyvagines Methode. Fuer allgemeines $r$ haben Mazur-Rubin \cite{MazurRubin2004} die Theorie der Kolyvagin-Systeme abstrakt entwickelt, und Burns-Sano \cite{BurnsSano2021} sowie Burns-Sakamoto-Sano \cite{BurnsSakamotoSano2025} haben diese auf hoeherrangige Euler-, Kolyvagin- und Stark-Systeme ueber allgemeinen Koeffizientenringen erweitert. Kim \cite{Kim2024} hat eine Kolyvagin-System-theoretische Verfeinerung der Gross-Zagier-Formel erhalten. Die \textit{Existenz} eines Kolyvagin-Systems hinreichender Tiefe fuer beliebigen Rang ist jedoch nicht etabliert.
...
Zusammenfassend: die Positivitaet (Axiom~II) ist etabliert, aber das Kompositionsgesetz (Axiome~I, III, IV) ist fuer Rang~$\geq 2$ unvollstaendig. Die BSD-Formel ist erzwungen, sobald die Euler-System-Komposition fuer beliebigen Rang etabliert ist.\footnote{Diese Arbeit ist Teil eines breiteren Programms \cite{Geiger2026e}; die Verbindungen sind rein motivational und werden in keinem Beweis verwendet.}
...
Die Iwasawa-Theorie liefert die klarste Realisierung von Axiom~III (monotone Kontrolle) und verbindet es natuerlich mit Axiom~IV (Euler-System-Kohaerenz). Die Hauptvermutung fuer elliptische Kurven \cite{Kato2004,SkinnerUrban2014} ist in den folgenden spezifischen Faellen etabliert:
\begin{itemize}
\item \textbf{Katos Teilbarkeit (eine Richtung):} Fuer alle Primzahlen $p$, bei denen $E$ gute ordinaere Reduktion hat und $\bar{\rho}_{E,p}$ surjektiv ist. Gilt fuer \textit{alle Raenge}, liefert aber nur die Inklusion $\mathrm{char}_\Lambda(\Sel^\vee) \subseteq (\mathcal{L}_p)$, d.h.\ das charakteristische Ideal des Selmer-Duals ist im vom $p$-adischen $L$-Funktion erzeugten Ideal enthalten, was eine obere Schranke fuer den Selmer-Modul liefert.
\item \textbf{Skinner-Urban (volle Gleichheit):} Fuer ordinaere Primzahlen $p$ mit $p \nmid a_p$, unter der Voraussetzung der Irreduzibilitaet von $\bar{\rho}_{E,p}$ und weiterer lokaler Hypothesen. Deckt \textit{keine} supersingularen Primzahlen ab.
...
\item \textbf{Nicht abgedeckt:} Primzahlen schlechter additiver Reduktion, Primzahlen bei denen $\bar{\rho}_{E,p}$ reduzibel ist, und der allgemeine Fall ohne residuale Irreduzibilitaet.
\end{itemize}
...
\end{equation}
wobei $\mathrm{pr}_{m,n}$ die natuerliche Projektion ist. Das Kontrolltheorem stellt sicher, dass $\Sel_{p^\infty}(E/\mathbb{Q}_n)$ durch $\Sel_{p^\infty}(E/\mathbb{Q}_\infty)$ via Restriktion mit beschraenktem Kern und Kokern bestimmt wird. Dieses kontrollierte kompatible System ist das $p$-adische Kompositionsgesetz, das die Arithmetik von $E$ ueber den Turm hinweg einschraenkt.
...
CRM/QG \cite{FST-Unified2026}\footnote{Curvature Relaxation Model; \href{https://doi.org/10.5281/zenodo.18728936}{doi:10.5281/zenodo.18728936}.} & Kapazitaet & RG-Fluss & Saettigungs-Thm \\
...
Das BSD-Framework verbindet sich ueber die Tamagawa-Zahlen $c_p$ in der Leitterm-Formel~\eqref{eq:bsd_formula} mit der Szpiro-Vermutung (aequivalent zur $abc$-Vermutung).

\subsubsection{Szpiro als gewichtete Tamagawa-Schranke}

\begin{conjecture}[Szpiro {\cite{Szpiro1981}}]
\label{conj:szpiro}
Fuer jedes $\varepsilon > 0$ existiert $C_\varepsilon > 0$, sodass fuer jede elliptische Kurve $E/\mathbb{Q}$ mit minimaler Diskriminante $\Delta_{\min}$ und Fuehrer $N$ gilt:
\begin{equation}
...
Fuer eine semistabile Kurve $E/\mathbb{Q}$ ist jede schlechte Reduktion multiplikativ. Der Fuehrer ist $N=\prod_{p\mid\Delta}p$, und bei split-multiplikativer Reduktion an $p$ ist die Tamagawa-Zahl gleich der Bewertung der minimalen Diskriminante: $c_p=v_p(\Delta_{\min})$ (\cite{Silverman2009}, Tabelle~IV.9.1). Szpiro ist daher eine \emph{primgewichtete} Tamagawa-Exponenten-Schranke:
\begin{equation}
\sum_{p\mid N} c_p\log p
= \log|\Delta_{\min}|
\leq (6+\varepsilon)\sum_{p\mid N}\log p + C_\varepsilon\,.
\label{eq:szpiro_tamagawa}
\end{equation}
Dies ist staerker und strukturierter als eine ungewichtete Schranke fuer $\prod_p c_p$: Ein grosser Exponent an einer grossen Primzahl traegt mehr Diskriminantenmasse als derselbe Exponent an einer kleinen Primzahl.
...
\end{equation}
Diese Identitaet kontrolliert das \emph{ungewichtete Produkt} der Tamagawa-Faktoren. Sie impliziert fuer sich genommen nicht Szpiro, weil Szpiro die primgewichtete Summe in~\eqref{eq:szpiro_tamagawa} kontrolliert. Eine Route von BSD zu Szpiro braucht daher neben den folgenden Zutaten ein zusaetzliches gewichtetes lokales Upgrade:
\begin{enumerate}
\item[\textbf{(B1)}] \textbf{Obere Schranke fuer $L$-Werte:} $L^{(r)}(E,1)/r! \leq C\cdot N^{r/2+\varepsilon}$. Unbedingt fuer $r=0$ (Konvexitaetsschranke); fuer allgemeines $r$ bedingt auf GRH.
\item[\textbf{(B2)}] \textbf{Untere Periodenschranke:} $\Omega_E \geq c\cdot N^{-1/2-\varepsilon}$. Dies folgt aus Schranken fuer den modularen Parametrisierungsgrad.
\item[\textbf{(B3)}] \textbf{Untere Regulator-Schranke:} $R_E \geq c\cdot(\log N)^r$. Dies verlangt eine effektive untere Schranke fuer die N\'eron--Tate-Hoehe nicht-torsioneller Punkte.
...
\item[\textbf{(B5)}] \textbf{Gewichtetes lokales Upgrade:} ein Mechanismus, der ungewichtete Kontrolle von $\prod_p c_p$ in Kontrolle von $\sum_{p\mid N}c_p\log p$ uebersetzt oder direkt jeden lokalen Diskriminantenbeitrag durch Fuehrerdaten beschraenkt.
\end{enumerate}
...
\end{equation}
Fuer semistabile Kurven vom Rang~$0$ ist dies eine starke ungewichtete Tamagawa-Produkt-Beschraenkung. Eine Szpiro-Schranke wird daraus erst, wenn (B5) die fehlenden Primgewichte liefert.

\begin{remark}[Zirkularitaet in Schritt (B3)]
\label{rem:szpiro_circularity}
Schon vor dem gewichteten Upgrade hat die Kette eine zweite Obstruktion: Zirkularitaet in (B3). Die beste bekannte effektive untere Hoehenschranke ist Silvermans $\hat{h}(P)\geq c\cdot\log N$ fuer nicht-torsionelle Punkte $P$~\cite{Silverman2009}; sie ist \textit{bedingt auf die $abc$-Vermutung}. Da $abc\Leftrightarrow$ Szpiro, waere die logische Struktur:
\[
...
\]
Um diese Zirkularitaet zu brechen, braucht man eine Hoehenschranke, die nicht von der Diskriminante oder von $abc$ abhaengt, und einen separaten Mechanismus fuer die primgewichteten lokalen Exponenten.
\end{remark}

\subsubsection{Eine Theta-Produkt-Bruecke}

Die folgende Identitaet verbindet Theta-Werte an Torsionspunkten mit der Dedekind-Eta-Funktion und damit mit der modularen Diskriminante. Sie liefert eine moegliche nicht-zirkulaere Verbindung zwischen Torsionsstruktur und Diskriminante.

% TOOL-NOTE: Die Theta-Eta-Produktidentitaet ist konsolidiert in
%   FST_MATHEMATICS/_tools/THETA_ETA_IDENTITY.md
% Identische Aussage und Notation. Dieselbe Identitaet wird in
% abc/abc_Theta_Tamagawa_EN.tex Proposition 4.1 verwendet.

\begin{proposition}[Theta-Eta-Produktidentitaet an $l$-Torsion]
\label{prop:theta_eta}
Fuer jede ungerade Primzahl $l$ und $\tau\in\mathcal{H}$, mit $q=e^{2\pi i\tau}$, gilt:
\begin{equation}
...
Setzt man $\omega=e^{2\pi i/l}$, dann ergeben die zyklotomische Identitaet $\prod_{j=1}^{l-1}(1-\omega^jx)=1+x+\cdots+x^{l-1}$ fuer jeden Faktor des unendlichen Produkts und die Sinus-Identitaet $\prod_{j=1}^{l-1}\sin(\pi j/l)=l/2^{l-1}$:
\[
\prod_{j=1}^{l-1}\theta_1(j/l|\tau)
= l\,q^{(l-1)/8}
\prod_{n=1}^{\infty}(1-q^n)^{l-3}(1-q^{ln})^2\,.
\]
Da $\eta(\tau)=q^{1/24}\prod_{n\geq1}(1-q^n)$ und $\eta(l\tau)=q^{l/24}\prod_{n\geq1}(1-q^{ln})$ gilt, besitzt das Eta-Produkt $l\eta(\tau)^{l-3}\eta(l\tau)^2$ genau den Vorfaktor $lq^{(l-3)/24+2l/24}=lq^{(l-1)/8}$ und dasselbe unendliche Produkt. Damit ist die Identitaet bewiesen.
\end{proof}
...
= \log l + \frac{l-3}{24}\log\frac{|\Delta(\tau)|}{(2\pi)^{12}}
+ \frac{2}{24}\log\frac{|\Delta(l\tau)|}{(2\pi)^{12}}\,.
\label{eq:theta_log_discriminant}
\end{equation}
Fuer grosses $l$ dominiert der Term $(l/24)\log|\Delta(\tau)|$: Das Produkt der Theta-Werte an $l$-Torsionspunkten kodiert die Diskriminante.
\end{remark}

\begin{remark}[Tate-Kurve und Szpiro-Verbindung]
\label{rem:tate_szpiro}
Fuer eine elliptische Kurve $E/\mathbb{Q}_p$ mit split-multiplikativer Reduktion (Tate-Kurve $E_q\cong\mathbb{G}_m/q^{\mathbb{Z}}$) entsprechen die $l$-Torsionspunkte den Klassen $\zeta_l^j\cdot q^{k/l}$ fuer $0\leq j,k<l$. Die Identitaet~\eqref{eq:theta_eta_identity} verbindet die $l$-Torsionsstruktur von $E$ mit $v_p(\Delta_{\min})=c_p$, \textit{ohne Hoehenschranken oder die $abc$-Vermutung zu verwenden}. Dies weist auf einen moeglichen nicht-zirkulaeren Pfad zu Szpiro hin: Wenn man die $l$-Torsions-Theta-Werte in Termen des Fuehrers $N$ beschraenken kann, etwa ueber die Galois-Darstellung $\bar{\rho}_{E,l}$ oder die explizite Weil-Formel fuer $L(E,s)$, dann wuerde die daraus folgende Schranke fuer $c_p$ direkt auf die Szpiro-Vermutung zielen.

Der fehlende Schritt ist eine primgewichtete lokale Fuehrer-Schranke der Form:
\begin{equation}
\sum_{p\mid N}\sum_{j=1}^{l-1}-\log|\theta_1(j/l|\tau_p)|_p
\leq C(l,\varepsilon)\cdot\log N\,,
\label{eq:theta_conductor_bound}
\end{equation}
die derzeit nicht etabliert ist, aber das richtige gewichtete Ziel formuliert. Die Beschraenkung der Theta-Werte ueber den Fuehrer verbindet sich natuerlich mit der Galois-Wirkung auf $E[l]$, wobei Serres Modularitaetsresultate und Level-Lowering die Verbindung zwischen $l$-Torsionsdarstellung und Fuehrer liefern.
\end{remark}

\begin{remark}[Bezug zu Mochizukis IUT-Ansatz]
\label{rem:iut_comparison}
Die Theta-Eta-Produktidentitaet~\eqref{eq:theta_eta_identity} operiert auf denselben Objekten -- Theta-Werten an Torsionspunkten von Tate-Kurven -- die auch in Mochizukis Inter-Universal Teichmuller Theory (IUT) auftreten. Der IUT-Ansatz verwendet jedoch Theta-Werte $q^{j^2}$, also quadratische Abhaengigkeit von $j$, die aus der multiradialen Darstellung stammt, waehrend die Identitaet~\eqref{eq:theta_eta_identity} $\theta_1(j/l|\tau)$ mit linearer Abhaengigkeit von $j$ betrifft. Diese Unterscheidung wird in~\cite{GeigerIUT2026} analysiert; dort wird gezeigt, dass der $O(l)$-gegen-$O(l^2)$-Skalierungsunterschied zwischen linearen und quadratischen Theta-Werten eine strukturelle Obstruktion gegen die Reproduktion der IUT-Schranke mit klassischen Methoden bildet. Der vorliegende Ansatz unterscheidet sich grundlegend: Er versucht nicht, den IUT-Mechanismus zu reproduzieren, sondern nutzt die Produktidentitaet als direkte Bruecke von Torsion zu Diskriminante innerhalb des BSD-Frameworks.
\end{remark}
...
\begin{enumerate}
\item \textbf{Axiomatisierung.} Vier Axiome -- endliche Kapazitaet, Hoehenpositivitaet, kontrolliertes Iwasawa-Wachstum, Euler-System-Komposition -- die gemeinsam die BSD-Formel als einzige Normalform erzwingen.
...
\begin{enumerate}
\item \textbf{$p$-adische BSD ueber Iwasawa-Theorie.} Der staerkste kurzfristige Fortschritt kommt wahrscheinlich aus der Erweiterung der Iwasawa-Hauptvermutung auf hoeherrangige Situationen unter Verwendung der Euler-Systeme von Loeffler-Skinner-Zerbes \cite{LSZ2020} und der hoeherrangigen Kolyvagin-Stark-System-Theorie von Burns-Sakamoto-Sano \cite{BurnsSano2021,BurnsSakamotoSano2025}.
...
Mazur und Rubin~\cite{MazurRubin2004} entwickelten die abstrakte Theorie der Kolyvagin-Systeme. Burns und Sano~\cite{BurnsSano2021} erweiterten diese auf hoeherrangige Systeme ueber allgemeinen Koeffizientenringen, und Burns, Sakamoto und Sano~\cite{BurnsSakamotoSano2025} bewiesen die Existenz einer kanonischen hoeheren Kolyvagin-Ableitungsabbildung zwischen Euler- und Kolyvagin-Systemen in beliebigem Rang, wie von Mazur und Rubin vermutet. Fuer Rang~$r$ benoetigt man ein ,,Rang-$r$-Kolyvagin-System''. Fuer CM-Kurven liefert der Endomorphismenring eine natuerliche Aufspaltung der Selmer-Gruppe in Eigenraeume.
\end{remark}

\begin{remark}[Juengster Fortschritt fuer Rang $\geq 2$]\label{rem:recent-bsd}
Bedeutender Fortschritt in Richtung hoeherrangiger BSD wurde in den letzten Jahren erzielt. Burungale, Castella und Kim~\cite{BurungaleCastellaKim2021} bewiesen Perrin-Rious Heegner-Punkt-Hauptvermutung, aufbauend auf Howards bipartiten Euler-Systemen und Wei Zhangs Arbeit zu Kolyvagines Vermutung, und etablierten die Iwasawa-Hauptvermutung fuer die Tate-Shafarevich-Gruppe ueber antizyklotomischen Erweiterungen. Kim~\cite{Kim2024} erhielt eine hoehere Gross-Zagier-Formel durch Vergleich von Heegner-Punkt-Kolyvagin-Systemen mit Kurihara-Zahlen und verfeinerte das strukturelle Verstaendnis von Selmer-Gruppen. 2024 konstruierten Loeffler und Zerbes \cite{LoefflerZerbes2024} ein universelles Euler-System fuer $\mathrm{GSp}(4)$ und zeigten, dass alle Euler-System-Klassen in einem eindimensionalen Raum liegen. Burungale, Skinner, Tian und Wan \cite{BSTW2024} bewiesen die Existenz $p$-adischer Zeta-Elemente fuer elliptische Kurven ueber imaginaer-quadratischen Zahlkoerpern, etablierten die Kobayashi-Vermutung fuer semistabile Kurven an supersingularen Primstellen und praesentieren die ersten unendlichen Familien nicht-CM-elliptischer Kurven, die die volle BSD-Vermutung erfuellen.
\end{remark}
...
\item \textbf{Degenerierte Hoehen:} Die Hoehenpaarung $\langle \cdot, \cdot \rangle_{\mathrm{ht}}$ hat einen nicht-trivialen Kern fuer $r \geq 2$, was die Determinante kollabieren laesst. Hoehensaettigung (Vermutung~\ref{conj:height_saturation}) schuetzt dagegen.
...
\subsection{Post-Zookeeper-Transfer: ein Hoeher-Rang-Modulprogramm}
\label{subsec:zookeeper-bsd-transfer}

Die Post-Zookeeper-Rolle dieses Papers ist nicht, einen neuen Beweis der
BSD-Vermutung in hoeherem Rang zu behaupten.  Sie besteht darin, die
Hoeher-Rang-Obstruktion in ein modulares Transferproblem zu zerlegen.  Die
fuenf relevanten Felder sind:
...
Approximation, Gap und Grenzuebergang muessen getrennt ausgewiesen werden.
Fuer BSD bleibt der rote Block die hoeher-rangige Selmer-Kontrolle und die
Hoehere-Gross--Zagier-Bruecke, insbesondere Modul~C in
Bemerkung~\ref{rem:hgz-modules}.

\begin{description}
  \item[Operator/Form.] Die Neron--Tate- und $p$-adischen Hoehenpaarungen
  ...
  Regulators.
  \item[Grenzuebergang.] Der Uebergang von $p$-adischer
  Leitterminformation und Selmer-Struktur zur archimedischen
  BSD-Leittermformel.
\end{description}

Daraus ergibt sich ein konservatives naechstes Ziel.  Statt einen
allgemeinen Rang-$r$-Beweis zu formulieren, sollte zuerst ein
Rang-zwei-Transfersatz isoliert werden:
\[
  \text{(hoeherer Gross--Zagier-Input)}
  + \text{(Selmer-Kontrolle)}
  + \text{($p$-adische Hoehen-Nichtausartung)}
  \Longrightarrow
  \text{Rang-zwei-Hoehensaettigung}.
\]
Kims Arbeiten zu hoeherem Gross--Zagier und Selmer/Iwasawa sowie der
$p$-adische BSD-Rahmen von Castella et al. liefern dafuer den natuerlichen
Literatur-Backbone.  Der erwartete Gewinn ist diagnostisch, nicht final:
Hoeherer Rang wird in vier separat testbare Module zerlegt, statt eine
einzige monolithische Luecke zu bleiben.

\subsection*{Ergebnisklassifikation}

Dieses Paper etabliert eine \textbf{strukturelle Normalform-Charakterisierung}: unter vier Positivitaetsaxiomen ist die BSD-Formel die einzige zulaessige Gestalt fuer den Leitterm von $L^{(r)}(E,1)$. Fuer Rang~$\leq 1$ ist dies eine genuige Umformulierung bewiesener Resultate; fuer Rang~$\geq 2$ ist es eine bedingte strukturelle Analyse.
...
Die BSD-Formel ist die einzige ,,Normalform'', die mit vier axiomatischen Zwangsbedingungen kompatibel ist: endliche arithmetische Kapazitaet ($\Sha$-Endlichkeit), kooperative Hoehenpositivitaet (N\'eron-Tate), kontrolliertes Iwasawa-Wachstum (kontrolliertes Wachstum in $\mathbb{Z}_p$-Tuermen) und Euler-System-Kompositionskohaerenz (Norm-Kompatibilitaet).
...
L.~Geiger (2026).
Structural limits of the classical route to Corollary~3.12: a theta-product diagnostic.
Preprint (Bernau).
