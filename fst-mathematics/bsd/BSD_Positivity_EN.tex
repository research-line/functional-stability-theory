...
The Gross-Zagier theorem \cite{GrossZagier1986} (extended to all Shimura curves by Yuan-Zhang-Zhang \cite{YZZ2013}) is the canonical example: it couples the first derivative $L'(E,1)$ to the height $\hat{h}(P_K)$ of a Heegner point and gives:
\begin{equation}
...
By a \textit{positivity normal-form theorem} we mean a result of the following type: given a set of structural axioms (positivity, finiteness, composition coherence), a canonical relationship between the analytic and geometric invariants of the object in question is selected within a specified product class. The Weil conjectures for the zeta function of a variety over $\mathbb{F}_q$ provide the archetype: Frobenius positivity (the Riemann hypothesis for varieties) constrains the factorization of the zeta function via eigenvalues on \'etale cohomology.

\subsection{Structure of this paper}

We develop four axioms (Section~\ref{sec:axioms}) which, together with the bridge hypotheses made explicit later, select the BSD leading-term formula as the expected canonical-product normal form. We explain how the rank $\leq 1$ case fits known theorems within this framework (Section~\ref{sec:rank01}), identify the precise obstructions for higher rank (Section~\ref{sec:higher_rank}), and discuss the relationship to Iwasawa theory and Euler systems (Section~\ref{sec:iwasawa}).
...
Every factor on the right is non-negative, and $R_E > 0$ whenever $r \geq 1$. The formula asserts that the analytic side -- which has no a priori reason to be non-negative or to have the ``right'' order of vanishing -- must match this positive structure once the BSD bridge is available.

This is the pattern of a \textit{positivity normal-form theorem}: the analytic object is constrained by the positivity of the geometric/arithmetic object to which it is equated.
...
We formulate four structural conditions expected of a coherent arithmetic theory of $L$-functions and Mordell-Weil groups.
...
\textit{Role in the structural argument.} If $\Sha$ were infinite, one could imagine a curve with $\rank E(\mathbb{Q}) = 0$ but $\ord_{s=1} L(E,s) = 2$: the ``missing rank'' would be hidden in $\Sha$. Axiom~I excludes this loophole.
...
\textit{Role in the structural argument.} Positivity ensures that the right-hand side of BSD has a definite sign structure: $R_E > 0$ is unavoidable once rank is positive. Any analytic formula for $L^{(r)}(E,1)$ must be compatible with this positivity.
...
\begin{enumerate}
  ...
  \item[\textbf{(U2)}] \textbf{Positivity of leading coefficient:} $\Phi(E) > 0$ whenever $r \geq 1$ (required by Axiom~II, since any coupling to $R_E > 0$ must preserve the sign).
  \item[\textbf{(U3)}] \textbf{Integrality of the $\Sha$ contribution:} the ratio $\Phi(E)/(\Omega_E \cdot R_E \cdot \prod_p c_p)$ is of the form $|\Sha|/|E(\mathbb{Q})_{\mathrm{tors}}|^2$ with $|\Sha| \in \mathbb{Z}_{>0}$ (as required by Axiom~I).
  \item[\textbf{(U4)}] \textbf{Compatibility with functional equation:} $\Phi(E)$ is compatible with the $p$-adic interpolation formula $L_p(E,0) \sim L(E,1)/\Omega_E^+$ and its higher derivatives (as required by Axiom~III, the Iwasawa main conjecture specialization).
...
The rank-0 case is fully ``saturated'' in the positivity framework: all four axioms are verified (Axiom~III at all primes of good ordinary reduction with surjective residual representation; the remaining primes are handled by complementary results or are finite in number), and the structural exclusion argument rules out $\rank \geq 1$.
\end{proof}
...
\begin{remark}
The rank-1 proof follows the same structural-exclusion pattern:
\begin{enumerate}
...
In summary: the positivity (Axiom~II) is established, but the composition law (Axioms~I, III, IV) is incomplete for rank~$\geq 2$. The BSD formula becomes the expected product normal form once the Euler-system composition is established for arbitrary rank. This paper is part of a broader programme \cite{Geiger2026e}; the connections are motivational and are not used as theorem input here.
...
Once such a coupling is established, the exclusion argument closes:
\begin{enumerate}
...
\item \textbf{Rank $\leq 1$ as a completed instance.} The Gross-Zagier + Kolyvagin argument naturally fits the positivity framework: it couples an analytic derivative to a non-negative height and uses composition (Euler systems) to exclude alternatives.

\item \textbf{Higher rank identified.} The higher-rank obstruction can be summarized by the absence of (H1) a universal higher Gross-Zagier formula and (H2) a complete Kolyvagin system -- i.e., the composition law remains incomplete.

\item \textbf{Exclusion of alternatives.} Within this restricted setup, no alternative leading-term formula is compatible with all four axioms, \textit{provided} a coupling between $L^{(r)}(E,1)$ and the height pairing exists.
\end{enumerate}
...
\item \textbf{Statistical BSD.} The work of Bhargava-Shankar \cite{BhargavaShankar2015} shows that a positive proportion of elliptic curves over $\mathbb{Q}$ have rank~0 and satisfy the BSD conjecture. These statistical results are at least consistent with the idea that the positivity framework tracks generic behavior.
\end{enumerate}
...
The rank $\leq 1$ proof is already compatible with this positivity reading: Gross-Zagier couples an analytic derivative to a non-negative height, and Kolyvagin's Euler system supplies Selmer control. For higher rank, the framework identifies two missing ingredients: non-degenerate higher Gross-Zagier height/cycle coupling and full higher-rank Selmer/Kolyvagin/Sha control. Height Saturation names the target but does not close either bridge.
...
\begin{quote}
\textit{``In this reading, BSD asks not merely for rational points, but for a reason why the $L$-function cannot register independent zeros without the corresponding arithmetic directions.''}
\end{quote}

\section*{AI Disclosure}
AI-based tools were used for mathematical formalization support, literature-verification support, translation assistance, consistency checks, and editorial refinement. They were not authors, co-authors, acknowledged contributors, ground-truth authorities, or independent validation instances. The research direction, mathematical claims, final editorial decisions, and responsibility for correctness remain solely with the author.
...
\textit{Transactions of the American Mathematical Society}, 377(5), 3691--3725.
DOI: 10.1090/tran/9125; arXiv:2203.12161.
