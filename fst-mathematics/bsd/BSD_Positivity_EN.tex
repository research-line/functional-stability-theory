...
\title{The BSD Conjecture as a Positivity Normal-Form:\\A Structural Survey}
...
\textit{Role in the No-Go argument.} The controlled growth formula~\eqref{eq:iwasawa_growth} (note: ``controlled'' refers to the asymptotically predictable growth governed by $\mu$ and $\lambda$, not strict monotonicity, since the $O(1)$ error allows bounded fluctuations) prevents the analytic rank from exhibiting unbounded ``oscillations'' between layers of the $\mathbb{Z}_p$-tower. Together with the main conjecture, it couples the $p$-adic $L$-function to the Selmer module, providing a $p$-adic positivity constraint.
...
For rank $\leq 1$, Axioms~I--IV are independently established theorems (Kolyvagin, N\'eron-Tate, Kato/Skinner-Urban, Heegner points). For rank $\geq 2$, Axioms~I and~IV are \textit{not} independent of BSD: finiteness of $\Sha$ (Axiom~I) is itself a consequence of the full BSD conjecture, and the existence of a complete Kolyvagin system of sufficient depth (Axiom~IV) is not established. The conditional result (Proposition~\ref{thm:bsd_normal}) should therefore be read as a \textit{structural characterisation}: it identifies the BSD formula as the unique normal form compatible with the axioms, rather than providing an independent proof of BSD.
\end{remark}
...
\item \textbf{Axiom~III $\leftrightarrow$ Controlled growth (Axiom~C).} As one ascends the $\mathbb{Z}_p$-tower, arithmetic complexity grows in a controlled, predictable fashion -- no oscillations or jumps in the Selmer rank occur.
...
\begin{proposition}[BSD Normal-Form Characterisation -- Conditional Structural Result]
\label{thm:bsd_normal}
\textbf{(Conditional on Axioms~I--IV.)} The following conclusions hold \textit{only under the assumption} that Axioms~I--IV are satisfied. For rank~$\leq 1$, this assumption is fulfilled by established theorems; for rank~$\geq 2$, Axioms~I and~IV themselves depend on unproven conjectures (see Remark~\ref{rem:axiom-independence}). \textbf{This is a structural characterisation, not a new proof of BSD.}
...
Proposition~\ref{thm:bsd_normal} is a \textit{conditional structural characterisation}: it shows that the BSD formula is the unique normal form \textit{given} Axioms~I--IV. \textbf{It does not provide an unconditional proof of BSD, and for rank~$\geq 2$ the argument is logically circular.} For rank~$\leq 1$, all four axioms are independently established theorems (Kolyvagin, N\'eron-Tate, Kato/Skinner-Urban, Heegner points), so the conclusion holds unconditionally and constitutes a genuine structural reformulation of known results. For rank~$\geq 2$, however, Axiom~I (finiteness of $\Sha$) is itself a consequence of the full BSD conjecture, and Axiom~IV (existence of a complete Kolyvagin system of sufficient depth) is not established independently of BSD. The proposition therefore has the logical structure ``BSD + known facts $\Rightarrow$ BSD is the unique normal form'' at rank~$\geq 2$, which is logically valid but mathematically trivial as a proof strategy. Its value lies in \textit{structural analysis}: identifying the BSD formula as the unique output of the four axioms clarifies \textit{which} components are needed for a proof, even though it does not provide one.
...
\label{ex:389a}
The elliptic curve $E: y^2 + y = x^3 + x^2 - 2x$ (Cremona label 389a1) has $\rank E(\mathbb{Q}) = 2$, generated by $P_1 = (-1,1)$ and $P_2 = (0,0)$.\footnote{The height matrix depends on the choice of generators; the regulator $R_E$ is invariant under change of $\mathbb{Z}$-basis of $E(\mathbb{Q})/E(\mathbb{Q})_{\mathrm{tors}}$ (since $\det(A^T M A) = (\det A)^2 \det M$ and $\det A = \pm 1$ for $A \in \mathrm{GL}(r, \mathbb{Z})$).} The height matrix is:
\[
...
\begin{remark}[Disc-averaging formulation]
\label{conj:height-saturation-identity}
The BSD leading-term formula can be expressed as a contour-integral limit, making the role of the Taylor coefficient explicit. This is a standard complex-analytic identity, not an independent conjecture. For an elliptic curve $E/\mathbb{Q}$ of analytic rank $r = \mathrm{ord}_{s=1} L(E,s)$:
\[
...
In summary: the positivity (Axiom~II) is established, but the composition law (Axioms~I, III, IV) is incomplete for rank~$\geq 2$. The BSD formula is forced once the Euler-system composition is established for arbitrary rank.\footnote{This paper is part of a broader programme \cite{Geiger2026e}; the connections are purely motivational and not used in any proof.}
...
\subsubsection{Szpiro as a Tamagawa bound}
...
For a semistable curve $E/\mathbb{Q}$, all bad reduction is multiplicative.  The conductor is $N = \prod_{p \mid \Delta} p$ and for split multiplicative reduction at~$p$, the Tamagawa number equals the valuation of the minimal discriminant: $c_p = v_p(\Delta_{\min})$ (\cite{Silverman2009}, Table~IV.9.1).  Thus Szpiro is a \emph{prime-weighted} Tamagawa-exponent bound:
\begin{equation}
...
\end{equation}
This is stronger and more structured than an unweighted bound on $\prod_p c_p$: a large exponent at a large prime carries more discriminant mass than the same exponent at a small prime.

\subsubsection{Diagnostic chain: BSD $+$ bounds expose the missing weighted step}
...
\item[\textbf{(B5)}] \textbf{Weighted local upgrade:} a mechanism converting unweighted control of $\prod_p c_p$ into control of $\sum_{p\mid N}c_p\log p$, or directly bounding each local discriminant contribution in terms of conductor data.
\end{enumerate}
...
\end{equation}
For semistable curves of rank~$0$, this gives a strong unweighted Tamagawa-product constraint.  It is \emph{not} yet a Szpiro bound unless (B5) supplies the missing prime weights.
...
\[
\text{BSD} + \underbrace{\text{abc}}_{\text{used in (B3)}} + \underbrace{\text{weighted local upgrade}}_{\text{missing (B5)}} \;\Longrightarrow\; \underbrace{\text{Szpiro}}_{\Leftrightarrow\, \text{abc}}\,.
\]
...
\begin{equation}
\sum_{j=1}^{l-1} \log|\theta_1(j/l|\tau)| = \log l + \frac{l-3}{24}\log\frac{|\Delta(\tau)|}{(2\pi)^{12}} + \frac{2}{24}\log\frac{|\Delta(l\tau)|}{(2\pi)^{12}}\,.
...
\begin{enumerate}
\item \textbf{Axiomatization.} Four axioms -- finite capacity, height positivity, controlled Iwasawa growth, Euler-system composition -- that jointly force the BSD formula as the unique normal form.
...
\begin{itemize}
\item For every curve with $\rank E(\mathbb{Q}) = r$, the ratio $L^{(r)}(E,1)/(r! \cdot \Omega_E \cdot R_E \cdot \prod c_p)$ must equal $|\Sha|/|E_{\mathrm{tors}}|^2$ -- a positive rational number, where $|\Sha|$ is itself a perfect square by the alternating Cassels--Tate pairing.
...
\[
  ...
  \Longrightarrow
  \text{rank-two height saturation}.
\]
...
This paper establishes a \textbf{structural normal-form characterisation}: under four positivity axioms, the BSD formula is the unique admissible shape for the leading term of $L^{(r)}(E,1)$. For rank~$\leq 1$, this is a genuine reformulation of proven results; for rank~$\geq 2$, it is a conditional structural analysis.
...
The BSD formula is the unique ``normal form'' compatible with four axiomatic constraints: finite arithmetic capacity ($\Sha$ finiteness), cooperative height positivity (N\'eron-Tate), controlled Iwasawa growth (predictable growth in $\mathbb{Z}_p$-towers), and Euler-system composition coherence (norm-compatibility).
