...
\hypersetup{
    ...
    pdfsubject={Bedingtes Navier-Stokes-Regularitaetsrahmenwerk},
    pdfkeywords={Navier-Stokes, Blow-up, Regularitaet, Attraktor, Dissipation}
}
...
\begin{abstract}
Wir schlagen ein bedingtes Regularitaetskriterium fuer die inkompressiblen 3D-Navier-Stokes-Gleichungen vor, das auf einem thermodynamischen Abstandsfunktional basiert. Anstatt den Geschwindigkeitsgradienten global zu beschraenken, konstruieren wir ein Attraktor-Abstandsfunktional $H(u \mid \mathcal{A})$, das den $L^2$-Abstand der aktiven Stroemung von einem Referenzattraktor mit explizitem Regularitaetspaket misst. Fuer die klassische 3D-Leray--Hopf-Theorie ist dieses Paket eine zusaetzliche stehende Hypothese, kein etabliertes Theorem. Unter diesem Attraktor-Paket und zwei weiteren Bedingungen -- (\textbf{Assumption~G}) dass die zeitabhaengige minimierende Projektion auf den Attraktor hinreichend regulaer ist, und (\textbf{Condition~D}) dass die kumulative viskose Dissipation die Gronwall-Kosten nahe einer hypothetischen Blow-up-Zeit dominiert -- zeigen wir, dass ein Blow-up in endlicher Zeit $H$ unter Null zwingen wuerde, ein Widerspruch zur Nichtnegativitaet. Das Argument reduziert die Regularitaetsfrage auf strukturelle Inputs: Attraktor-Regularitaet, Geometrie der Attraktorprojektion (Assumption~G) und das Verhaeltnis von Dissipation zu nichtlinearem Wachstum (Condition~D). Wir identifizieren hinreichende geometrische Bedingungen fuer~G (positiver Reach, log-Lipschitz-Projektionen, BV-Selektionen) und praesentieren einen $H^1$-Level-Lift, bei dem die Enstrophie-Dissipation -- die im Gegensatz zur Energie-Dissipation keine a-priori-Oberschranke unter Blow-up besitzt -- als divergenter Term dienen kann, sofern zusaetzlich eine Bihari-Dominanzbedingung gilt. Wir diskutieren den Status dieser Annahmen, ihre Verbindung zur Theorie inertialer Mannigfaltigkeiten und die Luecken, die das vorliegende Rahmenwerk von einem unbedingten Beweis trennen.
Diese Arbeit etabliert eine \emph{bedingte Reduktion}: Globale Regularitaet folgt aus dem Attraktor-Regularitaetspaket, Assumption~G und Condition~D; sie ist nicht allein aequivalent zur Existenz einer BV-regulaeren Selektion.
\end{abstract}
...
\end{equation}
In dieser Arbeit konstruieren wir ein thermodynamisches Ljapunow-Funktional, das den Abstand der aktiven Stroemung von einem Referenzattraktor misst. Wir zeigen, dass -- \emph{bedingt durch das Attraktor-Regularitaetspaket unten}, eine Regularitaetsannahme ueber die Attraktorprojektion (Assumption~G, Abschnitt~3) und eine \emph{Dissipations-Dominanz-Bedingung}~(D) -- jeder hypothetische Blow-up zu einem Widerspruch fuehrt: Die Enstrophie divergiert punktweise und treibt das nichtnegative Abstandsfunktional unter Null, sobald die kumulative viskose Dissipation die Gronwall-Kosten uebersteigt. \textbf{Entscheidend ist, dass Condition~D auf $L^2$-Ebene eine strukturelle Limitation darstellt:} Die Leray-Energieidentitaet beschraenkt das Dissipationsintegral von oben, waehrend die Gronwall-Kosten exponentiell wachsen koennen, was Condition~D zunehmend schwierig bei langsamem Blow-up macht. Das Argument identifiziert daher die $H^1$-Ebene als die natuerliche Sobolev-Ebene fuer die Obstruktionsstrategie, wo die Enstrophie-Dissipation keine a-priori-Oberschranke besitzt. Das resultierende bedingte Regularitaetskriterium reduziert das vollstaendige Regularitaetsproblem auf mehrere strukturelle Fragen: die Existenz eines ausreichend regulaeren Referenzattraktors, die Geometrie der Attraktorprojektion (Assumption~G) und die relativen Blow-up-Raten von Dissipation gegenueber Gronwall-Faktoren (Condition~D). Wir beanspruchen nicht, dass eine der Bedingungen leichter zu verifizieren ist als Regularitaet selbst; der Wert liegt in der Zerlegung und in der Identifikation konkreter Angriffsvektoren fuer jede Komponente.
...
Wir definieren ein verallgemeinertes Divergenzfunktional $H(u \mid \mathcal{A})$, das den Abstand des aktiven Geschwindigkeitsfeldes $u$ von einem Referenzattraktor $\mathcal{A}$ mit dem folgenden Regularitaetspaket darstellt.

\begin{definition}[Divergenz vom globalen Attraktor]
Sei $\Omega = \mathbb{T}^3$ der flache Torus. Wir arbeiten mit einer Referenzmenge $\mathcal{A}$ fuer die Navier--Stokes-Dynamik, die Axiom~\ref{prop:attractor-regularity} erfuellt. In Settings, in denen ein glatter kompakter globaler Attraktor bereits bekannt ist, kann $\mathcal{A}$ dieser Attraktor sein. Fuer das klassische 3D-Leray--Hopf-Problem ist dies ein bedingtes Referenzobjekt, kein etablierter globaler Leray--Hopf-Attraktor. Fuer jedes aktive Stroemungsfeld $u \in L^2(\Omega)$ ist das Abstandsfunktional:
\begin{equation}
...
\begin{axiom}[Attraktor-Regularitaetspaket]
...
\begin{enumerate}
    ...
    \item[\emph{(ii)}]  Jedes $v \in \mathcal{A}$ besitzt die unten benoetigte Sobolev-Regularitaet; insbesondere $v \in H^2(\Omega) \cap W^{1,\infty}(\Omega)$.
...
Attraktoren bereits verfuegbar sind, etwa in zweidimensionalen,
gedaempften, hyperviskosen oder regularisierten Modellen. Fuer das
...
Loesungen und schwache/Trajektorien-Attraktorobjekte in schwaecheren
Topologien, aber keinen kompakten $H^1$-Semiflussattraktor mit
gleichmaessig glatten Elementen, Eindeutigkeit und den hier benoetigten
Schranken. Jede Verwendung von Proposition/Axiom~\ref{prop:attractor-regularity}
unten ist daher eine bedingte Verwendung dieses Attraktor-Regularitaetspakets.

\medskip\noindent
\textbf{Selektionsabhaengigkeit.}\;
Da die Eindeutigkeit von Leray--Hopf-Loesungen offen ist, haengt der
Semifluss $S(t)$ von der Wahl der Loesungsselektion ab, und
verschiedene Selektionen koennen verschiedene Attraktoren erzeugen.
Das vorliegende Framework gilt fuer jede feste Selektion, die einen
Semifluss erzeugt, dessen $\omega$-Limesmenge
Axiom~\ref{prop:attractor-regularity} erfuellt. Die Schlussfolgerungen von
Satz~\ref{thm:main} gelten fuer jeden solchen Attraktor, sofern
die zugehoerige Naechste-Punkt-Projektion Assumption~G erfuellt.
...
vollstaendig umgeht, indem er mit Mengen vollstaendiger Trajektorien
...
Sei $u \in L^\infty_t H^1_x(\Omega)$ eine divergenzfreie Leray--Hopf-Loesung der Navier-Stokes-Gleichungen mit glattem Antrieb $f$, und sei $v \in L^\infty_t H^1_x(\Omega)$ ein divergenzfreies Feld (nicht notwendigerweise eine NS-Loesung) mit lokal integrierbarem $\partial_t v$. Definiere das Residuum $\mathcal{R}_v := \partial_t v + (v\cdot\nabla)v - \nu\Delta v - f$, das verschwindet, wenn $v$ selbst eine NS-Loesung ist. Die Zeitentwicklung von $E_{v}(t) = \frac{1}{2}\| u - v \|_{L^2}^2$ ist beschraenkt durch:
\begin{equation}
...
Wenn $v$ eine NS-Loesung ist, gilt $\mathcal{R}_v = 0$ und $C_2$ vereinfacht sich zu $\frac{1}{2}\|(v \cdot \nabla)v\|_{L^2}^2 + \frac{1}{2}\|f - \partial_t v\|_{L^2}^2 + \frac{\nu}{2}\|\nabla v\|_{L^2}^2$. Wenn $v(t)$ der zeitvariable Attraktor-Minimierer ist (Bemerkung~\ref{rem:minimiser-trajectory}), muss der Term $\partial_t v$ als die gesamte Aenderungsrate der Selektion interpretiert werden, und $\mathcal{R}_v \neq 0$ im Allgemeinen; die BV-Modifikation im Beweis von Satz~\ref{thm:main} (Gleichung~\eqref{eq:stieltjes-bound}) behandelt diesen Fall.
\end{lemma}
...
Unter Axiom~\ref{prop:attractor-regularity} gelten die gleichmaessigen Schranken $\sup_{v \in \mathcal{A}} C_1(t) < \infty$ und $\sup_{v \in \mathcal{A}} C_2(t) < \infty$ fuer die im Abstandsfunktional verwendeten Referenzfelder. Infimumbildung ueber $\mathcal{A}$ ueberfuehrt die individuelle Energieschranke in eine absolute Stabilitaetsgrenze fuer die Stroemungsdivergenz $H(u \mid \mathcal{A})$.
...
\[
\|\omega\|_{H^s}
\le C\,\|\omega\|_{L^2}^{1-s/k}\,\|\omega\|_{H^k}^{s/k},
\qquad
\|\omega\|_{H^k}
\le C'\|\nabla^k \omega\|_{L^2}+C''\|\omega\|_{L^2},
\]
fuer jedes ganzzahlige $k > s$; wir waehlen $k = \lceil s \rceil + 1$
...
eventuelle Attraktion in der Topologie, in der ein Attraktor verfuegbar
ist; exponentielles Tracking verlangt zusaetzliche Squeezing-,
Exponentialattraktor-, Inertialmannigfaltigkeits- oder
Regularisierungsstruktur. Fuer klassische 3D-Navier--Stokes bleibt
AGC2 ein zusaetzlicher bedingter Input.
...
\begin{definition}[Masstheoretischer Reach]\label{def:mu-reach}
Sei $\mu_{\mathcal{A}}$ ein ergodisches invariantes Mass, getragen auf dem
...
Naechste-Punkt-Projektion ist.
\end{definition}

\begin{proposition}[$\mu$-Reach als bedingter Weg zu G$'$]\label{prop:mu-reach}
Falls $\mathrm{reach}_\mu(\mathcal{A}) > 0$, die Leray--Hopf-Loesung $u(t)$
eventuell in der zugehoerigen Reach-Roehre bleibt, und das
Besetzungsmass der projizierten Trajektorie absolut stetig gegenueber
$\mu_{\mathcal{A}}$ ist mit endlicher Kreuzungs-/Sprungvariation durch
die Null-Reach-Menge, dann besitzt die Naechste-Punkt-Projektion eine
BV-Auswahl. Insbesondere gilt Assumption~G$'$. Die Besetzungs- und
Endlich-Variationsannahmen sind zusaetzliche Hypothesen; sie folgen
nicht allein aus der Ergodizitaet von $\mu_{\mathcal{A}}$.
\end{proposition}

\begin{proof}[Beweisskizze]
Nach Definition des $\mu$-Reach ist die Projektion $\pi_{\mathcal{A}}$
Lipschitz-1 in einer tubulaeren Umgebung $\mu_{\mathcal{A}}$-fast
jedes Punktes von $\mathcal{A}$.
Die Tracking-Annahme bringt $u(t)$ in eine Roehre, in der lokale
metrische Projektionen ausserhalb der Null-Reach-Menge definiert sind.
Positiver $\mu$-Reach liefert Lipschitz-1-Projektion an
$\mu_{\mathcal{A}}$-typischen Attraktorpunkten. Die
Besetzungsmass-Hypothese uebertraegt diese Typizitaet auf
Lebesgue-fast alle Zeiten der konkreten projizierten Trajektorie,
waehrend die Endlich-Variationsannahme die Ausnahme-Passagen durch
...
Die $\mu$-Reach-Bedingung ist \emph{strikt schwaecher} als die deterministische
Reach-Bedingung AGC1:
\begin{itemize}
\item AGC1 erfordert $\mathrm{reach}(\mathcal{A}) > 0$ \emph{ueberall}
  auf $\mathcal{A}$ -- aequivalent zur Existenz einer Inertialmannigfaltigkeit.
...
\end{itemize}
Fuer den 3D-Navier--Stokes-Attraktor wird erwartet, dass die singulaere Menge der
Naechste-Punkt-Projektion duenn ist
(konzentriert auf Selbstdurchschneidungsloki hoher Kodimension).
Falls bestaetigt, wuerde $\mu$-Reach $> 0$ folgen, ohne
die Existenz einer Inertialmannigfaltigkeit zu erfordern.

\textbf{Offen:} Ob $\mathrm{reach}_\mu(\mathcal{A}) > 0$ fuer den
...
lokalen Reach -- keine Reach-Null-Singularitaet wurde detektiert.
Beim Lorenz-Attraktor erklaert die glatte Blaetterstruktur
...
Die unbewiesenen Inputs sind \textup{(AGC1)} und die verstaerkte
Tracking-Annahme \textup{(AGC2)}, zusaetzlich zum
Attraktor-Regularitaetspaket. Dies reduziert einen Teil der
Regularitaetsfrage von einem PDE-Problem (``bleibt $u$ glatt?'') auf
geometrische und dynamische Fragen ueber den Referenzattraktor.
\end{remark}
...
Referenzattraktor, der Axiom~\ref{prop:attractor-regularity} erfuellt, und definiere
\[
...
die uniformen $H^2$- und $W^{1,\infty}$-Schranken, die unten benoetigt werden.
...
\end{equation}
\endgroup
mit zusaetzlichem BV/Stieltjes-Korrekturterm aus~\eqref{eq:H-integral-BV}
...
dominieren; diese Dominanz ist eine zusaetzliche offene Bedingung.
Die $H^1$-Ebene sollte daher als das \emph{natuerliche
Sobolev-Niveau} fuer diese Obstruktionsstrategie verstanden werden, nicht als optionale
Erweiterung.
\end{remark}
...
gueltiges $T_{\mathrm{crit}}$-Theorem wuerde eine unabhaengige Untergrenze
fuer die Gronwall-Kosten oder eine aequivalente quantitative Beschreibung
...
geschaerfte Bedingung~(D) gilt, ist endlicher Blow-up ausgeschlossen. Das
$L^2$-Energiebudget erklaert, warum~(D) als substanzielle Zusatzannahme
behandelt werden muss.
\end{remark}

\begin{remark}[Strukturelle Einschraenkung auf $L^2$-Ebene]\label{cor:L2-limitation}
...
Leray-Dissipationsbudget beruecksichtigen. Das macht den Satz nicht leer,
bedeutet aber, dass Dissipations-Dominanz nicht aus BKM oder der
Leray-Energieidentitaet allein folgt. Das natuerliche Sobolev-Niveau fuer
den naechsten Obstruktionsversuch ist $H^1$, wo die
...
dominiert das Anfangsbudget plus die Gronwall-Kosten fuer ein $T$
...
Prop.~\ref{prop:attractor-regularity} &
  ...
  \textbf{Axiom} & Offen fuer klassische 3D NS \\
Lem.~\ref{lem:BS} &
...
einzigen Eigenschaft: Fuer das klassische 3D-Problem braucht man weiter
das Attraktor-Regularitaetspaket, positiven Reach (oder BV-Auswahlersatz),
effektives Tracking in die Reach-Roehre und Bedingung~(D).
\end{remark}
...
\section[Navier-Stokes als Pattern-A-System]{Navier-Stokes-Regularitaet als Pattern-A-System}
...
\begin{enumerate}
\item[\textbf{(G$_1$)}] \textbf{Attraktorexistenz und Kompaktheit.} Der Referenzattraktor $\mathcal{A}$ existiert als kompakte Teilmenge der Arbeitstopologie und erfuellt die uniformen Schranken aus Axiom~\ref{prop:attractor-regularity}. \emph{Status:} Bedingt fuer klassische 3D-Navier--Stokes; schwache/Trajektorienattraktoren existieren in schwaecheren Settings, aber die hier benoetigte $H^1$-Kompaktheit und uniforme Glattheit sind ohne zusaetzliche Regularitaetsstruktur nicht bekannt.
...
Assumption~G laesst sich in einen minimalen \emph{Schnittstellenvertrag} zerlegen, der die exakten Regularitaetsdaten spezifiziert, die von der Attraktorgeometrie benoetigt werden:
\begin{description}
\item[Input 1 (Attraktorregularitaet):] $\sup_{v \in \mathcal{A}} \|v\|_{H^2} < \infty$. \emph{Status: bekannt} (folgt aus der Glattheit des globalen Attraktors).
\item[Input 2 (Selektionsregularitaet):] Die Naechste-Punkt-Abbildung $t \mapsto v(t) \in \mathcal{A}$ erfuellt $v \in BV([0,T]; H^1)$. \emph{Status: offen} -- dies ist die zentrale Luecke, adressiert durch das TLL/LDI-Framework der Begleitarbeit~\cite{Geiger2026-LDI}.
\item[Input 3 (Gronwall-Integrierbarkeit):] Die Gronwall-Koeffizienten $C_1(t)$ erfuellen $\int_0^{T^*} C_1(t)\,dt < \infty$. \emph{Status: folgt aus Input~2} (BV-Selektion impliziert lokal integrierbare Ableitungsschranken).
\end{description}
Diese Zerlegung verdeutlicht, dass der gesamte offene Inhalt von Assumption~G sich auf eine einzige Frage reduziert: \emph{Hat die Naechste-Punkt-Projektion auf den Attraktor beschraenkte Variation entlang von Leray--Hopf-Loesungen?} Die Begleitarbeit liefert eine hinreichende Bedingung (TLL + LDI) und validiert sie numerisch an drei Attraktoren.
\end{remark}
...
\begin{enumerate}
\item \emph{Unendliche Sprungakkumulation:} Der Minimierer $v(t) = \arg\min_{v \in \mathcal{A}} \|u(t) - v\|^2$ erfaehrt unendlich viele diskontinuierliche Spruenge in jeder Umgebung von $T^*$, wobei die Sprunggroessen nicht summierbar sind. Geometrisch entspricht dies dem Durchqueren unendlich vieler ``Graete'' der Voronoi-Tessellation von $\mathcal{A}$ durch die Trajektorie $u(t)$ in endlicher Zeit.
\item \emph{Cantor-artiges Schalten:} Die Menge der Schaltzeitpunkte $\{t : v(t^-) \neq v(t^+)\}$ hat positives Mass oder Cantor-artige Struktur, was BV-Regularitaet verhindert. Dies wuerde erfordern, dass der Attraktor auf den relevanten Skalen eine pathologisch fragmentierte Voronoi-Struktur besitzt.
\item \emph{Spiralakkumulation:} Die Trajektorie spiralt um eine Spitze oder Selbstdurchdringung von $\mathcal{A}$, wodurch die Projektion mit zunehmender Frequenz oszilliert. Dies ist der physikalisch plausibelste Versagensmodus und wuerde darauf hindeuten, dass die Attraktorgeometrie nahe potenzieller Blow-up-Punkte komplexer ist, als aktuelle Dimensionsabschaetzungen nahelegen.
\end{enumerate}
Das TLL/LDI-Framework der Begleitarbeit~\cite{Geiger2026-LDI} schuetzt gegen alle drei Modi: TLL begrenzt die Oszillationsrate logarithmisch, waehrend LDI sicherstellt, dass die kumulativen Oszillationskosten endlich bleiben.
\end{remark}
...
Die folgenden beobachtbaren Groessen in DNS oder numerischer Simulation wuerden Evidenz fuer oder gegen Bedingung~D liefern:
\begin{enumerate}
\item \emph{Enstrophie-Wachstumsrate:} Falls $\|\nabla u(t)\|_{L^2}^2 / H(u(t)|\mathcal{A})$ fuer $t \to T^*$ unbeschraenkt waechst, gilt Bedingung~D. Man ueberwache das Verhaeltnis $\|\nabla u\|^2 / \|u - v\|^2$ entlang nahezu-singulaerer Trajektorien.
\item \emph{Palinstrophie:} Das $H^1$-Niveau-Analogon erfordert $\int_0^{T^*} \|\Delta u\|_{L^2}^2\,dt = \infty$. Man verfolge die kumulative Palinstrophie in Simulationen mit adaptiver Aufloesung nahe potenziellem Blow-up.
\item \emph{Attraktorabstands-Zerfall:} Falls $H(u(t)|\mathcal{A}) \to 0$ schneller als $\|\nabla u\|^{-2}$, ist Bedingung~D erfuellt. Man vergleiche die Raten numerisch.
\end{enumerate}
...
liefern scharfe Attraktordimensions-Abschaetzungen via Lieb--Thirring-Ungleichungen,
die die klassischen nicht-fraktionalen Schranken verbessern und analoge Abschaetzungen
im vorliegenden Rahmenwerk anleiten koennten.
\end{remark}
...
\begin{remark}[Duchon--Robert-Defektmass als Bruecke zwischen Turbulenz und Attraktor-Regularitaet]
\label{rem:duchon-robert}
Duchon und Robert~\cite{DuchonRobert2000} fuehrten ein lokales
Energie-Defektmass $D(u)$ fuer schwache Loesungen der inkompressiblen
Euler- und Navier--Stokes-Gleichungen ein.  Fuer eine schwache
Loesung~$u$ erfasst die Distribution $D(u)$ die lokale Rate der
...
\[
  ...
  = -D(u).
\]
Eine zentrale physikalische Einschraenkung ist $D(u) \ge 0$ fast
ueberall fuer physikalisch zulaessige Loesungen (Energie fliesst nur
zu kleineren Skalen).

Dieses Defektmass verbindet sich auf drei Wegen direkt mit dem
FST-Attraktor-Framework:
\begin{enumerate}
\item \emph{Singularitaets-Lokalisierung auf dem Attraktor.}
Auf dem globalen Attraktor~$\mathcal{A}$ kodiert der Traeger von
$D(u)$ die Singularitaetsstruktur: Blow-up kann nur dort auftreten,
wo sich $D(u)$ konzentriert.  Falls der singulaere Traeger von $D(u)$
Hausdorff-Dimension
...
Dissipationsfunktional minimiert.  Das Defektmass $D(u)$ liefert
ein \emph{natuerliches Gewicht} fuer diese Minimierung: Ersetzt man
den gleichmaessigen $L^2$-Abstand durch den $D$-gewichteten Abstand
$\int D(u(t))\, \|u(t) - a(t)\|^2\, dt$, konzentriert sich die
Auswahl dort, wo inertiale Dissipation aktiv ist, und verbessert die
Regularitaet in ruhigen Regionen.
\item \emph{Bruecke zum Turbulenz-Begleitpaper.}
In~\cite{FST-TU2026} spielt das Defektmass $D(u)$ die Rolle des
anomalen Dissipationsflusses in der turbulenten Kaskade.  Das
vorliegende Paper betrifft die Regularitaet der Projektion
auf~$\mathcal{A}$ (Assumption~G); das Turbulenz-Paper betrifft die
spektrale Verteilung von~$D(u)$.  Eine vereinheitlichte
Schliessstrategie wuerde beides etablieren, indem gezeigt wird,
dass $D(u)$ ein Radon-Mass mit kontrolliertem
Hausdorff-dimensionalem Traeger ist -- genau die Bedingung, unter
der die BV-Auswahl nahezu-Lipschitz-Regularitaet erbt
...
% ------------------------------------------------------------------
\subsection{Warum die \texorpdfstring{$H^1$}{H1}-Ebene die Dissipationsluecke vermeidet}
% ------------------------------------------------------------------
...
\subsection{Post-Zookeeper-Transfer: gedaempfte Navier--Stokes und schlechte Mechanismen}
...
schaerfere Route fuer das, was als naechstes bewiesen werden muss.  Die
Navier--Stokes-Transferfelder sind:

Dies ist ein Transfer der RFEP-v1.8-Normalform, kein automatischer
PDE-Regularitaetssatz.  Der Zookeeper schliesst den
Riemann-$C^{(2)}$/MS2-Blocker im CORE; der Navier--Stokes-Ast
verifiziert Operator/Form, Defekt, Galerkin, Gap und Grenzuebergang
...
\begin{description}
  ...
  \item[Approximation.] Fourier--Galerkin-Trunkierungen, gedaempfte
  ...
  \item[Grenzuebergang.] Uebergang von gedaempften oder hyperviskosen
  Testsystemen zur klassischen dreidimensionalen Gleichung mit
  $\beta=1$.
\end{description}

Das konstruktive Zwischenziel ist ein Satz fuer gedaempfte
3D-Navier--Stokes-Gleichungen.  In Regimen, in denen Daempfung bessere
Attraktorregularitaet und Eindeutigkeit liefert, sollte man beweisen:
\[
  \text{Damping-Gap} + \text{Attraktorregularitaet}
  \Longrightarrow
  \mathrm{TLL/LDI}
  \Longrightarrow
  \mathrm{BV\text{-}Selektion}.
\]
Das loest das Millenniumsproblem nicht, wuerde aber die bisherigen
...
wird das bedingte Framework falsifizierbar: Eine glaubwuerdige
Regularitaetsroute muss sagen, welcher Mechanismus durch welchen Defekt
