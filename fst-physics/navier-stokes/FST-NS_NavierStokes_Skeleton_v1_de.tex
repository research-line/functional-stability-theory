...
Semifluss erzeugt, dessen $\omega$-Limesmenge kompakt in $H^1$ ist
(was fuer jede starke Selektion der Fall ist, aber fuer pathologische
schwache Selektionen offen bleibt). Die Schlussfolgerungen von
...
\begin{proposition}[$\mu$-Reach genuegt fuer Assumption~G]\label{prop:mu-reach}
Falls $\mathrm{reach}_\mu(\mathcal{A}) > 0$ und die Leray--Hopf-Loesung $u(t)$
die Bedingung $\operatorname{dist}(u(t), \mathcal{A}) < \mathrm{reach}_\mu(\mathcal{A})$
fuer alle $t \geq t_0$ erfuellt (was durch exponentielles Tracking, AGC2, gilt),
dann ist die Naechste-Punkt-Projektion $v(t) = \pi_{\mathcal{A}}(u(t))$
absolut stetig auf $[t_0, T]$ fuer jedes $T < \infty$,
und Assumption~G gilt.
\end{proposition}
...
Exponentielles Tracking (AGC2, bewiesen von Temam~\cite{Temam2001})
stellt sicher, dass $u(t) \in B(\mathcal{A}, \delta\,e^{-\lambda t})$ fuer $t \geq t_0$.
Da $u(t)$ $\mu_{\mathcal{A}}$-fast die gesamte Zeit nahe an
$\mu_{\mathcal{A}}$-typischen Punkten von $\mathcal{A}$ verbringt (durch Ergodizitaet der
projizierten Dynamik $v(t)$), gilt die Lipschitz-Eigenschaft von $\pi_{\mathcal{A}}$
fuer $\mu_{\mathcal{A}}$-fast jede Zeit $t$.
Die Menge der Ausnahmezeiten (wo $v(t)$ nahe einer Null-Reach-Singularitaet
von $\mathcal{A}$ liegt) hat $\mu_{\mathcal{A}}$-Mass Null
und damit Lebesgue-Mass Null (durch absolute Stetigkeit des
Bildmasses).
Daher ist $v(t) = \pi_{\mathcal{A}}(u(t))$ Lipschitz-1 f.u.\ in $t$,
also absolut stetig, und G gilt.
\end{proof}
...
\begin{theorem}[$L^2$-Ebene-Obstruktion]\label{thm:L2-obstruction}
Sei $u$ eine Leray--Hopf-Loesung auf $[0,T^*)$ mit $H(0) = H(u(0)|\mathcal{A}) > 0$.
Definiere
\[
  T_{\mathrm{crit}} \;:=\; \frac{1}{C_1^*}\,\ln\!\frac{E_0}{C_1^*\,H(0)},
\]
wobei $C_1^* = C_1^*(\mathcal{A},\nu,f)$ der Gronwall-Koeffizient aus Theorem~\ref{thm:main}
und $E_0 = \frac{1}{2\nu}\int_0^{T^*}\!\|f\|^2\,dt$ die Leray-Energieschranke ist.
Falls $T^* > T_{\mathrm{crit}}$, dann ist Bedingung~(D) \emph{notwendigerweise} verletzt:
die exponentiellen Gronwall-Kosten ueberschreiten das a-priori-Dissipationsbudget.
\end{theorem}

\begin{proof}
Integration der Energieungleichung fuer $H$ (Theorem~\ref{thm:main}) ueber $[0,T^*]$:
\[
  \frac{d}{dt}H \;\leq\; -\nu\|\nabla w\|^2 + C_1^*\,H + C_2^*,
\]
...
\end{equation}
Die Gronwall-Schranke $H(t) \leq H(0)\,e^{C_1^* t} + \frac{C_2^*}{C_1^*}(e^{C_1^* t}-1)$
ergibt
\[
  \int_0^{T^*}\!H\,dt
  \;\geq\; \frac{H(0)}{C_1^*}\bigl(e^{C_1^* T^*} - 1\bigr).
\]
Bedingung~(D) erfordert, dass die Dissipation (linke Seite von~\eqref{eq:diss-budget})
die Gronwall-Kosten (den $C_1^*\!\int H\,dt$-Term) dominiert.
Doch die Leray-Energieidentitaet beschraenkt die Dissipation:
$\nu\!\int_0^{T^*}\!\|\nabla u\|^2\,dt \leq E_0$.
Da $\|\nabla w\|^2 \leq 2\|\nabla u\|^2 + 2\|\nabla v\|^2$
und $\|\nabla v\|$ auf $\mathcal{A}$ gleichmaessig beschraenkt ist,
waechst die linke Seite von~\eqref{eq:diss-budget} hoechstens linear in $T^*$,
waehrend die rechte Seite wie $e^{C_1^* T^*}$ waechst.
Fuer $T^* > T_{\mathrm{crit}} := \frac{1}{C_1^*}\ln\frac{E_0}{C_1^*\,H(0)}$
ueberschreiten die exponentiellen Gronwall-Kosten das Dissipationsbudget, und (D) versagt.
\end{proof}

\begin{corollary}[Strukturelle Einschraenkung auf $L^2$-Ebene]\label{cor:L2-limitation}
Jede Obstruktionsstrategie basierend auf $H(u|\mathcal{A}) = \inf_{v \in \mathcal{A}}\frac{1}{2}\|u-v\|_{L^2}^2$
mit Gronwall-artiger Energiekontrolle liefert eine \emph{leere Aussage}
fuer hypothetische Blow-up-Zeiten $T^* > T_{\mathrm{crit}}$.
Das natuerliche Sobolev-Niveau fuer Attraktor-Abstands-Obstruktionsargumente
ist $H^1$, wo die Enstrophie-Dissipation $\int\|\Delta u\|^2\,dt$ keine a-priori-Oberschranke besitzt.
\end{corollary}
...
Dies ist ein RFEP-v1.8-Normalform-Transfer, kein automatischer
PDE-Regularitaetssatz.  Der Zookeeper schliesst den
Riemann-$C^{(2)}$/MS2-Blocker im CORE; der Navier--Stokes-Ast muss
Operator/Form, Defekt, Galerkin, Gap und Grenzuebergang weiterhin in
seinem eigenen analytischen Setting verifizieren.
\begin{description}
