...
Sei $u \in L^\infty_t H^1_x(\Omega)$ eine divergenzfreie Leray--Hopf-Lösung der Navier-Stokes-Gleichungen mit glattem Antrieb $f$, und sei $v \in L^\infty_t H^1_x(\Omega)$ ein divergenzfreies Feld (nicht notwendigerweise eine NS-Lösung) mit lokal integrierbarem $\partial_t v$. Die Zeitentwicklung von $E_{v}(t) = \frac{1}{2}\| u - v \|_{L^2}^2$ ist beschränkt durch:
\begin{equation}
    \frac{d}{dt} E_{v}(t) \le - \frac{\nu}{2} \|\nabla u\|_{L^2}^2 + C_1(t) E_{v}(t) + C_2(t),
\end{equation}
wobei $C_1(t) = 2\|\nabla v\|_{L^\infty} + 1$ und $C_2(t) = \frac{1}{2}\|(v \cdot \nabla)v\|_{L^2}^2 + \frac{1}{2}\|f - \partial_t v\|_{L^2}^2 + \frac{\nu}{2}\|\nabla v\|_{L^2}^2$. Falls $v$ selbst eine NS-Lösung ist, ist der Druckbeitrag in der Paarung mit dem divergenzfreien Feld $w=u-v$ unschädlich; ein separater unprojizierter Residuumsterm wird nicht benötigt.
Wenn $v(t)$ der zeitvariable Attraktor-Minimierer ist (Bemerkung~\ref{rem:minimiser-trajectory}), muss der Term $\partial_t v$ als die gesamte Änderungsrate der Selektion interpretiert werden; die BV-Modifikation im Beweis von Satz~\ref{thm:main} (Gleichung~\eqref{eq:stieltjes-bound}) behandelt diesen Fall.
\end{lemma}
...
% ------------------------------------------------------------------
\subsection*{Lemma B (Enstrophie-Bilanz und Limsup-Blow-up)}
% ------------------------------------------------------------------
\begin{lemma}[Enstrophie-Bilanz und Limsup-Blow-up]
\label{lem:enstrophy}
...
ist die Enstrophie im limsup-Sinn unbeschränkt:
...
$T^* < \infty$:
\begin{itemize}
  \item \textbf{Unbeschränkt im limsup:}\;
    BKM liefert
    \[
...
\end{itemize}
Limsup-Divergenz von $\|\nabla u(t)\|_{L^2}$ ist vereinbar mit
endlicher $L^2$-in-Zeit-Integrierbarkeit: zum Beispiel divergiert ein
monotones Modellprofil $(T^*-t)^{-\alpha}$ punktweise, ist aber integrierbar
für $\alpha < 1$; das PDE-Kriterium verlangt nur Unbeschränktheit entlang
einer Folge.
...
\begin{itemize}
  ...
  \item \textbf{Chistyakov BV-Auswahl}~\cite{Chistyakov2004}:
...
die relevanten starken Normen können nur entlang von Folgen unbeschränkt
werden, und die Frage ist, ob das Log-Gewicht dieses Wachstum verstärkt oder
absorbiert. Unter Blow-up bewegt sich die Lösung im limsup-Sinn
\emph{weg} vom Attraktor in $H^1$ (während Attraktorelemente gleichmäßig
beschränkt sind), kann aber in $L^2$ nahe bleiben -- die $L^2$-Energie bleibt
beschränkt nach der Leray--Hopf-Ungleichung. Das Zusammenspiel zwischen der
unbeschränkten Geschwindigkeitsskala und dem beschränkten (oder langsam
...
Dies ist die natürliche Kritikalitätsschwelle für dissipative PDEs,
...
\begin{conjecture}[Squeezing impliziert TLL]\label{conj:squeezing-tll}
...
\end{equation}
wobei $Q_N = I - P_N$ die Projektion auf die hohen Moden ist. Dann sollte die N\"achste-Punkt-Projektion $P_\mathcal{A}: \mathcal{A}^R \to \mathcal{A}$ die TLL-Bedingung (Definition~\ref{def:TLL}) mit $C_{\mathrm{TLL}} = C(N, \theta)$ erf\"ullen, wobei die Konstante nur von $N$ und $\theta$ abh\"angt.
\end{conjecture}

\begin{proof}[Heuristische Motivation (kein Beweis)]
Die Squeezing-Eigenschaft legt nahe, dass $\mathcal{A}$ in einem Lipschitz-Graphen \"uber den ersten $N$ Fourier-Moden enthalten ist (Foias--Temam, 1984). Auf einem solchen Graphen erf\"ullt die N\"achste-Punkt-Projektion
$\|P_\mathcal{A}(x) - P_\mathcal{A}(y)\| \leq (1 + L_N) \|P_N(x - y)\| + L_N \|Q_N(x - y)\|$,
wobei $L_N$ die Lipschitz-Konstante des Graphen ist. Der offene Schritt ist die logarithmische Kontrolle des Hochmoden-Fehlers $\|Q_N(x - P_\mathcal{A}(x))\|$ durch $d \cdot (1 + \log(R/d))$ f\"ur Punkte $x$ im Abstand $d$ von $\mathcal{A}$. Eine solche Kontrolle w\"urde TLL liefern, ist f\"ur klassische 3D-Navier--Stokes aber nicht bewiesen.
\end{proof}

\begin{remark}[Relevanz f\"ur 3D-Navier--Stokes]\label{rem:squeezing-ns}
Die Squeezing-Eigenschaft ist f\"ur 2D-Navier--Stokes (Foias--Temam 1984), Reaktions-Diffusions-Systeme und ged\"ampfte Wellengleichungen etabliert. F\"ur 3D-Navier--Stokes w\"urde Squeezing auf dem Attraktor aus der Eigenschaft der \emph{bestimmenden Moden} folgen, die f\"ur generische Anregung bekannt ist (Foias--Prodi~\cite{FoiasProdi1967}, Jones--Titi~\cite{JonesTiti1992}). Die L\"ucke besteht darin, dass bestimmende Moden das asymptotische Verhalten kontrollieren, w\"ahrend Squeezing gleichm\"a\ss ige Kontraktion in der Zeit erfordert. Vermutung~\ref{conj:squeezing-tll} markiert daher einen m\"oglichen TLL/LDI-Pfad zu Assumption~G, nicht einen etablierten Beweisschritt.
\end{remark}
...
\begin{remark}[Heuristische Implikationsrouten f\"ur G-NS]\label{prop:g-equivalences}
Die folgenden Aussagen notieren die hier betrachteten hinreichenden Strukturpfade:
\begin{enumerate}
...
\item[(iii)] \textbf{Starkes Squeezing:} Die Squeezing-Eigenschaft (Vermutung~\ref{conj:squeezing-tll}) gilt gleichm\"a\ss ig f\"ur alle L\"osungen auf dem Attraktor.
...
\end{enumerate}
Die im Paper verwendete robuste Richtung ist: Eine inertiale Mannigfaltigkeits-/Slaving-Struktur wie~(i) liefert, sofern sie mit den genannten Uniformit\"atsannahmen vorliegt, Assumption~G~(iv). Die Umkehrung (iv)$\Rightarrow$(i) ist nicht etabliert und wird nicht verwendet: BV-Regularit\"at der Selektion erzwingt f\"ur sich allein keine inertiale Mannigfaltigkeit.
\end{remark}

\begin{remark}[Status der Implikationsrouten]
\label{rem:iv-implies-i-status}
Die Route \textup{(ii)$\Rightarrow$(i)$\Rightarrow$(iii)$\Rightarrow$(iv)}
ist als Hierarchie hinreichender Bedingungen zu lesen: eine Spektrallücke
impliziert eine inertiale Mannigfaltigkeit, diese motiviert Squeezing, und
Squeezing würde über Vermutung~\ref{conj:squeezing-tll} Assumption~G liefern.
Der letzte Schritt enthält jedoch eine offene log-Lipschitz-Kontrolle.
Die Route ist daher heuristisch und nicht eine Kette streng bewiesener
Implikationen.
\end{remark}
...
müsste die zeitliche Kohärenz der Minimierer-Trajektorie ausnutzen
...
Zur Orientierung trennt Tabelle~\ref{tab:conditional-input-ledger} den
formalen Status der Inputs von ihrer Funktion im Beweis.
...
\hline
\textbf{Input} & \textbf{Status in dieser Arbeit} & \textbf{Funktion im Argument} \\
\hline
Attraktor-Regularitätspaket &
  Stehendes Axiom für klassische 3D-Navier--Stokes; verfügbar nur unter Annahme der angegebenen Kompaktheits- und Glattheitsschranken des Referenzattraktors. &
  Liefert die uniformen Konstanten und die Kompaktheit für die Abschätzungen von $H(u\mid\mathcal{A})$. \\
Assumption~G/G$'$ &
  Offener geometrischer Selektions-Input; die Arbeit nennt hinreichende Wege über positiven Reach, Tracking oder BV-Selektionen. &
  Überträgt die feste Referenzabschätzung auf das zeitabhängige minimierende Attraktorelement und kontrolliert die Stieltjes-Korrektur. \\
Bedingung~D &
  Substanzielle analytische Dominanzannahme; sie folgt nicht allein aus Leray-Energie oder BKM. &
  Macht die rechte Seite der integrierten $H$-Schranke negativ und erzeugt den Widerspruch $H(T)<0$. \\
$H^1$-Lift &
  Programmatische Erweiterung, motiviert durch die $L^2$-Budgetgrenze. &
  Verschiebt die Obstruktion auf ein Sobolev-Niveau, auf dem Enstrophie-Dissipation keine a-priori-Oberschranke hat; neue Bihari-artige Dominanz bleibt offen. \\
\hline
\end{tabularx}
\caption{Bedingter Input-Status zu Satz~\ref{thm:main}.}
...
erfüllt; für großes~$T^*$ (langsamer Blow-up nahe dem Attraktor)
...
Unter Blow-up ($T \nearrow T^* < \infty$):
Lemma~\ref{lem:enstrophy} gibt
$\limsup_{t\nearrow T^*}\|\nabla u(t)\|_{L^2}^2=\infty$, aber die
...
\medskip
Innerhalb des breiteren FST-Programms kann der bedingte Ausschluss von Blow-up in endlicher Zeit in den 3D-Navier-Stokes-Gleichungen als Kandidat für die \textbf{Pattern-A-Stabilitätslogik} gelesen werden (siehe \cite{FST-Unified2026}). Dies ist eine Interpretation der bedingten Architektur, kein zusätzlicher Satz. Der Ansatz ersetzt die Suche nach globalen a-priori-Schranken für $\|\nabla u\|_{L^\infty}$ durch die folgende vorgeschlagene Zerlegung:

\begin{enumerate}[label=\alph*)]
    \item \textbf{Analytische Rang-Endlichkeit (Null-Tail):} Ein putatives Blow-up-Szenario müsste über endlich viele effektive Instabilitätskanäle organisiert sein. In welchem Umfang Hochfrequenzdissipation einen solchen Null-Tail liefert, bleibt analytischer Input und ist hier keine bewiesene Folge.
    \item \textbf{Attraktorprojektion als physikalische Eichung:} Potentielle Singularitätsrichtungen werden mit dem Referenzattraktor $\mathcal{A}$ verglichen, der Axiom~\ref{prop:attractor-regularity} erfüllt. Diese Projektion ist nur unter der zusätzlichen Selektionsregularität Assumption~G verwendbar.
    \item \textbf{Eingeschränkte Positivität (H-Funktional):} Die Nichtnegativität des Abstandsfunktionals $H(u \mid \mathcal{A})$ wird erst dann obstruktiv, wenn Condition~D integrierte Dominanz der viskosen Dissipation über Gronwall-, BV- und Projektionskosten liefert.
\end{enumerate}

Somit würde globale Regularität in diesem Programm als bedingte Stabilitätsaussage unter der Eich-Beschränkung der Attraktorstruktur interpretiert. Das Dissipationsintegral wirkt als putative irreversible ``Kosten'', während der Attraktorabstand einen nichtnegativen ``Etat'' bereitstellt; der tatsächliche Widerspruch entsteht aber nur unter den expliziten Hypothesen von Satz~\ref{thm:main}. Die thermodynamische Analogie ist daher heuristisch und kein eigenständiges Beweisprinzip.
...
nicht unbedingt beseitigt. Sie wird in explizite Inputs verschoben:
das Attraktor-Regularitätspaket, Assumption~G und Condition~D.
Unter diesen Inputs würde Blow-up in endlicher Zeit die Budget-Ungleichung
von Satz~\ref{thm:main} über die nichtnegative Barriere
$H(u \mid \mathcal{A}) \ge 0$ treiben. Ohne diese Inputs folgt aus
Blow-up allein kein Schluss $H\to-\infty$.
...
Für klassische 3D-Navier--Stokes sind diese Eigenschaften Teil des
stehenden Attraktor-Regularitätspakets und nicht als Konsequenzen der
schwachen Attraktortheorie zu lesen. Die verbleibende Lücke ist
Assumption~G, die die \emph{zeitliche Regularität} der Projektion
betrifft, nicht die räumliche Regularität der Attraktorelemente.
\end{remark}
...
\end{enumerate}
Das TLL/LDI-Framework der Begleitarbeit~\cite{Geiger2026-LDI} schützt gegen alle drei Modi: TLL begrenzt die Oszillationsrate logarithmisch, während LDI sicherstellt, dass die kumulativen Oszillationskosten endlich bleiben.
\end{remark}
...
\begin{remark}[Fenster-Version von Bedingung (D)]
\label{rem:condition-d-limsup}
Eine rein punktweise oder intermittierende Aussage wie
\[
\limsup_{t \to T^*} \frac{\|\nabla u(t)\|^2}{H(u(t)|A)} = +\infty.
\]
ist nur ein Diagnostikum und ersetzt Bedingung~(D) nicht. Satz~\ref{thm:main}
verlangt integrierte Dominanz über den gesamten positiven Kostenblock. Eine
zulässige Abschwächung ist eine Fenster-Version: Entlang von Intervallen
$I_n=[s_n,T_n]$ mit $T_n\nearrow T^*$ muss die integrierte viskose Dissipation
auf $I_n$ das Budget bei~$s_n$ sowie die Gronwall-, BV- und
Projektionskorrekturkosten auf demselben Intervall dominieren. Diese Fassung
passt besser zur PDE-Perspektive, weil Enstrophie-Blow-up nicht monoton sein
muss; unbeschränkte punktweise Verhältnisse allein implizieren Condition~(D)
aber nicht.
\end{remark}
...
\begin{enumerate}
\item \emph{Enstrophie-Wachstumsrate:} Wachstum von $\|\nabla u(t)\|_{L^2}^2 / H(u(t)|\mathcal{A})$ ist nur dann Evidenz für Bedingung~D, wenn es auf Fenstern anhält, deren integrierte Dissipation die Gronwall-, BV- und Projektionskorrekturkosten dominiert. Man überwache sowohl das punktweise Verhältnis $\|\nabla u\|^2 / \|u - v\|^2$ als auch die zugehörige Fensterbilanz entlang nahezu-singulärer Trajektorien.
...
\item \emph{Attraktorabstands-Zerfall:} Schneller Zerfall von $H(u(t)|\mathcal{A})$ relativ zu $\|\nabla u\|^{-2}$ ist nur unterstützend, wenn der daraus entstehende Dissipationsvorteil die Integration gegen die positiven Kostenterme überlebt. Man vergleiche die Fensterraten numerisch.
\end{enumerate}
...
\end{equation}
In dieser Sprache verlangt der bedingte Satz einen nichtnegativen effektiven Wert, nachdem Gronwall-, BV- und Projektionskosten einbezogen sind. Der Attraktorabstand $H(u\mid\mathcal{A})$ spielt die Rolle eines endlichen ``Budgets'', das dem System zur Verfügung steht, während das Dissipationsintegral $\int_0^T \nu\|\nabla u\|_{L^2}^2\,dt$ die kumulativen Kosten darstellt, die jede Blow-up-Trajektorie bezahlen muss. Da das Budget nichtnegativ ist, reduziert sich die bedingte Frage darauf, ob die kumulativen Dissipationskosten die Gronwall-Kosten nahe der Blow-up-Zeit übersteigen -- genau die Dissipations-Dominanz-Bedingung~(D) von Satz~\ref{thm:main}.

Dieser Standpunkt verbindet sich heuristisch mit der Legendre--Fenchel-Dualität in der konvexen Analysis: Interpretiert man die Dissipation als konvexes ``Entropieproduktions''-Funktional $S$ und den nichtlinearen Transfer als sein konjugiertes ``Freie-Energie-Kosten''-Funktional $F$, so ähnelt die Dominanzbedingung $S \ge F$ einem PDE-Analogon eines Zweite-Hauptsatz-Constraints. Diese Analogie wird nicht als eigenständiges Regularitätskriterium verwendet.
...
Jüngere Entwicklungen in der Mean-Field-Game-Theorie (MFG) formulieren das gekoppelte HJB--Fokker-Planck-System als PDE auf dem Wasserstein-Raum der Wahrscheinlichkeitsmasse (Cardaliaguet--Delarue--Lasry--Lions~\cite{CDLL2019}). Die NS-Freie-Energie $H(u|A)$ lässt sich heuristisch als Wertfunktionsanalogon lesen: Der ,,Agent`` (Fluidpaket) minimiert laufende Kosten (Enstrophie) unter Advektions-Diffusions-Dynamik, während die ,,Population`` (Geschwindigkeitsfeld) über die Navier--Stokes-Gleichungen evolviert. Der Attraktor $A$ spielt dann die Rolle eines Kandidaten für ein Gleichgewicht. Diese Perspektive markiert einen möglichen Transferpfad; eine Äquivalenz zwischen NS-Attraktorregularität und Wohlgestelltheit einer MFG-Mastergleichung wird hier nicht behauptet.
\end{remark}
...
(Lemma~\ref{lem:enstrophy}) große Auslenkungen von $H$ und
...
Diese Arbeit ist mit dem Pattern-A-Master-Stabilitätsprinzip aus dem vereinheitlichten Framework \cite{FST-Unified2026} kompatibel, beweist aber keinen neuen Pattern-A-Spektrallückensatz für 3D-Navier--Stokes. Eine mögliche Instanziierung würde einen präzise definierten Transferoperator, strikte Konvexität der relevanten renormierten freien Energie und eine verifizierte exponentielle Relaxationsrate zum Referenzattraktor benötigen.
\end{remark}

\begin{remark}[Wasserstein-Konvexität und Talagrand-Schranke]
\label{rem:talagrand}
Derzeit ist dies nur eine Transfer-Vorlage. Eine Talagrand- oder Wasserstein-Konvexitätsschätzung für den NS-Attraktor würde ein explizit gewähltes Wahrscheinlichkeitsmaß auf dem Trajektorienraum, eine verifizierte geodätische Konvexität des Freie-Energie-Funktionals und den zugehörigen Log-Sobolev- oder Poincaré-Input benötigen. Keine dieser maßtheoretischen Hypothesen wird in diesem Paper bewiesen, und Satz~\ref{thm:main} verwendet keine Talagrand-Schranke. Die Yang--Mills-Analogie ist daher als mögliche Forschungsrichtung zu lesen, nicht als universelles FST-Theorem, das im Navier--Stokes-Setting bereits verfügbar wäre.
\end{remark}
...
Die klassische punktweise Projektion $v(t) = \pi_A(u(t))$ ist strukturell unzureichend: BV-Regularit\"at ist eine zeitintegrierte Eigenschaft, aber punktweise Projektion ignoriert zeitliche Koh\"arenz. Die hier untersuchte Route ersetzt sie durch eine \emph{Trajektorienselektion}. Dies kann die Abhängigkeit von punktweisem positivem Reach, Inertialmannigfaltigkeiten oder Spektrallücken reduzieren, benötigt aber weiterhin die unten explizit genannten Approximierbarkeits-, Kompaktheits- und Recovery-Annahmen.
...
\end{equation}
Für eine \emph{feste} Vergleichstrajektorie $a_0 \in \mathcal{A}_T$ ist die rechte Seite von~\eqref{eq:apriori} im Allgemeinen nur $O(1/\varepsilon)$, weil $\int\|u-a_0\|^2$ nicht verschwinden muss. Eine gleichmäßige TV-Schranke folgt daher erst unter einer zusätzlichen Approximierbarkeitsannahme: Es muss eine $\varepsilon$-abhängige Vergleichstrajektorie $a_0^{(\varepsilon)} \in \mathcal{A}_T$ geben mit $\int_0^T\|u-a_0^{(\varepsilon)}\|^2\,dt=O(\varepsilon)$, also $O(\sqrt{\varepsilon})$-Nähe in $L^2$. Diese Annahme ist eine Form von Assumption~G/Trajektoriennähe und kann im Blow-up-Regime fehlschlagen.

Unter dieser zusätzlichen Annahme gilt
\begin{equation}\label{eq:tv-bound}
  \mathrm{TV}(v_\varepsilon) \;\leq\; \mathcal{F}_\varepsilon[a_0^{(\varepsilon)}] \;\leq\; C \quad \text{gleichm\"assig in } \varepsilon,
\end{equation}
wobei $C$ nicht von $\varepsilon$ abhängt. Dies ist die \textbf{wesentliche Verbesserung} gegen\"uber der unreskalierten Formulierung, bei der nur $\varepsilon\,\mathrm{TV} \leq C$ gilt und TV divergiert; die Verbesserung hängt jedoch an der Approximierbarkeitsannahme.
...
\begin{proof}[Beweisskizze]
Nach (S1) liegt $v_\varepsilon(t) \in V$ für fast alle $t$. Nach (S2) sind die Zeitoszillationen in $V'$ kontrolliert. Die kompakte Einbettung $V \hookrightarrow\!\!\!\hookrightarrow H$ und ein Interpolationsargument (Simon \cite{Simon1987}, Theorem~5) liefern Äquikontinuität in $H$, woraus relative Kompaktheit via Arzel\`a--Ascoli in $L^2(0,T;H)$ folgt.
\end{proof}
...
\end{enumerate}
Eigenschaft (2) ist \emph{genau} die von Assumption~G ben\"otigte BV-Selektionsschranke, aber sie ist an die Approximierbarkeits- und Recovery-Annahmen oben gebunden. Sie entsteht nicht durch ``Division durch $\varepsilon$'' (was in der unreskalierten Formulierung ung\"ultig ist), sondern, sofern die EDP-Hypothesen erfüllt sind, aus dem Mielke--Rossi--Savar\'e-EDP-Konvergenzrahmen \cite{MielkeRossiSavare2016} und dessen relaxierter Wiggly-Energy-Variante \cite{DondlFrenzelMielke2019}: die Energie-Dissipations-Bilanz propagiert durch den $\Gamma$-Grenzwert via Unterhalbstetigkeit und Kettenregel-Argumente, wie bei Balanced-Viscosity-L\"osungen \cite{MielkeRossiSavare2016}.
...
\begin{remark}[Recovery via Young-Maß-Relaxation]\label{rem:young-measure}
...
\begin{enumerate}
\item \textbf{Young-Maß-Relaxation:} Man erlaubt mikroskopisch ,,chatternde'' Selektionen (schnelle Oszillationen zwischen Attraktorpunkten) und arbeitet auf der relaxierten Hülle $\mathcal{Y}(\mathcal{A}_T)$ maßwertiger Trajektorien. Der $\Gamma$-Grenzwert existiert auf $\mathcal{Y}(\mathcal{A}_T)$, und ein anschließendes \emph{Selektionstheorem} (z.\,B.\ Artsteins Theorem \cite{Artstein1995}) extrahiert unter zusätzlicher Regularität eine einwertige Grenzwert-Selektion.
...
\end{enumerate}
Für den NS-Trajektorienattraktor ist Ansatz~(1) natürlicher, da Trajektorienattraktoren eine natürliche Wahrscheinlichkeitsstruktur tragen (statistische Lösungen / Vishik--Fursikov-Maße), die das benötigte Young-Maß-Framework bereitstellt.
\end{remark}
...
\begin{remark}[Reduzierte Abhängigkeit von punktweiser geometrischer Regularit\"at]\label{rem:no-reach-needed}
Die Route setzt positiven Reach oder eine Inertialmannigfaltigkeit nicht als primitive Annahme voraus, ist aber nicht geometriefrei. Die Schranke benötigt weiterhin:
\begin{itemize}
...
\item die $O(\varepsilon)$-Trajektorienapproximierbarkeit sowie die Recovery-/Selektionsinputs aus den Schritten~2 und~5.
\end{itemize}
Positive Reach von $A$, eine Spektrallückenbedingung oder eine Inertialmannigfaltigkeit werden damit im engen Sinn der punktweisen Projektion nicht benötigt. Die No-Go-Zone wird aber nicht vollständig umgangen, sondern durch die konkreten EDP-/DS3-/Recovery-Gates oben ersetzt.
\end{remark}
...
\label{rem:bv-lorenz}
Die Balanced-Viscosity-Selektion $v_\varepsilon$ aus Definition~\ref{def:bv-selection} wurde auf dem Lorenz-Attraktor ($\sigma=10$, $\rho=28$, $\beta=8/3$) mittels zeitgemittelter Moreau--Yosida-Projektion getestet. Vier Diagnostiken stützen die theoretischen Vorhersagen: (i)~Simon-BV-Kompaktheit: $\mathrm{TV}(v_\varepsilon)$ bleibt beschränkt für $\varepsilon \to 0$ (Variation $< 7\%$ für Fenster $W=50$); (ii)~Balanced-Viscosity-Konvergenz: sukzessive Differenzen $\|v_{\varepsilon_1} - v_{\varepsilon_2}\|_{L^2}$ bilden eine Cauchy-Folge ($0{,}171, 0{,}171, 0{,}034, 0{,}004$); (iii)~Aubin--Lions-Regularität: $\|v_\varepsilon\|_{L^2}$ und $\|v_\varepsilon'\|_{L^2}$ beschränkt; (iv)~bounded-dissipation-Sanity-Check: das endlichdimensionale Dissipationsintegral ist endlich ($\approx 7 \times 10^5$). Dies ist keine Verifikation von Bedingung~D für Navier-Stokes, denn Bedingung~D ist eine Dominanzbedingung gegen Gronwall-/Bihari-Kosten nahe Blow-up. Skript: \texttt{compute\_bv\_selection.py}.
\end{remark}

\begin{remark}[Multi-Attraktor-Validierung]
\label{rem:multi-attractor}
Die Balanced-Viscosity-Selektion wurde auf drei chaotischen Attraktoren getestet: Lorenz ($\sigma=10$, $\rho=28$), R\"ossler ($a=0{,}2$, $c=5{,}7$) und Chen ($a=35$, $c=28$). Alle drei bestehen den Simon-BV-Beschränktheitstest ($\mathrm{TV}(v_\varepsilon)$ beschränkt für $\varepsilon \to 0$), zeigen Cauchy-Konvergenz in $L^2$ (geometrischer Abfall sukzessiver Differenzen) und besitzen endliche bounded-dissipation-Diagnostikintegrale. Diese endlichdimensionalen Diagnostiken stützen den BV-Selektionsmechanismus über verschiedene Attraktor-Topologien hinweg; sie belegen nicht Bedingung~D für Navier-Stokes. Der R\"ossler-Attraktor zeigt die schnellste Konvergenz (Differenzen $0{,}057 \to 0{,}001$), während der Chen-Attraktor die höchste diagnostische Dissipation aufweist ($\approx 1{,}4 \times 10^6$), aber dennoch konvergiert. Skript: \texttt{compute\_bv\_multi\_attractor.py}.
\end{remark}
...
(Proposition~\ref{prop:attractor-regularity}) garantiert nur die dort
explizit geforderten $H^2$- und $W^{1,\infty}$-Schranken. Diese reichen
für die hier verwendeten räumlichen Abschätzungen; höhere $H^k$-Kontrolle
wäre eine zusätzliche Verstärkung des Pakets.
...
warum die $H^1$-Ebene ein plausiblerer Ort für einen Widerspruch ist,
obwohl weiterhin die unten formulierte Bihari-Dominanzbedingung benötigt wird.
...
\end{equation}
Unter Blow-up liefert das Fortsetzungskriterium
\[
  \limsup_{T\nearrow T^*}\|\nabla u(T)\|^2=\infty.
\]
Die linke Seite ist daher entlang einer Folge $T_n\nearrow T^*$ unbeschränkt.
Da der nichtlineare Term
...
divergieren; falls diese Divergenz zusätzlich die Bihari-/Projektionskosten
dominiert, wird das $H^{(1)}$-Funktional durch dasselbe Budget-Argument wie in
Satz~\ref{thm:main} unter Null getrieben, aber nun mit einem
\emph{tatsächlich divergenten} Dissipationsterm.
...
Nicht-Eindeutigkeits-Szenarien, insbesondere Taos gemittelter Blow-up~\cite{Tao2016}
und die Buckmaster--Vicol-Konstruktion nicht-eindeutiger schwacher L\"osungen~\cite{BuckmasterVicol2019}, motivieren einen Katalog schlechter
...
Bounded-dissipation-Diagnostik (nicht Bedingung D) & 3/3 bestanden \\
\hline
...
(Bemerkung~\ref{rem:divergence-landscape}). Erzwungen wird
$\limsup_{t\nearrow T^*}\|\nabla u(t)\|_{L^2}^2=\infty$,
nicht Divergenz des Zeitintegrals. Ob diese limsup-Unbeschränktheit schnell
...
Trajektorien-Ebene-Regularität (TLL) kombiniert mit Log-Abstand-Integrierbarkeit (LDI) reduziert. Der zweite Pfad nimmt keine inertiale Mannigfaltigkeit als primitiven Input, sondern ersetzt diese Barriere durch das TLL/LDI-Verifikationsproblem.
