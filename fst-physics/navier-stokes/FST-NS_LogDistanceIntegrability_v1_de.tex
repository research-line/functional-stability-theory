...
\paragraph{Hauptbeitraege.}
Diese Arbeit leistet drei Beitraege: (i)~Wir fuehren die \emph{trajektorienweise log-Lipschitz}-Bedingung (TLL) und die \emph{Log-Distanz-Integrierbarkeit} (LDI) als die natuerliche Regularitaetsskala fuer BV-Projektionen auf fraktale Attraktoren ein, streng zwischen Lipschitz (zu stark) und H\"older (zu schwach). (ii)~Wir beweisen eine BV-Kettenregel (Theorem~\ref{thm:BV-chain}), die zeigt, dass TLL\,+\,LDI fuer beschraenkte-Variations-Selektionsregularitaet genuegt. (iii)~Wir identifizieren hinreichende geometrische Bedingungen (beschraenkte Variation der Log-Distanz, starke tangentiale Kegelbedingung), die mit der fraktalen Geometrie dissipativer PDE-Attraktoren vertraeglich sind.

\begin{remark}[Input-Output-Vertrag]
\label{rem:api-contract}
Das zentrale Ergebnis dieser Arbeit laesst sich als Input-Output-Vertrag formulieren:
\begin{description}
\item[Input:] Eine absolut stetige Trajektorie $u \in AC([0,T]; H)$, ein kompakter Attraktor $\mathcal{A} \subset H$ und die Naechste-Punkt-Projektion $P : \mathcal{A}^R \to \mathcal{A}$.
\item[Bedingungen:] (TLL) Die Projektion $P$ ist trajektorienweise log-Lipschitz entlang $u$; (LDI) das Log-Distanz-Gewicht $\log(R/d(t))$ ist $\|u'\|$-integrierbar.
\item[Output:] Die zusammengesetzte Selektion $v(t) := P(u(t))$ gehoert zu $BV([0,T]; H)$ mit der expliziten Schranke $\mathrm{Var}(v) \leq C_{\mathrm{TLL}} \int_0^T \|u'(t)\| \,\Omega(t)\,dt$.
\end{description}
Dieser Vertrag ist so konzipiert, dass er direkt mit dem Begleitpaper~\cite{FST-NS2026} zusammenwirkt: Der Output (BV-Selektion) ist praezise Annahme~$G'$ dieses Papers.

\smallskip\noindent
\textbf{Assumption~$G'$} (aus~\cite{FST-NS2026}):
\emph{Es existiert eine Naechstpunkt-Selektion
$v(t) = P(u(t)) \in \mathcal{A}$, sodass
$v \in BV([0,T];\,H)$ fuer jedes $T < T^*$.}

\smallskip\noindent
Kombiniert mit Condition~D aus~\cite{FST-NS2026} (einer Energieungleichung fuer den Attraktorabstand) impliziert Assumption~$G'$ globale Regularitaet fuer die 3D-Navier--Stokes-Gleichungen. Der Attraktor $\mathcal{A}$ darf fraktale Geometrie besitzen -- TLL und LDI sind mit nicht-ganzzahliger Hausdorff-Dimension vertraeglich.
\end{remark}
...
\begin{remark}[Eindeutigkeit fuer nichtkonvexe Mengen]\label{rem:uniqueness-caveat}
Das Mittelpunktargument im Beweis erfordert, dass $\frac{1}{2}(a_1 + a_2)$ naeher an $u(t)$ liegt als sowohl $a_1$ als auch $a_2$, was aus der strikten Konvexitaet der Norm folgt. Fuer nichtkonvexe $\mathcal{A}$ muss der Mittelpunkt jedoch nicht in $\mathcal{A}$ liegen. Die Eindeutigkeit der minimalen metrischen Ableitungsselektion gilt daher \emph{generisch} (fuer f.ue.~$t$, sodass der naechste Punkt in $\mathcal{A}$ eindeutig ist), kann aber auf einer Nullmenge versagen, auf der $\mathcal{A}$ mehrere aequidistante Punkte besitzt. Fuer Attraktoren dissipativer PDEs hat die Menge der mehrdeutigen Zeitpunkte nach Federers Theorem ueber die Singularitaeten von Distanzfunktionen das Mass Null.
\end{remark}
...
\begin{remark}[STC als realistischer Kandidat fuer Navier--Stokes]
\label{rem:stc-realistic}
Von den drei hinreichenden Bedingungen fuer LDI (Variante~A: BV-Log-Distanz; Variante~B: Sublevel-Zeitkontrolle; exponentielles Tracking) ist die \emph{Sublevel-Zeitkontrolle} (STC) der realistischste Kandidat fuer 3D-Navier--Stokes:
\begin{enumerate}
\item \emph{Exponentielles Tracking} ($d(t) \leq C_0 e^{-\lambda t}$) ist das ,,einfache Regime`` -- gueltig fern von Blow-up, wo der Attraktor exponentiell anziehend ist. Nahe potentieller Singularitaeten koennen die Tracking-Raten degenerieren, was exponentielles Tracking fragil macht.
\item \emph{STC} ($|\{d(t) \leq \varepsilon\}| \leq C_* \varepsilon^\alpha$) ist robust: Es erfordert lediglich, dass die Trajektorie auf keiner Skala zu lange nahe dem Attraktor verweilt. Zwei PDE-Mechanismen machen STC fuer Navier--Stokes plausibel:
\begin{itemize}
\item \emph{Dissipative Zeitskalen-Separation:} Nahe dem Attraktor ist die Niedermoddynamik langsam (kontrolliert durch den Attraktor), waehrend die Hochmoddynamik schnell ist (viskose Daempfung). Diese Separation begrenzt die Verweilzeit auf jeder Distanzskala.
...
\end{itemize}
\item \emph{BV-Log-Distanz} (Variante~A) erfordert monotone Annaeherung an den Attraktor, was generisch durch oszillierende Loesungen verletzt wird.
\end{enumerate}
\end{remark}
...
\begin{itemize}
  ...
  \item \textbf{BV-Schranke haengt von $\|u'\|$ ab.}\;
    Die Schranke \eqref{eq:BV-bound} lautet
    $\Var(v) \le C_{\mathrm{TLL}} \int_0^T \|u'\|\,\Omega\,dt$.
    Fuer Navier--Stokes haengt $\|u'\|$ von der Loesungsnorm ab,
    ...
    Die Details dieser Absorption haengen vom
    Begleitpaper~\cite{FST-NS2026} ab und liegen ausserhalb des
...
\begin{quote}
\textbf{Selektionsregularitaetsproblem.}
Sei $u(t)$ eine Leray--Hopf-Loesung der 3D-Navier--Stokes-Gleichungen und $\mathcal{A}$ der globale Attraktor. Erfuellt die Naechstpunkt-Projektion $P_\mathcal{A}$ die TLL-Bedingung entlang $u(t)$? Aequivalent: Erlaubt die Attraktorgeometrie BV-regulaere Selektionen?

Eine bejahende Antwort, kombiniert mit der BV-Kettenregel dieser Arbeit, wuerde Assumption~G der Begleitarbeit \cite{FST-NS2026} und damit globale Regularitaet liefern.
\end{quote}
...
% ==================================================================
\section{Zeitgemittelte Selektion und BV-Regularitaet (spekulativ)}
...
Abschnitte~\ref{sec:main}--\ref{sec:applications} haengen nicht von
diesem Material ab.}
...
Energiedissipationsschranke) wird erwartet, dass die regularisierte
Selektion~$v_\varepsilon$ folgende Eigenschaften erfuellt:
\begin{enumerate}[label=(\roman*)]
\item \emph{Eindeutigkeit:}  $v_\varepsilon(t)$ ist f.ue.\ eindeutig
  (strikte Konvexitaet des zeitintegrierten Kostenfunktionals auf~$H$;
  fuer die Minimierung ueber das nicht-konvexe Kompaktum~$\mathcal{A}$
  erfordert Eindeutigkeit ein separates Argument ueber
  Hausdorff-Stabilitaet der Minimierer-Menge, das offen bleibt).
\item \emph{BV-Schranke:}
  $\Var(v_\varepsilon;\,[0,T])
   ...
   \;<\; \infty$,
  wobei $C_\varepsilon = T/(2\varepsilon) \to \infty$ fuer
  $\varepsilon \to 0$. Eine \emph{uniforme} BV-Schranke (unabhaengig
  von~$\varepsilon$) wuerde zusaetzlichen geometrischen Input erfordern;
  siehe Bemerkung~\ref{rem:epsilon-limit}.
\item \emph{TLL-Vererbung:}  Falls~$u$ TLL mit Konstante
  $C_{\mathrm{TLL}}$ erfuellt, wird erwartet, dass $v_\varepsilon$ TLL mit
  Konstante $C_{\mathrm{TLL}} + O(\varepsilon)$ erfuellt. Ein rigoroser
...
\item \emph{Gronwall-Kompatibilitaet:}  Die Gronwall-Abschaetzungen
  des Hauptresultats (Satz~\ref{thm:BV-chain}) bleiben voraussichtlich fuer
  ...
  der Konstanten $\alpha, \beta, \gamma$; dies wird auf zukuenftige Arbeit verschoben.
\end{enumerate}
\end{conjecture}

\begin{proof}[Partielle Begruendung]
\emph{(i)}\; Die Abbildung
$a \mapsto \int_{t-\varepsilon}^{t+\varepsilon} \|u(s)-a\|^2\,ds$
ist strikt konvex auf~$H$. Die Minimierung erfolgt jedoch ueber das
kompakte, im Allgemeinen \emph{nicht-konvexe} $\mathcal{A}$.
Eindeutigkeit des Minimierers ueber einer nicht-konvexen Menge erfordert
ein separates Argument (z.B.\ ueber Hausdorff-Stabilitaet der
Minimierer-Menge oder ein generisches Transversalitaetsargument).
Dieser Schritt bleibt fuer allgemeine Attraktoren offen; er gilt,
...
% ==================================================================
\section{Post-Zookeeper-Transfer: Vom Proof of Life zu gedaempften 3D-Navier--Stokes}
\label{sec:zookeeper-ldi-transfer}

Die Zookeeper-Normalform macht dieses Paper zum natuerlichen Testtraeger
des Navier--Stokes-Asts.  Hier sind Defekt, Gap und Selektionsmechanismus
bereits explizit:

In der aktualisierten CORE-Lesart heisst das: RFEP v1.8 importiert eine
Normalform-Checkliste aus dem Zookeeper-Abschluss von Riemann
$C^{(2)}$/MS2.  Das ist noch kein Beweis der dreidimensionalen
Navier--Stokes-Regularitaet; der Transfer von gedaempften zu klassischen
...
\begin{description}
  ...
  Reaktions-Diffusion $\to$ gedaempfte 3D-Navier--Stokes $\to$
  klassische 3D-Navier--Stokes.
  \item[Gap.] TLL, Squeezing, exponentielles Tracking und Damping-Gaps.
  \item[Grenzuebergang.] Von endlichdimensionalen Attraktoren zu
  PDE-Attraktoren und von gedaempften/hyperviskosen Systemen zur
  klassischen Gleichung mit $\beta=1$.
\end{description}

Das unmittelbare naechste Ziel ist daher nicht das volle
Millenniumsproblem.  Es ist ein Transfersatz fuer gedaempfte
...
Regularitaet liefert, soll Daempfung plus Attraktorregularitaet
TLL/LDI und damit BV-Selektion implizieren.  Die bestehenden Lorenz- und
Kuramoto--Sivashinsky-Tests werden dadurch zu den ersten zwei Stufen einer
groesseren Leiter, nicht zu isolierter numerischer Evidenz.
...
\emph{Ergebnisse} (alle fuenf Diagnosen bestanden):
...
Ein systematischer Parameterscan ueber $\rho$ (einschliesslich
Near-Onset-Werten $\rho \approx 24$) wuerde die Evidenz
verstaerken. Trotz dieser Vorbehalte bestaetigt der Test, dass die
TLL+LDI-Bedingungen nicht leer sind: Sie sind in mindestens einem
konkreten chaotischen dynamischen System erfuellt.
\end{remark}
...
\emph{Ergebnisse} (primaerer Lauf, $N = 128$):
...
Wesentliche Beobachtungen: (i)~Alle vier Diagnosen bestehen bei jeder Aufloesung -- das
TLL/LDI-Framework ist \emph{kein} Artefakt endlicher Diskretisierung.
(ii)~Das BV-Verhaeltnis ist gleichmaessig klein ($< 0{,}15$), was zeigt, dass die
...
Die Attraktordimension ($\sim 8$--$12$ fuer $L = 32\pi$) liegt deutlich ueber
dem Lorenz-Wert ($\sim 2{,}06$), was dies zu einem wesentlich haerteren Test macht.
(iv)~Exponentielles Tracking schlaegt fehl (erwartet: Die Testtrajektorie wandert
...
\begin{theorem}[Squeezing impliziert TLL fuer endlich-dimensionale Attraktoren]\label{thm:squeezing-tll}
...
$\theta \in (0,1)$ und $T > 0$, sodass fuer alle $u_0, v_0$ in der
...
Dann gilt die Trajektorien-Log-Lipschitz-Bedingung (TLL, Definition~\ref{def:TLL})
mit
\[
...
in einem Lipschitz-Graphen ueber dem endlich-dimensionalen Unterraum $P_N H$ liegt.
...
kontrahiert, ist die Einschraenkung $P_N|_{\mathcal{A}}$ injektiv mit
Lipschitz-Inverser \cite{FoiasTemam1979,Mane1981}.
Konkret existiert $L = L(N, d_F) < \infty$, sodass
$\|Q_N(u - v)\| \leq L\,\|P_N(u - v)\|$ fuer alle $u, v \in \mathcal{A}$.

\emph{Schritt~2 (Naechstpunkt-Projektion ist lokal Lipschitz auf dem Graphen).}
Es muss gezeigt werden, dass die \emph{Naechstpunkt-Projektion} $\pi_{\mathcal{A}}$
(nicht die lineare Projektion $P_N$) lokal Lipschitz in einer Roehrenumgebung
des Lipschitz-Graphen $\mathcal{A} = \mathrm{graph}(\Phi)$ ist,
wobei $\Phi: P_N H \to Q_N H$ mit $\mathrm{Lip}(\Phi) \leq L$.

Ein Lipschitz-Graph mit Konstante $L$ ist \emph{prox-regulaer} im Sinne
von Poliquin--Rockafellar--Thibault~\cite{PoliquinRockafellarThibault2000}:
Die Reichweite erfuellt $\mathrm{reach}(\mathrm{graph}(\Phi)) \geq r_0 > 0$
(abhaengig von $L$ und der Kruemmung der Einbettung), und auf der
$r_0$-Roehrenumgebung ist die Naechstpunkt-Projektion
$\pi_{\mathcal{A}}$ einwertig und Lipschitz. Genauer gilt
fuer beliebige $x, y$ mit $\dist(x, \mathcal{A}) < r_0$ und
...
\[
  ...
  \,\|x - y\|.
\]
Fuer Punkte $u(t)$ auf der Trajektorie mit $d(t) = \dist(u(t), \mathcal{A})$
bleibt der Nenner von Null weg beschraenkt, solange $d(t) < r_0/2$
(was schliesslich durch die absorbierende-Kugel-Eigenschaft gilt). Dies ergibt
$\|\pi_{\mathcal{A}}(u(t)) - \pi_{\mathcal{A}}(u(t+h))\|
\leq 2\sqrt{1+L^2}\,\|u(t+h) - u(t)\|$
fuer $u(t), u(t+h)$ in der $(r_0/2)$-Roehre.

\emph{Schritt~3 (Log-Faktor aus exponentiellem Approach).}
Fuer Trajektorien $u(t)$ in der absorbierenden Kugel, die noch nicht auf $\mathcal{A}$ liegen,
liefert die Squeezing-Eigenschaft exponentiellen Approach:
$d(u(t), \mathcal{A}) \leq C\,\theta^{t/T}\,d(u(0), \mathcal{A})$.
Logarithmierung ergibt $\log(R/d(u(t), \mathcal{A})) \geq (t/T)\,|\log\theta| - C'$.
Der TLL-Modul $\Omega(t) = 1 + \log(R/d(t))$ waechst daher hoechstens linear
in $t$, waehrend die exponentielle Kontraktion in den hohen Moden den
kompensierenden Abfall liefert. Kombination mit den Schritten~1--2 und den
exponentiellen Tracking-Abschaetzungen von Eden--Foias--Nicolaenko--Temam \cite{EdenFoiasNicolaenkoTemam1994}
ergibt die Schranke $C_{\mathrm{TLL}} \leq C(N, d_F, \theta)\,(1 + |\log\theta|)$.
\end{proof}

\begin{remark}[Hyperviskose Navier--Stokes als vollstaendiger TLL/LDI-Testfall]
\label{rem:hyperviscous}
Fuer die hyperviskose NS-Gleichung $\partial_t u + (-\Delta)^\beta u + (u\cdot\nabla)u = -\nabla p + f$
mit $\beta \geq 5/4$ auf $\mathbb{T}^3$ existieren Inertialmannigfaltigkeiten
(Mallet-Paret--Sell~\cite{MalletParetSell1988}, Romanov~\cite{Romanov2000}). Die Squeezing-Eigenschaft gilt
mit expliziten Konstanten, sodass Theorem~\ref{thm:squeezing-tll} direkt anwendbar ist:
TLL gilt mit $C_{\mathrm{TLL}} \leq C(\beta, \nu, f)$.
Kombiniert mit LDI (das aus dem exponentiellen Tracking auf der
Inertialmannigfaltigkeit folgt) ergibt dies einen \emph{vollstaendigen geschlossenen Zyklus}:
\[
  \text{IM existiert} \;\Rightarrow\; \text{Squeezing} \;\Rightarrow\; \text{TLL}
  \;\Rightarrow\; \text{TLL + LDI} \;\Rightarrow\; \text{BV-Selektion (G')}
  \;\Rightarrow\; \text{Regularitaet (Thm~3.1 des Begleitpapiers)}
\]
Dies ist das erste System, in dem die gesamte TLL/LDI $\to$ BV $\to$ Regularitaets-Kette
ohne offene Annahmen schliesst.
Fuer klassische NS ($\beta = 1$) gilt die Squeezing-Eigenschaft, aber
die Existenz einer Inertialmannigfaltigkeit ist offen -- dies ist genau die Luecke,
die der $\mu$-Reach-Ansatz (Begleitpapier) adressiert.
\end{remark}
