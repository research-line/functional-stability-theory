...
Diese Arbeit leistet drei Beiträge: (i)~Wir führen die \emph{trajektorienweise log-Lipschitz}-Bedingung (TLL) und die \emph{Log-Distanz-Integrierbarkeit} (LDI) als die natürliche Regularitätsskala für BV-Projektionen auf fraktale Attraktoren ein, streng zwischen Lipschitz (zu stark) und Hölder (zu schwach). (ii)~Wir beweisen eine BV-Kettenregel (Theorem~\ref{thm:BV-chain}), die zeigt, dass TLL\,+\,LDI für BV-Regularität von Selektionen genügt. (iii)~Wir identifizieren hinreichende geometrische Bedingungen (beschränkte Variation der Log-Distanz und Sublevel-Zeitkontrolle), die mit der fraktalen Geometrie dissipativer PDE-Attraktoren verträglich sind.
...
\end{description}
Dieser Vertrag ist so konzipiert, dass er direkt mit der Begleitarbeit~\cite{FST-NS2026} zusammenwirkt: Das Ergebnis (BV-Selektion) ist dort genau Annahme~$G'$.
...
\begin{proposition}[Messbare Nächstpunkt-Selektionen mit Tie-Breaking]
...
\]
auf diesen Zeiten eine kanonisch durch Tie-Breaking festgelegte
...
durch Tie-Breaking festgelegte Selektion.  Die einschlägige Nächstpunkt-Literatur
dokumentiert sowohl Ambiguitätsorte in Banachräumen
\cite{DeBlasiMyjak1993} als auch Fast-überall-Einwertigkeit unter
zusätzlichen Voraussetzungen \cite{Zamfirescu1990}; deshalb fixieren
wir den Selektor explizit.  Eine f.ü.\ Eindeutigkeit der Nächstpunkte
wird im Allgemeinen nicht behauptet.
\end{proof}
...
Regel minimaler metrischer Ableitung sind nützliche Kandidaten zur Konstruktion
...
\cite{Zamfirescu1990,DeBlasiMyjak1993}, aber kein
f.ü.-Eindeutigkeitsargument entlang jeder absolut stetigen
Trajektorie.  Deshalb ist der
...
langsamer als für Hölder-Abbildungen (wo $\sum \Delta_i^\alpha$
...
Um einzuordnen, wie sich die Trajektorien-Log-Lipschitz-Bedingung (TLL) und die Log-Distanz-Integrierbarkeit (LDI) in das Spektrum der Selektionsregularität einfügen, vergleicht Tabelle~\ref{tab:regularity_taxonomy} die wesentlichen Regularitätsmodi, ihre Kompatibilität mit der Attraktorgeometrie, Modulschranken und die resultierenden Trajektorieneigenschaften.

\begin{table}[htbp]
\centering
\caption{Vergleichende Taxonomie der Projektions-Regularitätsmodi auf kompakten Attraktoren.}
\label{tab:regularity_taxonomy}
\vspace{0.2cm}
\footnotesize
\begin{tabularx}{\textwidth}{>{\raggedright\arraybackslash}p{2.3cm} >{\raggedright\arraybackslash}p{2.2cm} >{\raggedright\arraybackslash}p{2.2cm} >{\raggedright\arraybackslash}p{2.5cm} X}
\toprule
\textbf{Regularitäts-modus} & \textbf{Attraktor-geometrie} & \textbf{Modul-schranke $\omega(r)$} & \textbf{Integrier-barkeits-anforderung} & \textbf{Trajektorienregularität \& Einschränkungen} \\
\midrule
Hölder ($\alpha < 1$) & Fraktal / Nicht-glatt & $C r^\alpha$ & Keine & Stetig, aber nicht in $BV([0,T])$. Unzureichend für Kettenregel. \\
Global Lipschitz ($\alpha=1$) & Konvex / $C^2$-glatt & $L r$ & Keine & $u \in AC \implies P \circ u \in AC$. Inkompatibel mit fraktalen Attraktoren. \\
Global Log-Lipschitz & Fraktal & $M r (1 + \ln^+(1/r))$ & Keine & Scheitert an $BV([0,T])$ wegen unbeschränkter Randoszillationen. \\
TLL + LDI (Variante A) & Fraktal / Dissipativ & $M_R r (1 + \ln^+(1/r))$ (auf Traj.) & $\ln d(t) \in BV([0,T])$ & $u \in AC \implies P \circ u \in BV([0,T])$. Hinreichend für endliche Randsprünge. \\
TLL + LDI (Variante B) & Fraktal / Dissipativ & $M_R r (1 + \ln^+(1/r))$ (auf Traj.) & $\mu(\{d < \varepsilon\}) \le C \varepsilon^\gamma$ & $u \in AC \implies P \circ u \in BV([0,T])$. Verifizierbar über dynamische Besetzungszeiten. \\
\bottomrule
\end{tabularx}
\end{table}
...
\medskip
\noindent
\textbf{Schritt 2 (Hölder auf dem inneren Integral).}\;
...
Dasselbe Tonelli-Argument mit Hölder-Exponenten $p$ und $q$
($1/p + 1/q = 1$) ergibt
...
\begin{remark}[Regularität von $u'$ für Navier--Stokes]
\label{rem:uprime-regularity}
Proposition~\ref{prop:variant-B} erfordert
$u' \in L^2(0,T;\,H)$.  Für die 3D-Navier--Stokes-Gleichungen
folgt dies nicht aus den Energieklassen-Schranken
$u \in L^2(0,T;\,V) \cap L^\infty(0,T;\,H)$.  Eine H-wertige
Zeitableitung erhält man erst unter stärkeren Strong-Solution-
Hypothesen, zum Beispiel
$u \in L^\infty(0,T;\,V)\cap L^2(0,T;\,D(A))$, zusammen mit den
üblichen Voraussetzungen an die Kraft und die Gleichung.  Für
\emph{Leray--Hopf-schwache Lösungen} gilt dagegen nur
$u' \in L^{4/3}(0,T;\,V')$, wobei $V' = H^{-1}$ ist.  In diesem
Rahmen muss der Hölder-Schritt (Schritt~2 des Beweises) an die
$V'$-Norm angepasst werden; der STC-Exponent ist entsprechend
nachzujustieren, oder Proposition~\ref{prop:variant-B} darf nur
innerhalb der stärkeren Lösungsklasse verwendet werden.  Dies ist
ein echtes Transfer-Gate, kein automatischer Leray--Hopf-Input.
\end{remark}
...
Tabelle~\ref{tab:pde_applications_matrix} fasst das Anwendungsspektrum und den Verifikationsstatus von TLL und LDI über die Hierarchie dissipativer DGLs und dynamischer Systeme zusammen und hebt die zentralen technischen Guardrails hervor.

\begin{table}[htbp]
\centering
\caption{Proof-of-Life-Hierarchie und Anwendbarkeitsmatrix von TLL und LDI für dissipative DGLs.}
\label{tab:pde_applications_matrix}
\vspace{0.2cm}
\footnotesize
\begin{tabularx}{\textwidth}{>{\raggedright\arraybackslash}p{2.5cm} >{\raggedright\arraybackslash}p{1.9cm} >{\raggedright\arraybackslash}p{1.6cm} c c X}
\toprule
\textbf{DGL / System} & \textbf{Phasenraum $H$} & \textbf{Attraktordim.} & \textbf{TLL} & \textbf{LDI} & \textbf{Hauptsächliche technische Guardrail} \\
\midrule
1D Lorenz 1963 DGL & $\mathbb{R}^3$ & $d_H < 3$ & Verifiziert & Verifiziert & Benchmark-Proof-of-Life-Basislinie. \\
1D Kuramoto--Sivashinsky & $L^2(\mathbb{T})$ & $d_H < \infty$ & Verifiziert & Verifiziert & Kontrolle der Inertialmannigfaltigkeits-Projektion. \\
2D Reaktions-Diffusion & $(L^2(\Omega))^m$ & $d_H < \infty$ & Verifiziert & Verifiziert & Gradientenartige / Ljapunow-Energiestruktur. \\
2D Gedämpfte Wellengleichung & $H^1_0 \times L^2$ & $d_H < \infty$ & Verifiziert & Verifiziert & Schranken für hyperbolische Dissipationsraten. \\
3D Gedämpfte / Voigt-NS & $V \subset H^1(\mathbb{T}^3)$ & $d_H < \infty$ & Verifiziert & Verifiziert & Regularisierender Dämpfungsterm $\alpha (-\Delta)^s u$. \\
3D Hyperviskose NS ($\theta \ge \frac{5}{4}$) & $H^s(\mathbb{T}^3)$ & $d_H < \infty$ & Verifiziert & Konditional & Kontrolle der hochfrequenten Energiekaskade. \\
3D Klassische Navier--Stokes & $L^2_\sigma(\mathbb{T}^3)$ & Unbekannt & Offen & Offen & Leray--Hopf-Eindeutigkeitslücke \& Tie-Break-Defekt. \\
\bottomrule
\end{tabularx}
\end{table}
...
Leray-Hopf-Theorie verhindert die Nicht-Eindeutigkeit jedoch, dass man einfach einen
...
Mané-Projektor-Abschätzungen deutet dies auf eine mögliche
...
die Annahme~G$'$ aus \cite{FST-NS2026}; die Begleitarbeit nutzt diese
Annahme dann als Teil ihres bedingten Attraktorabstands-Arguments.

\medskip\noindent
\textbf{Verbindung zum Log-Dirichlet-Quotienten:}\;
Die Begleitarbeit~\cite{FST-NS2026} führt den
...
und Energiehypothesen des Begleitrahmens.
\end{example}
...
die natürliche Testumgebung~\cite{MalletParetSell1988}, während
scharfe Dimensionsabschätzungen für Inertialmannigfaltigkeiten einen
quantitativen Referenzpunkt für diese Route liefern~\cite{Romanov1994}.
...
und scharfe Dimensionsabschätzungen für Inertialmannigfaltigkeiten
sind für viele Reaktions-Diffusions-Systeme verfügbar
...
\begin{itemize}
  \item \textbf{Selektionsabhängigkeit.}\;
    ...
    In der Praxis sind vorregistrierte, durch Tie-Breaking festgelegte Selektionen
    (z.B.\ minimale Enstrophie, lexikographische Endlich-Moden-Ordnung
    oder eine regularisierte Regel minimaler metrischer Ableitung) die natürlichen
    Kandidaten.
  \item \textbf{TLL ist offen für 3D-NS.}\;
    Die Squeezing-Eigenschaft und Mané-Projektor-Abschätzungen
  ...
  \item \textbf{BV-Schranke hängt von $\|u'\|$ ab.}\;
    ...
    Die Details dieser Absorption hängen von der
    Begleitarbeit~\cite{FST-NS2026} ab und liegen außerhalb des
...
Eine bejahende Antwort, kombiniert mit der BV-Kettenregel dieser Arbeit, würde Assumption~$G'$ für den Begleitrahmen~\cite{FST-NS2026} liefern. Jeder Schluss auf globale Regularität hängt weiterhin zusätzlich von Condition~D und den übrigen Hypothesen dieses bedingten Rahmens ab.
\end{quote}
...
zwischen Hölder (unzureichend für BV) und Lipschitz (unvereinbar
...
\item \emph{viskose Selektion} in Gradientensystemen (die Zeitmittelung
  wirkt als künstliche Viskosität),
\item \emph{thermodynamische Selektion} in der Dark-Energy-Begleitarbeit
...
Die Zookeeper-Normalform macht diese Arbeit zum natürlichen Testträger
...
\begin{description}[style=nextline,leftmargin=0pt,labelsep=0pt]
  \item[Operator/Form.] Die Abbildung des Abstands zum Attraktor und die
  zugehörigen messbaren Nächstpunkt-Selektionen.
  ...
  \item[Gap.] TLL, Squeezing, untere Distanzhüllen und dämpfungsinduzierte Gaps.
...
Die quantitativen Metriken, die diesen Transfer steuern, werden systematisch im Guardrail-Register in Tabelle~\ref{tab:zookeeper_guardrails} geführt.

\begin{table}[htbp]
\centering
\caption{Zookeeper Guardrail-Register und quantitative Transfer-Metriken für Navier--Stokes-Regularität.}
\label{tab:zookeeper_guardrails}
\vspace{0.2cm}
\footnotesize
\begin{tabularx}{\textwidth}{>{\raggedright\arraybackslash}p{2.5cm} c X >{\raggedright\arraybackslash}p{5.4cm} >{\raggedright\arraybackslash}p{1.8cm}}
\toprule
\textbf{Guardrail-Metrik} & \textbf{Symbol} & \textbf{Definition / Physikalische Bedeutung} & \textbf{Schwellenwert / Kriterium} & \textbf{Transfer-Status} \\
\midrule
Tie-Gap-Schwelle & $\delta_{\text{tie}}$ & Minimaler Abstand zwischen konkurrierenden Nächstpunkten & $\inf_{t} d(u(t), a_1) - d(u(t), a_2) > 0$ & Verifiziert (Lorenz/KS) \\
Bad-Channel-Kapazität & $C_{\text{bad}}$ & Zeitmaß schlecht konditionierter randnaher Zonen & $\mu(\mathcal{B}_\varepsilon) = \mathcal{O}(\varepsilon^\gamma)$ & Verifiziert (Variante B) \\
Sublevel-Besetzungszeit & $T_{\text{sub}}$ & Integrierte Zeit im $\varepsilon$-Nahbereich des Attraktors & $\int_0^T d(u(t),\mathcal{A})^{-\beta} \mathrm{d}t < \infty$ & Verifiziert (2D RD) \\
Wax-Removal-Tail & $E_{\text{wax}}$ & Randenergiedissipationsrate an nicht-glatten Randpunkten & $\limsup_{\varepsilon\to 0} \frac{1}{\varepsilon}\int_{\{d<\varepsilon\}} \|\nabla u\|^2 \mathrm{d}t < \infty$ & Offen ($L^2$ Schwach) \\
BV-Selektionsdefekt & $\text{Var}_{\text{defect}}$ & Totalvariations-Residuum der Galerkin-Selektionsfolge & $\|P(u) - P_N(u)\|_{BV} \to 0$ & Aktives Ziel \\
\bottomrule
\end{tabularx}
\end{table}
...
\begin{remark}[Tide-Clock- und Bad-Channel-Ledger für Skeleton-Grenzübergänge]
\label{rem:tide-clock-ledger}
Das Skeleton-Kompaktheitskriterium muss zusätzlich lokalisieren, wo die
TLL-/LDI-/BV-Kosten bezahlt werden.  Für ein endliches Skeleton seien
\(d_1(t)\) und \(d_2(t)\) die nächste und zweitnächste
Skeleton-Distanz, und es sei \(g_{\rm tie}(t)=d_2(t)-d_1(t)\).  Für
\(K(\varepsilon)=\{t:g_{\rm tie}(t)\le \varepsilon\}\) sollten
künftige Berichte nicht nur globales \(C_{\rm TLL}\), LDI/\(L^1\) und
BV-Verhältnis erfassen, sondern auch den LDI-Anteil
\(\int_{K(\varepsilon)}\|u'\|\Omega/\int\|u'\|\Omega\), den
BV-Anteil im selben Fenster, Switching-Anteil, High-TLL-Anteil und
Second-Branch-Variationsstress.

Ein erster KS-Smoke bei \(N=64\) und \(T_{\rm test}=50\) ergibt
\(C_{\rm TLL}(95/99/\max)=1.45042/40.9229/58.03\),
LDI/\(L^1=1.00838\) und einen angepassten Tide-Clock-Exponenten
\(\alpha_{\rm tie}=1.14757\).  Das engste \(1\%\)-Tie-Fenster trägt
etwa \(0.99\%\) der LDI-Kosten und \(0.69\%\) der BV-Kosten, während
die \(5\%\)- und \(10\%\)-Fenster etwa \(4.72\%\)/\(9.55\%\) der LDI,
aber \(11.77\%\)/\(22.04\%\) der BV tragen.  Dieser Smoke-Test zeigt
also keinen versteckten LDI-Tail in der kleinsten Near-Tie-Menge, aber
sichtbare BV-Konzentration in breiteren Tie-Korridoren.  Das ist eine
diagnostische Guardrail, kein Navier--Stokes-Beweis.  Ein zukünftiges
No-Hidden-Backflow-Lemma sollte daher verlangen, dass
Tie-Fenster-LDI-Anteile, BV-Anteile, Branch-Stress und Wax-Removal-Tail
entlang einer vorregistrierten \(\varepsilon_j\)-Leiter summierbar sind
oder verschwinden.
\end{remark}
...
\begin{enumerate}
\item \emph{Reaktions-Diffusion (2D):} Glatte Nichtlinearität, gut verstandene Attraktoren. TLL und LDI sind hier plausible Zielgrößen, weil die parabolische Glättung stärkere Regularität liefert als im kritischen Navier--Stokes-Fall.
\item \emph{Hyperviskose NSE ($(-\Delta)^\beta$, $\beta \geq 5/4$):} Mögliches Brückensystem, in dem Inertialmannigfaltigkeits- und Glättungsabschätzungen die zusätzliche Röhrenzweig-Kontrolle für TLL liefern könnten. Dies ist eine Testroute, keine automatische TLL-Garantie.
...
Wesentliche Beobachtungen: (i) Die TLL-Konstante ist am 95.\ Perzentil klein ($C_{\mathrm{TLL}} \approx 0{,}19$), doch das Maximum entlang der Testtrajektorie beträgt $1{,}94$ und liegt damit ungefähr um eine Größenordnung höher. Da TLL für \emph{alle} $t$ gelten muss, ist für die BV-Schranke das Maximum maßgeblich. (ii) Das LDI/L$^1$-Verhältnis beträgt $6{,}86$ und zeigt, dass das Log-Distanz-Gewicht das Integral um weniger als eine Größenordnung vergrößert. (iii) Die BV-Kettenregelschranke wird mit Spielraum erfüllt (Verhältnis $0{,}52 < 1$). (iv) Sublevel-Zeitkontrolle gilt mit Exponent $\alpha \approx 1{,}1$, konsistent mit der erwarteten Potenzgesetz-Annäherung an den Attraktor.
...
Wesentliche Beobachtungen: (i)~Alle vier Diagnosen bestehen in dieser Testsuite bei jeder Auflösung, sodass das
TLL/LDI-Signal nicht offensichtlich ein Artefakt der gewählten Diskretisierung ist.
...
Die komplexe Ginzburg--Landau-Gleichung $\partial_t u = (1+i\alpha)\Delta u + u - (1+i\beta)|u|^2 u$ besitzt für bestimmte Parameterbereiche eine endlich-dimensionale Inertialmannigfaltigkeit. Vollständige Verifikation von TLL+LDI auf diesem System, gefolgt von einem Lifting-Argument auf 2D-NS (via Parameterfortsetzung in der Nichtlinearität), würde einen konkreten Testweg für das NS-LDI-Transferprogramm liefern. Dies sollte jedoch nicht als direkter Weg zum vollen klassischen 3D-NS-Problem ohne die oben isolierten uniformen Projektionszweig-Abschätzungen verstanden werden.
\end{remark}
...
Dies ist das Mané-Projektionstheorem angewandt auf die Squeezing-Dichotomie:
...
\begin{aligned}
  ...
  &\Rightarrow \text{konditionale Regularität in der Begleitarbeit}.
\end{aligned}
\]
Dies ist als beweisnahe Testroute zu lesen, nicht als unbedingter
...
Die BV $\to$ $L^\infty$-Einbettung in Proposition~\ref{prop:variant-B} erfordert die zusätzliche Bedingung $d(t) > 0$ f.ü. Die Trajektorie darf den Attraktor also nicht auf einer Menge positiven Maßes berühren. Dies ist eine Annahme ohne Kontakt auf positiver Maßmenge, keine automatische Eigenschaft von Leray--Hopf-Lösungen. Für starke analytische Lösungen kann sie unter zusätzlichen Hypothesen gerechtfertigt werden; auf Intervallen, auf denen die Trajektorie auf dem Attraktor liegt, ist stattdessen Variante~A zu verwenden.
\end{remark}
...
\begin{remark}[Perspektive der Variationsanalyse]
\label{rem:variational-analysis}
Die Theorie der metrischen Regularität und Variationsanalyse (Rockafellar--Wets~\cite{RockafellarWets1998}, Dontchev--Rockafellar~\cite{DontchevRockafellar2009}) bietet eine natürliche Heimat für die TLL- und LDI-Bedingungen: TLL steht in Beziehung zur Aubin-Eigenschaft der Attraktorprojektion, und LDI ist eine Calmness-Bedingung für die Distanzfunktion. Das vollständige Instrumentarium der Variationsanalyse -- einschließlich Ekelands Variationsprinzip, des Mordukhovich-Kriteriums und der Attouch--Théra-Dualität -- ist in diesem Kontext anwendbar, aber bislang wenig erforscht.
\end{remark}
...
\begin{enumerate}
\item \textbf{Selektionsregularität über Trajektorienattraktoren.}\; Formulierung der minimalen metrischen Ableitungsselektion im Chepyzhov--Vishik-Trajektorienattraktor-Rahmen, wo Nichteindeutigkeit durch Betrachtung ganzer Trajektorien statt Anfangsdaten behandelt wird.
...
KI-gestützte Werkzeuge wurden in unterstützender Funktion für
mathematische Formalisierung, Literatur- und Metadatenprüfung,
redaktionelle Verfeinerung und Qualitätssicherung eingesetzt.  Der
Autor hat die Forschungsrichtung konzipiert, alle mathematischen
Behauptungen ausgewählt und bewertet und trägt die abschließende
Verantwortung für den gesamten Inhalt.
