...
the NS-LDI v1.4 release-candidate paper files. The latest Navier-Stokes Zenodo release is
...
release until the next upload is complete. The NS-LDI files are synchronized
for the pending Zenodo v1.4 version upload.
...
and the NS-LDI v1.4 release-candidate package only. Local working notes and the
