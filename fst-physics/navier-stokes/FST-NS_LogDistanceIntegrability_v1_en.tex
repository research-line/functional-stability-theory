...
This paper makes three contributions: (i)~We introduce the \emph{trajectory log-Lipschitz} (TLL) condition and the \emph{log-distance integrability} (LDI) condition as a trajectory-adapted regularity scale for BV projections onto fractal attractors, weaker than global Lipschitz projection regularity but stronger than a purely H\"older-type heuristic. (ii)~We prove a BV chain rule (Theorem~\ref{thm:BV-chain}) showing that TLL\,+\,LDI suffices for bounded-variation selection regularity. (iii)~We identify sufficient geometric conditions (bounded variation of the log-distance and sublevel-time control) that are compatible with the fractal geometry of dissipative PDE attractors.
...
\begin{proposition}[Measurable and tie-broken nearest-point selections]
...
finite-dimensional or Moreau--Yosida regularisation), then
\[
...
among the minimisers may be used.
\end{proposition}
...
and measurable; if the minimiser is unique for a.e.\ $t$, the stated
...
minimal enstrophy, lexicographic finite-mode order, or a regularised
...
prox-regular or $C^{1,1}$ in a tubular neighbourhood), the
...
\begin{itemize}
  \item \textbf{Selection dependence.}\;
    ...
    enstrophy, finite-mode lexicographic order, or a regularised
...
limit with pre-registered regularised selectors
...
that may have better BV behaviour after additional geometric input.

\begin{definition}[Moreau--Yosida regularised selection]
\label{def:my-selection}
For $\varepsilon > 0$, define the \emph{time-averaged minimiser set}
\[
  \Pi_\varepsilon(t)
  \;:=\;
...
dissipation bound), a pre-registered regularised selection
...
\item If\; $\sup_\varepsilon \Var(v_\varepsilon;\,[0,T]) < \infty$,
  ...
  regularisation and tie-breaking scheme.
...
compactness problem.  For a regularised system let
...
The skeleton compactness criterion also has to localise where the
...
test suite, so the TLL/LDI signal is not obviously an artefact of the chosen
finite discretisation.
