...
This paper establishes a \emph{doubly conditional framework}: under the joint validity of Assumption~G (BV-regular selection on the attractor) and Condition~D (dissipation dominance near blow-up), finite-time blow-up is excluded (Theorem~\ref{thm:main}).  This is \emph{not} a claim of a new unconditional regularity result; the value lies in the decomposition of the regularity question into two structurally distinct components---one geometric (Assumption~G), one analytical (Condition~D)---and in identifying concrete attack vectors for each. The companion paper on log-distance integrability \cite{Geiger2026-LDI} provides sufficient geometric conditions for Assumption~G.
\end{abstract}
...
$\omega$-limit set is compact in $H^1$ (which is the case for any
strong selection, but remains open for pathological weak selections).
...
\begin{proposition}[$\mu$-reach suffices for Assumption~G]\label{prop:mu-reach}
If $\mathrm{reach}_\mu(\mathcal{A}) > 0$ and the Leray--Hopf solution $u(t)$
satisfies $\operatorname{dist}(u(t), \mathcal{A}) < \mathrm{reach}_\mu(\mathcal{A})$
for all $t \geq t_0$ (which holds by exponential tracking, AGC2),
then the nearest-point projection $v(t) = \pi_{\mathcal{A}}(u(t))$ is
absolutely continuous on $[t_0, T]$ for every $T < \infty$,
and Assumption~G holds.
\end{proposition}
...
Exponential tracking (AGC2, established by Temam~\cite{Temam2001})
ensures $u(t) \in B(\mathcal{A}, \delta\,e^{-\lambda t})$ for $t \geq t_0$.
Since $u(t)$ spends $\mu_{\mathcal{A}}$-almost all time near
$\mu_{\mathcal{A}}$-typical points of $\mathcal{A}$ (by ergodicity of the
projected dynamics $v(t)$), the Lipschitz property of $\pi_{\mathcal{A}}$
applies for $\mu_{\mathcal{A}}$-almost every time $t$.
The set of exceptional times (where $v(t)$ is near a zero-reach
singularity of $\mathcal{A}$) has $\mu_{\mathcal{A}}$-measure zero
and hence Lebesgue measure zero (by absolute continuity of the
push-forward measure).
Therefore $v(t) = \pi_{\mathcal{A}}(u(t))$ is Lipschitz-1 a.e.\ in $t$,
hence absolutely continuous, and G holds.
\end{proof}
...
cost grows \emph{exponentially} in $T^*$.  Specifically, for any
...
\begin{equation}\label{eq:L2-limitation}
  ...
  \;=:\; E_0(T^*),
\end{equation}
while the RHS grows as $\int_0^{T^*}C_1 H\,dt \geq C_1^*\,H(0)\,T^*\,e^{C_1^* T^*}$.
For $T^* > T_{\mathrm{crit}} := \frac{1}{C_1^*}\ln\frac{E_0}{C_1^* H(0)}$,
Condition~(D) is \textbf{necessarily violated}.
This means that at the $L^2$-level, Theorem~\ref{thm:main}
is an \textbf{empty statement} for slow blow-up scenarios
(large~$T^*$).  This asymmetry is not an artefact of the proof
technique but reflects the fundamental energy budget of the
Navier--Stokes equations at the $L^2$ level.
...
\begin{theorem}[$L^2$-level obstruction]\label{thm:L2-obstruction}
Let $u$ be a Leray--Hopf solution on $[0,T^*)$ with $H(0) = H(u(0)|\mathcal{A}) > 0$.
Define
\[
  T_{\mathrm{crit}} \;:=\; \frac{1}{C_1^*}\,\ln\!\frac{E_0}{C_1^*\,H(0)},
\]
where $C_1^* = C_1^*(\mathcal{A},\nu,f)$ is the Gronwall coefficient from Theorem~\ref{thm:main}
and $E_0 = \frac{1}{2\nu}\int_0^{T^*}\!\|f\|^2\,dt$ is the Leray energy bound.
If $T^* > T_{\mathrm{crit}}$, then Condition~(D) is \emph{necessarily} violated:
the exponential Gronwall cost exceeds the a priori dissipation bound.
\end{theorem}

\begin{proof}
Integrating the energy inequality for $H$ (Theorem~\ref{thm:main}) over $[0,T^*]$:
\[
  \frac{d}{dt}H \;\leq\; -\nu\|\nabla w\|^2 + C_1^*\,H + C_2^*,
\]
we obtain, after rearrangement,
\begin{equation}\label{eq:diss-budget}
  \nu\!\int_0^{T^*}\!\|\nabla w\|^2\,dt
  \;\leq\; H(0) - H(T^*) + C_1^*\!\int_0^{T^*}\!H\,dt + C_2^*\,T^*.
\end{equation}
The Gronwall bound $H(t) \leq H(0)\,e^{C_1^* t} + \frac{C_2^*}{C_1^*}(e^{C_1^* t}-1)$
gives
\[
  \int_0^{T^*}\!H\,dt
  \;\geq\; \frac{H(0)}{C_1^*}\bigl(e^{C_1^* T^*} - 1\bigr).
\]
Condition~(D) requires the dissipation (left-hand side of~\eqref{eq:diss-budget})
to dominate the Gronwall cost (the $C_1^*\!\int H\,dt$ term).
But the Leray energy identity bounds the dissipation:
$\nu\!\int_0^{T^*}\!\|\nabla u\|^2\,dt \leq E_0$.
Since $\|\nabla w\|^2 \leq 2\|\nabla u\|^2 + 2\|\nabla v\|^2$
and $\|\nabla v\|$ is uniformly bounded on $\mathcal{A}$,
the left-hand side of~\eqref{eq:diss-budget} grows at most linearly in $T^*$,
while the right-hand side grows as $e^{C_1^* T^*}$.
For $T^* > T_{\mathrm{crit}} := \frac{1}{C_1^*}\ln\frac{E_0}{C_1^*\,H(0)}$,
the exponential Gronwall cost exceeds the dissipation budget, and (D) fails.
\end{proof}

\begin{corollary}[$L^2$-level structural limitation]\label{cor:L2-limitation}
Any obstruction strategy based on $H(u|\mathcal{A}) = \inf_{v \in \mathcal{A}}\frac{1}{2}\|u-v\|_{L^2}^2$
with Gronwall-type energy control yields an \emph{empty statement}
for hypothetical blow-up times $T^* > T_{\mathrm{crit}}$.
The natural Sobolev level for attractor-distance obstruction arguments
is $H^1$, where the enstrophy dissipation $\int\|\Delta u\|^2\,dt$ has no a priori upper bound.
\end{corollary}
...
\begin{enumerate}
\item[\textbf{(G$_1$)}] \textbf{Attractor existence and compactness.} The global attractor $\mathcal{A}$ exists as a compact subset of $H^1(\Omega)$ and is finite-dimensional ($d_H(\mathcal{A}) < \infty$). \emph{Status:} Established for the weak Leray--Hopf attractor without assuming unconditional regularity (Foias--Temam, Sell, Chepyzhov--Vishik; cf.\ Remark~\ref{rem:circularity}). This component is \textbf{independent} of the regularity question.
...
\begin{description}
\item[Input 1 (Attractor regularity):] $\sup_{v \in \mathcal{A}} \|v\|_{H^2} < \infty$. \emph{Status: known} (follows from smoothness of the global attractor).
...
The conclusion requires that the divergent dissipation outpaces
the (growing) Gronwall cost.  Because the Gronwall coefficient
...
Proposition~\ref{thm:H1-main} replaces the dissipation-dominance
...
Proposition~\ref{thm:H1-main} is doubly conditional: it isolates
...
establish a \emph{doubly conditional} framework for regularity.  This
framework does not claim to reduce the regularity problem to simpler
components; rather, it decomposes the question into two structurally
distinct open problems, each admitting independent investigation.
