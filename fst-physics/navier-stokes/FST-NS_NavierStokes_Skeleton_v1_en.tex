...
\begin{proposition}[Implication chain for G-NS]\label{prop:g-equivalences}
...
\begin{enumerate}[label=\alph*)]
    ...
    \item \textbf{Attractor-Projection as Physical Gauge:} The "finite negativity" (potential singularity directions) is neutralized by projecting the active flow onto the reference attractor $\mathcal{A}$ satisfying Axiom~\ref{prop:attractor-regularity}. This projection acts as the physical gauge.
...
\begin{enumerate}
\item \emph{Enstrophy growth rate:} If $\|\nabla u(t)\|_{L^2}^2 / H(u(t)|\mathcal{A})$ grows without bound as $t \to T^*$, Condition~D holds. Monitor the ratio $\|\nabla u\|^2 / \|u - v\|^2$ along near-singular trajectories.
...
\item \emph{Attractor distance decay:} If $H(u(t)|\mathcal{A}) \to 0$ faster than $\|\nabla u\|^{-2}$, Condition~D is satisfied. Compare the rates numerically.
\end{enumerate}
...
Recent developments in mean field game (MFG) theory formulate the coupled HJB--Fokker-Planck system as a PDE on the Wasserstein space of probability measures (Cardaliaguet--Delarue--Lasry--Lions~\cite{CDLL2019}). The NS free energy $H(u|A)$ can be interpreted as a value function in this MFG framework: the ``agent'' (fluid parcel) minimises a running cost (enstrophy) subject to advection-diffusion dynamics, while the ``population'' (velocity field) evolves via the Navier--Stokes equations. The attractor $A$ corresponds to the MFG equilibrium. This perspective suggests that regularity of the NS attractor is equivalent to well-posedness of the associated master equation in Wasserstein space.
\end{remark}
...
\end{enumerate}
Property (2) is \emph{precisely} the BV selection bound required by Assumption~G. It arises not from ``dividing by $\varepsilon$'' (which is invalid in the unrescaled setting) but from the Mielke--Rossi--Savar\'e EDP-convergence framework \cite{MielkeRossiSavare2016} and its relaxed wiggly-energy variant \cite{DondlFrenzelMielke2019}: the energy-dissipation balance propagates through the $\Gamma$-limit via lower semicontinuity and chain-rule arguments, as in balanced-viscosity solutions \cite{MielkeRossiSavare2016}.
...
(Axiom~\ref{prop:attractor-regularity}) guarantees
...
diverge; if it also dominates the Bihari/projection costs, the
