...
In the present paper, we adopt a variational approach. Rather than deriving the
K41 spectrum from dimensional analysis alone, we demonstrate that it is the
\emph{preferred energy profile} within the stated K41-normalised
...
\begin{enumerate}
  ...
  \item Relaxed flux constraints and the minimax notation are recorded as
        scope limits: if the exact K41 profile remains feasible, the
        relaxed problem is trivial, and the minimax form remains a decoupled
...
Our variational perspective aligns with and complements recent developments in information thermodynamics and variational fluid dynamics. In particular, Tanogami~\cite{Tanogami2023,Tanogami2025} established an information-thermodynamic bound on energy cascades using Kullback--Leibler divergence and proved scale locality of information flow in shell models. Furthermore, modern variational principles for Kolmogorov scaling---such as the stochastic Schrödinger--Bridge framework of Sanchis-Agudo and Vinuesa~\cite{SanchisAgudoVinuesa2025}, the adaptive fractional operator formulation of L\'opez~\cite{Lopez2026}, Onsager's $L^2$ dissipation principle studied by Said~\cite{Said2025}, and the spectral cutoff dissipation model of Mishra~\cite{Mishra2026}---underline the centrality of variational extremal principles in turbulence theory. In addition, polarization geometry frameworks by Skalidis~\cite{Skalidis2026} highlight how geometrical state transitions govern spectral scaling in turbulent cascades. Mathematically, restricting to admissible comparison classes is essential to exclude wild unphysical states arising in non-uniqueness theories of weak Navier--Stokes solutions~\cite{BuckmasterVicol2019,Isett2018}.
...
For orientation, Table~\ref{tab:claim-status-ledger} delineates the
...
The relaxed-flux and minimax branches record scope limits and a formal
penalty representation, not further dynamical theorems.}
...
= \mathbf{E}^*$. The use of relative entropy to quantify scale-to-scale cascade deviations is information-thermodynamically grounded by Tanogami~\cite{Tanogami2023,Tanogami2025}.
\end{remark}
...
relative selection theorem within this stated K41-normalised class.  It
...
\item The reference $\mathbf{E}^*$ is not arbitrary within the normalised
  ...
  admissible class, rather than a proof excluding alternative modelling
  conventions.
\end{enumerate}
...
\begin{remark}
Proposition~\ref{prop:dfc-slack} is a scope limit rather than a positive
...
ingredient, the KL minimum remains centred at the reference profile by construction.
\end{remark}
...
structure of the turbulent cascade.  A genuine game-theoretic model would require
...
The joint minimisation over the K41-normalised admissible class extends
beyond the formal observation that a KL divergence is minimised at its reference.
Kolmogorov’s original derivation relies on dimensional analysis, while
Theorem~\ref{thm:K41} establishes that the same profile is attainable under
...
The Zookeeper structural transfer delineates the boundaries of the present
paper's claims. This paper is Paper A in the turbulence split: it establishes the
...
\begin{center}
\small
\begin{tabular}{|p{0.50\textwidth}|p{0.16\textwidth}|p{0.18\textwidth}|}
\hline
...
\hline
\multicolumn{3}{|l|}{\textit{Scope limits and formal rewrites:}} \\
\hline
Relaxed-flux slack is trivial unless $E^*$ is excluded/perturbed & Scope limit & Prop.~\ref{prop:dfc-slack} \\
...
\href{https://doi.org/10.1017/S022112062000518}{doi:10.1017/S022112062000518}.
