...
% ===================================================================
\begin{abstract}
We establish a reference-normalised variational principle that characterises
...
fluctuations around the variational minimum reproduce the
Kolmogorov--Obukhov (K62) exponent formula.  The result is unconditional
as a static finite/inertial-range variational statement inside the stated
K41-normalised class; it is not a dynamical Navier--Stokes selection
theorem.  Anomalous dissipation under additional hypotheses on the
...
\emph{free-energy preferred} distribution inside a K41-normalised
...
theory~\cite{Rockafellar70,BonnansShapiro00}.
\end{proof}
...
\begin{equation}\label{eq:zeta_p}
  ...
  \frac{\mu}{18}\,p(p-3),
\end{equation}
which reproduce the K62 (Kolmogorov--Obukhov) refined similarity
hypothesis with intermittency parameter $\mu$~\cite{K62,Frisch95}.  Under
this ansatz, the
...
A numerical evaluation of $\mathcal{F}[\mathbf{E}]$ over six synthetic energy spectra illustrates the reference-centred KL landscape: $\mathcal{F}[\mathbf{E}_{\mathrm{K41}}] = 0$ exactly, while $\mathcal{F}[\mathbf{E}_{\mathrm{K62}}] = 2.0 \times 10^{-4}$ (intermittency correction), $\mathcal{F}[\mathbf{E}_{k^{-2}}] = 1.74$ (steep), $\mathcal{F}[\mathbf{E}_{k^{-4/3}}] = 5.94$ (shallow), and $\mathcal{F}[\mathbf{E}_{\mathrm{thermal}}] = 7.55$ (equipartition). A perturbation test ($300$ random perturbations $\delta \mathbf{E}$ at three noise levels $\|\delta \mathbf{E}\|/\|\mathbf{E}^*\| \in \{0.01, 0.1, 1.0\}$) yields $\mathcal{F}[\mathbf{E}^* + \delta \mathbf{E}] > 0$ for \emph{all} $300$ samples, illustrating strict convexity numerically. Numerical scripts: \texttt{compute\_F\_spectrum.py}.
\end{remark}

We propose the following reproducible diagnostic protocol for DNS testing:
\begin{enumerate}
...
The joint minimisation over the K41-normalised admissible class is not merely the statement that a KL divergence is minimised at its reference: Kolmogorov's original derivation relies on dimensional analysis, while Theorem~\ref{thm:K41} records that the same profile is attainable under the normalised constant-flux closure and is the unique energy minimiser in the resulting joint feasible set.  This is a structural consistency and selection statement, not a fully intrinsic derivation of K41.
...
strength: the static variational result in the present paper remains clean,
...
\hline
\multicolumn{3}{|l|}{\textit{DNS-falsifiable diagnostics:}} \\
\hline
$\mathcal{F}[\mathbf{E}_{\mathrm{DNS}}] \to 0$ as $R_\lambda \to \infty$ & Diagnostic prediction & Sec.~\ref{sec:numerics} \\
\hline
\end{tabular}
\end{center}
