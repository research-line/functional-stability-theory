...
% ===================================================================
\begin{abstract}
We establish a variational principle that characterises the Kolmogorov
$k^{-5/3}$ energy spectrum as the unique global minimiser of a
scale-resolved free-energy functional.  Starting from a
...
reference equilibrium.  We introduce the class of admissible
energy-flux operators---all local, dimensionally consistent, monotone
closures satisfying axioms (A1) and (A3)---and prove that the K41
distribution is the unique minimiser of $\mathcal{F}[\mathbf{E}]$ under
the joint minimisation over energy profiles and closures, subject to
constant scale-to-scale energy flux (Theorem~\ref{thm:K41}).  The
minimiser is non-degenerate: the Hessian of $\mathcal{F}$ at
$\mathbf{E}^*$ is diagonal with entries $1/E_j^* > 0$, providing
  scale-dependent stiffness that grows as $k^{5/3}$.  We also record two
...
naturally as Hessian fluctuations around the variational minimum.  The
result is unconditional: no hypothesis on the Navier--Stokes dynamics is
required.  Anomalous dissipation under additional hypotheses on the
nonlinear cascade will be treated in a companion paper~\cite{Geiger2026b}.
\end{abstract}

\begin{center}
\small\emph{Preprint note. This is Paper A in the turbulence split. It isolates the unconditional K41 variational-minimiser theorem. The companion Paper B treats anomalous dissipation and the Downhill Flux Condition as a conditional cascade route.}
\end{center}
...
\begin{enumerate}
  ...
  \item The minimiser is non-degenerate, with the Hessian providing
        scale-dependent stiffness $1/E_j^* \propto k_j^{5/3}$ that
        penalises small-scale perturbations more severely than large-scale
        ones---the variational origin of the forward cascade.
...
\begin{enumerate}
  \item[\textup{(i)}]
    For any admissible flux family $\Pi \in \mathscr{A}$ for which the
    constant-flux constraint $\Pi_j[\mathbf{E}] = \varepsilon$ admits a
    stationary profile $\mathbf{E}^{(\Pi)}$, the K41 distribution
    $E_j^* = C_K\,\varepsilon^{2/3}\,k_j^{-5/3}\,\Delta k_j$
...
\begin{proof}
\textbf{Step 1: Feasible stationary profiles.}\;
Fix an admissible $\Pi \in \mathscr{A}$ and restrict attention to the
feasible case in which the coupled constant-flux system
$\Pi_j[\mathbf{E}] = \varepsilon$ has a positive stationary profile
$\mathbf{E}^{(\Pi)}$.  Local monotonicity in $E_j$ is not, by itself, a
global existence theorem for the coupled shell system; existence for a
specific closure is therefore treated as a feasibility condition.  This is
the only scope needed below, because the joint problem compares profiles
that already satisfy the flux constraint.
...
\item The reference $\mathbf{E}^*$ is not arbitrary within the normalised
  ...
  $E(k)\propto k^{-5/3}$.  Broader closure families can be designed around
  other references; the theorem is therefore a selection statement inside
  the stated normalised admissible class, not a proof that no other
  modelling convention can be written down.
\end{enumerate}
...
\smallskip\noindent
\textbf{Step 6: Equivalence with the hard-constraint formulation.}\;
The penalty formulation~\eqref{eq:minimax} is the augmented Lagrangian
relaxation of the hard-constraint problem $\Pi_j = \varepsilon$ in
Theorem~\ref{thm:K41}.  As $\lambda \to \infty$, any saddle point
of~\eqref{eq:minimax} must satisfy $\Pi_j \to \varepsilon$ (otherwise
the penalty term $-\lambda(\Pi_j - \varepsilon)^2 \to -\infty$ would
violate the saddle-point inequality).  In the limit, the minimax
problem reduces to the constrained minimisation~\eqref{eq:min}, and
the saddle point converges to the K41 minimiser of
Theorem~\ref{thm:K41}.  The equivalence of the penalty and
hard-constraint formulations follows from classical convex duality
...
(Taylor expansion to second order, controlled remainder via
$x\ln(x/y) - x + y \ge (x-y)^2/(2y)$).  A \emph{global} UCU3 in the
...
A numerical evaluation of $\mathcal{F}[\mathbf{E}]$ over six synthetic energy spectra confirms that the K41 spectrum is the unique minimiser: $\mathcal{F}[\mathbf{E}_{\mathrm{K41}}] = 0$ exactly, while $\mathcal{F}[\mathbf{E}_{\mathrm{K62}}] = 2.0 \times 10^{-4}$ (intermittency correction), $\mathcal{F}[\mathbf{E}_{k^{-2}}] = 1.74$ (steep), $\mathcal{F}[\mathbf{E}_{k^{-4/3}}] = 5.94$ (shallow), and $\mathcal{F}[\mathbf{E}_{\mathrm{thermal}}] = 7.55$ (equipartition). A perturbation test ($300$ random perturbations $\delta \mathbf{E}$ at three noise levels $\|\delta \mathbf{E}\|/\|\mathbf{E}^*\| \in \{0.01, 0.1, 1.0\}$) yields $\mathcal{F}[\mathbf{E}^* + \delta \mathbf{E}] > 0$ for \emph{all} $300$ samples, confirming strict convexity numerically. Numerical scripts: \texttt{compute\_F\_spectrum.py}.
\end{remark}

We propose the following reproducible test protocol for DNS verification:
\begin{enumerate}
...
\begin{itemize}
  \item \textbf{Anomalous dissipation.}\;
    In the companion paper~\cite{Geiger2026b}, we introduce a hierarchy of
    ...
    prove that these imply a lower bound on the dissipation rate in the
...
claim.  This paper is Paper A in the turbulence split: it proves the
...
strength: the unconditional result in the present paper remains clean,
while the conditional defect theory has its own natural follow-up.
...
\hline
\hline
\multicolumn{3}{|l|}{\textit{Theorems (rigorous, unconditional):}} \\
\hline
$\mathcal{F}[\mathbf{E}] \geq 0$, $= 0$ iff $\mathbf{E} = \mathbf{E}^*$ & Theorem & Thm.~\ref{thm:K41} \\
K41 unique joint minimiser & Theorem & Thm.~\ref{thm:K41} \\
(A2) from (A1) + dimensional analysis & Proposition & Prop.~\ref{prop:A2-derived} \\
\hline
...
K41 as decoupled penalty saddle point & Formal representation & Prop.~\ref{prop:minimax} \\
\hline
\multicolumn{3}{|l|}{\textit{Numerically confirmed:}} \\
\hline
$\mathcal{F}[\mathbf{E}_{\mathrm{K41}}] = 0$ (minimiser) & Exact & Rem.~\ref{rem:numerical-F} \\
$\mathcal{F} > 0$ for all 300 perturbations & Numerical & Rem.~\ref{rem:numerical-F} \\
\hline
\multicolumn{3}{|l|}{\textit{DNS-falsifiable predictions:}} \\
\hline
$\mathcal{F}[\mathbf{E}_{\mathrm{DNS}}] \to 0$ as $R_\lambda \to \infty$ & Prediction & Sec.~\ref{sec:numerics} \\
\hline
\end{tabular}
\end{center}
...
The author thanks the Claude AI system for assistance with literature review, mathematical verification, and manuscript preparation.
