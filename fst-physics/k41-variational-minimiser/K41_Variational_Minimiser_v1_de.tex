...
Exponentenformel (K62). Das Ergebnis gilt unbedingt als statisches
...
\emph{freie-energie-bevorzugte} Verteilung innerhalb einer
...
Formulierung mit harten Nebenbedingungen folgt aus der klassischen Konvexen-Dualitäts-Theorie~\cite{Rockafellar70,BonnansShapiro00}.
\end{proof}
...
\begin{equation}\label{eq:zeta_p}
  ...
  \frac{\mu}{18}\,p(p-3),
\end{equation}
was die verfeinerte Ähnlichkeitshypothese (refined similarity hypothesis) von K62 (Kolmogorov--Obukhov)
mit Intermittenzparameter $\mu$ reproduziert~\cite{K62,Frisch95}. Unter
diesem Ansatz wird der Parameter $\mu$ durch die Varianz von $\ln(E_j/E_j^*)$ unter
...
Vanishing-Viscosity-Konstruktionen. Diese Trennung explizit zu halten, ist eine Stärke: Das statische Variationsresultat im vorliegenden Paper bleibt rein,
...
\hline
\multicolumn{3}{|l|}{\textit{DNS-falsifizierbare Diagnostiken:}} \\
\hline
$\mathcal{F}[\mathbf{E}_{\mathrm{DNS}}] \to 0$ für $R_\lambda \to \infty$ & Diagnostische Vorhersage & Abschn.~\ref{sec:numerics} \\
\hline
\end{tabular}
\end{center}
