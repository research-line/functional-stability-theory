...
\thanks{KI-Tools (Claude von Anthropic) wurden zur mathematischen Formalisierung, Literaturprüfung und redaktionellen Verfeinerung verwendet. Alle mathematischen Inhalte wurden vom Autor entwickelt, geprüft und freigegeben.}}
...
% ===================================================================
\begin{abstract}
Wir etablieren ein Variationsprinzip, welches das Kolmogorov
$k^{-5/3}$-Energiespektrum als eindeutigen globalen Minimierer eines
skalenaufgelösten Freie-Energie-Funktionals charakterisiert. Ausgehend von einer
...
Referenzgleichgewicht. Wir führen die Klasse zulässiger
Energiefluss-Operatoren ein --- alle lokalen, dimensionskonsistenten, monotonen
Schließungen (closures), die die Axiome (A1) und (A3) erfüllen --- und beweisen, dass die K41-Verteilung
der eindeutige Minimierer von $\mathcal{F}[\mathbf{E}]$ unter
der gemeinsamen Minimierung über Energieprofile und Schließungen ist, vorausgesetzt,
der Skala-zu-Skala-Energiefluss ist konstant (Theorem~\ref{thm:K41}). Der
Minimierer ist nicht-entartet: die Hesse-Matrix von $\mathcal{F}$ bei
$\mathbf{E}^*$ ist diagonal mit Einträgen $1/E_j^* > 0$, was eine
  skalenabhängige Steifigkeit liefert, die wie $k^{5/3}$ wächst. Zusätzlich
...
Intermittenzkorrekturen vom Kolmogorov--Obukhov-Typ (K62) entstehen
auf natürliche Weise als Hesse-Fluktuationen um das Variationsminimum. Das
Ergebnis gilt ohne Einschränkungen: Es wird keine Hypothese über die Navier--Stokes-Dynamik
benötigt. Anomale Dissipation unter zusätzlichen Annahmen über die
nichtlineare Kaskade wird in einer Begleitarbeit behandelt~\cite{Geiger2026b}.
\end{abstract}

\begin{center}
\small\emph{Preprint-Hinweis. Dies ist Paper A der Turbulenz-Aufteilung. Es isoliert den unbedingten K41-Variationsminimierer-Satz. Die Begleitarbeit Paper B behandelt anomale Dissipation und die Downhill-Flux-Bedingung als konditionale Kaskadenroute.}
\end{center}
...
aus ersten Prinzipien (first principles) erreicht.
...
In diesem Paper wählen wir einen anderen Ansatz. Anstatt das
K41-Spektrum allein aus der Dimensionsanalyse abzuleiten, zeigen wir, dass es die
\emph{thermodynamisch bevorzugte} Verteilung der Energie über alle Skalen ist.
...
\begin{enumerate}
  \item Das K41-Spektrum ist der \emph{eindeutige} Minimierer von $\mathcal{F}$
        ...
        gemeinsam über alle zulässigen lokalen Flussoperatoren und Energieprofile
        läuft (Theorem~\ref{thm:K41}). Dies vermeidet Zirkularität: Das
        Variationsargument setzt keine spezifische Schließung (closure) voraus.
  \item Der Minimierer ist nicht-entartet, wobei die Hesse-Matrix eine
        skalenabhängige Steifigkeit $1/E_j^* \propto k_j^{5/3}$ liefert, die
        kleinskalige Störungen stärker bestraft als großskalige --- der 
        variationelle Ursprung der Vorwärts-Kaskade.
...
Dieses Paper ist in sich geschlossen; alle Hauptresultate stützen sich ausschließlich auf Definitionen
...
zulässigen Bereich ein, aber das Minimum liegt strikt im Inneren, was K41 als
gemeinsamen Minimierer bestätigt.}
...
Axiom~\textup{(A2)} ist keine unabhängige Annahme, sondern eine Konsequenz aus Lokalität~\textup{(A1)} und Dimensionsanalyse. Insbesondere ist die Form $\Pi_j = k_j\,E_j^{3/2}\,\Phi_j(\cdot)$ der \emph{eindeutige} dimensionskonsistente lokale Flussausdruck.
\end{proposition}

\begin{proof}
Nach dem Buckinghamschen $\Pi$-Theorem muss der Fluss $\Pi_j$ ein Produkt aus Potenzen der verfügbaren dimensionalen Größen $k_j$ und~$E_j$ sein, multipliziert mit einer dimensionslosen Funktion dimensionsloser Verhältnisse. Schreibe $\Pi_j = k_j^{a}\,E_j^{b}\,\Phi_j(\ldots)$ mit $[\Pi_j] = [L^2\,T^{-3}]$ (Energiefluss pro Masseneinheit), $[k_j] = [L^{-1}]$ und $[E_j] = [L^2\,T^{-2}]$.
...
\]
Daraus folgt $\Pi_j = k_j\,E_j^{3/2}\,\Phi_j$, und nach (A1) kann die dimensionslose Funktion $\Phi_j$ nur von den lokalen Verhältnissen $E_{j-1}/E_j$ und $E_{j+1}/E_j$ abhängen. Die Eddy-Turnover-Zeit $\tau_j = (k_j^2\,E_j)^{-1/2}$ ist gleichermaßen die eindeutige dimensionskonsistente lokale Zeitskala.
\end{proof}
...
erweitert die Modellklasse so, dass alle physikalisch sinnvollen lokalen Schließungen eingeschlossen sind:
EDQNM-artige Schließungen, Leiths Diffusionsmodell und trunkierte Triaden-Interaktionen
erfüllen alle (A1)--(A3). Die Familie $\mathscr{A}$ ist somit
eine breite, physikalisch motivierte Klasse und keine einzelne algebraische Relation.
Proposition~\ref{prop:A2-derived} zeigt, dass (A2) keine belanglose Modellierungswahl ist, sondern eine notwendige Konsequenz der Dimensionsanalyse, wodurch das Axiomensystem weiter auf zwei unabhängige Annahmen reduziert wird: Lokalität~(A1) und Monotonie~(A3).
\end{remark}
...
Betrachte das \emph{gemeinsame} (joint) Minimierungsproblem
...
\begin{enumerate}
  \item[\textup{(i)}]
    Für jede zulässige Flussfamilie $\Pi \in \mathscr{A}$, für welche die
    Bedingung konstanten Flusses $\Pi_j[\mathbf{E}] = \varepsilon$ ein
    positives stationäres Profil $\mathbf{E}^{(\Pi)}$ besitzt, ist die
    K41-Verteilung
    $E_j^* = C_K\,\varepsilon^{2/3}\,k_j^{-5/3}\,\Delta k_j$
    der eindeutige globale Minimierer von~$\mathcal{F}$, und sie wird
  ...
  \item[\textup{(ii)}]
    ...
    Nebenbedingungs-Mannigfaltigkeit (für jedes zulässige $\Pi$) ist strikt positiv
    ...
    $1/E_j^* \propto k_j^{5/3}$, siehe Schritt~5b unten.)
\end{enumerate}
\end{theorem}

\begin{proof}
\textbf{Schritt 1: Zulässige stationäre Profile.}\;
Fixiere ein zulässiges $\Pi \in \mathscr{A}$ und beschränke die Betrachtung
auf den Fall, in dem das gekoppelte System
$\Pi_j[\mathbf{E}] = \varepsilon$ ein positives stationäres Profil
$\mathbf{E}^{(\Pi)}$ besitzt. Lokale Monotonie in $E_j$ ist für sich
genommen kein globaler Existenzsatz für das gekoppelte Schalensystem; die
Existenz für eine konkrete Schließung wird daher als Zulässigkeitsbedingung
behandelt. Mehr wird im Folgenden nicht benötigt, da das gemeinsame Problem
nur Profile vergleicht, die die Flussbedingung bereits erfüllen.

\textbf{Schritt 2: Untere Schranke via Konvexität.}\;
Da jeder Summand $f(E_j) = E_j\ln(E_j/E_j^*) - E_j + E_j^*$
strikt konvex mit globalem Minimum $f(E_j^*) = 0$ ist, haben wir
...
Schritt~3 (Erreichbarkeit): Es ist die Existenz einer zulässigen Schließung,
die $\Pi_j[\mathbf{E}^*] = \varepsilon$ realisiert, welche K41
unter allen möglichen Referenzspektren herausstellt. Ein Ersetzen von $\mathbf{E}^*$ durch
etwa $k^{-3}$ würde keine zulässige Schließung ergeben, die
die Bedingung konstanten Flusses am angeblichen Minimierer erfüllt (siehe
Bemerkung~\ref{rem:circularity}).
...
Die diagonalen Einträge $1/E_j^* \propto k_j^{5/3}$ der Hesse-Matrix wachsen mit
der Wellenzahl, sodass kleinskalige Störungen des K41-Profils
strenger bestraft werden als großskalige. Diese skalenabhängige
Steifigkeit ist der variationelle Ursprung der Vorwärts-Kaskade: Abweichungen
bei hohen Wellenzahlen sind energetisch kostspielig und treiben das System auf der lokalen Eddy-Turnover-Zeitskala zum
K41-Profil zurück. Der Ein-Parameter-Freiheitsgrad in der Kolmogorov--Heisenberg-Schließung ($\Phi_j \equiv \alpha$)
...
\[
  \min_j\frac{1}{E_j^*}
  \;=\;
  \frac{1}{C_K\,\varepsilon^{2/3}\,k_1^{-2/3}\,\Delta k_1}
  \;>\; 0.
\]
...
\item Die Referenz $\mathbf{E}^*$ innerhalb der normalisierten
  ...
  $E(k)\propto k^{-5/3}$. Breitere Schließungsfamilien können natürlich
  um andere Referenzen konstruiert werden; das Theorem ist daher eine
  Selektionsaussage innerhalb der angegebenen normalisierten zulässigen
  Klasse, kein Beweis, dass keine andere Modellkonvention formulierbar ist.
\end{enumerate}
...
Die Variationsstruktur von Theorem~\ref{thm:K41} lässt eine aufschlussreiche
spieltheoretische Interpretation zu. Betrachtet man den Trägheitsbereich als eine
hierarchische \emph{Reaktionskette}: große Wirbel auf der Skala $k_j$ transferieren
Energie an Wirbel auf der Skala $k_{j+1}$, wobei jede Skala als eine „Art“ agiert,
deren Fitness durch ihren Energieanteil gesteuert wird. Die zugehörige Replikator-Dynamik
auf dem Simplex der normierten Schalenenergien
...
Populationsdurchschnitt ist. Das K41-Profil $\mathbf{p}^*$ ist ein \emph{Nash-Gleichgewicht} dieser Dynamik: Keine einzelne Skala kann ihren Energieanteil
durch einseitige Abweichung erhöhen. Die freie Energie $\mathcal{F}$ dient als
strenge Ljapunow-Funktion für die Replikator-Gleichung, was die
asymptotische Stabilität von $\mathbf{p}^*$ sicherstellt.
Dies spiegelt das wohlbekannte Resultat wider, dass die Minimierung der Gibbs'schen freien Energie
in chemischen Reaktionsnetzwerken das Detaillierte-Bilanz-Gleichgewicht
als eindeutiges Nash-Gleichgewicht der Massenwirkungskinetik liefert.
\end{remark}
...
\smallskip\noindent
\textbf{Schritt 6: Äquivalenz zur harten Nebenbedingungsformulierung.}\;
Die Penalty-Formulierung~\eqref{eq:minimax} ist die augmentierte Lagrange-Relaxation
des Problems mit harten Bedingungen $\Pi_j = \varepsilon$ in
Theorem~\ref{thm:K41}. Für $\lambda \to \infty$ muss jeder Sattelpunkt
von~\eqref{eq:minimax} $\Pi_j \to \varepsilon$ erfüllen (andernfalls
würde der Strafterm $-\lambda(\Pi_j - \varepsilon)^2 \to -\infty$
die Sattelpunkt-Ungleichung verletzen). Im Grenzwert reduziert sich das Minimax-Problem
auf die restringierte Minimierung~\eqref{eq:min}, und
der Sattelpunkt konvergiert gegen den K41-Minimierer aus
Theorem~\ref{thm:K41}. Die Äquivalenz der Penalty- und der
Formulierung mit harten Nebenbedingungen folgt aus der klassischen Konvexen-Dualitäts-Theorie~\cite{Rockafellar70}.
\end{proof}
...
\[
  ...
  \;\ge\;
  \tfrac{1}{2}\,\|\mathbf{E}-\mathbf{E}^*\|_*^{\,2},
\]
d.h.\ \emph{lokales} UCU3 gilt mit der Konstante $g_* = 1/2$ in der gewichteten
...
$x\ln(x/y) - x + y \ge (x-y)^2/(2y)$). Ein \emph{globales} UCU3 im
...
In unserem Rahmen entsteht Intermittenz durch Fluktuationen der Schalenenergien $\mathbf{E}$ um den K41-Minimierer $\mathbf{E}^*$. Die Entwicklung
von $\mathcal{F}$ bis zur zweiten Ordnung liefert:
\begin{equation}\label{eq:F_expand}
  ...
  \;+\; O(|\delta\mathbf{E}|^3).
\end{equation}
Die Hesse-Matrix $H_{jj} = 1/E_j^*$ impliziert, dass Fluktuationen auf kleinen Skalen
(große $j$, kleines $E_j^*$) stärker unterdrückt werden als Fluktuationen
auf großen Skalen. Diese Asymmetrie ist der variationelle Ursprung
der Intermittenz.

\subsection{Verbindung zum K62-Lognormal-Modell}

Wenn wir die Fluktuationen $\delta E_j$ als Gaußverteilt in der Variable
...
\begin{equation}\label{eq:zeta_p}
  ...
  \frac{\mu}{18}\,p(p-3),
\end{equation}
was exakt die verfeinerte Ähnlichkeitshypothese (refined similarity hypothesis) von K62 (Kolmogorov--Obukhov)
mit Intermittenzparameter $\mu$ ist~\cite{K62,Frisch95}. Der
Parameter $\mu$ wird durch die Varianz von $\ln(E_j/E_j^*)$ unter
dem Gibbs-Maß $\exp(-\mathcal{F}/\Theta)$ bestimmt, wobei die effektive
Temperatur $\Theta > 0$ die Stärke der turbulenten Fluktuationen um das K41-Gleichgewicht
parametrisiert (analog zur Temperatur in einem kanonischen Ensemble). Im Limes $\Theta \to 0$ konzentriert sich das Gibbs-Maß
auf den K41-Minimierer und $\mu \to 0$ (keine Intermittenz); für
endliches $\Theta$ skaliert die lognormale Varianz als
$\mu \propto \Theta\,E_j^*$, wodurch der Intermittenzparameter
mit der Intensität der Kaskadenfluktuationen in Verbindung gebracht wird.
...
% ===================================================================
\section{Numerische Evidenz}\label{sec:numerics}

\begin{remark}[Numerische Verifikation von Theorem~\ref{thm:K41}]\label{rem:numerical-F}
Eine numerische Auswertung von $\mathcal{F}[\mathbf{E}]$ über sechs synthetische Energiespektren bestätigt, dass das K41-Spektrum der eindeutige Minimierer ist: $\mathcal{F}[\mathbf{E}_{\mathrm{K41}}] = 0$ exakt, während $\mathcal{F}[\mathbf{E}_{\mathrm{K62}}] = 2{,}0 \times 10^{-4}$ (Intermittenzkorrektur), $\mathcal{F}[\mathbf{E}_{k^{-2}}] = 1{,}74$ (steil), $\mathcal{F}[\mathbf{E}_{k^{-4/3}}] = 5{,}94$ (flach) und $\mathcal{F}[\mathbf{E}_{\mathrm{thermal}}] = 7{,}55$ (Gleichverteilung). Ein Perturbationstest ($300$ zufällige Störungen $\delta \mathbf{E}$ bei drei Rauschleveln $\|\delta \mathbf{E}\|/\|\mathbf{E}^*\| \in \{0{,}01,\, 0{,}1,\, 1{,}0\}$) liefert $\mathcal{F}[\mathbf{E}^* + \delta \mathbf{E}] > 0$ für \emph{alle} $300$ Samples, was die strikte Konvexität numerisch bestätigt. Numerische Skripte: \texttt{compute\_F\_spectrum.py}.
\end{remark}

Wir schlagen folgendes reproduzierbares Testprotokoll zur DNS-Verifizierung vor:
\begin{enumerate}
...
\item Verifiziere, dass $\mathcal{F}$ mit zunehmendem $R_\lambda$ abnimmt
...
statistisch unabhängigen Zeit-Snapshots innerhalb des stationären Regimes angewendet werden;
...
Die gemeinsame Minimierung über alle zulässigen Schließungen ist keine tautologische Umformulierung von K41: Kolmogorovs ursprüngliche Ableitung beruht auf Dimensionsanalyse, während unser Theorem zeigt, dass K41 der eindeutige Minimierer eines wohldefinierten Variationsproblems ist, was eine strukturelle Erklärung für die Universalität liefert.

\subsection{Die Rolle der Schließungsfamilie}

Die Umformulierung von Theorem~\ref{thm:K41} via der zulässigen Flussfamilie (Definition~\ref{def:admissible}) schwächt die Abhängigkeit von jeglichem spezifischen Schließungsmodell signifikant ab. Das K41-Spektrum wird nicht durch eine einzelne algebraische Relation selektiert, sondern als der eindeutige Minimierer quer über \emph{alle} lokalen, dimensionskonsistenten, monotonen Flussoperatoren.
...
\item \textbf{Eindeutigkeit der Referenz.}\; Während Bemerkung~\ref{rem:circularity} argumentiert, dass $\mathbf{E}^*$ die einzig physikalisch konsistente Referenz ist, würde eine vollkommen intrinsische Charakterisierung von $\mathbf{E}^*$ (ohne \emph{a priori} Rückgriff auf K41) die Argumentation stärken. Bemerkung~\ref{rem:khm-intrinsic} stellt einen Schritt in diese Richtung dar.
\end{enumerate}
...
\begin{itemize}
  \item \textbf{Anomale Dissipation.}\;
    ...
    beweisen, dass diese im Limes verschwindender Viskosität eine untere Schranke für die Dissipationsrate implizieren.
...
CCM/Fourier-Zweig, aber daraus folgt keine automatische Turbulenz-Aussage von
jener Schließung. Das vorliegende Theorem bleibt daher exakt das reine
K41-Variations-Theorem.
...
gewichtete schalen-gemittelte Formen des $4/5$-Gesetzes sowie Stresstests aus konvexer Integration und
Vanishing-Viscosity-Konstruktionen. Diese Trennung explizit zu halten, ist eine Stärke: Das unbedingte Resultat im vorliegenden Paper bleibt rein,
...
\hline
\hline
\multicolumn{3}{|l|}{\textit{Theoreme (rigoros, unbedingt):}} \\
\hline
$\mathcal{F}[\mathbf{E}] \geq 0$, $= 0$ gdw.\ $\mathbf{E} = \mathbf{E}^*$ & Theorem & Thm.~\ref{thm:K41} \\
K41 eindeutiger gemeinsamer Minimierer & Theorem & Thm.~\ref{thm:K41} \\
(A2) aus (A1) + Dimensionsanalyse & Proposition & Prop.~\ref{prop:A2-derived} \\
\hline
\multicolumn{3}{|l|}{\textit{Geltungsbereich-Leitplanken und formale Umschreibungen:}} \\
\hline
Relaxierter Flussspielraum ist trivial, solange $E^*$ zulässig bleibt & Leitplanke & Prop.~\ref{prop:dfc-slack} \\
K41 als entkoppelter Penalty-Sattelpunkt & Formale Darstellung & Prop.~\ref{prop:minimax} \\
\hline
\multicolumn{3}{|l|}{\textit{Numerisch bestätigt:}} \\
\hline
$\mathcal{F}[\mathbf{E}_{\mathrm{K41}}] = 0$ (Minimierer) & Exakt & Rem.~\ref{rem:numerical-F} \\
$\mathcal{F} > 0$ für alle 300 Perturbationen & Numerisch & Rem.~\ref{rem:numerical-F} \\
\hline
\multicolumn{3}{|l|}{\textit{DNS-falsifizierbare Vorhersagen:}} \\
\hline
$\mathcal{F}[\mathbf{E}_{\mathrm{DNS}}] \to 0$ für $R_\lambda \to \infty$ & Vorhersage & Abschn.~\ref{sec:numerics} \\
\hline
\end{tabular}
\end{center}
...
Der Autor dankt dem KI-System Claude für die Unterstützung bei der Literaturrecherche, mathematischen Verifizierung und Manuskripterstellung.

% ===================================================================
\section*{KI-Offenlegung}
Bei der Erstellung dieses Manuskripts wurden KI-gestützte Tools (Claude, Anthropic) in einer unterstützenden Funktion verwendet, insbesondere zur Literaturrecherche, Textstrukturierung und Formulierungshilfe. Die konzeptionelle Gestaltung, wissenschaftliche Argumentation und die Verantwortung für den gesamten Inhalt liegen ausschließlich beim Autor.
