#!/usr/bin/env python3
# coding: utf-8
"""
Kritischer Test: chi_5 (Legendre mod 5), ein EVEN Charakter.

Wenn die revidierte Meta-Paper-Vorhersage stimmt (Transfer nur fuer even chars),
sollte chi_5 klare EVEN-Dominance zeigen (wie Riemann).

chi_5 = Legendre-Symbol mod 5:
  chi_5(1) = 1, chi_5(2) = -1, chi_5(3) = -1, chi_5(4) = 1, chi_5(0) = 0.
  chi_5(-1) = chi_5(4) = +1 (EVEN character).
"""
import numpy as np
import sympy
from scipy.special import digamma
import sys
sys.stdout.reconfigure(line_buffering=True)

def chi5(n):
    """Legendre-Symbol mod 5."""
    if n % 5 == 0: return 0
    r = n % 5
    return {1: 1, 2: -1, 3: -1, 4: 1}[r]

# Sanity checks
assert chi5(-1) == chi5(4) == 1  # EVEN character
assert chi5(5) == 0
assert chi5(2) == -1
print("[chi_5] primitive Legendre mod 5, chi_5(-1) = +1 (EVEN)")

def build_W_matrix_fast(sector, N, L, primes, chi_vals, kappa):
    if sector == 'cos':
        bs = 0
    else:
        bs = 1
    idx = np.arange(N) + bs
    nn = idx[:, None]; mm = idx[None, :]

    W = np.zeros((N, N))
    shift = (2*kappa+1)/4.0
    tau = np.pi * idx / L
    diag = np.array([digamma(shift + 1j*t/2).real - np.log(np.pi) for t in tau])
    np.fill_diagonal(W, diag)

    if sector == 'cos':
        norm = np.where(idx > 0, 1.0/np.sqrt(L), 1.0/np.sqrt(2*L))
    else:
        norm = np.full(N, 1.0/np.sqrt(L))
    norm_matrix = np.outer(norm, norm)

    def A_manual(k_arr, phi_arr, t, L):
        result = np.zeros_like(k_arr, dtype=float)
        mask = (k_arr != 0)
        kk = k_arr[mask]; pp = phi_arr[mask]
        result[mask] = (L / (kk * np.pi)) * np.sin(kk * np.pi * t / L + pp)
        result[~mask] = np.cos(phi_arr[~mask]) * t
        return result

    for p, cp in zip(primes, chi_vals):
        if cp == 0: continue
        lp = np.log(p)
        mm_max = int(2*L/lp)
        for me in range(1, mm_max+1):
            d = me * lp
            if d >= 2*L: break
            weight = (cp**me) * lp / (p**(me/2.0))
            for d_signed in [d, -d]:
                a = max(-L, -L + d_signed)
                b = min(L, L + d_signed)
                if a >= b: continue
                k1 = nn - mm; k2 = nn + mm
                phi1 = mm * np.pi * d_signed / L
                phi2 = -mm * np.pi * d_signed / L
                phi1_full = np.broadcast_to(phi1, (N, N)).copy()
                phi2_full = np.broadcast_to(phi2, (N, N)).copy()
                I1 = 0.5*(A_manual(k1, phi1_full, b, L) - A_manual(k1, phi1_full, a, L))
                I2 = 0.5*(A_manual(k2, phi2_full, b, L) - A_manual(k2, phi2_full, a, L))
                if sector == 'cos':
                    overlap_matrix = I1 + I2
                else:
                    overlap_matrix = I1 - I2
                W += weight * overlap_matrix * norm_matrix
    return 0.5 * (W + W.T)

def gap_at(chi_fn, lam, N, kappa):
    """Gap-Berechnung fuer gegebenen Charakter."""
    L = np.log(lam)
    primes = [p for p in sympy.primerange(2, int(lam)+1) if chi_fn(p) != 0]
    chi_vals = [chi_fn(p) for p in primes]
    W_c = build_W_matrix_fast('cos', N, L, primes, chi_vals, kappa)
    W_s = build_W_matrix_fast('sin', N, L, primes, chi_vals, kappa)
    ec = np.linalg.eigvalsh(W_c); es = np.linalg.eigvalsh(W_s)
    return ec[0], es[0], es[0] - ec[0]

# ============================================================
# EVEN character chi_5: kappa = 0 (even gamma factor)
# Vorhersage: klare EVEN-Dominance (Gap > 0)
# ============================================================
print("\n=== chi_5 (EVEN character) — Fine Scan ===")
print(f"{'lambda':>7}  {'N':>4}  {'lam1+':>10}  {'lam1-':>10}  {'gap':>10}  {'gap/sqrt(L)':>14}  {'dom':>5}")
lam_values = [30, 50, 75, 100, 150, 200, 300, 500, 700, 1000, 2000, 5000]
data_even = []
for lam in lam_values:
    L = np.log(lam)
    N = min(80, max(20, int(2.0 * L * L)))
    ec, es, g = gap_at(chi5, lam, N, kappa=0)  # kappa=0 fuer even character
    dom = 'ODD' if g < 0 else 'EVEN'
    print(f"{lam:7d}  {N:4d}  {ec:+10.4f}  {es:+10.4f}  {g:+10.4f}  {g/np.sqrt(L):+14.4f}  {dom:>5}", flush=True)
    data_even.append((lam, N, g, dom))

# Dominance-Statistik
n_even = sum(1 for d in data_even if d[3] == 'EVEN')
n_odd = sum(1 for d in data_even if d[3] == 'ODD')
print(f"\nDominance-Stats (chi_5): {n_even} EVEN / {n_odd} ODD von {len(data_even)}")

# ============================================================
# Kontrolle: Riemann (chi_0 trivial) bei denselben lambda
# ============================================================
print("\n=== Kontrolle: chi_0 (Riemann, trivial, EVEN) ===")
print(f"{'lambda':>7}  {'N':>4}  {'lam1+':>10}  {'lam1-':>10}  {'gap':>10}  {'gap/sqrt(L)':>14}  {'dom':>5}")

def chi_trivial(n):
    return 1 if n > 0 else 0

for lam in [100, 500, 1000, 5000]:
    L = np.log(lam)
    N = min(80, max(20, int(2.0 * L * L)))
    ec, es, g = gap_at(chi_trivial, lam, N, kappa=0)
    dom = 'ODD' if g < 0 else 'EVEN'
    print(f"{lam:7d}  {N:4d}  {ec:+10.4f}  {es:+10.4f}  {g:+10.4f}  {g/np.sqrt(L):+14.4f}  {dom:>5}", flush=True)
